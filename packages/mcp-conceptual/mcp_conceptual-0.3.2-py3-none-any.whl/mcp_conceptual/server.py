...
try:
    ...
    if key_manager.admin_username and key_manager.admin_password:
        print(f"✓ Using admin API for client keys (username: {key_manager.admin_username})", file=sys.stderr)
    ...
    print("  - CONCEPTUAL_ADMIN_USERNAME and CONCEPTUAL_ADMIN_PASSWORD (fetch from API)", file=sys.stderr)
    sys.exit(1)
...
async def get_weekly_metrics_tool(
    ...
    result = await get_weekly_metrics(
        ...
        comparison_label, platform, include_insights, include_budget_pacing
    )
    return result[0].text

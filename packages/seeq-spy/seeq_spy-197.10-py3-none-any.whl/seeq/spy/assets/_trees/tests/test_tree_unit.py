...
def test_validate_properties():
    ...
    _, error_series = _validate.validate(session, df, stage='input')
    error_list = [
    ...
    for i in range(len(error_list)):
        row_validation = _validate.property_validations(session, df.iloc[i], 'input')
    ...
    _, error_series = _validate.validate(session, df, stage='final')
    final_error_list = [
    ...
    for i in range(len(final_error_list)):
        row_validation = _validate.property_validations(session, df.iloc[i], 'final')
        assert row_validation[i][1] in error_series.loc[i]
    ...
    session.options.compatibility = 190
    df = KeyedDataFrame([
        {'Name': 'Workbook', 'Type': 'Workbook'}
    ])
    row_validation = _validate.property_validations(session, df.iloc[0], stage='input')
    assert all(result[0] for result in row_validation)
    df = KeyedDataFrame([
        {'Name': 'Workbook', 'Type': 'Workbook', 'Path': 'Example', 'Depth': 1}
    ])
    row_validation = _validate.property_validations(session, df.iloc[0], stage='final')
    assert all(result[0] for result in row_validation)
...
def test_formula_parameter_pointing_to_asset_fail():
    ...
    tree = Tree('test_tree_system11', workbook=workbook, session=session)
    tree.insert('Asset1')

...
from seeq.sdk import WorkbooksApi, WorkbookInputV1
...
@pytest.mark.system
def test_pull_vantage():
    if not spy.utils.is_server_version_at_least(66, 30, 1):
        pytest.skip('Server version does not support Vantage rooms as workbooks')
    ...
    try:
        status = spy.Status()
        vantage_workbooks = spy.workbooks.pull(workbook_id, status=status)
        assert not vantage_workbooks
        assert status.df.empty
        assert status.warnings == [
            'Some items are being skipped. Only workbooks of type: Topic, Analysis are currently supported.'
        ]

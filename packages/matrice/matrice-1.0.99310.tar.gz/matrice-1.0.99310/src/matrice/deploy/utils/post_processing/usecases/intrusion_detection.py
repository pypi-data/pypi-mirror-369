...
class IntrusionUseCase(BaseProcessor):
    ...
    def _generate_tracking_stats(self, counting_summary: Dict, zone_analysis: Dict, config: IntrusionConfig, frame_id: str, alerts: Any=[], stream_info: Optional[Dict[str, Any]] = None) -> List[Dict]:
        ...
        # Get zone tracking info for consistent totals
        ...
        # Get global unique count from zone tracking
        global_total_unique_count = 0
        if zone_tracking_info:
            # Use the global unique person count directly
            global_total_unique_count = self.get_global_unique_person_count()
        else:
            global_total_unique_count = total_unique_count
        
        for category in config.person_categories or ["person"]:
            # Use global unique count for total
            category_total_count = global_total_unique_count
        ...
        # Always generate human text, even when no one is in zones
        human_text_lines = []
        current_timestamp = self._get_current_timestamp_str(stream_info, frame_id=frame_id)
        start_timestamp = self._get_start_timestamp_str(stream_info)
        ...
        # Check if anyone is currently in any zone
        current_zone_activity = False
        if zone_analysis:
            for zone_name, zone_data in zone_analysis.items():
                if isinstance(zone_data, dict) and zone_data.get("current_count", 0) > 0:
                    current_zone_activity = True
                    break
                elif isinstance(zone_data, (int, list)) and (zone_data > 0 if isinstance(zone_data, int) else len(zone_data) > 0):
                    current_zone_activity = True
                    break
        
        if current_zone_activity:
            # Someone is currently in a zone
            human_text_lines.append(f"\t- Person detected in Prohibited Boarding Gate: {total_people}")
        else:
            # No one is currently in any zone
            human_text_lines.append(f"\t- No person currently in Prohibited Boarding Gate")
        
        human_text_lines.append("")
        human_text_lines.append(f"TOTAL SINCE @ {start_timestamp}:")
        
        # Use global tracking for total unique people who have ever been in any zone
        # This ensures the same person leaving and returning doesn't increase the count
        global_total_unique_count = 0
        ...
        if zone_tracking_info:
            ...
            # Show zone-specific breakdown
            for zone_name, zone_info in zone_tracking_info.items():
                zone_total_count = zone_info.get("total_count", 0)
                human_text_lines.append(f"\t- Zone name: {zone_name}")
                human_text_lines.append(f"\t\t- Total unique people detected in this zone: {zone_total_count}")
            
            # Show global total
            human_text_lines.append(f"\t- Total unique people across all zones: {global_total_unique_count}")
        else:
            # Fallback to total unique count if no zone tracking
            global_total_unique_count = total_unique_count
            human_text_lines.append(f"\t- Total unique people detected: {global_total_unique_count}")

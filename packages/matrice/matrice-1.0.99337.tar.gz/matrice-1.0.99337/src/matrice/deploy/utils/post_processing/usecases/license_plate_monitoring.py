...
class LicensePlateMonitorUseCase(BaseProcessor):
    ...
    def _update_detections_with_ocr(self, detections: List[Dict[str, Any]], ocr_analysis: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Update detections with OCR results based on detection_id or track_id."""
        ocr_dict = {rec["detection_id"]: rec["plate_text"] for rec in ocr_analysis}
        for det in detections:
            det_id = det.get("detection_id", det.get("id", det.get("track_id")))
            if det_id in ocr_dict:
                det["plate_text"] = ocr_dict[det_id]
            else:
                det["plate_text"] = None
        return detections

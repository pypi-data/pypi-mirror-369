...
class ColorDetectionUseCase(BaseProcessor):
    ...
    def _generate_tracking_stats(
        ...
        color_track_ids = defaultdict(set)
        per_category_count: Dict[str, int] = {}
        category_track_ids = defaultdict(set)
        ...
        for det in counting_summary.get("detections", []):
            ...
            if color and track_id is not None:
                # Only count each track ID once per color in current frame
                if track_id not in color_track_ids[color]:
                    color_track_ids[color].add(track_id)
                    current_color_count[color] = current_color_count.get(color, 0) + 1
            
            if category and track_id is not None:
                # Only count each track ID once per category in current frame
                if track_id not in category_track_ids[category]:
                    category_track_ids[category].add(track_id)
                    per_category_count[category] = per_category_count.get(category, 0) + 1
        
        # Calculate total detections as sum of unique track IDs
        total_detections = sum(len(track_ids) for track_ids in category_track_ids.values())
        ...
        # Always show current frame colors, even if no detections
        if current_color_count:
            color_counts_text = ", ".join([f"{count} {color}" for color, count in current_color_count.items()])
            human_text_lines.append(f"\t- Colors: {color_counts_text}")
        else:
            human_text_lines.append(f"\t- Colors: None")
        ...
        # Add color-wise totals - show all colors, even with 0 count
        if total_color_counts_dict:
            ...
            human_text_lines.append("\t- Colors: None")
        # Build arrays using the unique track ID counts
        ...
        # Join all lines to create final human text
        human_text = "\n".join(human_text_lines)
        reset_settings = [
                {
                    "interval_type": "daily",

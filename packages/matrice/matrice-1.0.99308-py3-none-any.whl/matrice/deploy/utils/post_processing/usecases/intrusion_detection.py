...
class IntrusionUseCase(BaseProcessor):
    ...
    def _generate_tracking_stats(self, counting_summary: Dict, zone_analysis: Dict, config: IntrusionConfig, frame_id: str, alerts: Any=[], stream_info: Optional[Dict[str, Any]] = None) -> List[Dict]:
        ...
        # Show cumulative totals for each zone
        if zone_tracking_info:
            for zone_name, zone_info in zone_tracking_info.items():
                total_count = zone_info.get("total_count", 0)
                human_text_lines.append(f"\t- Zone name: {zone_name}")
                human_text_lines.append(f"\t\t- Total unique people detected in Prohibited Boarding Gate: {total_count}")
        else:
            # Fallback to total unique count if no zone tracking
            human_text_lines.append(f"\t- Total unique people detected: {total_unique_count}")

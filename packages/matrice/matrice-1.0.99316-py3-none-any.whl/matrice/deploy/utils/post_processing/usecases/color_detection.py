...
class ColorDetectionUseCase(BaseProcessor):
    ...
    def _update_color_tracking_state(self, detections: List[Dict]):
        ...
        # Convert color_record to set for efficient lookups if it's not already
        if not hasattr(self, '_color_record_set'):
            self._color_record_set = set(self.color_record)
        ...
        for det in detections:
            ...
            if cat and track_id is not None:
                ...
                if track_id not in self._color_record_set:
                    ...
                    self._color_record_set.add(track_id)
    ...
    def get_current_frame_color_counts(self, detections: List[Dict]) -> Dict[str, int]:
        """Return current frame unique track_id count per color."""
        color_to_ids = defaultdict(set)
        for det in detections:
            color = det.get('main_color')
            track_id = det.get('track_id')
            if color and track_id is not None:
                color_to_ids[color].add(track_id)
        return {color: len(ids) for color, ids in color_to_ids.items()}

    def get_current_frame_category_counts(self, detections: List[Dict]) -> Dict[str, int]:
        """Return current frame unique track_id count per category."""
        category_to_ids = defaultdict(set)
        for det in detections:
            category = det.get('category')
            track_id = det.get('track_id')
            if category and track_id is not None:
                category_to_ids[category].add(track_id)
        return {category: len(ids) for category, ids in category_to_ids.items()}

    def get_current_frame_total_count(self, detections: List[Dict]) -> int:
        """Return current frame total unique track_id count."""
    ...
    def _update_color_tracking_state_from_analysis(self, color_analysis: List[Dict[str, Any]]) -> None:
        ...
        # Ensure color_record_set exists
        if not hasattr(self, '_color_record_set'):
            self._color_record_set = set(self.color_record)
    ...
    def _calculate_color_summary(self, color_analysis: List[Dict], config: ColorDetectionConfig) -> Dict[str, Any]:
        # Use track IDs for unique counting
        category_colors = defaultdict(lambda: defaultdict(set))
        detections = []
        counts = {}
        ...
        for record in color_analysis:
            ...
            if track_id is not None:
                category_colors[category][main_color].add(track_id)
        ...
        category_colors_counts = defaultdict(lambda: defaultdict(int))
        for category, colors in category_colors.items():
            for color, track_ids in colors.items():
                category_colors_counts[category][color] = len(track_ids)
        
        counts = {category: len(track_ids) for category, track_ids in counts.items()}
        print("-------------------category_colors-------------------")
        print(category_colors_counts)
        print("-------------------category_colors-------------------")
        ...
        for category_data in category_colors_counts.values():
        ...
        for category, colors in category_colors_counts.items():
            if colors:
    ...
    def _generate_tracking_stats(
        ...
        # Compute current frame counts using track IDs for unique counting
        current_color_count = self.get_current_frame_color_counts(counting_summary.get("detections", []))
        per_category_count = self.get_current_frame_category_counts(counting_summary.get("detections", []))
        total_detections = self.get_current_frame_total_count(counting_summary.get("detections", []))

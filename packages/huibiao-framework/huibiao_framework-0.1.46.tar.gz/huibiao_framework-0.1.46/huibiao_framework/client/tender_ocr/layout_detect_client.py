...
from huibiao_framework.client.data_model.ocr.layout_detect_vo import DocumentLayoutDetectVo
...
class LayoutDetectionClient(HuibiaoAbstractClient):
    ...
    async def query_by_image_base64(
        self, image_base64: str, reqid: str="", session_id: str = ""
    ) -> DocumentLayoutDetectVo:
        reqid = reqid if reqid is not None else ""
        session_id = session_id if session_id is not None else reqid
        ...
        try:
            async with self.session.post(
                ModelConfig.LAYOUT_DETECTION_TYY_URL, json=payload, headers=headers
            ) as resp:
                sp_time = time.time() - start_time
                logger.debug(
                    f"{session_tag},resp-{resp.status},cost {sp_time:.2f}s"
                )
    ...
    async def query_by_image_path(self, image_path: str, reqid: str="", session_id: str = "") -> DocumentLayoutDetectVo:
        image_base64 = await ImageUtils.encode_base64(image_path)
        return await self.query_by_image_base64(image_base64=image_base64, reqid=reqid, session_id=session_id)

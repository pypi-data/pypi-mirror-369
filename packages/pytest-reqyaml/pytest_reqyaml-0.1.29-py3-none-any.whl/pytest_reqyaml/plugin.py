...
class YamlItem(pytest.Item):
    ...
    # 运行测试用例
    def runtest(self):
        json_assertion, statu_code_assertion = self.request_case.run()
        if json_assertion is not None:
            assert json_assertion
        if statu_code_assertion is not None:
            assert statu_code_assertion
...
class RequestCase:
    ...
    def run(self):
        ...
        if self.asserts is not None:
            ...
            return asser_json, res.status_code == statu_code

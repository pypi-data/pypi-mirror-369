...
class YamlItem(pytest.Item):
    class RequestCase:
        ...
        def run(self):
            ...
            if self.asserts is not None:
                ...
                logger.info(json_assertion)
                # json断言，使用simple_eval进行逻辑断言，返回真或假
                asser_json = simple_eval(json_assertion)
                assert_status = res.status_code == statu_code
                return asser_json and assert_status
            return True
    ...
    def __init__(self, request, name, parent, **kwargs):
        ...
        self.request_case = self.RequestCase(request)

    # 运行测试用例
    def runtest(self):
        success = self.request_case.run()
        assert success

...
def stream_multipart_files(requested_files: List[str],
    ...
    def generate() -> Iterator[bytes]:
        ...
        for file_path in requested_files:
            ...
            chunk_size = min(16384, file_size) if file_size < 32768 else 16384
            # Yield file content in chunks
            yield from stream_file_range(file_path, 0, file_size - 1, chunk_size)
...
def stream_file_range(file_path: str, start: int = 0, end: Optional[int] = None,
                      chunk_size: int = 16384) -> Iterator[bytes]:

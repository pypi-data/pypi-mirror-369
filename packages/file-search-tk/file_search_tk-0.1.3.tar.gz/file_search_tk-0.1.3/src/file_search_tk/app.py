...
class FileSearchApp:
    ...
    def _configure_layout(self):
        ...
        # Bottom frame layout with better spacing
    ...
    def _on_fetch_click(self):
        ...
        if not query:
            messagebox.showwarning("Input Required", "Please enter a search query to find files.")
            self.query_entry.focus()
            return
            
        if len(query) < 2:
            messagebox.showwarning("Query Too Short", "Please enter at least 2 characters for your search query.")
            self.query_entry.focus()
            return
    ...
    def _fetch_success(self, folder: Path, files: List[Path]):
        ...
        # Update search default if empty
        ...
        if files:
            ...
            messagebox.showinfo("Search Complete", 
                              f"Found {len(files)} files containing '{self.current_query}'.\n"
                              f"Select a file to view its contents.")
        else:
            self._log_message(f"No files found containing '{self.current_query}'", LogLevel.INFO)
            messagebox.showinfo("No Results", 
                              f"No files were found containing '{self.current_query}'.\n"
                              f"Try a different search term or check if the query exists in your files.")
    ...
    def _on_file_select(self, event):
        ...
        self._log_message(f"Opening file: {filename}", LogLevel.INFO)

    ...
    def handle_health_status(self) -> bool:
        ...
        try:
            ...
            if "Failed to resolve 'api.ibkr.com'" in str(e):
                _LOGGER.error(f'Connection to IBKR servers failed during reauthentication. Check internet connection between IBind and \'api.ibkr.com\'')
                return False
            elif 'An attempt was made to access a socket in a way forbidden by its access permissions' in str(e):
                _LOGGER.error(f'Connection to IBKR servers blocked during reauthentication. Check that nothing is blocking connectivity of the application')
                return False
            else:

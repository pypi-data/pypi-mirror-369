...
class AsyncDataAccess:
    ...
    @staticmethod
    def _ensure_consistent_numeric_types(df: pl.DataFrame, batch_df: pl.DataFrame) -> pl.DataFrame:
        """
        Ensure consistent data types for numeric columns to avoid type conflicts during concatenation.
        
        Args:
            df: Main DataFrame
            batch_df: Batch DataFrame to be concatenated
            
        Returns:
            batch_df with consistent numeric types
        """
        ...
        try:
            ...
            for result in sensor_results:
                if isinstance(result, pl.DataFrame) and not result.is_empty():
                    ...
                    result = self._ensure_consistent_numeric_types(df, result)
        ...
        while cursor["end"]:
            ...
            try:
                ...
                if data:
                    ...
                    # Ensure consistent data types for numeric columns
                    batch_df = self._ensure_consistent_numeric_types(df, batch_df)
    ...
    async def _fetch_sensor_time_range_data(
        ...
        while cursor["start"] and cursor["end"]:
            ...
            try:
                ...
                data = response_content["data"]
                if data:
                    batch_df = pl.DataFrame(data, strict=False)
                    # Ensure consistent data types for numeric columns
                    batch_df = self._ensure_consistent_numeric_types(df, batch_df)
                    df = pl.concat([df, batch_df])
    ...
    async def get_filtered_operation_data(
        ...
        try:
            ...
            async with self.logger.timer("Get Filtered Operation Data Query:", self.extra_params):
                while True:
                    try:
                        # Use shared connector with proper session management
                        connector = await self._get_shared_connector()
            ...
            for sensor in sensor_list:
                if df is not None:
                    ...
                    operation_val = df.filter(pl.col("sensor") == sensor)["operation"][0]
                else:
                    operation_val = operation

                info = response_data["data"].get(f"{device_id}_{sensor}_{operation_val}")
                if info:
            ...
            if value_list:
                df_result = pl.DataFrame(
                    {"sensor": retrieved_sensors, "time": time_list, "value": value_list}, strict=False
                )

...
class Module(ABC):
    ...
    def _validate_json_schema(self, input_data: dict[str, Any], schema: dict[str, Any]) -> dict[str, Any]:
        """
        ...
        for field, prop_schema in properties.items():
            if field in input_data:
                ...
                if field not in [e.split(':')[0].replace('Missing required field', '').strip() for e in errors]:
                    errors.append(f"Missing required field: {field}")
    ...
    def _validate_flat_schema(self, input_data: dict[str, Any], schema: dict[str, Any]) -> dict[str, Any]:
        """
    ...
    def handle_lambda(self, event: dict[str, Any], context: Any) -> dict[str, Any]:
        ...
        exec_context = {
            'job_id': event.get('job_id', context.request_id if hasattr(context, 'request_id') else 'local'),

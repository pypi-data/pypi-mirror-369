...
class PaymentEntryWindow(NextGenWindow):
    ...
    def select_status_in_dropdown(
        ...
        if len(status.split()) > 2:
            ...
            return self.select_status_in_dropdown_that_begin_with_and_end_with(beginning, end, pane)
        self.select_status_in_service_row(pane, status, status_key)
        return True

    def select_status_in_dropdown_that_begin_with_and_end_with(
        self, start_with: str, ends_with: str, pane: EditWrapper
    ) -> bool:
        """This method selects the status in the drop down that begins with the value provided.
        ...
        Returns:
            bool: True if the status is found and selected, False otherwise
        """
        ...
        for item in combo_box_element.descendants(control_type="ListItem"):
            ...
            if status.lower().startswith(start_with.lower()) and status.lower().endswith(ends_with.lower()):
                self.select_status_in_service_row(pane, status)
                return True
        return False
    ...
    def check_bad_debt(self, timeout: int = 2, window_title: str = "NextGen") -> bool:
        ...
        if bad_debt_text.exists(timeout=timeout, retry_interval=0.01):
            alert_window.child_window(title="OK", control_type="Button").click_input()
            balance_window = self.window.child_window(title="BalanceControl", control_type="Window").wait("visible", 10)
            balance_window.child_window(title="cmdCancel", control_type="Button").click_input()
            return True
        return False

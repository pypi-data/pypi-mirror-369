...
class JianyingController:
    ...
    def export_draft(self, draft_name: str, output_path: Optional[str] = None, *,
        ...
        """导出指定的CapCut草稿, **目前仅支持CapCut6及以下版本**

        **注意: 需要确认有导出草稿的权限(不使用VIP功能或已开通VIP), 否则可能陷入死循环**

        Args:
            draft_name (`str`): 要导出的CapCut草稿名称
            output_path (`str`, optional): 导出路径, 支持指向文件夹或直接指向文件, 不指定则使用CapCut默认路径.
            resolution (`Export_resolution`, optional): 导出分辨率, 默认不改变CapCut导出窗口中的设置.
            framerate (`Export_framerate`, optional): 导出帧率, 默认不改变CapCut导出窗口中的设置.
            timeout (`float`, optional): 导出超时时间(秒), 默认为20分钟.

        Raises:
            `DraftNotFound`: 未找到指定名称的CapCut草稿
            `AutomationError`: CapCut操作失败
        """
        ...
        if not draft_name_text.Exists(0):
            raise exceptions.DraftNotFound(f"未找到名为{draft_name}的CapCut草稿")
    ...
    def get_window(self) -> None:
        ...
        if not self.app.Exists(0):
            raise AutomationError("CapCut窗口未找到")

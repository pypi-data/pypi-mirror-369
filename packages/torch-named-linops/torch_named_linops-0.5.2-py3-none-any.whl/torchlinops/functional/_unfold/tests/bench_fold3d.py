...
def main():
    ...
    # Test correctness
    ishape = (N, *nblocks, *block_dim)
    x = torch.arange(prod(ishape), device=device).reshape(ishape)
    Bx_triton = fold(x, im_size, block_dim, stride, mask)
    Bx_sp = sp.blocks_to_array(from_pytorch(x), (N, *im_size), block_dim, stride)
    assert torch.allclose(Bx_triton, to_pytorch(Bx_sp), rtol=1e-2)

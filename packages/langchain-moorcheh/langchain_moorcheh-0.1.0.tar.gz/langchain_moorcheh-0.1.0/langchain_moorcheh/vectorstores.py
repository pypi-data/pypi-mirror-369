...
class MoorchehVectorStore(VectorStore):
        ...
        **kwargs: Any,
        ...
        try:
            ...
            for result in results:
                page_content = result.get("text") or ""
   ...
   #Similarity_search with score method
    ...
    def get_by_ids(self, ids: List[str]) -> List[Document]:
        ...
        try:
            resp = self._client.get_documents(namespace_name=self.namespace, ids=ids)
            ...
            # Preserve input order using the **Document** objects
            by_id = {d.id: d for d in docs if d.id is not None}
            return [by_id[i] for i in ids if i in by_id]

        except APIError as e:
            # 207 multi-status or "missing" case → just return empty

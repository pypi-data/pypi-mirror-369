...
from material.FormatHandler import FormatHandler

class Normalizer(FormatHandler):
  ...
  def _set_system_fileds(self,entity):
    ...
    entity['mat_chan'] = config.get('chan','article') # article gallery shortvideo qa
    entity['mat_lang'] = config.get('lang','cn') # cn en
    entity['mat_id'] = BluesAlgorithm.md5(entity['mat_url'])
    entity['mat_site'] = BluesURL.get_main_domain(entity['mat_url']) # ifeng bbc

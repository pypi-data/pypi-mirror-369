...
class Image():
  ...
  def download_image(self,image_CS_WE,image_dir=None,parent_CS_WE=None,timeout=None):
    '''
    Returns:
    ...
    urls = self.get_img_urls(image_CS_WEs,parent_CS_WE,timeout)
    if urls:
  ...
  def get_img_urls(self,image_CS_WE,parent_CS_WE=None,timeout=None):
    '''
    ...
    for image_CS_WE in image_CS_WEs:
      if not image_CS_WE:
        continue
      current_urls = self.__get_img_urls(image_CS_WE,parent_CS_WE,timeout)
      if current_urls:
        urls.extend(current_urls)
    return urls

  def __get_img_urls(self,target_CS_WE,parent_CS_WE=None,timeout=None):
    '''

...
class RoboTooter:
    ...
    def register_plugin(self, plugin_module: str) -> None:
        ...
        if hasattr(plugin_klass, "plugin_filters"):
            has_something = True
            self.config.plugin_filters.extend(plugin_klass.plugin_filters)
        if hasattr(plugin_klass, "plugin_bots"):
            has_something = True
            self.config.plugin_bots.extend(plugin_klass.plugin_bots)
    ...
    def unregister_plugin(self, plugin_module: str) -> None:
        ...
        if hasattr(plugin_klass, "plugin_filters"):
            has_something = True
            for filter_name in plugin_klass.plugin_filters:
                if filter_name in self.config.plugin_filters:
                    del self.config.plugin_filters[filter_name]
                    has_something = True
        if hasattr(plugin_klass, "plugin_bots"):
            has_something = True
            for plugin_bot in plugin_klass.plugin_bots:
                if plugin_bot in self.config.plugin_bots:
                    del self.config.plugin_bots[plugin_bot]
                    has_something = True
        if has_something:
            self.save_configuration()

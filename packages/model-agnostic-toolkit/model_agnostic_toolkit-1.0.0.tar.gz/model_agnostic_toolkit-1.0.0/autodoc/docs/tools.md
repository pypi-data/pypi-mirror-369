...
All tools inherit from the `Tool` class. One or more objects of this class are passed to an `analyzer` and from there on used for computing feature importance or interaction results with different `datasets` and `models`.

**→ [Find a short intro on tools here](getting_started.md#tools).**

In the following, see how to work with tools directly and find out about all available tools.

## Working with Tools

### Creating Tools

To create a `Tool` object you just have to instantiate any of the derived specific tool implementations.
```python
pfi_tool = PFIImportance()
```

> **Hint**: The classes `Tool`, `ImportanceTool` and `InteractionTool` themselves are abstract classes and cannot be instantiated directly.
...
Multiple tools are usually passed as a list to an `analyzer` upon its initialization and managed through it from there on.
...
`Tool` objects are usually passed to an `analyzer` during its initialization and run through it when running an analysis. It also provides functionality for plotting results. Outside of an analyzer, however, a tool's running and plotting methods can be accessed as well.

To compute a `tool`'s results for a given `dataset` `data`, `model` `model` and specified `features`, call its `run(...)` method. This returns the corresponding result dictionary.
```python
...
> **Hint**: Learn about more available plots and customization in the chapter on plotting.
...
The following methods are available for all objects of the `Tool` class and inheriting classes.
...
> ```python
> from model_agnostic_toolkit.tools import PDPImportance, HStatisticInteraction, LIME
> ```

A detailed information on available tools is available at: [Tool documentation](../model_agnostic_toolkit.tools)
...
- [`PFIImportance`](../model_agnostic_toolkit.tools.pfi)
...
- [`SHAPImportance`](../model_agnostic_toolkit.tools.shap)
...
- [`VIPImportance`](../model_agnostic_toolkit.tools.vip)
...
- [`ALEImportance`](../model_agnostic_toolkit.tools.ale)
...
- [`PDPImportance`](../model_agnostic_toolkit.tools.pdp)
...
- [`HStatisticInteraction`](../model_agnostic_toolkit.tools.h_statistic)
...
- [`VIPInteraction`](../model_agnostic_toolkit.tools.vip)
...
- [`ALEInteraction`](../model_agnostic_toolkit.tools.ale)
...
- [`PDPInteraction`](../model_agnostic_toolkit.tools.pdp)

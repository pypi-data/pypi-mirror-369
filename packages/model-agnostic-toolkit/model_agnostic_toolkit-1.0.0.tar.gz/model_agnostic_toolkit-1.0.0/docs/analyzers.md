...
> Follow the embedded links (e.g. [`Analyzer`]) to take a look.
...
In the following sections, see how to [work with analyzers](#working-with-analyzers) and find out about [available types of analyzers](#available-analyzers).
...
```python
# not recommended!
ana = Analyzer(tool_type=ToolType.IMPORTANCE, model=model, dataset=data)
```
...
Having initialized an [`Analyzer`] object, you can start an analysis simply by calling its [`run_analysis(...)`] method. You just need to provide the `features` parameter, a list of features (for importance analyis or individual analysis) or feature pairs (for interaction analysis) of your `data` that should be taken into account for the analysis, e.g. all of them. The [`run_analysis(...)`] method returns a dictionary with complete results for all tools, indexed by the tools' names, and results regarding the model's performance. These results will also be stored by the analyzer itself.
```python
...
Alternatively, the [results can be accessed](#accessing-results) individually for each tool and passed to the respective tool's [`plot(...)`](tools.md#running-and-plotting) method. To obtain more different plots and get more customization options, use any method from the [plotting module](plotting.md) to obtain a figure.
...
Some tools internally depend on the results of other tools (e.g. the [`VIPInteraction`] depends on [`PDPInteraction`] results). If both tools are part of the same analyzer, the PDP results are automatically reused for the VIP analysis. The order the tools are run in does not matter.
```python
...
[`Analyzer`]: ../model_agnostic_toolkit/analyzer.py#L14
[`ImportanceAnalyzer`]: ../model_agnostic_toolkit/analyzer.py#L541
[`InteractionAnalyzer`]: ../model_agnostic_toolkit/analyzer.py#L550
...
[`add_reuse_analyzers(...)`]: ../model_agnostic_toolkit/analyzer.py#L111
[`add_tools(...)`]: ../model_agnostic_toolkit/analyzer.py#L65
[`evaluate_model(...)`]: ../model_agnostic_toolkit/analyzer.py#L155
[`fit_model(...)`]: ../model_agnostic_toolkit/analyzer.py#L137
[`plot(...)`]: ../model_agnostic_toolkit/tools/tool.py#L72
[`plot_results(...)`]: ../model_agnostic_toolkit/analyzer.py#L273
[`export_plots(...)`]: ../model_agnostic_toolkit/analyzer.py#L315
[`remove_tools(...)`]: ../model_agnostic_toolkit/analyzer.py#L93
[`result_exists(...)`]: ../model_agnostic_toolkit/analyzer.py#L348
[`run_analysis(...)`]: ../model_agnostic_toolkit/analyzer.py#L242
[`run_tool(...)`]: ../model_agnostic_toolkit/analyzer.py#L172

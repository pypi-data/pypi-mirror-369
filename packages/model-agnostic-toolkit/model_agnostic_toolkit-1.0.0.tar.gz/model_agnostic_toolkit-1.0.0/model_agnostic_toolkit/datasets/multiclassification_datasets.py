...
class MultiClassificationOneDataset(ArtificialDataset):
    """Classification One Dataset containing random base features and artificial interactions for a classification task, generated synthetically.
    Consists of twenty observed random base features with no hidden interactions.
    """

    def __init__(
            self,
            name: str = "Classification One",
            file: str = "./data/classification_one_dataset.hdf5",
            n: int = 1000,
            test_size: float = 0.2,
        ...
        """Initializes Classification One dataset.py.

        Args:
            name (str, optional): Name of dataset.py instance. Defaults to 'Classification One'.
            file (str, optional): Relative or absolute path to dataset.py file. If a dataset.py exists here already and is loaded, the following arguments are ignored. Defaults to './data/classification_one_dataset.hdf5'.
    ...
    def _generate(self) -> tuple[pd.DataFrame, pd.Series]:
        """Generates artificial Classification One dataset.py with twenty random base features and no artificial interaction for a classification task.
        ...
        2. A target is computed as `y = w * [b0 b1 b2 b3 b4 b5 b6 b7 b8 b9 ]ᵀ + N(0,s)` with weights `w = [w0 w1 w2 w3 w4 w5 w6w w7 w8 w9]`.
           and afterwards classified: y < 3: 'one', y >= 3: 'two'.
        Returns:
        ...
        # make classification task of regression problem
        target[target < 1.5] = 0
        target[1.5 <= target <= 3] = 1
        target[target > 3] = 2
...
class MultiClassificationTwoDataset(ArtificialDataset):
    """Classification Two Dataset containing random base features and artificial interactions for a classification task, generated synthetically.
    Consists of twenty observed random base features with ten hidden interactions.
    """

    def __init__(
            self,
            name: str = "Classification Two",
            file: str = "./data/multiclassification_two_dataset.hdf5",
            n: int = 1000,
            test_size: float = 0.2,
            weights: list[float] = None,
            noise_variance: float = 0.1,
        ...
        Args:
            name (str, optional): Name of dataset instance. Defaults to 'Classification Two'.
            file (str, optional): Relative or absolute path to dataset file. If a dataset exists here already and is loaded, the following arguments are ignored. Defaults to './data/classification_two_dataset.hdf5'.
            n (int, optional): Number of samples to create when generating new data. Defaults to 1000.
            test_size (float, optional): Proportion of all samples used for the testing data. Defaults to 0.2.
    ...
    def _generate(self) -> tuple[pd.DataFrame, pd.Series]:
        """Generates artificial Classification Two dataset with twenty random base features and ten artificial interaction for a classification task.
        Data is generated as follows:
        ...
        and afterwards classified: y < 2: 'one', y >= 2: 'two'.
        ...
        # make classification task of regression problem
        target[target < 1.5] = 0
        target[np.bitwise_and(target >= 1.5, target <= 3)] = 1
        target[target > 3] = 2
...
class MultiClassificationThreeDataset(ArtificialDataset):
    """Classification Three Dataset containing random base features and artificial interactions for a classification task, generated synthetically.
    Consists of twenty observed random base features with five hidden interactions.
    """

    def __init__(
            self,
            name: str = "Classification Three",
            file: str = "./data/classification_three_dataset.hdf5",
            n: int = 1000,
            test_size: float = 0.2,
            weights: list[float] = None,
            noise_variance: float = 0.1,
        ...
        Args:
            name (str, optional): Name of dataset instance. Defaults to 'Classification Three'.
            file (str, optional): Relative or absolute path to dataset file. If a dataset exists here already and is loaded, the following arguments are ignored. Defaults to './data/classification_three_dataset.hdf5'.
            n (int, optional): Number of samples to create when generating new data. Defaults to 1000.
            test_size (float, optional): Proportion of all samples used for the testing data. Defaults to 0.2.
    ...
    def _generate(self) -> tuple[pd.DataFrame, pd.Series]:
        """Generates artificial Classification Three dataset with twenty random base features and 5 artificial interactions for a classification task.
        Data is generated as follows:
        ...
        and afterwards classified: y < 2.5: 'one', y >= 2.5: 'two'.
...
class MultiClassificationFourDataset(ArtificialDataset):
    """Classification Four Dataset containing random base features and no artificial interactions for a Classification task, generated synthetically.
    Consists of twenty observed random base features with no hidden interactions.
    """

    def __init__(
            self,
            name: str = "Classification Four",
            file: str = "./data/classification_four_dataset.hdf5",
            n: int = 1000,
            test_size: float = 0.2,
            noise_variance: float = 0.1,
            force: bool = False,
        ...
        Args:
            name (str, optional): Name of dataset.py instance. Defaults to 'Classification Four'.
            file (str, optional): Relative or absolute path to dataset.py file. If a dataset.py exists here already and is loaded, the following arguments are ignored. Defaults to './data/classification_four_dataset.hdf5'.
            n (int, optional): Number of samples to create when generating new data. Defaults to 1000.
            test_size (float, optional): Proportion of all samples used for the testing data. Defaults to 0.2.
            noise_variance (float, optional): Variance of Gaussian noise that is added to every base of interaction feature and to the target. Defaults to 0.1.
            force (bool, optional): Whether to force the generation of a new dataset.py and overwrite a possibly existing one in `file`. Defaults to False.
    ...
    def _generate(self) -> tuple[pd.DataFrame, pd.Series]:
        """Generates artificial Classification Four dataset.py with twenty random base features and no artificial interaction for a classification task.
        The target is independent of the random base features.
        Data is generated as follows:

        1. Twenty random base features `b0` to `b19` are sampled i.i.d. from `U(0,1) + N(0,s)` with variance `s` (sigma) for the Gaussian noise.

        2. A target is computed as `N(0,s)` and afterwards classified: y < 0: 'one', y >= 0: 'two'.

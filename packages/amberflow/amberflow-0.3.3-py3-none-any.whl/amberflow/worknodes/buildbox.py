...
from amberflow.artifacts.structure import BinderLigandPDB, BaseComplexStructureFile, get_biomolecule
...
"""
        ...
        **kwargs,
        ...
        for sysname in self.input_artifacts.keys():
            ...
            self.rejoin_with_box(
                self.complexes[sysname], boxed_complexes, self.solvent_selection, biomolecule, complex_out_filepath
            )

            binder_out_filepath = self.out_dirs[sysname] / f"binder_{sysname}.pdb"
            self.rejoin_with_box(
                self.binders[sysname], boxed_binders, self.solvent_selection, biomolecule, binder_out_filepath
            )
    ...
    def rejoin_with_box(
        self, sys_atoms: AtomGroup, box_filepath: Path, box_selection: str, biomolecule: str, out_filepath: Path
    ):
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
                ...
                for art in arts:
                    if isinstance(art, BaseComplexStructureFile):
                        ...
                        with warnings.catch_warnings():
                            warnings.simplefilter("ignore")
                            lig.write(lig_pdb)
                        oriented_pdb = self.center_align_principal_axes(lig_pdb, orient_dir)

                        # set a different resnum for each binder, so that tleap can handle them correctly
                        with warnings.catch_warnings():
                            warnings.simplefilter("ignore")
                            u = Universe(oriented_pdb, in_memory=True)
                            u.atoms.residues.resnums = i
                            u.atoms.residues.resids = i
                            i += 1
                            u.atoms.write(oriented_pdb)
                        centered_aligned_binders[sysname] = oriented_pdb
    ...
    def write_superuniverses(
        ...
        for sysname, pdb in complexes.items():
            ...
            self.complexes_binders[sysname] = u.select_atoms(f"resname {self.resname}")

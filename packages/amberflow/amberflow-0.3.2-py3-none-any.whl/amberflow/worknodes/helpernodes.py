...
from amberflow.artifacts import (BaseArtifact, ArtifactContainer, BaseTargetStructureFile, BinderLigandPDB,
                                 BaseBinderStructureFile, ArtifactRegistry, BaseStructureFile)
...
class WorkNodeFilter(BaseSingleWorkNode):
    ...
    def _run(
        ...
        if len(output_artifacts) == 0:
            err_msg = f"No artifacts of type {self.artifact_types} found in input artifacts."
...
class JoinTargetBinder(BaseSingleWorkNode):
    ...
    def _run(
        ...
        file_artifact = ArtifactRegistry.create_instance_by_filename(out_pdb,
                                                                     tags=self.tags[tar_type] + self.tags[bin_type])
...
class AddChainid(BaseSingleWorkNode):
    ...
    def fill_output_artifacts(self, output_dir: dirpath_t, sysname: str) -> ArtifactContainer:
        output_artifacts = [
            ArtifactRegistry.create_instance_by_name(art.__class__.__name__, output_dir / art.filepath.name) for artifacts in
            self.input_artifacts.values() for art in artifacts]

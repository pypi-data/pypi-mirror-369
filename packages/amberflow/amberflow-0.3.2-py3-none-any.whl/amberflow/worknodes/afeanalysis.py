...
from amberflow.artifacts import (
    MdoutStates,
    BoreschRestraints,
    Remlog, EdgeMBARxml, Datdir, TargetDatdir, ReferenceDatdir, ArtifactContainer, EdgeMBARhtml
)
...
class Amber2Dats(BaseAnalysis):
    ...
    def _run(
        ...
        trial_dir = self.datdir_type(self.work_dir, edge=edge, environment=self.environment, stage=self.stage,
                                     trial=self.trial,
                                     states=mdouts.states.keys(), makedir=True)
        if self.try_and_skip(sysname=sysname, trial_dir=trial_dir):
            return self.output_artifacts

        # Restraints
        boresch_file = (
            Path(self.input_artifacts["BoreschRestraints"])
    ...
    def try_and_skip(self, sysname: str, trial_dir: Datdir) -> bool:
        if self.skippable:
            try:
                self.output_artifacts = self.fill_output_artifacts(sysname=sysname, trial_dir=trial_dir)
...
class GetXML(BaseAnalysis):
    ...
    def _run(
        ...
        if self.try_and_skip(sysname=sysname, edges_xml=xml_path):
            return self.output_artifacts
    ...
    # noinspection DuplicatedCode
    def try_and_skip(self, sysname: str, edges_xml: Path) -> bool:
        if self.skippable:
            try:
...
class EdgeMBAR(BaseAnalysis):
    ...
    def _run(
        ...
        if self.try_and_skip(sysname=sysname, out_html=out_html):
            return self.output_artifacts
    ...
    # noinspection DuplicatedCode
    def try_and_skip(self, sysname: str, out_html: Path) -> bool:
        if self.skippable:
            try:
                self.output_artifacts = self.fill_output_artifacts(sysname=sysname, out_html=out_html)
                self.node_logger.info(f"Skipped {self.id} WorkNode.")

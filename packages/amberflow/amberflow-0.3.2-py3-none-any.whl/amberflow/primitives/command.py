...
from amberflow.primitives import (
    ...
    filepath_t,
    RemoteExecutor,
    LocalExecutor, SLURMError, DataMover,
)
...
class BaseCommand(metaclass=CommandRegistryMeta):
    ...
    def __init__(self, remote_server: Optional[str] = None, remote_base_dir: Optional[str] = None,
                 local_base_dir: Optional[dirpath_t] = None, data_mover: Optional[DataMover] = None, **kwargs: Any):
        ...
        if is_remote:
            executor = RemoteExecutor(remote_server, remote_base_dir, local_base_dir, data_mover=data_mover)
        else:
    ...
    def replace(self, **changes: Any) -> "BaseCommand":
        ...
        for key, value in changes.items():
            if key not in current_args:
                raise TypeError(
                    f"'{key}' is not a valid keyword argument for {self.__class__.__name__}"
                )
            current_args[key] = value

        return self.__class__(**current_args)

    # --- End: Immutability Additions ---

    def run(self, cmd: list[str], *args, cwd: filepath_t, logger: Logger, walltime_mins: Optional[int] = None,
            force: bool = False,
            **kwargs) -> None:
...
class DefaultCommand(BaseCommand):
    ...
    def run(
        ...
        # we're not yet using the `force` or `walltime_mins` parameters here,
        self.executor.run(cmd, cwd, logger, **kwargs)
        if expected:
...
class SLURMScript:
    ...
    def write_slurm_script(self, cmd: list[str], cwd: dirpath_t,
                           sbatch_params: Optional[dict[str, Any]] = None) -> tuple[Path, str]:
...
class SLURMCommand(BaseCommand):
    ...
    _TERMINAL_STATES: Final[tuple[str]] = ("CD", "COMPLETED", "CG", "F", "FAILED", "CA", "CANCELLED", "TO", "TIMEOUT",
                                           "NF", "NODE_FAIL")
    ...
    def __init__(
        ...
        if script_generator is None:
            ...
            assert isinstance(script_generator,
                              SLURMScript), f"Expected 'script_generator' to be an instance of SLURMScript, got {type(script_generator)}"
    ...
    def run(
        ...
        if state is None or force:
            ...
            p = self.executor.run(["sbatch", script_path.name], cwd, logger, download=False, **kwargs)
    ...
    def _check_job_status(self, job_state_pkl: filepath_t, cwd: dirpath_t, logger: Logger, update: bool = True) -> dict[
        str, str]:
        state = self._read_job_state(job_state_pkl, logger)
        # Set check=False to avoid raising an error if the job is not found
        p = self.executor.run(["squeue", f"-j {state['jobid']}", "-h", "-o", "%t"], cwd, logger, check=False,
                              download=False,
                              upload=False)
        if p.returncode == 0:
    ...
    def _wait_for_job_completion(self, job_state_pkl: filepath_t, cwd: dirpath_t, logger: Logger,
                                 walltime_mins: Optional[int] = None) -> None:
        ...
        while True:
            ...
            if time.monotonic() - start_time > timeout_seconds:
                ...
                self.executor.run(f"scancel {state['jobid']}", cwd, logger, download=False, upload=False)
                logger.error(err_msg)

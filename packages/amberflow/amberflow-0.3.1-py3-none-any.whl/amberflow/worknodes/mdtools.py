...
from amberflow.artifacts.topology import BaseTopologyFile
...
class HMR(BaseSingleWorkNode):
    ...
    def _run(
        ...
        **kwargs,
        ...
        if self._try_and_skip(sysname, in_top.name, self.tags[top_typename]):
        ...
        self.output_artifacts = self.fill_output_artifacts(sysname, in_top.name, self.tags[top_typename])
        return self.output_artifacts

    def _try_and_skip(self, sysname: str, outname: str, tags: tuple[str, ...]) -> bool:
        if self.skippable:
            try:
                self.output_artifacts = self.fill_output_artifacts(sysname, outname, tags)

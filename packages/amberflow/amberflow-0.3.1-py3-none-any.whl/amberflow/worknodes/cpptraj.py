...
from amberflow.primitives import DEFAULT_RESOURCES_PATH, dirpath_t, filepath_t, BaseCommand
from amberflow.worknodes import noderesource, worknodehelper, BaseSingleWorkNode, check_cpp_log
...
class CpptrajAnalysis(BaseAnalysis):
...
class ImageFit(CpptrajAnalysis):
    def __init__(
        self,
        wnid: str,
        *args,
    ...
    def _run(
        ...
        **kwargs,
        ...
        if self._try_and_skip(out_top, out_trj, sysname):
        ...
        self.output_artifacts = self.fill_output_artifacts(out_top, out_trj, sysname)
    ...
    def fill_output_artifacts(self, out_top: filepath_t, out_trj: filepath_t, sysname: str) -> ArtifactContainer:
    ...
    def _try_and_skip(self, out_top: filepath_t, out_trj: filepath_t, sysname: str) -> bool:
        if self.skippable:
            try:
                self.output_artifacts = self.fill_output_artifacts(out_top, out_trj, sysname)
...
class CpptrajDefault(CpptrajAnalysis):
    def __init__(
        self,
        wnid: str,
        *args,
    ...
    def _run(
        ...
        **kwargs,
        ...
        out_top = self.work_dir / in_top.name
        shutil.copy(in_top, out_top)
        self.output_artifacts = self.fill_output_artifacts(out_top, sysname)
    ...
    def fill_output_artifacts(self, out_top: filepath_t, sysname: str) -> ArtifactContainer:
        return ArtifactContainer(
            sysname,
            (
                ArtifactRegistry.create_instance_by_filename(

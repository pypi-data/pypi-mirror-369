...
"""
    ...
    def _run(
        ...
        biomolecule = self.get_biomolecule(self.input_artifacts, self.node_logger)
        ...
        self.write_superuniverses(centered_aligned_complexes, centered_aligned_binders, all_binders_filepath,
        ...
        for sysname in self.input_artifacts.keys():
            ...
            self.rejoin_with_box(self.complexes[sysname], boxed_complexes, self.solvent_selection, biomolecule,
                                 complex_out_filepath)

            binder_out_filepath = self.out_dirs[sysname] / f"binder_{sysname}.pdb"
            self.rejoin_with_box(self.binders[sysname], boxed_binders, self.solvent_selection, biomolecule,
                                 binder_out_filepath)
    ...
    def merge_without_clash(
        ...
        node_logger.debug(f"Found {len(extra_solute_atoms)} atoms in the solute that are not part of the {biomolecule=}")
    ...
    def rejoin_with_box(self, sys_atoms: AtomGroup, box_filepath: Path, box_selection: str, biomolecule: str,
                        out_filepath: Path):
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
    ...
    def center_align_complexes_and_binders(self, input_artifacts: BatchArtifacts, biomolecule: str) -> tuple[
            ...
            for arts in art_container.values():
                for art in arts:
                    if isinstance(art, BaseComplexStructureFile):
                        ...
                        centered_aligned_complexes[sysname] = self.center_align_principal_axes(art.filepath, orient_dir,
                                                                                               neutralize_first)
    ...
    @staticmethod
    def get_biomolecule(input_artifacts: BatchArtifacts, node_logger: Logger) -> str:
        ...
        if "nucleic" in flattened_tags:
            if "protein" in flattened_tags:
                err_msg = f"Invalid system. Found both nucleic and protein tags ({tags}) in {some_art_container}"
                node_logger.error(err_msg)
                raise ValueError(err_msg)
            else:
                return "nucleic"
            ...
            err_msg = f"Could not determine biomolecule type from tags ({tags}) in {some_art_container}"
            node_logger.error(err_msg)
            raise ValueError(err_msg)

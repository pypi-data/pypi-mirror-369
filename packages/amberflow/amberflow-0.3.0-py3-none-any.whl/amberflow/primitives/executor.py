...
__all__ = ("Executor", "LocalExecutor", "RemoteExecutor")
...
class Executor(ABC):
    """An abstract base class for an immutable command execution context.
        ...
        Attributes:
            remote_server (Optional[str]): The hostname or IP address of a remote server.
            remote_base_dir (Optional[pathlib.Path]): The base working directory on the
                remote machine.
            local_base_dir (Optional[pathlib.Path]): The base working directory on the
                local machine.
        """
    ...
    def replace(self, **changes: Any) -> "Executor":
        """
        ...
        current_args = {
            slot: getattr(self, slot) for slot in constructor_slots if hasattr(self, slot)
        }
    ...
    @staticmethod
    def _run_command(command: str, cwd: dirpath_t, logger: Logger, env: Optional[dict] = None,
                     check: bool = True) -> sp.CompletedProcess:
        try:
            p = sp.run(command, stdout=sp.PIPE, stderr=sp.PIPE, text=True, cwd=str(cwd), env=env, check=check,
                       shell=True)
            if p.stdout:
...
class LocalExecutor(Executor):
    ...
    def run(
        ...
        return super()._run_command(cmd_str, cwd, logger, check=True, env=env)
...
class RemoteExecutor(Executor):
    ...
    initialized: bool
    __slots__ = Executor.__slots__ + ("remote_server", "remote_base_dir", "local_base_dir")
    # __slots__ = ("remote_server", "remote_base_dir", "local_base_dir", "initialized")

    def __init__(self, remote_server: Optional[str] = None, remote_base_dir: Optional[dirpath_t] = None,
    ...
    def run(
        ...
        if kwargs.get("upload", True):
            ...
            logger.info(f"Syncing files from {cwd} to {self.remote_server}:{remote_cwd}")
        ...
        if kwargs.get("download", True):
            logger.info(f"Syncing results from {self.remote_server}:{remote_cwd} back to {cwd}")
    ...
    def run_remote_command(self, command: str, cwd: dirpath_t, logger: Logger,
                           check: bool = True) -> sp.CompletedProcess:
        cmd = f'ssh {self.remote_server} "{command}"'
        logger.debug(f"Running remote command:\n{cmd}")
        return super()._run_command(cmd, cwd, logger, check=check)

    def rsync(self, src: dirpath_t, dest: dirpath_t, cwd: dirpath_t, logger: Logger,
        ...
        return super()._run_command(cmd_str, cwd, logger, check=True)
    ...
    def download(self, local_filepath: Union[dirpath_t, filepath_t], logger: Logger) -> None:
        ...
        super()._run_command(cmd_str, cwd, logger, check=True)

...
class BaseLambdaMDWorkNode(BaseMDWorkNode):
    ...
    def try_and_skip(
            ...
            sysname: str,
            reference_rst7: Optional[Path] = None,
    ...
    def fill_output_artifacts(
        ...
        if len(rst7_states) != len(schdl):
            raise ValueError(f"Expected {len(schdl)} states but found {len(rst7_states)}. ")

        artifacts = [
            self.artifact_builder["BaseTopologyFile"](out_top),
            rst7_states,
            MdoutStates(out_first_mdout),
            schdl,
        ]
        if out_boresch is not None:
            artifacts.append(BoreschRestraints(out_boresch))
        if reference_rst7 is not None:
            artifacts.append(self.artifact_builder["BaseStructureReferenceFile"](reference_rst7))
...
class LambdaMDRun(BaseLambdaMDWorkNode):
    ...
    def _run(
        ...
        ref_rst7 = (
            self.work_dir / Path(self.input_artifacts["BaseStructureReferenceFile"]).name
            if "BaseStructureReferenceFile" in self.input_artifacts
            else None
        )
        ...
        elif len(schdl) > self.min_cores:
            raise ValueError(
                f"Not enough cores. Number of cores: {self.min_cores}, number of input states: {len(in_states_rst7)}, "
                f"number of lambda values: {len(schdl)}."
            )
        ...
        if ref_rst7 is not None:
            # Again, copy the reference to the working dir
            in_ref = Path(self.input_artifacts["BaseStructureReferenceFile"])
            shutil.copy(in_ref, ref_rst7)
        ...
        for clambda, state_rst7 in zip(schdl.formatted(), in_states_rst7.values()):
            ...
            if self.ifmbar:
                ...
                mbar_lambdas = [f"mbar_lambda({i}) = {l}" for i, l in enumerate(schdl.formatted(), 1)]
            ...
            shutil.copy(state_rst7, self.work_dir / in_rst7_name)
            ...
            ref = ref_rst7.name if ref_rst7 is not None else in_rst7_name
        ...
        if lambda_sch is not None:
            ...
            for group_line in groupfile_lines:
                group_line += f" -lambda_sch {lambda_sch.name}"
...
class QuickLambdaSchedule(BaseSingleWorkNode):
    ...
    def inverse_s2_interpolation(self, y_values, x_min: float = 0., x_max: float = 1., kind: Literal["cubic"] = "cubic"):

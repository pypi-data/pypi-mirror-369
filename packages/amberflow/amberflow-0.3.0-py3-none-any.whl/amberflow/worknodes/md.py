...
from amberflow.worknodes import worknodehelper, BaseSingleWorkNode, noderesource
...
class BaseMDWorkNode(BaseSingleWorkNode):
    ...
    SUPPORTS: dict[str, Sequence[Union[int, str]]] = {
        "ntmin": (0, 1, 2),
        "engine": ("pmemd", "pmemd.cuda", "pmemd.MPI", "pmemd.cuda.MPI", "sander"),  # Base engines
    ...
    def __init__(
        ...
        self.engine = self.check_supported(engine, "engine")
        ...
        if self.min_cores > 1:
            ...
            self.engine = f"mpirun -np {self.min_cores} " + self.engine
    ...
    def _try_and_skip(self, sysname: str, prefix: str, tags: tuple[str, ...]) -> bool:
        if self.skippable:
            try:
                self.output_artifacts = self._fill_output_artifacts(
                    self.work_dir, prefix=prefix, tags=tags, sysname=sysname
                )
    ...
    def _fill_output_artifacts(
            self, output_dir: dirpath_t, prefix: str, tags: tuple[str, ...], sysname: str,
    ) -> ArtifactContainer:
        ...
        try:
            art_ctner = ArtifactContainer(
                ...
                (
                    ...
                    ArtifactRegistry.create_instance_by_filename(mdout, tags=tuple()),
                ),
            )
    ...
    def _write_mdin(self, output_dir: dirpath_t, sysname: str) -> Path:
        if self.mdin is None:
            err_msg = f"Bad MDWorkNode. '.mdin' contents were not generated."
...
@worknodehelper(
    ...
    output_artifact_types=(BaseStructureFile, BaseTopologyFile, BaseTrajectoryFile, MdoutMD),
)
class MDRun(BaseMDWorkNode):
    ...
    def _run(
        ...
        if super()._try_and_skip(sysname, prefix, tags):
        ...
        sbatch_params = {"ntasks-per-node": self.cores} if sbatch_params is None else sbatch_params | {
            "ntasks-per-node": self.cores}

        mdin_file = self.write_mdin(self.work_dir, sysname)
        self._run_md(
            cwd=self.work_dir,
            sysname=sysname,
            i=mdin_file,
            ...
            ref=Path(self.input_artifacts["BaseStructureFile"]),
            output_name_template=f"{prefix}_{sysname}",
            gpus=gpus,
            sbatch_params=sbatch_params,
        )

        self.output_artifacts = self._fill_output_artifacts(self.work_dir, prefix=prefix, tags=tags, sysname=sysname)

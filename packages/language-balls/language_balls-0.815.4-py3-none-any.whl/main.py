def main():
    ...
    # Try to find logos.jpeg in various locations
    logo_path = None
    search_paths = [
        # Current directory
        "logos.jpeg",
        # Relative to module
        os.path.join(os.path.dirname(__file__), "logos.jpeg"),
        # In site-packages data directories with glob pattern
    ]
    
    # Add site-packages paths with wildcard version
    try:
        import site
        for site_dir in site.getsitepackages():
            search_paths.extend(glob.glob(os.path.join(site_dir, "language_balls-*.data", "data", "logos.jpeg")))
    except:
        pass
    ...
    if not logo_path:
        print("Error: Could not find logos.jpeg")
        return
        
    try:
        width, height, channels, data = dpg.load_image(logo_path)
        if data is None:
            raise FileNotFoundError(f"Could not load image from {logo_path}")
    except Exception as e:
        print(f"Error loading logos.jpeg: {e}")
        return

    ...
    def receive_full_response(self, sock, buffer_size=4096) -> bytes:
        ...
        try:
            while True:
                ...
                # Join all chunks and try to decode/parse only occasionally
                # to avoid expensive operations on every chunk
                if len(chunks) % 3 == 0 or len(chunk) < buffer_size:  # Every 3 chunks or when chunk is smaller (likely last chunk)
                    data = b''.join(chunks)
                    try:
                        ...
                        # UTF-8 character split across chunks, continue reading
                        logger.debug(f"UTF-8 decode error, continuing to read more data...")
                        continue
                    except json.JSONDecodeError:
                        # Not complete JSON yet, continue reading
                        logger.debug(f"JSON incomplete, continuing to read more data...")
                        continue
                    except Exception as e:
                        logger.warning(f"Error processing response chunk: {str(e)}")
                        continue
            
            # Final attempt with all data
            if chunks:
                data = b''.join(chunks)
                decoded_data = data.decode('utf-8')
            ...
            if chunks:
                ...
                try:
                    ...
                    logger.error(f"Received {len(data)} bytes but could not parse as valid JSON")
                    pass

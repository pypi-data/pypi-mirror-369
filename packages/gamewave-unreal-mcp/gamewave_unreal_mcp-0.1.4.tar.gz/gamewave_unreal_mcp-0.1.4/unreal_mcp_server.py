...
mcp = FastMCP(
    ...
    transport='uvicorn',      # Define the transport here
    host='127.0.0.1',          # Define the host here for security
    port=55558                 # Use a safe, uncommon port here
)
...
from python_executor_tools.python_executor_tools import register_python_executor_tools
...
def info():
    ...
    - The tools mirror the functionality that drives Unreal Engine's own context menus and action palettes in the Blueprint editor, giving you programmatic access to the same rich action discovery system.

    ### Python Executor Tools
    - Execute python code in unreal engine. This tool uses Unreal's IPythonScriptPlugin plugin to execute python code in unreal engine.
"""

def main():
  """Run the MCP server as a persistent web server."""
  mcp.run()

if __name__ == "__main__":
    logger.info("Starting MCP server with uvicorn web transport")
    main()
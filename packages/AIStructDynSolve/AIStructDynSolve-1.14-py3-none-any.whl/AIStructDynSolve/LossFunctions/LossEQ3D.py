...
class LossEQ3D:
    def __init__(
        ...
        self.duration_time, self.eq_time, eq_accX, eq_accY, eq_accZ, self.dimensionsless_Pt = inputEQ.get_EQparm()
        if train_params is not None and "eqx" in {key.lower() for key in train_params}:
            eq_accX = torch.tensor(eq_accX, device=DEVICE, dtype=DEFAULT_DTYPE) / self.dimensionsless_Pt    # Make sure to set the dtype as needed
            self.eq_accX_scale = torch.ones_like(eq_accX) / eq_accX
            self.eq_accX = torch.nn.Parameter(eq_accX * self.eq_accX_scale, requires_grad=True)
        else:
            self.eq_accX = eq_accX

        if train_params is not None and "eqy" in {key.lower() for key in train_params}:
            eq_accY = torch.tensor(eq_accY, device=DEVICE, dtype=DEFAULT_DTYPE) / self.dimensionsless_Pt    # Make sure to set the dtype as needed
            self.eq_accY_scale = torch.ones_like(eq_accY) / eq_accY
            self.eq_accY = torch.nn.Parameter(eq_accY * self.eq_accY_scale, requires_grad=True)
        else:
            self.eq_accY = eq_accY

        if train_params is not None and "eqz" in {key.lower() for key in train_params}:
            eq_accZ = torch.tensor(eq_accZ, device=DEVICE, dtype=DEFAULT_DTYPE) / self.dimensionsless_Pt    # Make sure to set the dtype as needed
            self.eq_accZ_scale = torch.ones_like(eq_accZ) / eq_accZ
            self.eq_accZ = torch.nn.Parameter(eq_accZ * self.eq_accZ_scale, requires_grad=True)
        else:
    ...
    def lossODE(self):
        ...
        if isinstance(self.eq_accX, torch.Tensor):
            eq_time_tensor = torch.tensor(self.eq_time, dtype=DEFAULT_DTYPE, device=DEVICE)
            x_query = t_x_in.view(-1)
            acc_interpolationX = self.torch_interp_1d(x_query, eq_time_tensor, self.eq_accX / self.eq_accX_scale).unsqueeze(-1)
            acc_interpolationY = self.torch_interp_1d(x_query, eq_time_tensor, self.eq_accY / self.eq_accY_scale).unsqueeze(-1)
            acc_interpolationZ = self.torch_interp_1d(x_query, eq_time_tensor, self.eq_accZ / self.eq_accZ_scale).unsqueeze(-1)
            # 拼接成 (len(x_in), 3)
            acc_interpolation = torch.cat([acc_interpolationX, acc_interpolationY, acc_interpolationZ], dim=-1)
            acc = acc_interpolation.repeat(1, self.DOF // 3)
        else:
    ...
    def get_eq_accX_scale(self):
        return self.eq_accX_scale

    def get_eq_accY_scale(self):
        return self.eq_accY_scale

    def get_eq_accZ_scale(self):
        return self.eq_accZ_scale
    ...
    def get_trainable_params(self):
        ...
        if self.train_params is not None:
            for param in self.train_params:
                ...
                elif param_lower == "eqx":
                    params.append(self.eq_accX)
                elif param_lower == "eqy":
                    params.append(self.eq_accY)
                elif param_lower == "eqz":
                    params.append(self.eq_accZ)
                else:

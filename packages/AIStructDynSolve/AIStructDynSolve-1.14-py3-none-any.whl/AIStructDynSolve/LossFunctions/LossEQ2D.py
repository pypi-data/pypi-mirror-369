...
class LossEQ2D:
    def __init__(
        ...
        self.duration_time, self.eq_time, eq_accX, eq_accY, self.dimensionsless_Pt = inputEQ.get_EQparm()
    ...
    def lossODE(self):
        ...
        if isinstance(self.eq_accX, torch.Tensor):
            ...
            # 拼接成 (len(x_in), 2)
            acc_interpolation = torch.cat([acc_interpolationX, acc_interpolationY], dim=-1)
            acc = acc_interpolation.repeat(1, self.DOF // 2)
        else:

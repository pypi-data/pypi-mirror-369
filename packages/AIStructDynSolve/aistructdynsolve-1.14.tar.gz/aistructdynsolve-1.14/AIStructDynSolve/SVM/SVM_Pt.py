...
class SVM_Pt:
    def __new__(cls, M, C, K, t, Pt, gamma, sigma2, InitialU=None, InitialU_dot=None, PHI=None):
        """
        ...
        instance = super(SVM_Pt, cls).__new__(cls)
        ...
        if not isinstance(Pt, np.ndarray):
            P = np.vstack(Pt).T /dimensionsless_unit
        else:
            # 如果EQ已经是np.array矩阵，直接赋值
            P = Pt /dimensionsless_unit
        # 如果没有提供 InitialU 和 InitialU_dot，初始化为 0.0
        instance.InitialU = np.zeros(M.shape[0]) if InitialU is None else InitialU
        instance.InitialU_dot = np.zeros(M.shape[0]) if InitialU_dot is None else InitialU_dot
        # 调用 Implement_SVMPt 计算并直接返回结果
        return instance.Implement_SVMPt(instance.M, instance.C, instance.K, t, P, instance.InitialU, instance.InitialU_dot, PHI)

    def SVMPt_method(self, M, C, K, time, Pt, U_t0, U_dt0):
        """
        实现 SVMPt 算法进行动态结构分析。
    ...
    def Implement_SVMPt(self,  M, C, K, t, Pt, U_t0, U_dt0, PHI=None):
        """
        实现 SVMPt 方法，处理物理坐标和模态坐标。
        ...
        if PHI is None:
            U_I = U_t0
            U_dtI = U_dt0
            Result_I = self.SVMPt_method(M, C, K, t, Pt, U_I, U_dtI)
            U = Result_I['U']
            U_dt = Result_I['U_dt']
            ...
            # 实例化一个新的 SVMPt_method 以使用模态矩阵
            Result_I = self.SVMPt_method(M_modal, C_modal, K_modal, t, P_eff, U_I, U_dtI)
            U_modal = Result_I['U']

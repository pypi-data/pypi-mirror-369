...
class LossEQ:
    def __init__(
        ...
        # 从 inputEQ 类实例获取 duration_time, eq_time, eq_acc参数
        self.duration_time, self.eq_time, eq_acc,self.dimensionsless_Pt = inputEQ.get_EQparm()
        if train_params is not None and "eq" in {key.lower() for key in train_params}:
            eq_acc = torch.tensor(eq_acc, device=DEVICE, dtype=DEFAULT_DTYPE) / self.dimensionsless_Pt   # Make sure to set the dtype as needed
            self.eq_acc_scale = torch.ones_like(eq_acc) / eq_acc
            self.eq_acc = torch.nn.Parameter(eq_acc * self.eq_acc_scale, requires_grad=True)
            #print(eq_acc.size())
        else:
            self.eq_acc = eq_acc
    ...
    def lossODE(self):
        ...
        # Interpolate external acceleration
        if isinstance(self.eq_acc, torch.Tensor):
            eq_time_tensor = torch.tensor(self.eq_time, dtype=DEFAULT_DTYPE, device=DEVICE)
            x_query = t_x_in.view(-1)
            acc_interpolation = self.torch_interp_1d(x_query, eq_time_tensor, self.eq_acc/self.eq_acc_scale).unsqueeze(-1)
            acc = acc_interpolation.repeat(1, self.DOF)
        else:
            acc_interpolation = np.interp(x_in, self.eq_time, self.eq_acc/self.dimensionsless_Pt)
            acc = torch.from_numpy(acc_interpolation).repeat(1, self.DOF).to(DEFAULT_DTYPE).to(DEVICE)
        ...
        residual = torch.matmul(self.mass, u_collection_dotdot.T) + torch.matmul(self.damping, u_collection_dot.T) + torch.matmul(self.stiffness, u_collection.T) + torch.matmul(self.mass, acc.T)
    ...
    def get_eq_acc_scale(self):
        return self.eq_acc_scale

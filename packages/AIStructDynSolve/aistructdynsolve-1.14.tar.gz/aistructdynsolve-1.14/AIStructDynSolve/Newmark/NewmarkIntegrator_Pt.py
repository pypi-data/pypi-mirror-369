...
class NewmarkIntegrator_Pt:
    def __new__(cls, M, C, K, eigenvectors, time, Pt, InitialU=None, InitialU_dot=None, integration_type='linear', Deltatime =0.01):
        """
        初始化并直接计算结果。
        ...
        """
        instance = super(NewmarkIntegrator_Pt, cls).__new__(cls)
        ...
        #Pt = np.array(Pt).T  # 转换为 numpy 数组
        #instance.Pt = np.array([np.interp(instance.time, time, Pt[:, i]) for i in range(Pt.shape[1])]).T
        instance.Pt = np.array([np.interp(instance.time, time, value) for value in Pt]).T

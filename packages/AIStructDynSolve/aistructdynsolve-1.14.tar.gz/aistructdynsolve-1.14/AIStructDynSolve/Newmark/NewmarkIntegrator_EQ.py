...
class NewmarkIntegrator_EQ:
    def __new__(cls, M, C, K, eigenvectors, time, EQ, InitialU=None, InitialU_dot=None, integration_type='linear', Deltatime =0.01):
        """
        初始化并直接计算结果。
        ...
        """
        instance = super(NewmarkIntegrator_EQ, cls).__new__(cls)
        ...
        instance.EQ = np.interp(instance.time, time, EQ)
        ...
        P0 = -instance.M @ np.ones(instance.n) * instance.EQ[0]
        ...
        for i in range(instance.N - 1):
            P = -instance.M @ np.ones(instance.n) * instance.EQ[i]
            P_m = instance.eigenvectors.T @ P
            P1 = -instance.M @ np.ones(instance.n) * instance.EQ[i + 1]

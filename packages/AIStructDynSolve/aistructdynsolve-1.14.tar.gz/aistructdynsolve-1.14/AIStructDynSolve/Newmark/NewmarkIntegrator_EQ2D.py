...
class NewmarkIntegrator_EQ2D:
    def __new__(cls, M, C, K, eigenvectors, time, EQX, EQY, InitialU=None, InitialU_dot=None, integration_type='linear', Deltatime =0.01):
        """
        初始化并直接计算结果。
        ...
        """
        instance = super(NewmarkIntegrator_EQ2D, cls).__new__(cls)
        ...
        EQ_vec0[::2] *= instance.EQX[0]  # 偶数索引：EQX
        EQ_vec0[1::2] *= instance.EQY[0]  # 奇数索引：EQY
        ...
        for i in range(instance.N - 1):
            ...
            EQ_vec[::2] *= instance.EQX[i]  # 偶数索引：EQX
            EQ_vec[1::2] *= instance.EQY[i]  # 奇数索引：EQY
            P = -instance.M @ EQ_vec
            P_m = instance.eigenvectors.T @ P
            EQ_vec1 = np.ones(instance.n)
            EQ_vec1[::2] *= instance.EQX[i+1]  # 偶数索引：EQX
            EQ_vec1[1::2] *= instance.EQY[i+1]  # 奇数索引：EQY

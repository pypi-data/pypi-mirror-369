...
class MultiDiscriminatorWrapper(Model):
    ...
    def generator(
        ...
        y: Tensor,
        y_hat: Tensor,
        feat_lambda: float = 2.0,
        generator_lambda: float = 1.0,
    ) -> Tuple[Tensor, Tensor]:
        """This is experimental, to see if the training is affected or not by eval mode"""
        feat_loss: Tensor = 0
        gen_loss: Tensor = 0
        for disc in self.disc:
            f_loss, g_loss = disc.gen_step2(y, y_hat, feat_lambda, generator_lambda)
            gen_loss += g_loss
            feat_loss += f_loss
    ...
    def discriminator(
        ...
        y: Tensor,
        y_hat: Tensor,
        lambda_loss_r: float = 1,
        lambda_loss_g: float = 1,
    ) -> Tensor:
        """This is experimental, to see if the training is affected or not by eval mode"""
        disc_loss: Tensor = 0
        for disc in self.disc:
            disc_loss += disc.disc_step2(y, y_hat, lambda_loss_r, lambda_loss_g)
        return disc_loss
...
class _MultiDiscriminatorT(ConvNets):
    ...
    def gen_step(self, y: Tensor, y_hat: Tensor) -> tuple[Tensor, Tensor, List[float]]:
        ...
        _, y_hat_gen, feat_map_real, feat_map_gen = self.forward(y, y_hat)
    ...
    def disc_step(
        ...
        y_hat_real, y_hat_gen, _, _ = self.forward(y, y_hat)
    ...
    def gen_step2(
        ...
        """In tests"""
        _, y_hat_gen, feat_map_real, feat_map_gen = self.forward(y, y_hat)
        loss_feat = self.feature_loss2(feat_map_real, feat_map_gen, feat_lambda)
        loss_generator = self.generator_loss2(y_hat_gen, generator_lambda)
        return loss_feat, loss_generator

    def disc_step2(
        self,
        y: Tensor,
        y_hat: Tensor,
        lambda_loss_r: float = 1.0,
        lambda_loss_g: float = 1.0,
    ) -> Tensor:
        """In tests"""
        y_hat_real, y_hat_gen, _, _ = self.train_step(y, y_hat)
        return self.discriminator_loss2(
            y_hat_real,
            y_hat_gen,
            lambda_loss_r,
            lambda_loss_g,
    ...
    def discriminator_loss2(
        ...
        return sum(
            [
                (torch.mean((1 - dr) ** 2) * lambda_loss_r)
                + (torch.mean(dg**2) * lambda_loss_g)
                for dr, dg in zip(real_outputs, fake_outputs)
            ]
        )

    @staticmethod
    def feature_loss2(fmap_r, fmap_g, lambda_loss: float = 2.0) -> Tensor:
        """In Tests"""
        loss = 0
        for dr, dg in zip(fmap_r, fmap_g):
            loss += sum(torch.mean(torch.abs(rl - gl)) for rl, gl in zip(dr, dg))
        return loss * lambda_loss

    @staticmethod
    def generator_loss2(
        disc_out, lambda_loss: float = 1.0
    ) -> Tuple[Tensor, List[float]]:
        loss = 0
        for dg in disc_out:
            loss += -torch.mean((1 - dg) ** 2).log()
        return loss
...
class DiscriminatorCQT(ConvNets):
    ...
    def forward(self, x: torch.tensor) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        ...
        z = torch.permute(z, (0, 1, 3, 2)).contiguous()  # [B, C, W, T] -> [B, C, T, W]

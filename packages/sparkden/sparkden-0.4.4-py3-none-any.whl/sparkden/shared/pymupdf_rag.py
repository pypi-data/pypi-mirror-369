# type: ignore
# copied from pymupdf4llm.helpers.pymupdf_rag.to_markdown for customize write_images logic
# add a parameter to to_markdown to control write_images logic
...
class IdentifyHeaders:
    ...
    def __init__(
        ...
        doc: str,
        pages: list = None,
        body_limit: float = 10,  # force this to be body text
...
def to_markdown(
    ...
    *,
    ...
    if callable(hdr_info):
        ...
        toc_headers = TocHeaders(doc)
        hdr_info = toc_headers if toc_headers.TOC else IdentifyHeaders(doc)
        get_header_id = hdr_info.get_header_id
    ...
    def save_image(parms, rect, i):
        ...
        if write_images is True:
            if on_write_image is not None:
                image_name = f"{parms.filename}/page{page.number}/{i}.{IMG_EXTENSION}"
                return on_write_image(pix.tobytes(IMG_EXTENSION), image_name)

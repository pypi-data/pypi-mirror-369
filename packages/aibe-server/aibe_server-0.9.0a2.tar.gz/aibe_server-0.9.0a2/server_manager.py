...
def start_server():
    ...
    try:        
        ...
        process = subprocess.Popen([
            sys.executable, "-m", "aibe_server.main", "--detached"
        ...
        if status:
            ...
            print("X Server start failed - not responding")
            return False
...
def stop_server():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"X Server stop failed: {result.stderr}")
            print("Server may still be running - check permissions?")
            return False
            
    except Exception as e:
        print(f"X Server stop failed: {e}")
        return False

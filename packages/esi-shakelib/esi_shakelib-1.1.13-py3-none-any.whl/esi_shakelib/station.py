...
TABLES = OrderedDict(
    (
        ...
        (
            "imt",
            OrderedDict((("id", "integer primary key"), ("imt_type", "text"))),
        ),
        (
            "amp",
            OrderedDict(
...
def convert_units(imt_type, input_units, amplitude):
    """
    Convert amp units based on units strings.
    """
...
class StationList(object):
    ...
    def loadFromFiles(cls, filelist, min_nresp=3, dbfile=":memory:"):
        ...
        Args:
            ...
            min_nresp (int):
                The minimum number of DYFI observations required to form and
                valid observation. Default is 3.
            dbfile (str):
    ...
    def _loadFromXML(self, xmlfiles, min_nresp):
        ...
        for xmlfile in xmlfiles:
            stationdict, ims = self._filter_station(
                xmlfile, stationdict, min_nresp
            )
            imtset |= ims
    ...
    def _parse_shakemap_json(
        self, jfile, min_nresp, sta_set, imt_set, amp_set
    ):
        with open(jfile, "r", encoding="utf-8") as jfp:
            stas = json.load(jfp)

        if "type" not in stas:
            logging.warning(
                "%s appears to contain no stations, skipping", jfile
            )
            return ([], [], [])
        if stas["type"] != "FeatureCollection":
            logging.warning(
                "%s is not a ShakeMap JSON stationlist, skipping", jfile
            )
            return ([], [], [])
        ...
        for feature in stas["features"]:
            ...
            for ic, comp in enumerate(feature["properties"]["channels"]):
                ...
                for amp in comp["amplitudes"]:
                    if amp["name"].startswith("psa"):
                        imt_type = PSA_TO_SA[amp["name"]]
                    else:
                    ...
                    if "ln_sigma" in amp:
                        ...
                        stddev = 0
                    # We don't care about previous flagging, we'll do our own
                    # flag = amp["flag"]
                    flag = "0"
                    units = amp["units"]
                    if (
    ...
    def _parse_groundpacket_json(self, jsonfile, sta_set, imt_set, amp_set):
        ...
        for feature in packet.features:
            ...
            station_lon, station_lat, station_elev = (
            ...
            for stream in feature.properties.streams:
                for trace in stream.traces:
                    ...
                    if channel_code.lower().startswith("rot"):
                        ...
                        if channel_code[-1].upper() in ORIENTATIONS:
                            orient = ORIENTATIONS[channel_code[-1].upper()]
                        else:
                            orient = None
                        ...
                        if isinstance(metric.values, float):
                            ...
                            for idx, amplitude in enumerate(metric.values[0]):
                                ...
                                if period >= 1:
                                    ...
                                    imt_type = (
                                        f"{metric.properties.name}({period:g})"
                                    )
                                imt_set.add(imt_type)
                                amp_id = (
    ...
    def _loadFromJSON(self, jsonfiles, min_nresp):
        ...
        sta_set = {z[0] for z in self.cursor.fetchall()}
    ...
    def getGeoJson(self):
        ...
        for sta in sta_rows:
            ...
            self.cursor.execute(
                ...
                "FROM amp a, imt i "
                f"WHERE a.station_id = '{str(sta[0])}' "
                "AND a.imt_id = i.id"
            )
            amp_rows = self.cursor.fetchall()
            if sta[8] == 0:
                if len(amp_rows) != 1:
                    logging.warning("Couldn't find intensity for MMI station.")
                    continue
    ...
    def _loadFromDict(self, stationdict, imtset):
        ...
        amp_set = set(
            [str(v[0]) + "." + str(v[1]) + "." + str(v[2]) for v in amp_rows]
        )
        ...
        for sta_id, station_tpl in stationdict.items():
            station_attributes, comp_dict = station_tpl
            instrumented = int(
                station_attributes["netid"].lower() not in CIIM_TUPLE
            )
            for original_channel, cdict in comp_dict.items():
                pgm_dict = cdict["amps"]
                if len(
                    comp_dict
                ) == 1 and original_channel == station_attributes.get(
                    "name", ""
                ):
                    orientation = "H"
                else:
    ...
    def getStationDictionary(
        ...
        Args:
            ...
            component (str):
                ...
                "RotD100", etc. Invalid components will simply not return any
                IMTs.
            min_nresp (int):
        ...
        dstr = ", ".join(columns)
        self.cursor.execute(
            f"SELECT {dstr} FROM station where instrumented = {instrumented}"
        )
        ...
        self.cursor.execute(
            "SELECT "
            "a.amp, i.imt_type, a.station_id, a.stddev, a.nresp, "
            "a.original_channel "
            "FROM "
    ...
    def _fixOrientations(self):
        ...
        for row in sta_rows:
            ...
            if len(channel_list) == 3:
                ochars = [x[-1].upper() for x in channel_list]
                if "1" in ochars and "2" in ochars and "Z" in ochars:
                    hinds = [
                        i for i, x in enumerate(ochars) if x in ("1", "2")
                    ]
                    sql = (
                        "UPDATE amp "
                        "SET orientation = 'H'"
                    ...
                    self.db.commit()
                    hinds = [i for i, x in enumerate(ochars) if x == "Z"]
                    sql = (
                        "UPDATE amp "
                        "SET orientation = 'Z' "
                        "WHERE station_id = ? "
                        "AND original_channel = ?"
                    )
                    self.cursor.execute(
                        sql,
                        (
    ...
    def _getGroundMotions(comp, imt_translate):
        ...
        for pgm in comp:
            ...
            if "value" in pgm.attrib:
                ...
                logging.warning("No value for amp %s", pgm.tag)
                continue
    ...
    def _filter_station(self, xmlfile, stationdict, min_nresp):
        ...
        Args:
            ...
            stationdict (dict):
                the dictionary of stations that the stations in this file
                should be added to.
            min_nresp (int):
        ...
        for sl in root.iter("stationlist"):
            if "reference" in sl.attrib:
                ...
                self.db.commit()
                query = (
                    f"SELECT id FROM reference WHERE longref is ('{longref}')"
                )
                self.cursor.execute(query)
...
def _getOrientation(orig_channel, orient):
    ...
    elif orig_channel[-1].upper() in ("N", "E", "Z"):
        orientation = orig_channel[-1].upper()
    elif orig_channel == "UNK":  # Channel is "UNK"; assume horizontal
        orientation = "H"
    elif orig_channel.upper() == "H1" or orig_channel.upper() == "H2":
        orientation = "H"
...
def _getOrientationSet(chan_names):
    ...
    if len(chan_names) == 3:
        term_chars = [
            chan_names[0][-1].upper(),
            chan_names[1][-1].upper(),
            chan_names[2][-1].upper(),
        ]
        if "1" in term_chars and "Z" in term_chars:
            orientations = [("Z" if x == "Z" else "H") for x in term_chars]
            return orientations
    orientations = []

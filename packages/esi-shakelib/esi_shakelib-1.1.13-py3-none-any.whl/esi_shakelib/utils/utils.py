...
from esi_shakelib.multigmpe import set_sites_depth_parameters
...
def replace_dyfi(stationfile, dyfi_xml, min_nresp=3):
    ...
    Args:
        ...
        min_nresp (int): The minimum number of DYFI responses required for an
                         observation to be added to the stationlist.
                         Default = 3.
    Returns:
...
def _get_extent_from_multigmpe(rupture, config, ipe):
    ...
    # Rupture context
    rx = RuptureContext()
    rx.mag = np.full(size, origin.mag)
    rx.rake = np.zeros(size)
    # From WC94...
    # rx.width = 10 ** (-0.76 + 0.27 * rx.mag)
    # rx.dip = 90.0
    # rx.ztor = origin.depth
    # Using parameters from the point rupture...
    rx.width = np.full(size, rupture.getWidth())
    rx.dip = np.full(size, rupture.getDip())
    rx.ztor = np.full(size, rupture.getDepthToTop())
    rx.hypo_depth = np.full(size, origin.depth)
    ...
    #
    logging.debug(
        "Extent: %f, %f, %f, %f", lonmin[0], lonmax[0], latmin[0], latmax[0]
    )
    return (
...
def get_shakelib_version():
    """
    Get the code version of this package.
    """
    return importlib.metadata.version("esi_shakelib")
...
def get_gmpe_coef_table(gmpe):
    ...
    raise NotImplementedError(
        f"GMPE {gmpe} does not contain sa_coeffs attribute."
    )

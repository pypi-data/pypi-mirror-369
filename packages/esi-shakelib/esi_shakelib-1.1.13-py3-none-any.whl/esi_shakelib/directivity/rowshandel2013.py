...
class Rowshandel2013(object):
    ...
    def __computeXiPrime(self):
        ...
        nsubs = np.zeros(np.prod(slat.shape))
        nsubp = np.zeros(np.prod(slat.shape))

        xi_prime_s = np.zeros(np.prod(slat.shape))
        xi_prime_p = np.zeros(np.prod(slat.shape))
        ...
        xi_prime_unscaled = xi_prime_unscaled + np.reshape(
            xi_prime_tmp, slat.shape
        )

...
# local imports
from esi_shakelib.directivity.bayless2013 import Bayless2013
...
def test_ss3():
    ...
    x = np.linspace(-60.0, 60.0, 21)
    y = np.linspace(-60.0, 138.0, 34)
...
def test_ss3_m6():
    ...
    x = np.linspace(0.0, 20.0, 6)
    y = np.linspace(0.0, 90.0, 11)
    site_x, site_y = np.meshgrid(x, y)
    shp = site_x.shape
    site_x = site_x.flatten()
    site_y = site_y.flatten()
...
def test_rv4():
    ...
    x = np.linspace(-50.0, 50.0, 11)
    y = np.linspace(-50.0, 50.0, 11)
    site_x, site_y = np.meshgrid(x, y)
    shp = site_x.shape
    site_x = site_x.flatten()
    site_y = site_y.flatten()
    ...
    # fd[5][5] appears differently from fd_test[5][5],
    # at least on the Gitlab CI runner it's 9.25557826e-02 , not 1.01875104e-01,
    fd_test = np.array(
        [
            [
                0.00000000e00,
    ...
    print(fd.flatten())
    print(fd_test.flatten())
    np.testing.assert_allclose(fd, fd_test, rtol=2e-4, verbose=True)
...
def test_so6():
    ...
    # Sites
    x = np.linspace(-80.0, 80.0, 21)
    y = np.linspace(-50.0, 130.0, 21)
    site_x, site_y = np.meshgrid(x, y)
    shp = site_x.shape
    site_x = site_x.flatten()
    site_y = site_y.flatten()

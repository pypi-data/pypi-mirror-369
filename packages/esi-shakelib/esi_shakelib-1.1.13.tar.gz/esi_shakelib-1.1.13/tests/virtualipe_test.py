...
# local imports
from esi_shakelib.gmice.wgrw12 import WGRW12
...
def test_virtualipe():
    ...
    mmi_const_vs30, mmi_sd_const_vs30 = ipe.get_mean_and_stddevs(
        sx, rx, dx, MMI(), sd_types
    )
    ...
    mmi_variable_vs30, mmi_sd_variable_vs30 = ipe.get_mean_and_stddevs(
        sx, rx, dx, MMI(), sd_types
    )
    ...
    with pytest.raises(ValueError) as e:
        mmi_psa, mmi_sd_psa = ipe.get_mean_and_stddevs(
            sx, rx, dx, PGA(), sd_types
        )
    #
    ...
    assert np.allclose(
        td["mmi_sd_variable_vs30_total"], mmi_sd_variable_vs30_intra[0]
    )
    assert np.allclose(
        td["mmi_sd_variable_vs30_intra"], mmi_sd_variable_vs30_intra[1]
    )
    assert np.allclose(
        td["mmi_sd_variable_vs30_inter"], mmi_sd_variable_vs30_intra[2]
    )
    ...
    mmi_fd, mmi_fdsd = ipe.get_mean_and_stddevs(
        sx, rx, dx, MMI(), sd_types, fd=0
    )

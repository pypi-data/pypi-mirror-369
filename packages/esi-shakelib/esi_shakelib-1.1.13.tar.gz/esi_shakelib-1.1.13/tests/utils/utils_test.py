...
from esi_shakelib.utils.utils import get_extent, is_stable, replace_dyfi
...
def test_replace_dyfi():
    ...
    scursor.execute(
        "SELECT count(*) from station where instrumented=0 and network != "
        "'DYFI'"
    )
...
def test_get_extent_small_point():
    ...
    #
    config = {
        "extent": {"mmi": {"threshold": 4.0, "mindist": 100, "maxdist": 1000}}
    }
    ...
    np.testing.assert_allclose(W, 102.23333333333333)
    np.testing.assert_allclose(E, 104.53333333333333)
    np.testing.assert_allclose(S, 30.0)
    np.testing.assert_allclose(N, 31.966666666666667)
...
def test_get_extent_small_complex():
    ...
    W, E, S, N = get_extent(config=config, ipe=ipe, rupture=rupture)
    np.testing.assert_allclose(W, 96.383333333333333)
    np.testing.assert_allclose(E, 113.55)
    np.testing.assert_allclose(S, 24.25)
    np.testing.assert_allclose(N, 38.766666666666667)
...
def test_get_extent_aspect():
    ...
    W, E, S, N = get_extent(config=config, ipe=ipe, rupture=rupture)
    np.testing.assert_allclose(W, 93.733333333333333)
    np.testing.assert_allclose(E, 118.41666666666667)
    np.testing.assert_allclose(S, 20.833333333333333)
    np.testing.assert_allclose(N, 38.2)
    #
    ...
    rupture = get_rupture(origin, rrep)
    W, E, S, N = get_extent(config=config, ipe=ipe, rupture=rupture)
    np.testing.assert_allclose(W, 89.366666666666667)
    np.testing.assert_allclose(E, 113.61666666666667)
    np.testing.assert_allclose(S, 17.283333333333333)
    np.testing.assert_allclose(N, 41.616666666666667)
...
def test_get_extent_stable_large():
    ...
    rupture = get_rupture(origin)
    W, E, S, N = get_extent(config=config, ipe=ipe, rupture=rupture)
    np.testing.assert_allclose(W, -104.45)
    np.testing.assert_allclose(E, -89.18333333333333)
    np.testing.assert_allclose(S, 29.3)
    np.testing.assert_allclose(N, 41.583333333333333)


# @pytest.mark.skip(reason="Function not currently used and needs strec configuration in the future")
def test_is_stable():

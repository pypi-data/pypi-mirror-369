...
sigs_target = np.array(
    [
        [0.5021999, 0.55200067, 0.60183448, 0.65169375, 0.6118045, 0.70157305],
        [
            0.50366136,
            0.55333061,
            0.60305453,
        ...
        [0.50918036, 0.55835888, 0.60767149, 0.657088, 0.61754728, 0.70658661],
        [0.50893265, 0.558133, 0.60746394, 0.65689607, 0.61734305, 0.70640813],
        [
            0.50913428,
            0.55831685,
            0.60763287,
        ...
        [0.50494378, 0.55449817, 0.604126, 0.65381054, 0.61405881, 0.70353978],
        [
            0.50554264,
            0.55504357,
            0.60462663,
...
def test_bk17():
    ...
    for i, imc in enumerate(imc_in):
        ...
        for imt in imt_in:
            tmp = bk17.convertAmps(
                imt, amps_in, rrup_in, np.array([mags_in[i]])
            )
    ...
    if do_test is True:
        ...
        tmp = bk17.convertAmps(
            imt_in[0], amps_in, rrup_in, np.array([mags_in[0]])
        )
        bk17 = BooreKishida2017(imc_out[0], imc_in[0])
        orig_amps = bk17.convertAmps(
            imt_in[0], tmp, rrup_in, np.array([mags_in[0]])
        )
    ...
    bk17 = BooreKishida2017(
        "Average Horizontal", "Average Horizontal (RotD50)"
    )
    ...
    if bk17.path[1].value == "Greater of two horizontal":
        ...
        correct = False
    assert correct == True
    bk17 = BooreKishida2017("Horizontal", "Random horizontal")
    assert len(bk17.path) == 3
    ...
    bk17 = BooreKishida2017(
        "Horizontal Maximum Direction (RotD100)", "Average Horizontal (RotD50)"
    )
    mid = bk17.convertAmps(imt_in[0], amps_in, rrup_in, np.array([mags_in[0]]))
    bk17 = BooreKishida2017(
        "Average Horizontal (RotD50)", "Average Horizontal (GMRotI50)"
    )
    last = bk17.convertAmps(imt_in[0], mid, rrup_in, np.array([mags_in[0]]))
    bk17 = BooreKishida2017(
        "Horizontal Maximum Direction (RotD100)",
        "Average Horizontal (GMRotI50)",
    )
    full = bk17.convertAmps(
        imt_in[0], amps_in, rrup_in, np.array([mags_in[0]])
    )
    ...
    with pytest.raises(ValueError) as e:
        bk17.convertAmps(imt_in[0], amps_in, None, np.array([mags_in[0]]))
    with pytest.raises(ValueError) as e:
    ...
    with pytest.raises(ValueError) as e:
        bk17.convertAmpsOnce(
            IMT("wrong"), [10.0], rrup_in, np.array([mags_in[0]])
        )

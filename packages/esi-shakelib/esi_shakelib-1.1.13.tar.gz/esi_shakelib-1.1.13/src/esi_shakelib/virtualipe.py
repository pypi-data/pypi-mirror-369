...
class VirtualIPE(GMPE):
    ...
    def __fromFuncs__(cls, gmpe, gmice, rupture_obj=None):
        ...
        if gmpe.ALL_GMPES_HAVE_PGV is True and any(
            "PGV" in t.__name__
            for t in gmice.DEFINED_FOR_INTENSITY_MEASURE_TYPES
        ):
            self.imt = PGV()
        elif any(
            "PGA" in t.__name__
            for t in gmpe.DEFINED_FOR_INTENSITY_MEASURE_TYPES
        ) and any(
            "PGA" in t.__name__
            for t in gmice.DEFINED_FOR_INTENSITY_MEASURE_TYPES
        ):
    ...
    def get_mean_and_stddevs(self, sx, rx, dx, imt, stddev_types, fd=None):
        ...
        #
        mgm, sdev = self.gmpe.get_mean_and_stddevs(
            sx, rx, dx, self.imt, stddev_types
        )

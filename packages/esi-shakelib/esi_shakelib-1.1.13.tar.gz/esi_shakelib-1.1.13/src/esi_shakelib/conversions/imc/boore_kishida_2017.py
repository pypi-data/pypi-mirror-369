...
# Local imports
from esi_shakelib.conversions.convert_imc import ComponentConverter
...
class BooreKishida2017(ComponentConverter):
    ...
    def __init__(self, imc_in, imc_out):
        super().__init__()
        if isinstance(imc_in, IMC):
            self.imc_in = imc_in
        elif isinstance(imc_in, str):
            ...
            self.imc_in = IMC.GEOMETRIC_MEAN
        if isinstance(imc_out, IMC):
            self.imc_out = imc_out
        elif isinstance(imc_out, str):
        ...
        self.conversion_graph = {
            ...
            IMC.GEOMETRIC_MEAN: set(
                [IMC.RotD50, IMC.GREATER_OF_TWO_HORIZONTAL]
            ),
            ...
            IMC.MEDIAN_HORIZONTAL: set(
                [IMC.GREATER_OF_TWO_HORIZONTAL, IMC.RotD50]
            ),
    ...
    def _imcPairToFile(imc_in, imc_out):
        ...
        """
        datadir = (
            importlib_resources.files("esi_shakelib.conversions.imc") / "data"
        )
    ...
    def _verifyConversion(self, imc_in, imc_out=None):
        ...
        if filename is None:
            raise ValueError(
                f"Can't find a conversion file for {imc_in} and {imc_out}"
            )

...
def contour(imtdict, imtype, filter_size, gmice):
    ...
    if np.all(np.isnan(fgrid)):  # data is totally empty; no contours
        ...
        if grid_max - grid_min:
            intervals = getContourLevels(
                grid_min, grid_max, itype=interval_type
            )
    ...
    for cval in intervals:
        ...
        for ic, coords in enumerate(contours):  # coords is a line segment
            ...
            mylons = np.around(
                coords[:, 1] * lonspan / nlon + lonstart, decimals=6
            )
            mylats = np.around(
        ...
        if len(new_contours):
            ...
            if imtype == "MMI":
                ...
                props["weight"] = 4
            line_strings.append(
                {"geometry": mapping(mls), "properties": props}
            )
    return line_strings
...
def getContourLevels(dmin, dmax, itype="log"):
    ...
    if itype == "log":
        ...
        levels = np.arange(
            np.ceil(dmin * 2) / 2, np.floor(dmax * 2) / 2 + 0.5, 0.5
        )
    return levels

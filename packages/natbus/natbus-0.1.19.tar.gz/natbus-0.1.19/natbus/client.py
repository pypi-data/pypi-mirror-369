...
class NatsBus:
    ...
    async def connect(self) -> None:
        ...
        # Create or update the stream’s subject set if requested.
        if self.cfg.stream_name and self.cfg.stream_subjects:
            name = self.cfg.stream_name
            desired = sorted(set(self.cfg.stream_subjects))

            if self.cfg.stream_create:
                try:
                    # Only pass supported kwargs; do NOT pass allow_rollup
                    await self._js.add_stream(
                        name=name,
                        subjects=desired
                    )
                except js_err.APIError:
                    # Falls through to update path below
                    pass

            if self.cfg.stream_update_subjects:
                try:
                    info = await self._js.stream_info(name)
                    current = sorted(set(info.config.subjects or []))
                    if current != desired:
                        union_subjects = sorted(set(current) | set(desired))

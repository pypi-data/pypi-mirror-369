...
class NatsBus:
    ...
    async def _ensure_push_consumer(
        ...
        *,
        ...
        try:
            info = await self._js.consumer_info(stream, durable, timeout=self.cfg.js_api_timeout_s)
            c = info.config
            ...
            if actual_q != queue:
                await self._js.delete_consumer(stream, durable, timeout=self.cfg.js_api_timeout_s)
            ...
            await self._js.add_consumer(
                ...
                timeout=self.cfg.js_api_timeout_s,
            )
            ...
            raise RuntimeError(
                f"JS API timeout creating/binding consumer stream={stream} subject={subject} durable={durable}"
            )
    ...
    async def connect(self) -> None:
        ...
        if self.cfg.stream_name:
            try:
                info = await self._js.stream_info(self.cfg.stream_name, timeout=self.cfg.js_api_timeout_s)
        ...
        if self.cfg.stream_create and self.cfg.stream_name and self.cfg.stream_subjects:
            try:
                await self._js.add_stream(
                    StreamConfig(name=self.cfg.stream_name, subjects=list(self.cfg.stream_subjects)),
                    timeout=self.cfg.js_api_timeout_s,
                )
                ...
                # already exists or not permitted; we’ll try update path if enabled
                pass
            ...
            try:
                ...
                if not desired.issubset(current):
                    ...
                    await self._js.update_stream(
                        StreamConfig(name=self.cfg.stream_name, subjects=merged),
                        timeout=self.cfg.js_api_timeout_s,
                    )

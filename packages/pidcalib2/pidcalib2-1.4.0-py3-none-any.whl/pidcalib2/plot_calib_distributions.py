...
from pidcalib2 import argparse_tools, binning, pid_data, utils
...
def decode_arguments(args: List[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            "This tool allows you to plot distributions of variables in the "
            "calibration datasets. You can supply the same cuts and custom "
            "binnings that you would use for make_eff_hists. If you wish to plot "
            "a variable for which no binning exists, a uniform binning with 50 "
            "bins will be used. You can change the number of bins using --bins "
            "and force a uniform binning even if another binning is defined via "
            "--force-uniform."
            "\n\n"
            "A plot for every requested variable will be "
            "created in the --output-dir directory. The format of the plots can "
            "be controlled by --format. Furthermore, "
            "plot_calib_distributions.pkl will be saved in the same directory, "
            "containing all the histograms, should the user want to make the "
            "plots manually."
        ),
    )
    parser.add_argument(
...
def create_plot_histograms(
    ...
    for path in (
        ...
        df = pid_data.root_to_dataframe(
            path, tree_paths, list(branch_names.values()), True, particle=particle_label
        )
        ...
        if df is not None:
            ...
            if "sweight_dir" in calib_sample:
                path_sweight = (
                    str(calib_sample["sweight_dir"]) + str(pathlib.Path(path).stem)
                    + "_sweights.root"
                )
                log.debug(f"Reading sWeights from {path_sweight}")
                df[sweight_branch] = pid_data.root_to_dataframe(
                    path_sweight, tree_paths, ["sweight"], True, particle_label
                )
            ...
            if df[sweight_branch].isna().any():
                ...
                new_len = df.shape[0]
                log.debug(
                    f"Droped {orig_len - new_len}/{orig_len} events with NaN sWeights "
                    "(usually due to events being out of the range of the sWeight fit)"
                )
                if new_len == 0:
                    # this is to cover situations
                    # where no event in the dataframe survives
                    continue
...
def save_plots(
    ...
    for var, hist in total_hists.items():
        ...
        plt.xlabel(var)
        plt.tight_layout()
        path = output_dir / pathlib.Path(f"{var}.{format}")

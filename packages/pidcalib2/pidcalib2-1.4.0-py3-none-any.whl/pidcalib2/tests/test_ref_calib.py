...
def test_ref_calib_aliases(test_path, tmp_path):
    config = {
        "sample": "2024_WithUT_block8",
        "magnet": "up",
        "bin_vars": '{"P": "P", "ETA": "ETA", "nPVs": "nPVs"}',
        "output_file": str(tmp_path / "test_data/test_data_PIDCalibResults.root"),
        "histo_dir": str(test_path / "test_data"),
        "merge": False,
        "ref_file": str(test_path / "test_data/ref_test_data_run3.root"),
        "ref_tree": "DecayTree",
        "ref_pars": "{'Bac': ['K', 'DLLK>0.0']}",
        "verbose": True,
    }
    assert ref_calib.ref_calib(config) == pytest.approx(0.9539051776330348)

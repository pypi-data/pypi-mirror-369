...
def copy_tree_and_set_as_friend(
    ...
    # If the tree is in a subdirectory, make sure the copied tree is placed in
    # the same subdirectory
    subdir_path = None
    if "/" in dest_tree_name:
        subdir_path = str(Path(dest_tree_name).parent)
        dest_file.cd(subdir_path)

    log.info(f"Copying {source_tree_name} to {dest_file_name}")
    source_tree.CloneTree().Write()

    dest_tree.AddFriend(
        f"{subdir_path}/{source_tree_name}" if subdir_path else source_tree_name
    )

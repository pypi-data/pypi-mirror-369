...
def test_plot_calib_distributions(tmp_path):
    ...
    assert saved_hist.metadata == {"name": "DLLK"}
...
@pytest.mark.xrootd
@pytest.mark.slow
def test_plot_calib_distributions_run_3(tmp_path):
    config = {
        "particle": "Pi",
        "magnet": "down",
        "sample": "test",
    ...
    with open(tmp_path / "plot_calib_distributions.pkl", "rb") as f:
        saved_hist = pickle.load(f)
    assert saved_hist.metadata == {"name": "DLLK"}
    assert saved_hist.values().sum() == pytest.approx(119982.8066788379)
    assert saved_hist.variances().sum() == pytest.approx(139989.2620579896)
...
def test_create_plot_histograms():
    ...
    calib_sample = {
        "files": [
            "root://eoslhcb.cern.ch//eos/lhcb/grid/prod/lhcb/anaprod/lhcb/LHCb/Collision15/PIDCALIB.ROOT/00064787/0000/00064787_00000037_1.pidcalib.root",  # noqa
        ]
    }
...
def test_save_plots(tmp_path):
    ...
    hist = bh.Histogram(bh.axis.Regular(40, -2, 10))
    hist.fill(data)
    hists["test_var"] = hist

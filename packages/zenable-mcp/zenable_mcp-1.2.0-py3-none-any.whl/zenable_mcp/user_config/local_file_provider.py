...
def _find_git_root(current_dir: Path) -> Optional[Path]:
    ...
    try:
        ...
        LOG.debug("Not in a git repository or gitpython not installed")
        return None
...
class LocalFileProvider(FileProvider):
    ...
    def get_file(self, file_path: Path) -> File:
        ...
        try:
            return File(
                path=str(file_path), content=file_path.read_text(encoding="utf-8")
            )
        except Exception:
            LOG.exception(f"Failed to read file {file_path}")
            raise

...
def read_file(name, opts):
    """Read the input file for a template."""
    if opts.dryrun:
        return
...
def create_modules_toc_file(master_package, modules, opts, name='index'):
    ...
    text = re.sub('\\$modules', text, template)

    # Add contributors
    root_dir = os.path.abspath(__file__).rsplit(os.path.sep, 3)[0]
    print('ROOT DIR: {}'.format(root_dir))
    with open(os.path.join(root_dir, 'CONTRIBUTORS'), 'r') as fin:
        contributors = fin.read().splitlines()
    contributors[-1] = 'and {}'.format(contributors[-1])
    text = re.sub('\\$contributors', ', '.join(contributors), text)

    write_file(os.path.join('..', name), text, opts)
...
Note: By default this script will not overwrite already created files.""")
    parser.add_option("-n", "--doc-header", action="store", dest="header", help="Documentation Header (default=WORC)", default="WORC")

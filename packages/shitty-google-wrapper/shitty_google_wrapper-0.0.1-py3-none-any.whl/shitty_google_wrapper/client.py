...
class GoogleApiClient:
    ...
    def uploadFile(self, local_data, destination_name, destination_folder=None):
        """
        ...
        try:
            ...
            existing_file = self._getFileByName(destination_name)
        ...
        if destination_folder:
            self.moveFile(destination_name, destination_folder)
    ...
    def deleteFileByName(self, file_name):
        ...
        file_info = self._getFileByName(file_name)
        self.driveService.files().delete(fileId=file_info['id']).execute()
        if self.verbose: print(f"File '{file_name}' (ID: {file_info['id']}) deleted successfully.")

    def moveFile(self, file_name, folder_name):
    ...
    while True:
        try:
            ...
            if action == 'exit':
                ...
                file_content = client.downloadFileByName(parts[1])
                print("-" * 20 + f"\n{file_content}\n" + "-" * 20)
                ...
                client.uploadFile(local_data=parts[2], destination_name=parts[1])
            
            elif action == 'delete':
                client.deleteFileByName(parts[1])
            
            elif action == 'move':
                client.moveFile(file_name=parts[1], folder_name=parts[2])

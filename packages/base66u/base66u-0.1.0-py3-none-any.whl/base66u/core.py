...
from .exceptions import Base66UError
...
def b66u_decode(s: str, with_checksum: bool = False) -> bytes:
    ...
    if with_checksum:
        if len(s) < 4:
            raise Base66UError("Too short for checksum")
        sep = s.rfind("~")
        if sep == -1 or len(s) - sep != 4:
            raise Base66UError("Missing or misplaced checksum")
        chk = s[sep+1:]
        core = s[:sep]
        if core.endswith("~~"):
            core = core[:-1]  # unescape
        elif core.endswith("~"):
            raise Base66UError("Dangling separator")
    ...
    for ch in payload:
        try:
            ...
            raise Base66UError(f"Invalid character: {repr(ch)}")
        n = n * BASE + d
    ...
    if with_checksum:
        ...
        if crc != decode_checksum(chk):
            raise Base66UError("Checksum mismatch")

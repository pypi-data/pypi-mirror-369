...
from data_getters import get_alzheimer, get_amide, get_mice, \
    get_data, get_bacteria_images_ms2
from bernn.dl.models.pytorch.aedann import ReverseLayerF
from aedacnn import AutoEncoder32
from aedacnn import AutoEncoder64

from aedacnn import SHAPAutoEncoder32
from aedacnn import SHAPAutoEncoder64
from bernn.dl.models.pytorch.utils.loggings import TensorboardLoggingAE, \
    log_metrics, log_input_ordination, LogConfusionMatrix, log_plots, \
        log_neptune, log_shap, log_mlflow, make_data
...
from bernn.dl.models.pytorch.utils.utils import get_optimizer, to_categorical, \
    get_empty_dicts, get_empty_traces, log_traces, get_best_values, \
        add_to_logger, add_to_neptune, \
            add_to_mlflow
import mlflow
import warnings
...
def keep_top_features(data, path, args):
    ...
    for group in ['all', 'train', 'valid', 'test']:
        data['inputs'][group] =\
              data['inputs'][group].loc[:, top_features.iloc[:, 0].values[:args.n_features]]
...
class TrainAE:
    ...
    def __init__(self, args, path, fix_thres=-1, load_tb=False, log_metrics=False, 
                 keep_models=True, log_inputs=True, log_plots=False, log_tb=False,
                 log_neptune=False, log_mlflow=True, groupkfold=True):
        """
    ...
    def make_samples_weights(self):
        """
        Makes the samples weights for the training set. The weights are 
        calculated as the inverse of the frequency of each class in the training set.
        Returns:
            ...
            if len(event_acc.Tags()['tensors']) > 2 and self.load_tb:
                ...
                for epoch in range(0, self.args.n_epochs):
                    ...
                    if np.mean(values['valid']['mcc'][-self.args.n_agg:]) > self.best_mcc and len(
                        ...
                        im.save("rec.jpeg")
                        im = Image.fromarray(lists['test']['inputs'][-1][0][0] * 255).convert("L")
                        im.save("input.jpeg")
    ...
    def warmup_loop(self, optimizer_ae, ae, celoss, loader, triplet_loss, mseloss, best_loss, warmup, epoch,
        ...
        if (self.args.early_warmup_stop != 0 and self.warmup_counter == self.args.early_warmup_stop) and warmup:  # or warmup_counter == 100:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--new_size', type=int, default=64, help='')
    ...
    if args.new_size == 32:
        ...
        SHAPAutoEncoder = SHAPAutoEncoder32
    if args.new_size == 64:

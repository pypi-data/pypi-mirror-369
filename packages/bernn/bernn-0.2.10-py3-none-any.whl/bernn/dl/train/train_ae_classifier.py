...
class TrainAE:
    ...
    def train(self, params):
        ...
        if len(event_acc.Tags()['tensors']) > 2 and self.load_tb:
            ...
            if self.pools:
                loaders = get_loaders(data, self.args.random_recs, self.samples_weights,
            ...
            shap_ae.load_state_dict(torch.load(f'{self.complete_log_path}/model.pth'))
        ...
        if np.mean(best_mccs) > self.best_mcc:
            try:
                if os.path.exists(
                        f'logs/best_models/ae_classifier/{self.args.dataset}/{self.args.dloss}_vae{self.args.variational}'):
                    shutil.rmtree(
                        f'logs/best_models/ae_classifier/{self.args.dataset}/{self.args.dloss}_vae{self.args.variational}',
                        ignore_errors=True)

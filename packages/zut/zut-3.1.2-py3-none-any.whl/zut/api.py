...
from zut import get_logger, NotFound
...
class ApiClient:
    ...
    def get(self, endpoint: str, *, params: Mapping|None = None, headers: MutableMapping[str,str]|None = None) -> dict[str,Any]:
        result = self.request(endpoint, method='GET', params=params, headers=headers)
        if not isinstance(result, dict):
            raise ApiClientError("Response is not a dictionnary", result, message_maxlen=self.error_message_maxlen)
        return result


    def iter(self, endpoint: str, *, params: Mapping|None = None, headers: MutableMapping[str,str]|None = None) -> Generator[dict[str,Any],Any,None]:
        data = self.request(endpoint, method='GET', params=params, headers=headers)
    ...
    def get_list(self, endpoint: str, *, params: Mapping|None = None, headers: MutableMapping[str,str]|None = None) -> list[Any]:
        return [data for data in self.iter(endpoint, params=params, headers=headers)]


    def post(self, endpoint: str, data = None, *, params: Mapping|None = None, headers: MutableMapping[str,str]|None = None, content_type: str|None = None, content_length: int|None = None, content_filename: str |None= None) -> dict[str,Any]:
        result = self.request(endpoint, data, method='POST', params=params, headers=headers, content_type=content_type, content_length=content_length, content_filename=content_filename)
        if not isinstance(result, dict):
            raise ApiClientError("Response is not a dictionnary", result, message_maxlen=self.error_message_maxlen)
        return result
    

    def put(self, endpoint: str, data = None, *, params: Mapping|None = None, headers: MutableMapping[str,str]|None = None, content_type: str|None = None, content_length: int|None = None, content_filename: str |None= None) -> dict[str,Any]:
        result = self.request(endpoint, data, method='PUT', params=params, headers=headers, content_type=content_type, content_length=content_length, content_filename=content_filename)
        if not isinstance(result, dict):
            raise ApiClientError("Response is not a dictionnary", result, message_maxlen=self.error_message_maxlen)
        return result
   

    def request(self, endpoint: str, data = None, *, method = None, params: Mapping|None = None, headers: MutableMapping[str,str]|None = None, content_type: str|None = None, content_length: int|None = None, content_filename: str|None = None) -> list[Any]|dict[str,Any]:
        ...
        try:
            ...
            if error.code == HTTPStatus.NOT_FOUND.value:
                raise NotFound(_prepare_error_message(error, http_data, message_maxlen=self.error_message_maxlen))
            else:
                raise ApiClientError(error, http_data, message_maxlen=self.error_message_maxlen) from None

        except Exception as error:
            raise ApiClientError(error, message_maxlen=self.error_message_maxlen) from None
...
class ApiClientError(Exception):
    def __init__(self, orig: str|Exception|None, data: list|dict|str|None = None, *, message_maxlen: int|None = 400):        
        ...
        if isinstance(orig, HTTPError):
            self.nature = 'HTTP'
            self.code = orig.status
            self.reason = orig.reason
        elif isinstance(orig, URLError):
            self.nature = 'URL'
            self.code = orig.errno
            self.reason = str(orig.reason) if not isinstance(orig.reason, str) else orig.reason
        ...
        self.message = _prepare_error_message(orig, data, message_maxlen=message_maxlen)

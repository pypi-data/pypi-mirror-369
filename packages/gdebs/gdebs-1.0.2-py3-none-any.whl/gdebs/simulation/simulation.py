...
class Simulation:
    ...
    def simulate_year(self) -> Dict[str, int]:
        ...
        if self.settings.get('added_ge_carriers', False):
            # Check if population has dropped to 10% of original size
            ...
            if self.original_population_size is not None:
                ...
                if current_population <= population_threshold:
                    # Population has declined to 500 or less - stop adding GE carriers
                    self.settings['added_ge_carriers'] = False
                    print(f"Population declined to {current_population} (10% threshold reached). Stopping GE carrier addition.")
                else:

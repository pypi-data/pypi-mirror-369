...
from msgraph.generated.users.item.followed_sites.followed_sites_request_builder import (
...
async def get_followed_sites(
    ...
    request_configuration = RequestConfiguration(
        query_parameters=FollowedSitesRequestBuilder.FollowedSitesRequestBuilderGetQueryParameters(
            top=limit,
        ),
    )

    response = await client.me.followed_sites.get(request_configuration=request_configuration)

    if not response or not response.value:
        return {"sites": [], "count": 0}

    return {
        "sites": [serialize_site(site) for site in response.value if site],
        "count": len([site for site in response.value if site]),
    }

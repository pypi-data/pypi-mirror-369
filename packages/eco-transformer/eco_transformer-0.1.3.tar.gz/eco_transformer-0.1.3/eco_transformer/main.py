...
# ===== CLI ENTRY POINT =====
def main_cli():
    parser = argparse.ArgumentParser(description="Run eco-transformer experiments")
    parser.add_argument("--model_name", default="tiiuae/falcon-rw-1b", help="Hugging Face model name")
    args = parser.parse_args()

    # Default prompts and references
    prompts = [
    ...
    results = transformer(args.model_name, prompts, references)

    print("\n--- BASELINE OUTPUTS ---")
    for i, (ppl, bleu, rouge, gen) in enumerate(results['baseline']['outputs']):
        print(f"Prompt {i+1}: {prompts[i]}")
        print(f"Generated: {gen}\n")

    print("\n--- OPTIMIZED OUTPUTS ---")
    for i, (ppl, bleu, rouge, gen) in enumerate(results['optimized']['outputs']):
        print(f"Prompt {i+1}: {prompts[i]}")
        print(f"Generated: {gen}\n")
...
# Allows terminal call as "eco_transformer"

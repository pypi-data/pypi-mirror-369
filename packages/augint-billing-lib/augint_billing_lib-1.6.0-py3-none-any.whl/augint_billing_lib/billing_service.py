...
- StripeEventProcessor for handling Stripe events
...
class BillingService:
    ...
    Attributes:
        ...
        stripe_event_processor: Component for Stripe event processing
    ...
    def __init__(
        ...
        eventbridge_bus: str | None | object = _EVENTBRIDGE_UNSET,
        ...
        try:
            ...
            self.stripe_event_processor = StripeEventProcessor(
                stripe_manager=self.stripe_manager, db_client=self.db_client
            )
            # Backward compatibility alias
            self.webhook_processor = self.stripe_event_processor
    ...
    def track_usage(
        ...
        if self.event_publisher:
            try:
                ...
                usage_timestamp = timestamp or datetime.now(UTC)
    ...
    def generate_invoice(
        ...
        if send_to_stripe and self.stripe_key:
            ...
            invoice = Invoice(
                ...
                due_date=calculate_due_date(datetime.now(UTC)),
                line_items=[
                    {
    ...
    def process_stripe_event(
        ...
        come from webhooks, EventBridge, queues, or any other source. This method
        directly delegates to the StripeEventProcessor.
        ...
        Example:
            >>> # Process event from EventBridge
            >>> result = service.process_stripe_event(
            ...     event_type="invoice.payment_succeeded",
            ...     event_data={"id": "inv_123", "amount_paid": 5000},
            ...     event_id="evt_123"
            ... )
        """
        return self.stripe_event_processor.process_stripe_event(  # type: ignore[no-any-return]

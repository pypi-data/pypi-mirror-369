...
from .event_types import StripeEventData
...
def validate_usage(usage_record: UsageRecord) -> bool:
    ...
    if usage_record.timestamp.replace(tzinfo=UTC) > now + timedelta(minutes=5):
    ...
    if usage_record.timestamp.replace(tzinfo=UTC) < ninety_days_ago:
...
def get_billing_period_bounds(date: datetime | None = None) -> tuple[datetime, datetime]:
    ...
    # Start of the month
    period_start = datetime(date.year, date.month, 1, 0, 0, 0, tzinfo=UTC)

    # End of the month (start of next month)
    december = 12
    if date.month == december:
        period_end = datetime(date.year + 1, 1, 1, 0, 0, 0, tzinfo=UTC)
    else:
        period_end = datetime(date.year, date.month + 1, 1, 0, 0, 0, tzinfo=UTC)
...
def calculate_proration(
    ...
    billing_period_start = billing_period_start.replace(tzinfo=UTC)
    billing_period_end = billing_period_end.replace(tzinfo=UTC)
...
def verify_webhook_signature(payload: bytes, signature: str, secret: str) -> bool:
    """Verify generic webhook signature for security.
...
def verify_stripe_signature(
    payload: bytes, signature: str, signature_secret: str
) -> StripeEventData:
    """Verify Stripe webhook signature and return the event.

    This utility function validates Stripe webhook signatures for secure
    event delivery. It should be used at the infrastructure layer (e.g.,
    Lambda handler) before passing events to the processor.

    Args:
        payload: Raw webhook payload bytes
        signature: Stripe signature header value
        signature_secret: Your webhook endpoint's secret

    Returns:
        Validated and parsed Stripe event data

    Raises:
        SignatureVerificationError: If signature validation fails
        EventValidationError: If payload is invalid

    Example:
        >>> # In your Lambda handler or API endpoint
        >>> try:
        ...     event = verify_stripe_signature(payload, signature, secret)
    ...
    if not signature_secret:
        raise SignatureVerificationError(
            "Signature secret required for webhook validation",
            signature_type="stripe",
            source="webhook",
        )

    try:
        event: StripeEventData = stripe.Webhook.construct_event(
            payload, signature, signature_secret
        )  # type: ignore[no-untyped-call]
        return event
    except ValueError as e:
        raise EventValidationError(f"Invalid payload: {e!s}") from e
    except Exception as e:
        # Handle Stripe's SignatureVerificationError
        if e.__class__.__name__ == "SignatureVerificationError":
            raise SignatureVerificationError(
                f"Invalid signature: {e!s}", signature_type="stripe", source="webhook"
            ) from e
        raise


def parse_stripe_event(payload: bytes) -> dict[str, Any]:
    """Parse a Stripe event payload without signature verification.

    Use this only in trusted environments where signature verification
    is handled elsewhere or not required (e.g., EventBridge).
    ...
    Raises:
        EventValidationError: If payload is invalid JSON
    """
    try:
        return json.loads(payload)  # type: ignore[no-any-return]
    except json.JSONDecodeError as e:
        raise EventValidationError(f"Invalid JSON payload: {e!s}") from e

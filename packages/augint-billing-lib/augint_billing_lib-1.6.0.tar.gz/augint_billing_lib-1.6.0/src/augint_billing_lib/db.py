...
from .cache import CacheProtocol, LambdaMemoryCache, cache_invalidate, cached
from .config import get_settings
from .exceptions import DynamoDBError
from .retry import ServiceRetryConfigs, is_retryable_error, retry_with_backoff
...
class DynamoDBClient:
    ...
    def __init__(
        ...
        cache: CacheProtocol | None = _CACHE_UNSET,  # type: ignore[assignment]
    ):
        ...
        Args:
            ...
            cache: Optional cache implementation (creates default if not provided)
        """
        ...
        # Initialize cache with proper handling of explicit None vs not provided
        self._cache: CacheProtocol | None
        if cache is _CACHE_UNSET:
            # No cache parameter provided, use default
            self._cache = LambdaMemoryCache()
        elif cache is None:
            # Cache explicitly disabled
            self._cache = None
        else:
            # Cache explicitly provided
            self._cache = cache
    ...
    def _handle_error(self, operation: str, error: Exception) -> NoReturn:
        ...
        Raises:
            DynamoDBError: Wrapped error with context, or re-raises retryable errors
        """
        logger.error(f"DynamoDB {operation} failed: {error}")

        # Check if this is a retryable error - if so, let it bubble up to retry decorator
        if is_retryable_error(error, ServiceRetryConfigs.DYNAMODB.retryable_exceptions):
            raise error

        # Handle non-retryable errors with proper wrapping
    ...
    @cache_invalidate(["*"])  # Clear all cache on writes (simple strategy)
    ...
    @cache_invalidate(["*"])  # Clear cache on update (simple strategy for now)
    @retry_with_backoff(**ServiceRetryConfigs.DYNAMODB.to_dict())
    def update_item(
        self,
        pk: str,
    ...
    @cache_invalidate(["*"])  # Clear cache on delete (simple strategy for now)
    @retry_with_backoff(**ServiceRetryConfigs.DYNAMODB.to_dict())
    ...
    @cached(ttl=300, key_pattern="customer:{customer_id}")
    ...
    def get_usage_by_date(
        self, date: datetime, customer_id: str | None = None
    ) -> list[dict[str, Any]]:
    ...
    @cached(ttl=300, key_pattern="billing_period:{customer_id}:{year}:{month}")
    def get_billing_period(self, customer_id: str, year: int, month: int) -> dict[str, Any] | None:
    ...
    @cached(
        ttl=3600, key_pattern="pricing_tiers:{plan_type}"
    )  # Cache for 1 hour (pricing changes rarely)
    ...
    @cached(ttl=600, key_pattern="invoice:{invoice_id}")  # Cache for 10 minutes
    ...
    @cache_invalidate(["billing_period:{customer_id}:{year}:{month}"])
    ...
    @retry_with_backoff(**ServiceRetryConfigs.DYNAMODB.to_dict())
    def list_customers(
        self, limit: int | None = None, last_evaluated_key: dict[str, Any] | None = None
    ) -> list[dict[str, Any]]:
        """List all customers.
...
class AsyncDynamoDBClient:
    ...
    def __init__(self, sync_client: DynamoDBClient | None = None):

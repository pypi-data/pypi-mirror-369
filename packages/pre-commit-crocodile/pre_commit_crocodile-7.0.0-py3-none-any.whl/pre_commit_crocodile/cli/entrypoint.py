    ...
    def cli(options: Namespace, ) -> Result:
        ...
        if options.configure and Gcil.exists():
            ...
            if options.components:
                components_sources = False
            elif options.no_components:
                components_sources = True
            else:
                components_sources = not any(Bundle.COMPONENTS_DOMAIN in url
                                             for url in Git.urls())

            # Bind components version
            if not components_sources:
                ...
                if f'{Bundle.COMPONENT_COMMITS}@{Version.get()}' not in local_ci_data:
                    ...
                    with open(
                        ...
                        if f'{Bundle.COMPONENT_COMMITS}@' not in local_ci_data:
                            f.write(
                                Gcil.EOL.join([
                                    'include:',
                                    f'  - component: {Bundle.COMPONENT_COMMITS}@{Version.get()}',
                                    '    inputs:',
                                    '      stage: test',
                                    f'      name: {Bundle.COMPONENT_COMMITS.rsplit("/", 1)[-1]}',
                                    '',
                                    '',
                                ]))
                        else:
                            local_ci_data = Gcil.EOL.join([
                                f'{line[0:line.rfind("@")]}@{Version.get()}' #
                                if f'component: {Bundle.COMPONENT_COMMITS}@' in line else
                                line #

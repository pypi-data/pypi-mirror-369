    ...
    if tm_threshold is not None and tm_unit is not None:  # if there is a temporal threshold and temporal unit
        ...
        for section_unit in sections_pz['unit'].unique():  # for each unique unit of the presence zones
            ...
            presencezones_list.append({  # append to the presence zones list...
                ...
                'presencezones':  # the temporally overlapping presences as a MultiPoint (if there are any, else None)
...
def generate_absences(
    ...
    # if using along-the-line variation, lines can be 'cookie-cuttered' to ensure absences are not within presence zones
    if var in ['a', 'along']:
        lines['absencelines'] = (
            lines.apply(lambda r:  # take lines and presence zones and...
                        ...
                        else r['absencelines'], axis=1))  # ...otherwise lines are left as they were

    lines['dfbal'] = lines.geometry.length.cumsum()  # calculate distance from beginning for each line
    lines_all = lines.geometry.union_all()  # put all lines into single geometry
    points_list = []  # list for the points
    i = 0  # initialise i
    while i < target:  # while count of points is less than the target to be generated
        # generate a point (depends on variation)
        if var in ['a', 'along']:  # along-the-line variation - randomly sample point along lines
            point = lines_all.interpolate(random.uniform(a=0, b=lines_all.length))  # interpolate point along lines
            dfbal = line_locate_point(line=lines_all, other=point)  # get distance from beginning of lines to point
            line = lines.iloc[lines[lines['dfbal'] > dfbal]['dfbal'].idxmin()]  # get line along which point lies
            points_list.append({  # append...
                'point': point,  # ...point
                'date': line['date'],  # ...date
                'dfbal': dfbal})  # ...distance from beginning of lines to point
            i += 1  # increase the count
        elif var in ['f', 'from']:  # from-the-line variation - randomly sample point from absence lines
            dist = random.uniform(a=0, b=lines_all.length-0.001)  # randomly select distance along absence line
            point_a = lines_all.interpolate(dist)  # make point at that distance
            point_b = lines_all.interpolate(dist+0.001)  # make point at that distance plus a tiny (arbitrary) distance
            ...
            dfbal = line_locate_point(line=lines_all, other=point_a)  # get distance from beginning of absence lines to point a
            line = lines.iloc[lines[lines['dfbal'] > dfbal]['dfbal'].idxmin()]  # get absence line along which point a lies
            if not point.intersects(line['presencezones']):  # if point not in corresponding presence zones...
                points_list.append({  # ...append...
                    'point': point,  # ...point
                    'date': line['date'],  # ...date
            ...
            raise Exception
    points = gpd.GeoDataFrame(points_list, geometry='point', crs=lines.crs)  # GeoDataFrame
    return points
...
def absences_delimit(
    ...
    absencelines_cols = ['section_id', 'date', 'absencelines', 'presencezones']
    if block is not None:
        check_dtype(par='block', obj=block, dtypes=str, none_allowed=True)
        check_cols(df=absencelines, cols=block)
        absencelines_cols = absencelines_cols + [block]
    absencelines = absencelines[absencelines_cols]  # necessary
    # absencelines is a GeoDataFrame
    #   with columns: 'section_id', 'date', 'absencelines', 'presencezones', and, optionally, block
    #   of which, column 'absencelines' is the geometry column

    if block is not None and how is not None:
        ...
        rate = target / float(absencelines.length.sum())
        absences_list = []
        for uniq in absencelines[block].unique():
            absencelines_block = absencelines.copy()[absencelines[block] == uniq].reset_index(drop=True)

            if how in ['target', 't']:
                target_block = target
            elif how in ['average', 'a']:
                target_block = int(np.ceil(target / absencelines[block].nunique()))
            elif how in ['effort', 'e']:
                target_block = int(np.ceil(absencelines_block.length.sum() * rate))
            elif how in ['presences', 'p']:
                target_block = len(presences[presences[block] == uniq]) * target
            else:  # should never be reached given check_opt() above
                target_block = None

            if target_block > 0:  # if the target is more than 0
                absences_block = generate_absences(
                    lines=absencelines_block,
                    var=var,
                    target=target_block,
                    dfls=dfls)
                absences_block[block] = uniq
    ...
    absences_cols = ['point_id', 'point', 'date']  # columns to keep
...
def samples_point(datapoints: gpd.GeoDataFrame, presences: gpd.GeoDataFrame, absences: gpd.GeoDataFrame,
    ...
    if sections is not None:  # if Sections provided
        ...
        absences = pd.merge_asof(absences.sort_values('dfbsl'),  # merge the thinned absences with datapoints by...
                                 ...
                                 on='dfbsl', direction='backward')  # ...the nearest DFBSL going backwards as...

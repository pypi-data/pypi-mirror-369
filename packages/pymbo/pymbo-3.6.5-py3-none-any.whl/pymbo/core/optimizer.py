...
class SimpleParameterTransformer:
    ...
    def validate_parameter_constraints(self, params_dict: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """
        Validates that parameters satisfy their Target and Range constraints.
        ...
        for name, config in self.params_config.items():
            ...
            if goal == "Target" and "target_value" in config:
                target = config["target_value"]
                tolerance = config.get("constraint_tolerance", 1e-3)
                if abs(value - target) > tolerance:
                    violations.append(f"Parameter '{name}' violates Target constraint: "
                                    f"{value} not within {tolerance} of target {target}")
    ...
    def enforce_parameter_constraints(self, params_dict: Dict[str, Any]) -> Dict[str, Any]:
        """
        Enforces parameter constraints by clipping values to feasible regions.
        ...
        for name, config in self.params_config.items():
            ...
            if goal == "Target" and "target_value" in config:
                # For Target constraints, clamp to target value if outside tolerance
                target = config["target_value"]
                tolerance = config.get("constraint_tolerance", 1e-3)
                if abs(value - target) > tolerance:
                    constrained_params[name] = target
                    logger.debug(f"Enforced Target constraint for '{name}': {value} -> {target}")
...
class EnhancedMultiObjectiveOptimizer:
    ...
    def _setup_parameter_constraints(self) -> None:
        """
        Sets up parameter constraints for Target and Range goals following academic standards.
        
        Target goals create equality constraints of the form: |param - target| <= tolerance
        ...
        for name, config in self.params_config.items():
            ...
            if goal == "Target":
                # Target constraint: equality constraint with tolerance
                target_value = config.get("target_value")
                if target_value is not None:
                    # Transform target to normalized space
                    param_bounds = self.parameter_transformer.bounds[param_idx]
                    normalized_target = (target_value - param_bounds[0]) / (param_bounds[1] - param_bounds[0])
                    normalized_target = max(0.0, min(1.0, normalized_target))
                    
                    # Create constraint function for |x[param_idx] - target| <= tolerance
                    def create_target_constraint(idx, target, tol):
                        def constraint_func(x):
                            return tol - torch.abs(x[..., idx] - target)
                        return constraint_func
                    
                    constraint = create_target_constraint(param_idx, normalized_target, self.constraint_tolerance)
                    self.parameter_constraints.append(constraint)
                    
                    logger.info(f"Added Target constraint for parameter '{name}': target={target_value} "
                               f"(normalized: {normalized_target:.4f})")
    ...
    def predict_responses_at(
        ...
        try:
            ...
            X_test = param_tensor.unsqueeze(0).to(self.device, self.dtype)  # Add a batch dimension and move to device

...
class CurrencyField(ChoiceField):
    ...
    def _get_base_currencies(self) -> list[str]:
        ...
        """
        try:
            currencies = [item['code'] for item in self._market_data.get_currencies(fields='code', status__eq='active')]
        ...
        if not currencies:
            currencies = list(settings.ENTITY_BASE_CURRENCY_DEFAULT_LIST)

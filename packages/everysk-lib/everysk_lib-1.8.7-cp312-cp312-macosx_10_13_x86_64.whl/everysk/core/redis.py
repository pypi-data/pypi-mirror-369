...
from everysk.core.fields import FloatField, StrField, BoolField
...
class RedisClient(BaseObject):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        if RedisClient._connection.get(self._connection_key(), None) is None:
            self._connect()
...
class RedisCache(RedisClient):
    ...
    def get_multi(self, keys: list[str]) -> dict:
        ...
        return {key: self._decode(value) for key, value in zip(keys, data_list)}

    def set(self, key: bytes, value: Any, timeout: int = None) -> bool:
        """
    ...
    def set_multi(self, data_dict: dict, timeout: int = None) -> None:
        ...
        if False in ret:
            log.error('Error RedisCache set_multi', extra={
                'labels': {
    ...
    def incr(self, key: str, delta: int = 1, initial_value: Any = None, timeout: int | None = None) -> int:
        ...
        if ret[1] is None:
            log.error('Error RedisCache incr', extra={
                'labels': {
                    'REDIS_NAMESPACE': settings.REDIS_NAMESPACE,
                    'key': key,
                    'delta': delta,
    ...
    def decr(self, key: str, delta: int = 1, initial_value: Any = None, timeout: int = None) -> int | None:
        """
        ...
        key_ = self.get_hash_key(key)
        with self.connection.pipeline(True) as pipe:
            while True:
                try:
                    pipe.watch(key_)
    ...
    def rpush(self, name: str, *values, timeout: int | None = None) -> bool:
        ...
        datas = []
        for value in values:
            datas.append(self._encode(value))
...
class RedisLock(RedisClient):
    ...
    def acquire(self, token: str | None = None, blocking: bool = None, blocking_timeout: float = None) -> bool:
        """

...
def simple_poll_delete(
    grpc_client: GrpcClient, task_map: Dict[metadata_pb2.ResourceID, str]
):
    """Simple polling for delete/prune operations using task IDs."""
    StatusDisplayer(grpc_client, []).simple_poll(task_map)
...
class StatusDisplayer:
    ...
    def simple_poll(self, task_map: Dict[Tuple[str, str, int], str]):
        ...
        for (res_type, name, variant), task_id in task_map.items():
            print(f"  - {name}:{variant} (task {task_id})")
        print()
            ...
            for res_key, task_id in incomplete.items():
                ...
                if status_val not in (
                    ...
                    if status_val != metadata_pb2.ResourceStatus.READY:
                        ...
                        raise RuntimeError(
                            f"Task {task_id} {status_name.lower()} for {name}:{variant}{error_detail}"
                        )
                    else:
                        print(
                            f"[OK] {name}:{variant} - "

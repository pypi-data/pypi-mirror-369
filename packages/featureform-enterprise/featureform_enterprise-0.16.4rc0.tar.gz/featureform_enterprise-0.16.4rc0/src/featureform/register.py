...
class ResourceClient:
    ...
    **Using the Resource Client:**
    ...
    def _execute_cleanup(self, resources_to_delete, asynchronous):
        ...
        # Print summary
        self._print_cleanup_summary(asynchronous, stats)

        return deleted_resources

    def _print_cleanup_summary(self, asynchronous, stats):
        """
        ...
        if asynchronous:
            ...
            print(f"  Queued for deletion: {stats['queued']}")
            if stats['failed'] > 0:
                print(f"  Failed: {stats['failed']}")
            print("\nResources will be deleted in the background")
        else:
            print("Cleanup Complete")
            print(f"  Deleted: {stats['queued']}")
            if stats['failed'] > 0:
                print(f"  Failed: {stats['failed']}")
        ...
        if stats['errors'] and self.logger.level <= logging.DEBUG:
            print("\nDetailed errors:")
            for resource_id, error in stats['errors'][:5]:  # Show first 5 errors
    ...
    def _get_resource(
        ...
        """Fetch a resource via the GetResource RPC.

        Args:
            resource_id: The resource ID to fetch
            was_deleted: If True, include deleted resources in the search

        Returns:
            The resource if found, None otherwise
        """
        request = metadata_pb2.GetResourceRequest(
            resource_id=resource_id,
            was_deleted=was_deleted,
        )

        self.logger.debug(
            f"Fetching resource {resource_id.resource.name} "
            f"with variant {resource_id.resource.variant} of type"
            f" {resource_id.resource_type}..."
        )

        try:
            resp = self._stub.GetResource(iter([request]))

            # GetResource returns a stream, so get the first item
            try:
                return next(resp)
            except StopIteration:
                self.logger.warning(
                    f"No resource found for {resource_id.resource.name}:"
                    f"{resource_id.resource.variant}"
                )
                return None

        except grpc.RpcError as e:
            self.logger.error(
                f"gRPC error fetching resource {resource_id.resource.name}:"
                f"{resource_id.resource.variant} - {e.code()}: {e.details()}"
            )
            return None
        except Exception as e:
            self.logger.error(
                f"Unexpected error fetching resource {resource_id.resource.name}:"
                f"{resource_id.resource.variant} - {str(e)}"
            )
            return None

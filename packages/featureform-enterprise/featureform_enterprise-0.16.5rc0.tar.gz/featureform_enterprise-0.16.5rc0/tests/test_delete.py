...
class TestDelete:
    ...
    def client(self):
        ...
        client._stub.PruneResource.return_value = SimpleNamespace(results={})
        client._stub.MarkForDeletion.return_value = SimpleNamespace(results={})
    ...
    def test_delete_returns_task_id(self, client):
        response = metadata_pb2.MarkForDeletionResponse(task_id="1")
        client._stub.MarkForDeletion.return_value = response
        results = client.delete(
            "source_variant", variant="v1", resource_type=ResourceType.SOURCE_VARIANT
        )
        assert results[1] == "1"

    def test_prune_sync_polls_tasks(self, client):
        res_id = metadata_pb2.ResourceID(
            resource=metadata_pb2.NameVariant(name="source_variant", variant="v1"),
            resource_type=ResourceType.SOURCE_VARIANT.to_proto(),
        )
        response = metadata_pb2.PruneResourceResponse(
            results=[
                metadata_pb2.PruneResourceResult(resource=res_id, task_id="3")
            ]
        )
        client._stub.PruneResource.return_value = response
        client._stub.GetTaskStatuses.return_value = metadata_pb2.GetTaskStatusesResponse(
            task_statuses=[
                metadata_pb2.TaskStatus(
                    task_id="3",
                    status=metadata_pb2.ResourceStatus(status=metadata_pb2.ResourceStatus.READY),
                )
            ]
        )
        task_map = client.prune(
            "source_variant",
            variant="v1",
            resource_type=ResourceType.SOURCE_VARIANT,
            asynchronous=False,
        )
        assert (
            task_map[(ResourceType.SOURCE_VARIANT.to_proto()), "source_variant", "v1"]
            == "3"
        )
        client._stub.GetTaskStatuses.assert_called()

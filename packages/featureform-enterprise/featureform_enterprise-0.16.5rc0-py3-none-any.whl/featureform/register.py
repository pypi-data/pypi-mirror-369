...
class ResourceClient:
    ...
    **Using the Resource Client:**
    ...
    def prune(
        ...
        resources_to_tasks = {
            (r.resource.resource_type, r.resource.resource.name, r.resource.resource.variant): r.task_id
    ...
    def _cleanup(self, resource_state, mode, asynchronous=False):
        ...
        for res_type, name, variant in resources_to_keep:
            print(f"  - {res_type.name}: {name} (variant: {variant})")
    ...
    def _get_resources_to_keep(self, resource_state):
        ...
        for res in resource_state.sorted_list():
            if isinstance(res, ResourceVariant):
                resources_to_keep.add((
    ...
    def _find_resources_to_delete(self, resources_to_keep):
        ...
        Returns:
            list: Tuples of (type, name, variant) to delete
        """
    ...
    def _execute_cleanup(self, resources_to_delete, asynchronous):
        ...
        failed_resources = []  # Track failed deletions
        stats = {
            'queued': 0,
            'failed': 0,
            'errors': []
        ...
        for resource_type, name, variant in resources_to_delete:
            ...
            try:
                ...
                if resources_to_tasks:
                    ...
                    print(f" [OK] {operation}: {resource_id}")

            except Exception as e:
                err_msg = str(e)
                if "Could not find resource to delete" in err_msg:
                    # Just skip and maybe log at debug level
                ...
                self.logger.error(f"Failed to prune {resource_id}: {e}")
                print(f"  [FAIL] Failed: {resource_id}")

        # Print detailed summary with deleted and failed resources
        self._print_detailed_cleanup_summary(asynchronous, stats, deleted_resources, failed_resources)

        return deleted_resources

    def _print_detailed_cleanup_summary(self, asynchronous, stats, deleted_resources, failed_resources):
        """
        Print a detailed summary of the cleanup operation including deleted and failed resources.
        ...
        if stats['errors'] and self.logger.level <= logging.DEBUG:
            print("\nDETAILED ERRORS:")
            print("-" * 30)
            for resource_id, error in stats['errors'][:10]:  # Show first 10 errors
                print(f"  {resource_id}:")
                print(f"    Error: {error}")
        ...
        if not asynchronous:
            if stats['failed'] == 0:
                ...
                print(f"\n⚠ Cleanup completed with {stats['failed']} failures")
        else:
            print(f"\n➤ Resources will be deleted in the background")
    ...
    def get_resource(self, resource_type, name, variant, was_deleted=False):
        """Return the ResourceVariant for a given resource.

        Args:
            resource_type (ResourceType): The type of the resource.
            name (str): The resource name.
            variant (str): The resource variant.
            was_deleted (bool): Whether to include deleted resources.

        Returns:
            pb.ResourceVariant: The resource variant proto.
        """

        request = pb.GetResourceRequest(
            resource_id=pb.ResourceID(
                resource=pb.NameVariant(name=name, variant=variant),
                resource_type=resource_type,
            ),
            was_deleted=was_deleted,
        )
        return next(self._stub.GetResource(iter([request])))

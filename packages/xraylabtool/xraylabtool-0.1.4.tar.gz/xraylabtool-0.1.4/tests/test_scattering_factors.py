...
class TestScatteringFactorLoading:
    ...
    def test_invalid_element_symbol(self):
        ...
        # Test empty string
        with pytest.raises(ValueError, match="Element symbol must be a non-empty string"):
            load_scattering_factor_data('')
    ...
    def test_nonexistent_element_file(self):
        ...
        with pytest.raises(FileNotFoundError, match="Scattering factor data file not found"):
...
class TestPCHIPInterpolators:
    ...
    def test_create_interpolators_success(self):
        ...
        try:
            ...
            assert not np.isnan(f1_val).any() if hasattr(f1_val, 'any') else not np.isnan(f1_val)
            assert not np.isnan(f2_val).any() if hasattr(f2_val, 'any') else not np.isnan(f2_val)
        ...
        try:
            ...
            try:
                ...
                if not np.isnan(result):
                    pytest.skip("Interpolator allows extrapolation or returns boundary values")

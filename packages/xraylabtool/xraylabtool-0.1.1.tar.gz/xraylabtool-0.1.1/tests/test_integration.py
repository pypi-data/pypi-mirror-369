...
from xraylabtool import Refrac, SubRefrac
...
class TestBasicSetupAndInitialization:
    ...
    def test_basic_setup_and_initialization(self):
        ...
        assert len(data) == 2
        assert all(material in data.keys() for material in MATERIALS)
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_mismatched_array_lengths(self):
        ...
        with pytest.raises(ValueError, match=r".*Number of formulas.*must match.*number of densities.*"):
            Refrac(MATERIALS, ENERGIES, [1.0])  # Wrong density count
...
class TestSubRefracErrorHandling:
    ...
    @pytest.mark.skip(reason="Requires atomic data files to test invalid elements")
    def test_invalid_chemical_formula(self):
        ...
        with pytest.raises(ValueError, match=r".*Element.*not found.*"):
            SubRefrac("InvalidElement123", [8.0], 2.2)
...
class TestPerformanceBenchmarks:
    ...
    def test_benchmark_single_calculation(self, benchmark):
        ...
        def single_calculation():
            return SubRefrac("SiO2", np.arange(1.0, 20.1, 0.1), 2.2)
    ...
    def test_benchmark_multi_calculation(self, benchmark):
        ...
        def multi_calculation():
            return Refrac(formulas, energies, densities)
    ...
    def test_benchmark_energy_sweep(self, benchmark):
        ...
        def energy_sweep():
            ...
            return SubRefrac("SiO2", energies, 2.2)
    ...
    def test_benchmark_formula_complexity(self, benchmark):
        ...
        def complex_formula():
            ...
            return SubRefrac("SiO2", [5.0, 8.0, 10.0, 15.0, 20.0], 2.65)

...
class SmartFollowUpService:
    ...
    @staticmethod
    def validate_next_call_time_or_default(ai_agent_decision: SmartFollowUpDecision, next_follow_up_number: int, strategy: FollowUpStrategy) -> datetime:
        ...
        if ai_agent_decision.next_call_time is None:
            return datetime.now(ZoneInfo('UTC')) + timedelta(minutes=strategy.get_interval_for_followup(next_follow_up_number))
        else:
            return ai_agent_decision.next_call_time
    ...
    def update_based_on_decision(chat: Chat, decision: SmartFollowUpDecision, smart_follow_up_state: SmartFollowUpState, flow_preview : FlowPreview, follow_up_strategy: FollowUpStrategy, execution_context: ExecutionContext) -> None:
        ...
        if decision.action == SmartFollowUpDecisionAction.SEND or decision.action == SmartFollowUpDecisionAction.SUGGEST:
            ...
            SmartFollowUpService.validate_next_call_time_or_default(decision, smart_follow_up_state.consecutive_count, follow_up_strategy)

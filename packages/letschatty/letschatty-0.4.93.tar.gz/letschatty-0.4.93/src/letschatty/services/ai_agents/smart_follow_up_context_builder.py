...
from letschatty.services.ai_agents import cot_follow_up_examples
...
from letschatty.models.utils.custom_exceptions.custom_exceptions import HumanInterventionRequired, PostponeFollowUp
...
class SmartFollowUpContextBuilder(ContextBuilder):
    ...
    @staticmethod
    def build_prompt(chat_smart_follow_up_assigned: SmartFollowUpState, follow_up_strategy: FollowUpStrategy, company_info: EmpresaModel, chatty_ai_agent: ChattyAIAgent, chat: Chat, mode_in_chat: ChattyAIMode) -> str:

        """
        Builds a comprehensive prompt for the AI agent to make intelligent follow-up decisions.

        Args:
            chat_smart_follow_up_assigned: Current follow-up state for the chat
            follow_up_strategy: Strategy configuration to use

        Returns:
            Complete prompt string for the AI agent
        """
        if not chat.is_free_conversation_active and not follow_up_strategy.templates_allowed:
            mode_in_chat = ChattyAIMode.SUGGESTIONS
        context = ContextBuilder.common_prompt(agent=chatty_ai_agent, mode_in_chat=mode_in_chat, company_info=company_info)
        context += f"In this particular case, you've been tasked with deciding whether and how to follow up with customers who haven't responded. Your goal is to be helpful, contextually aware, and respectful while driving business objectives."
        context += ContextBuilder.relevant_contexts(chatty_ai_agent.contexts, chat)
        context += ContextBuilder.unbreakable_rules(agent=chatty_ai_agent)
        context += ContextBuilder.control_triggers(agent=chatty_ai_agent)
        context += SmartFollowUpContextBuilder.follow_up_strategy_context(chat_smart_follow_up_assigned, follow_up_strategy)
        context += f"And here are some real business applications of the follow up strategy: {ContextBuilder.relevant_examples(follow_up_strategy.examples, chat)}"
        return context
    ...
    def check_minimum_time_since_last_message(chat: Chat, follow_up_strategy: FollowUpStrategy,smart_follow_up_state: SmartFollowUpState) -> bool:
        expected_interval_hours = follow_up_strategy.get_interval_for_followup(smart_follow_up_state.consecutive_count)
        ...
        if time_since_last_message.total_seconds() < expected_interval_hours * 3600:
            raise PostponeFollowUp(time_delta= timedelta(seconds=expected_interval_hours * 3600 - time_since_last_message.total_seconds()), message="The follow up is postponed because it's too soon")
        return True
    ...
    def follow_up_strategy_context(chat_smart_follow_up_assigned: SmartFollowUpState, follow_up_strategy: FollowUpStrategy) -> str:
        ...
        if follow_up_strategy.maximum_consecutive_follow_ups <= chat_smart_follow_up_assigned.consecutive_count:
            raise HumanInterventionRequired(f"Se alcanzó el máximo de seguimientos consecutivos: {follow_up_strategy.maximum_consecutive_follow_ups}.")
        if follow_up_strategy.maximum_follow_ups_to_be_executed <= chat_smart_follow_up_assigned.total_followups_sent:
            raise HumanInterventionRequired(f"We've reached the maximum total follow ups to be executed limit of {follow_up_strategy.maximum_follow_ups_to_be_executed}, human intervention is required")

        if datetime.now(ZoneInfo('UTC')).weekday() >= 5:
            days_to_add = 1 if datetime.now(ZoneInfo('UTC')).weekday() == 5 else 2
            raise PostponeFollowUp(time_delta=timedelta(days=days_to_add), message="The follow up is postponed because it's a weekend")

        next_followup_number = chat_smart_follow_up_assigned.consecutive_count + 1
        next_interval_hours = follow_up_strategy.get_interval_for_followup(next_followup_number)
    ...
    def next_follow_up(chat_smart_follow_up_assigned: SmartFollowUpState, follow_up_strategy: FollowUpStrategy) -> str:
        next_call = datetime.now(ZoneInfo('UTC')) + timedelta(hours=follow_up_strategy.get_interval_for_followup(chat_smart_follow_up_assigned.consecutive_count + 1))
        next_interval_hours = follow_up_strategy.get_interval_for_followup(chat_smart_follow_up_assigned.consecutive_count + 1)
        context = f"As far as the next call, if you decide to send / suggest messages, the standard interval would be: {next_interval_hours} hour, so the next call would be: {next_call.isoformat()}. Unless you determine that the situation requires a different interval, in which case you should set the next call time accordingly. If you decide to skip, the next call time up to you based on the context and reason for skipping."

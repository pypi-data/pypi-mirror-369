...
class ExecutionFlowResult(BaseModel):
    ...
    time_pause: datetime.timedelta = Field(
...
class WorkflowExecution(CompanyAssetModel):
    ...
    result: Optional[ExecutionFlowResult] = Field(default=None)
    execution_description: list[str] = Field(default_factory=list)
    ended_at: Optional[datetime] = Field(default=None)

    @classmethod
    def start_execution(cls, flow_state:FlowStateAssignedToChat, flow_name:str) -> None:
        cls(
    ...
    def end_execution(self, result: ExecutionFlowResult) -> None:

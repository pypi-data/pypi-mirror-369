...
__all__ = ['greedy_matcher', 'hungarian_matcher']
    ...
    def calculate(self, input_matrix=None, is_profit_matrix=False):
        ...
        while len(self._results) != expected_results:
            ...
            if not zero_locations.any():
                # raise "Unable to find results. Algorithm has failed."
                break
...
class CoverZeros:
    ...
    def __find_marked_column_without_choice(self):
        ...
        raise "Could not find a column without a choice. Failed to cover matrix zeros. Algorithm has failed."
...
def greedy_matcher(
    ...
    # unassigned_m = list(range(M))
    unassigned_n = list(range(N))
    matched_indices = []

    for mindx, costs in enumerate(cost_matrix):
        if len(costs) < 1:
            continue

        nindx = np.argmax(costs)
        if nindx not in unassigned_n or costs[nindx] < cost_thr:
            continue

        # unassigned_m.remove(mindx)
        unassigned_n.remove(nindx)
        matched_indices.append((mindx, nindx))
...
def hungarian_matcher(
    cost_matrix: np.ndarray,
    cost_thr: float = None,
    optimal_cost: bool = True,
) -> List[Tuple[int, int]]:
    hungarian = Hungarian()
    if cost_thr is None:
        hungarian.calculate(cost_matrix, not optimal_cost)
        return sorted(hungarian.get_results())

    if np.prod(cost_matrix.shape) > 0:
        crop_thr_indices = (
            np.arange(cost_matrix.shape[0])[cost_matrix.max(axis=1) < cost_thr],
            np.arange(cost_matrix.shape[1])[cost_matrix.max(axis=0) < cost_thr],
        ) if optimal_cost else (
            np.arange(cost_matrix.shape[0])[cost_matrix.max(axis=1) > cost_thr],
            np.arange(cost_matrix.shape[1])[cost_matrix.max(axis=0) > cost_thr],
        )
    else:
    ...
    if len(crop_thr_indices[0]) * len(crop_thr_indices[1]) > 0:
        hungarian.calculate(cost_matrix[crop_thr_indices[0]][:, crop_thr_indices[1]], not optimal_cost)
        filtered_indices = sorted(hungarian.get_results(), key=lambda x: x[0])

        matched_indices = [
            (crop_thr_indices[0][inds[0]], crop_thr_indices[1][inds[1]]) for inds in filtered_indices if
                (cost_matrix[crop_thr_indices[0][inds[0]], crop_thr_indices[1][inds[1]]] < cost_thr)
            ] if optimal_cost else [
            (crop_thr_indices[0][inds[0]], crop_thr_indices[1][inds[1]]) for inds in filtered_indices if
                (cost_matrix[crop_thr_indices[0][inds[0]], crop_thr_indices[1][inds[1]]] > cost_thr)
            ]

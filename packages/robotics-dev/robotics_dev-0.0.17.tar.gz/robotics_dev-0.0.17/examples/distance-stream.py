...
async def handle_message(ros_message):
    ...
    if topic == '/camera/camera/depth/image_rect_raw/compressedDepth':
        data = ros_message.get('data', {})
        if data and 'data' in data:
            print(f"Received 3d camera depth frame")
            # Print the base64 image string if available
            if 'depth' in data:
                print(f"Distance: {data['depth']}")  # Print first 100 chars
            else:
                print("No depth value found in message")

...
def display_conversation(conv_data: Dict, show_full_history: bool = True):
    ...
    print(f"📅 Time: {timestamp_to_datetime(conv_data['request_ts'])}")
    print(f"🏷️  Topic: {conv_data['topic']}")
    print(f"📍 Source: {conv_data['source']}")
    print(f"🆔 Stream ID: {conv_data['stream_id']}")
    print("-" * 80)
...
def main():
    ...
    max_display = min(3, len(conversations))  # Show 3 conversations by default

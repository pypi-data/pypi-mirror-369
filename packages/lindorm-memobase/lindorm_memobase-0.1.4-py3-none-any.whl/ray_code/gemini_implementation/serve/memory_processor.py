...
@serve.deployment(
    ...
    autoscaling_config={
        "min_replicas": 1,              # 最小副本数，保证基础处理能力
        "max_replicas": 50,             # 最大副本数，根据实际资源调整
        "target_num_ongoing_requests_per_replica": 30,  # 每个副本的目标并发请求数
...
class MemoryProcessorDeployment:
    ...
    def __init__(self, memobase_config_path: str = "./config.yaml"):
        """
        初始化MemoryProcessor
        ...
        """
        # 使用LindormMemobase.from_yaml_file加载配置
        logger.info(f"MemoryProcessor loading config from: {memobase_config_path}")
        self.memobase = LindormMemobase.from_yaml_file(memobase_config_path)
        self.start_time = time.time()
        
        logger.info(f"MemoryProcessor initialized with config file: {memobase_config_path}")
...
class MemoryProcessorAPI:
    ...
    def __init__(self, memobase_config_path: str = "./config.yaml"):
        ...
        Args:
            memobase_config_path: LindormMemobase配置文件路径（未直接使用，但保留以保持一致性）
        """
        self.processor_handle = serve.get_deployment("MemoryProcessor").get_handle()
        logger.info(f"MemoryProcessorAPI initialized with config: {memobase_config_path}")

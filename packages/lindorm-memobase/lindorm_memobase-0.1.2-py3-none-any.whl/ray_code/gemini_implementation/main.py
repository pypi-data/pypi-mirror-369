...
from utils.helpers import get_kafka_config, get_database_config, HealthChecker, metrics_collector
...
class LongTermMemorySystem:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        # 系统参数
        self.num_kafka_consumers = self.config.get('num_kafka_consumers', 4)
        self.num_buffer_managers = self.config.get('num_buffer_managers', 10)
        self.kafka_partitions = self.config.get('kafka_partitions', list(range(8)))
    ...
    async def _deploy_memory_processor(self):
        ...
        # 部署MemoryProcessor
        memory_processor = MemoryProcessorDeployment.bind()
        serve.run(memory_processor, name="MemoryProcessor", route_prefix="/memory")
        
        # 可选：部署HTTP API
        if self.config.get('enable_http_api', False):
            api = MemoryProcessorAPI.bind()
    ...
    async def _create_buffer_manager_pool(self):
        ...
        # 创建BufferManager actors
        buffer_managers = [
            BufferManagerActor.remote(manager_id=i, db_config=self.db_config)
    ...
    async def _create_kafka_consumers(self):
        ...
        for i in range(self.num_kafka_consumers):
            ...
            # 从配置中获取批处理参数
            batch_timeout = self.config.get('kafka_batch_timeout_seconds', 10)
            max_batch_size = self.config.get('kafka_max_batch_size', 100)
    ...
    async def _system_health_check(self):
        ...
        # 检查Kafka连接
        kafka_healthy = await HealthChecker.check_kafka_connection(self.kafka_config)
        logger.info(f"Kafka connection: {'healthy' if kafka_healthy else 'unhealthy'}")
        
        # 检查数据库连接
        db_healthy = await HealthChecker.check_database_connection(self.db_config)
        ...
        if not (kafka_healthy and db_healthy):
...
async def main():
    ...
    # 系统配置
    config = {
        'num_kafka_consumers': 4,
        'num_buffer_managers': 10,
        'kafka_partitions': list(range(8)),
        'enable_http_api': True
    }
    
    # 创建系统实例
    system = LongTermMemorySystem(config)

...
from .utils._common import contains_versioned_url
...
class DifyClient(BaseModel):
    ...
    def completion_messages(
        ...
        models.CompletionResponse, Iterator[models.CompletionStreamResponse]
    ]:
        """
    ...
    def get_conversations(
        self, req: models.ConversationRequest, **kwargs
    ...
    def stop_completion_messages(
        ...
        return self._stop_stream(
            self._prepare_url(
                ENDPOINT_STOP_COMPLETION_MESSAGES, task_id=task_id
            ),
            req,
            **kwargs,
        )
    ...
    def stop_workflows(
        ...
        """
        return self._stop_stream(
            self._prepare_url(ENDPOINT_STOP_WORKFLOWS, task_id=task_id),
            req,
            **kwargs,
        )
    ...
    def _prepare_url(self, endpoint: str, **kwargs) -> str:
        ...
        if contains_versioned_url(self.api_base):
            return urljoin(
                self.api_base.rstrip("/") + "/", formatted_endpoint.lstrip("/")
            )
        else:
            base_with_version = urljoin(
                self.api_base.rstrip("/") + "/",
                self.api_version.lstrip("/") + "/",
            )
            return urljoin(base_with_version, formatted_endpoint.lstrip("/"))
...
class AsyncDifyClient(BaseModel):
    api_key: str
    api_base: Optional[str] = "https://api.dify.ai/v1"
    api_version: Optional[str] = "v1"
    verify_ssl: Optional[bool] = True
    ...
    async def arun_workflows(
        ...
        models.WorkflowsRunResponse,
        AsyncIterator[models.WorkflowsStreamResponse],
    ]:
        """
        Initiates the execution of a workflow, which can consist of multiple steps and actions.

...
class CompanyManagerBot:
    ...
    def optimize_loan_structure(self):
        ...
        if money > 300000 and loan > 50000:
            ...
            if paydown_amount > 0:
                ...
                if success:
                    self.client.send_chat(f"Paying down £{paydown_amount:,} of loan to reduce interest costs")
                    time.sleep(1)
    ...
    def demonstrate_money_transfer(self):
        ...
        other_companies = [cid for cid in companies.keys() if cid != int(self.client.game_state.company_id)]
        ...
        if other_companies and len(other_companies) > 0:
            ...
            if finances and finances['money'] > transfer_amount * 2:
                ...
                if success:
                    self.client.send_chat(f"Transferred £{transfer_amount:,} to Company {target_company} as goodwill gesture!")
        else:
    ...
    def on_chat(self, sender, message, _data):
        ...
        elif any(greeting in message_lower for greeting in ['hello', 'hi', 'hey']) and self.client.player_name.lower() in message_lower:
            responses = [
            ...
            try:
                ...
                if self.management_actions_performed % 5 == 0:
                    ...
                    if performance:
                        ...
                        time.sleep(0.5)
                        self.client.send_chat(f"Management cycles completed: {self.management_actions_performed}")
                        time.sleep(0.5)

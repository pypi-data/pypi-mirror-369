...
def test_methods_create_pydantic_v1_targeted(dyffapi: Client | DyffLocalPlatform, ctx):
...
def test_methods_create_pydantic_v2_targeted(dyffapi: Client | DyffLocalPlatform, ctx):
    ...
    assert (
        method.analysisImage.digest
        == "sha256:36b524c0f253014afe5b2f85931179e1fbfc7f540cfe5b47306718f1ff2ccf8b"
    )
...
def test_modules_create_pydantic_v1_test(dyffapi: Client | DyffLocalPlatform, ctx):
...
def test_modules_create_pydantic_v2_test(dyffapi: Client | DyffLocalPlatform, ctx):
...
def test_methods_create_pydantic_v1_runtime(dyffapi: Client | DyffLocalPlatform, ctx):
...
def test_methods_create_pydantic_v2_runtime(dyffapi: Client | DyffLocalPlatform, ctx):
...
def test_api_sets_analysis_image_for_new_methods(
    dyffapi: Client | DyffLocalPlatform, ctx
):
    """Test that new Methods automatically get analysisImage set by API."""

    # Test that API automatically sets analysisImage when none specified

    # Create Method without specifying analysisImage
    method = dyffapi.methods.create(
        MethodCreateRequest(
            ...
            output=MethodOutput(
                ...
                safetyCase=SafetyCaseSpec(
                    name="api_default_image_test",
                    description="Test API sets default analysisImage",
                ),
            ),
            modules=[ctx["module_pydantic_v2_test"].id],
            # NO analysisImage specified - should get current default (0.8.0)
        )
    )

    # Verify this Method got the current default image (0.8.0/Pydantic v2)
    assert method.analysisImage is not None
    assert (
        method.analysisImage.digest
        == "sha256:36b524c0f253014afe5b2f85931179e1fbfc7f540cfe5b47306718f1ff2ccf8b"
    ...
    print("Verified API sets analysisImage for new Methods")

    ctx["method_api_default"] = method


@pytest.mark.depends(
    on=[
        "tests/test_analysis.py::test_api_sets_analysis_image_for_new_methods",
    ]
)

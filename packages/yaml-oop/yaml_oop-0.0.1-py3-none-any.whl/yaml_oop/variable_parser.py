...
from yaml_oop.custom_errors import KeySealedError, ConflictingDeclarationError, NoOverrideError, InvalidVariableError
...
def process_variables(data, super_data, super_key_or_index, parent_variables: dict):
    modified_variables = add_variables(data, parent_variables.copy())
    replace_with_variables(data, super_data, super_key_or_index, modified_variables)
...
def replace_with_variables(data, super_data, super_key_or_index, variables: dict):
    ...
    if type(data) is dict:
        for key in list(data.keys()):
            if key == definitions.BASE_CONFIG_DECLARATION and definitions.VARIABLE_DECLARATION in data[key]:
                carryover_variables = {variable_key: value for variable_key, value in data[key][definitions.VARIABLE_DECLARATION].items() \
                                      if definitions.CARRYOVER_DECLARATION in variable_key}
                for carryover_key in carryover_variables:
                    ...
                    if parsed_carryover_key in variables:
                        data[key][definitions.VARIABLE_DECLARATION][carryover_key] = variables[parsed_carryover_key][0]
                        carryover_key = parser.remove_key_declaration(data[key][definitions.VARIABLE_DECLARATION], carryover_key, definitions.CARRYOVER_DECLARATION)
                        if definitions.ABSTRACT_DECLARATION in carryover_key_declarations:
                            parser.remove_key_declaration(data[key][definitions.VARIABLE_DECLARATION], carryover_key, definitions.ABSTRACT_DECLARATION)
            elif key == definitions.BASE_CONFIG_DECLARATION:
                pass # Do not read variables from base_configs as those are processed and passed separately during instantiation
                ...
                process_variables(data[key], data, key, variables)
            else:
                replace_value(data, key, variables)
    elif type(data) is list:
        i = 0
        while i < len(data):
            if data[i] == definitions.BASE_CONFIG_DECLARATION:
                pass # Do not read variables from base_configs as those are processed and passed separately during instantiation
            elif type(data[i]) is str and definitions.OPTIONAL_DECLARTION in parser.find_key_declarations(data[i]):
                ...
                element = data[i]
                process_variables(data[i], data, i, variables)
...
def replace_value(data, key_or_index, variables: dict):
    ...
    if type(data[key_or_index]) is str:
        ...
        if data[key_or_index] in variables.items():
            data[key_or_index] = variables[data[key_or_index]]
...
def find_variable_declarations(key: str) -> set:
    ...
    if key == "" or key is None:
        ...
        for item in key.split(" "):
            if item in definitions.VARIABLE_DECLARATIONS or item in definitions.VARIABLE_DECLARATION:

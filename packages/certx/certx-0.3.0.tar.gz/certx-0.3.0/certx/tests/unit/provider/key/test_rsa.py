...
class TestEcKeyProvider(unittest.TestCase):
    def setUp(self) -> None:
        self.provider = rsa.RsaKeyProvider(KeyAlgorithm.RSA_4096)

    def test_new_object_should_return_bad_request_with_unsupported_alg(self):
        self.assertRaises(exceptions.UnsupportedAlgorithm, rsa.RsaKeyProvider, KeyAlgorithm.ECC_256)

    def test_generate_private_key(self):
        key = self.provider.generate_private_key()
        self.assertIsNotNone(key)
        self.assertEqual(4096, key.key_size)
    ...
    def test_load_private_key_without_password(self):
        ...
        self.assertEqual(4096, pri_key.key_size)

...
class DatabaseObserver(Observer):
    ...
    def __init__(self, order=3):
        Observer.__init__(self, order=order, sets={"random": "random_stuff"})
        self.add_observer_method(self.database_observer_method, order=self.order[0], sets={}, gets={})
    ...
    def database_observer_method(self, database, state):
        ...
        state.add_to_cache(self, self.random, ["STRING_IN_A_LIST"], mode="w")

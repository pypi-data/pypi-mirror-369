...
class GetsSetsBasicObserver(Observer):
    name = "LessBasicObserver"

    def __init__(self, order=2, gets={"get_key": "candidates"}, sets={"set_key": "candidates"}):
        Observer.__init__(self, gets=gets, order=order, sets=sets)
        self.add_observer_method(self.basic_observer_method, order=self.order[0], sets=self.sets[0], gets=self.gets[0])

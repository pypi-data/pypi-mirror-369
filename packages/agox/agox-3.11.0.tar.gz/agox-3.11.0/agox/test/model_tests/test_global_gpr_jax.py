...
@pytest.mark.skip  # Temporary... I hope.
@pytest.mark.ray
...
def test_model(test_data_dict, update_kwargs, cmd_options):
    ...
    data = get_test_data(path, environment)[0:10]

    # Update the kwargs for the model.
    descriptor = JaxFingerprint.from_atoms(data[0], r_cut_radial=4.0, r_cut_angular=4.0)
    ...
    output = model_tester(
        ...
        tolerance=tolerance,
        force_tolerance=dict(rtol=1e-2, atol=1e-3),
    )

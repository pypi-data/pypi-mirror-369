...
class ReuseGenerator(GeneratorBaseClass):
    """
    Reuses the candidates that were not selected for evaluation in the previous iteration.
    """
    ...
    def __init__(
        self,
        order=8,
        gets={"candidates_key": "prioritized_candidates", "evaluated_key": "evaluated_candidates"},
        replace=True,
        **kwargs,
    ):
        super().__init__(order=order, gets=gets, replace=replace, **kwargs)

...
def list_tokens(
    ...
    Args:
        ...
        pattern:
            Pattern for filtering tokens, using the usual ``*`` and ``?`` wildcards.
            Only tokens matching to the pattern will be returned. 
            If ``None``, no filtering is performed.

        field:
            Metadata property field for filtering tokens.
            Only tokens found in the specified field will be returned.
            If ``None``, no filtering is performed.

        count:
            Whether to count the number of metadata files associated with each token.
    ...
    Returns:
        ...
        - ``count``, integer specifying the number of files associated with the token.
    ...
    if field is not None:
        query.append("field=" + urllib.parse.quote_plus(field))
    if count:

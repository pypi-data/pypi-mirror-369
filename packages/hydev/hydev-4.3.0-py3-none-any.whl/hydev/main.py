...
def read_local_config(local_path: str | Path = "pyproject.toml") -> TCfg:
    try:
        config_text = Path(local_path).read_text(encoding=DEFAULT_ENCODING)
...
class CommonCLITool(CLIToolBase):
    ...
    def is_uv(self) -> bool:
        ...
        return isinstance(build_sys, list) and len(build_sys) == 1 and str(build_sys[0]).startswith("hatchling")
    ...
    def make_cmd(self, extra_args: Sequence[str] = ()) -> Sequence[str]:
        ...
        run_prefix = self.command_run_prefix

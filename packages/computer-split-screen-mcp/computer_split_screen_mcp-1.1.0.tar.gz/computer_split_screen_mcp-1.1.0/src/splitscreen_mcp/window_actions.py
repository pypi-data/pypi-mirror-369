...
def bottom_half_window_mac():
    ...
    try:
        ...
        -- Find the NSScreen that actually contains a point
        on screenForPoint(px, pyBottom)
          set allScreens to current application's NSScreen's screens()
          repeat with sc in allScreens
            set fr to sc's frame()
            set sx to (current application's NSMinX(fr))
            set sy to (current application's NSMinY(fr))
            set sw to (current application's NSWidth(fr))
            set sh to (current application's NSHeight(fr))
            if (px ≥ sx and px < sx + sw and pyBottom ≥ sy and pyBottom < sy + sh) then return sc
          end repeat
          return current application's NSScreen's mainScreen()
        end screenForPoint
        ...
        tell application "System Events"
          ...
          try
            ...
            -- Window center (AX coords = top-left origin)
            set {wx, wy} to position of targetWin
            set {ww, wh} to size of targetWin
            set cx_top to (wx + ww / 2)
            set cy_top to (wy + wh / 2)

            -- Convert center Y to Cocoa bottom-left for screen picking
            set mainS to current application's NSScreen's mainScreen()
            set mainH to (current application's NSHeight(mainS's frame()))
            set cy_bottom to (mainH - cy_top)

            -- IMPORTANT: call handler on the current script, not System Events
            set sc to my screenForPoint(cx_top, cy_bottom)

            -- Read visibleFrame; re-read once for stability
            set v1 to sc's visibleFrame()
            set s1 to sc's frame()
            delay 0.02
            set v2 to sc's visibleFrame()
            if ((current application's NSEqualRects(v1, v2)) is false) then set v1 to v2

            -- Extract rects (Cocoa bottom-left origin) and convert Y for AX (top-left)
            ...
            set screenH to (current application's NSHeight(s1)) as integer
            set visTopY to (screenH - (visYBottom + visH))

            -- Target bottom half
            set halfH to (round (visH / 2))
            set yTarget to (visTopY + halfH)
            set hTarget to (visH - halfH)

            -- Clamp so bottom never exceeds visible bottom (tiny guard for safety)
            set guard to 4
            set visibleBottomTopY to (visTopY + visH)
            ...
            if canResize then
              set position of targetWin to {visX, yTarget}
              set size of targetWin to {visW, hTarget}
            end if
...
def bottom_left_quadrant_window_mac():
    ...
    try:
        ...
        -- Return the NSScreen that contains a point (px, pyBottom) in Cocoa (bottom-left) coords
        on screenForPoint(px, pyBottom)
          set allScreens to current application's NSScreen's screens()
          repeat with sc in allScreens
            set fr to sc's frame()
        ...
        tell application "System Events"
          ...
          try
            ...
            -- Window center (AX coords use top-left origin)
            set {wx, wy} to position of targetWin
            set {ww, wh} to size of targetWin
            set cx_top to (wx + ww / 2)
            set cy_top to (wy + wh / 2)

            -- Convert that center Y to Cocoa bottom-left for screen picking
            set mainS to current application's NSScreen's mainScreen()
            set mainH to (current application's NSHeight(mainS's frame()))
            set cy_bottom to (mainH - cy_top)

            -- IMPORTANT: call the handler on the current script
            set sc to my screenForPoint(cx_top, cy_bottom)
            ...
            -- Extract rects (Cocoa bottom-left), then convert Y to AX top-left
            set visX to (current application's NSMinX(v1)) as integer
            set visW to (current application's NSWidth(v1)) as integer
            set visYBottom to (current application's NSMinY(v1)) as integer
            set visH to (current application's NSHeight(v1)) as integer
    ...
    try:
        ...
        tell application "System Events"
          ...
          try
            ...
            -- Window center (AX uses top-left origin)
            set {wx, wy} to position of targetWin
            set {ww, wh} to size of targetWin
            set cx_top to (wx + ww / 2)
            set cy_top to (wy + wh / 2)
            ...
            -- IMPORTANT: call handler on the current script
            set sc to my screenForPoint(cx_top, cy_bottom)
            ...
            -- Guards & clamps (prevent overshooting visible right/bottom)

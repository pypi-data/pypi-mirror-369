...
def get_main_audio_paths(beatmap_dir):
    audio_files = set()
    for osu_file in glob.glob(os.path.join(beatmap_dir, "*.osu")):
        try:
            with open(osu_file, "r", encoding="utf-8", errors="ignore") as f:
                for line in f:
                    if line.startswith("AudioFilename:"):
                        audio_name = line.split(":", 1)[1].strip()
                        audio_path = os.path.abspath(os.path.join(beatmap_dir, audio_name))
...
def main():
    ...
    for directory in directories:
        if os.path.isdir(directory):
            ...
            os.chdir(directory)
            main_audio_files = get_main_audio_paths(".")
                ...
                for file in files:
                    ...
                    if (
                        ...
                        or (
                            ...
                            and os.path.basename(file_lowercase) not in main_audio_files
                        )
                        or (

...
class TodoToolSet(ToolSet):
    ...
    @tool
    async def add_todo(self, content: str, status: str = "pending", auto_break_down: bool = True, auto_start: bool = True, prevent_duplicates: bool = True) -> dict:
    ...
    def _is_complex_task(self, content: str) -> bool:
        ...
        complex_indicators = [
            "step by step", "generate figure", "analyze data", "create plot",
            "build model", "process", "workflow", "pipeline", "analysis"
        ]
        content_lower = content.lower()

...
class GovResearcherConfigSchema(TypedDict):
    """政府研究員助手配置 Schema - 可在 LangGraph UI 中設定"""

    # 模型選擇
    decomposition_model: str  # 題目拆解模型
...
def get_config_value(config: RunnableConfig, key: str, default_value: Any) -> Any:
    """統一獲取配置值的輔助函數"""
    return config.get("configurable", {}).get(key, default_value)
...
async def search_subtopic_node(
    ...
    # 獲取搜尋引擎配置
    search_vendor = get_config_value(config, "search_vendor", "perplexity")
...
def summary_response_node(
    ...
    summary_data = {
        ...
        "verification_results": (
            "幻覺驗證完成" if state.get("hallucination_check") else "未進行驗證"
        ),
    }
...
def should_compute(state: GovResearcherState) -> str:
    ...
    if state.get("needs_computation", False):
        ...
        return HALLUCINATION_VERIFICATION_NODE
...
class GovResearcherGraph:
    ...
    def _initialize_graph(self):
        """初始化 LangGraph 工作流"""
        workflow = StateGraph(
            GovResearcherState, config_schema=GovResearcherConfigSchema
        )
        ...
        workflow.add_conditional_edges(
            ...
            should_compute,
            [COMPUTATION_VERIFICATION_NODE, HALLUCINATION_VERIFICATION_NODE],
        )

        workflow.add_edge(
            COMPUTATION_VERIFICATION_NODE, HALLUCINATION_VERIFICATION_NODE
        )
        workflow.add_edge(HALLUCINATION_VERIFICATION_NODE, SUMMARY_RESPONSE_NODE)

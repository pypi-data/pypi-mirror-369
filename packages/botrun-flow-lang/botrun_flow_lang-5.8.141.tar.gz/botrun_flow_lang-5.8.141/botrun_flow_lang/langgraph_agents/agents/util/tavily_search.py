...
try:
    ...
    # 使用 async 版本以提高性能
    from tavily import tavily_search_async
except ImportError:
    TavilyClient = None
    tavily_search_async = None
...
async def respond_with_tavily_search(
    ...
    if tavily_search_async is None:
    ...
    try:
        # 使用 async 版本進行搜尋（參考 open_deep_research）
        logger.info(f"使用 Tavily 搜尋: {query}")
        
        # 構建搜尋查詢列表（與 open_deep_research 格式一致）
        ...
        if response and len(response) > 0:
            ...
            yield SearchEvent(
                chunk="未找到搜尋結果",
                raw_json={"error": "no results found"}
            )

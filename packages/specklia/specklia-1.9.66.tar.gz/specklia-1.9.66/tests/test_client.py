...
from specklia import chunked_transfer, Specklia
...
def test_add_points_to_dataset(
        test_client: Specklia,
        example_geodataframe: gpd.GeoDataFrame,
        patched_requests_with_response: Dict[str, MagicMock]):
...
def test_query_dataset(
    ...
    patched_requests_with_response['chunked_transfer_requests'].get.side_effect = [
        mock_chunk_response, mock_no_more_chunks_response]
...
def test_report_usage(patched_requests_with_response: Dict[str, MagicMock], example_usage_report: List[Dict]):

...
def determine_screen_size():
    """
    Determine the screen dimensions by user input.
    """
...
def load_screen_size():
    """
    Load screen dimensions from a file or determine them if the file does not exist.
    """
    ...
    for y in range(vertical_length):
        ...
        for x in range(horizontal_length):
            if [x, y] == mouse_position:
                if is_mouse_pressed:
                    line += "\033[7m  \033[0m"  # Solid block when mouse is pressed
                else:
                    line += "\033[7m  \033[0m"  # Hollow block when mouse is not pressed
            else:
...
def place_character(character, position):
    """
    Place a character at the specified position on the screen.
    """
...
def initialize():
    """
    Initialize the screen and load screen dimensions.
    """
    global screen
    load_screen_size()
...
def run():
    """
    Main loop to handle user input and update the screen.
    """
...
# Ensure the screen dimensions are loaded when the library is imported
load_screen_size()

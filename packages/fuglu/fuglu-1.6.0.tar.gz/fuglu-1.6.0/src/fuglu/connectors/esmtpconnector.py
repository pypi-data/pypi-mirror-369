...
class ESMTPHandler(ProtocolHandler):
    ...
    def re_inject(self, suspect):
        ...
        queueid = self.sess.forwardconn.queueid
        self.logger.debug(f'{suspect.id} reinjected message from={force_uString(suspect.from_address)} to={force_uString(suspect.recipients)} in={self.sess.forwardconn._host} with queueid={queueid} code={code} message={force_uString(answer)}')
        return code, answer, queueid
...
class ESMTPServer(BasicTCPServer):
    ...
    def __init__(self, controller, port=10125, address="127.0.0.1"):
        super().__init__(controller, port, address, ESMTPHandler)
...
class ESMTPPassthroughSession(object):
    ...
    def endsession(self, code, message):
        ...
        self.socket.send(force_bString(f"{code} {message}\r\n"))
        rawdata = b''
        data = ''
        while True:
            ...
            if len(lump):
                ...
                if (len(rawdata) >= 2) and rawdata[-2:] == b'\r\n':
                    ...
                    cmd = cmd.upper()
                    if cmd == b"QUIT":
                        self.socket.send(b"220 BYE\r\n")
                        self.closeconn()
                        return
                    self.socket.send(b"421 Cannot accept further commands\r\n")
                    self.closeconn()
                    return
            else:
                self.closeconn()
    ...
    def getincomingmail(self):
        ...
        self.socket.send(b"220 fuglu scanner ready \r\n")
    ...
    def forwardCommand(self, command):
        """forward esmtp command to outgoing postfix instance
        ...
        if self.forwardconn is None:
            ...
            if targethost == '${injecthost}':
                ...
                if not env_targethost:
                    raise ValueError(f"Could not extract target host from environment var '{targethost}'")
                targethost = env_targethost

            self.forwardconn = FugluSMTPClient(force_uString(targethost), self.config.getint('main', 'outgoingport'))
        ...
        if ret.find('\n'):
            ...
            for line in parts:
                line = line.strip()
                temprv.append(f'{code}-{line}')
            # replace - with space on last line
            temprv[-1] = f'{code} {line}'

            ret = '\r\n'.join(temprv)
        self.logger.debug(f'RECEIVE: "{ret}" ')
        return ret.strip()
    ...
    def doCommand(self, data):
        ...
        if cmd in ["EHLO", 'HELO']:
            ...
            if len(self.recipients) > 1 and self.config.getboolean('esmtpconnector', 'ignore_multiple_recipients'):
                return f"250 OK #{len(self.recipients)}", 1
            ...
            try:
                ...
                self.logger.error(f"could not create temp file in {self.config.get('main', 'tempdir')} due to {e.__class__.__name__}: {str(e)}")
                self.endsession(421, "Queue file write error")
    ...
    def stripAddress(self, address):
        ...
        # keeping this check as it was in the previous code
        if retaddr != '' and self._re_at.match(retaddr) is None:

    ...
    def smart_cm(f):
        ...
        if is_async:
            async def async_wrapper(self, *args, **kwargs):
                ...
                fun_input = args + tuple(kwargs.items())
                cstats, ucstats = get_statscounter(self)
                ...
                __function_cache = getattr(self, '_function_cache', {})
                setattr(self, '_function_cache', __function_cache)

                __property_input_cache = getattr(self, '_property_input_cache', {})
                setattr(self, '_property_input_cache', __property_input_cache)
                ...
                fdict, tdict = __function_cache.get(funcname, ({}, {}))
                ...
                if not climits.get('nocache') and func_allow_cache:
                    ...
                    if num_cache is not None and len(fdict) > num_cache:
                        time_sorted = sorted(tdict.items(), key=operator.itemgetter(1))
                        for k, _ in time_sorted:

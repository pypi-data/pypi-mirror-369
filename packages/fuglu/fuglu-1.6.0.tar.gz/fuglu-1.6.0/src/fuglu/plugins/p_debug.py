...
"""
    ...
    def pluginlist(self, suspect, pluginlist):
        ...
        if suspect.get_tag('incomingport') == debugport:
            self.logger.info(f'{suspect.id} Enabling debug mode for message on incoming port {debugport}')
        ...
        self.logger.debug(f"{suspect.id} debugport={debugport} incomingport={suspect.get_tag('incomingport')}")

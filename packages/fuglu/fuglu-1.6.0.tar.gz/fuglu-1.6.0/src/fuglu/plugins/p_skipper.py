...
class PluginSkipper(PrependerPlugin):
    ...
    def pluginlist(self, suspect, pluginlist):
        ...
        results = self.filter.get_args(suspect, True)
        # each arg should be a comma separated list of classnames to skip

        skipplugindict = {}
        for result in results:
            plugnames = result.arg.split(',')
            for plugname in plugnames:
                skipplugindict[plugname] = result.fieldname
...
class AppenderSkipper(PrependerPlugin):
    ...
    def appenderlist(self, suspect, appenderlist):
        ...
        listcopy = appenderlist[:]
        for plug in appenderlist:
            name = plug.__class__.__name__
    ...
    def _initfilter(self):
        ...
        if not os.path.exists(filename):
            self.logger.error(f'Filterfile not found for skipper: {filename}')
            return False

...
class _VacationReply(object):
    ...
    def __str__(self):
        return "<VacationReply(%s) sent=%s>" % (self.recipient, self.sent)
...
if SQL_EXTENSION_ENABLED:
    from sqlalchemy import Column, TEXT, TIMESTAMP, Integer, ForeignKey, Unicode, Boolean
    from sqlalchemy.orm import relationship, backref
    metadata = DeclarativeBase.metadata

    class Vacation(DeclarativeBase, _Vacation):
        ...
        awayuser = Column(Unicode(255), unique=True, nullable=False)
        subject = Column(Unicode(255), nullable=False)
        body = Column(TEXT, nullable=False)
        ignoresender = Column(TEXT, nullable=False)

    class VacationReply(DeclarativeBase, _VacationReply):
        __tablename__ = "vacationreply"
        id = Column(Integer, primary_key=True)
        vacation_id = Column(None, ForeignKey('vacation.id'))
        sent = Column(TIMESTAMP, nullable=False)
        recipient = Column(Unicode(255), nullable=False)
        
        vacation = relationship(Vacation, backref=backref('replies'))
        
else:
    class Vacation(_Vacation):
        pass
    class VacationReply(_VacationReply):
        pass
...
class VacationCache(object):
    ...
    def _loadvacation(self):
        ...
        # important to expunge or other sessions won't be able to use this
        # vacation object
...
"""
    ...
    def already_notified(self, vacation, recipient):
        ...
        log = dbsession.query(VacationReply).filter_by(vacation=vacation).filter(
            VacationReply.sent > utcnow() - timedelta(days=1)).filter_by(recipient=recipient).first()
        dbsession.expunge_all()
        if log is not None:
            self.logger.debug(

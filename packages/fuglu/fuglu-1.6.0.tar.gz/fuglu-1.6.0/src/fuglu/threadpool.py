...
class Worker(threading.Thread):
    ...
    def __init__(self, workerid, pool):
        super().__init__(name=f'Pool worker {workerid}')
    ...
    def run(self):
        ...
        event_loop = get_event_loop(f'worker.run-{self.workerid}')
        while self.stayalive:
            ...
            try:
                event_loop.run_until_complete(sesshandler.handlesession(self))
            except Exception as e:
                self.logger.error(f'Unhandled Exception : {e.__class__.__name__}: {str(e)}')

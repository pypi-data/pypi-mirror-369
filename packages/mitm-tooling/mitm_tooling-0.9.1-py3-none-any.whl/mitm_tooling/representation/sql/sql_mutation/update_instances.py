...
def _update_instances_small(
    ...
    :return:
    """
    t_type = sql_rep_schema.get_type_table(concept, type_name)
    if t_type is None:
        return False
    ...
    pk_cols = t_type.primary_key.columns
    ...
    for col_name in all_updated_cols:
        ...
        for idx, (_, update_dict) in enumerate(non_empty_updates.items()):
            ...
            pk_conditions = and_(*[t_type.c[pk] == bindparam(f'pk_{pk}_{idx}') for pk in pk_col_names])
            whens.append((pk_conditions, bindparam(f'val_{col_name}_{idx}')))

        # Only add CASE expression if there are updates for this column
        if whens:
            values_dict[col_name] = case(*whens, else_=t_type.c[col_name])

    # Execute update if we have columns to modify
    if values_dict:
        update_stmt = update(t_type).values(values_dict)
...
def _update_instances(
    ...
    if (t_type := sql_rep_schema.get_type_table(concept, type_name)) is not None:
        pk_col_names = [c.name for c in t_type.primary_key.columns]
        pk_bindparams = [bindparam(c.name, type_=c.type).label(c.name) for c in t_type.primary_key.columns]

        updated_col_names = set(itertools.chain(*updates.values()))  # union, consider intersection
        updated_cols = [t_type.columns[n] for n in updated_col_names]
        ...
        update_stmt = (
            t_type.update()
            .where(*(t_type.c[n] == values_table.c[n] for n in pk_col_names))

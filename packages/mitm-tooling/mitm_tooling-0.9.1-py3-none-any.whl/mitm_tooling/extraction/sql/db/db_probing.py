...
def sample_queryable(
    session: Session, from_clauses: Collection[sa.FromClause], sample_size: int = 100, query_via_pandas=False
    ...
    for from_clause in from_clauses:
        ...
        if query_via_pandas:
            ...
            df = pd.DataFrame.from_records(
                session.execute(selection).all(), columns=[c.name for c in from_clause.columns]
...
def create_db_probe(db_session: Session, db_meta: DBMetaInfo, sample_size: int = 100) -> DBProbe:
    ...
    table_probes = (
        (tm.short_table_identifier, create_table_probe(db_session, tm, sample_size=sample_size))

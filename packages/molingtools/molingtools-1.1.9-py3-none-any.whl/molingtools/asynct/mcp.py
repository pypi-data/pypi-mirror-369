...
class MCPClient:
    ...
    async def connect_to_http_server(self, server_name:str, url:str, debug_list_tools=False, **kwargs):
        read_stream, write_stream,_ = await self.exit_stack.enter_async_context(streamablehttp_client(url, **kwargs))
        session = await self.exit_stack.enter_async_context(ClientSession(read_stream, write_stream))
        return await self._connect_to_server(server_name, session, debug_list_tools=debug_list_tools)
    
    async def connect_to_sse_server(self, server_name:str, url:str, debug_list_tools=False, **kwargs):
        ...
        for server_name, server_config in config['mcpServers'].items():
            if server_config.get("command"):
                ...
                if server_config.get("type", 'http').lower() == 'sse':
                    ...
                    await self.connect_to_http_server(server_name, server_config["url"], debug_list_tools=debug_list_tools)
            else:
                warnings.warn(f"未指定command或url, 无法连接到 MCP 服务器 {server_name}")
        
    async def chat(self, messages:list[Messager|dict], max_tool_num=3, stream=True, **kwargs)->AsyncIterator[Messager]:

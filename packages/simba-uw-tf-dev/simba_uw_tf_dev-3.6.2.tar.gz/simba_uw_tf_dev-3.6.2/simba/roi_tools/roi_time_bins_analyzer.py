        ...
        for file_cnt, file_path in enumerate(self.data_paths):
            ...
            for animal_name, roi_name in list(itertools.product(self.bp_dict.keys(), video_shape_names)):
                ...
                for bin_cnt, bin_frms in enumerate(frame_bins):
                    ...
                    self.results.loc[len(self.results)] = [video_name, roi_name, animal_name, bp_name, bin_cnt, bin_start_time, bin_end_time, bin_start_frm, bin_end_frm, TOTAL_ROI_TIME,  distance]
            ...
            if self.non_roi_zone:
                for animal_name in self.bp_dict.keys():
                    ...
                    for bin_cnt, bin_frms in enumerate(frame_bins):
                        ...
                        self.results.loc[len(self.results)] = [video_name, OUTSIDE_ROI, animal_name, bp_name, bin_cnt, bin_start_time, bin_end_time, bin_start_frm, bin_end_frm, TOTAL_ROI_TIME,  distance]

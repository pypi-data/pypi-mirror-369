...
def to_datetime(
        ...
        default_tz: Union[str, timezone] = 'UTC+8'  # 默认时区改为 UTC+8
) -> Union[datetime, str]:
    """
    ...
    if s is None or s == '':
        ...
        # 2. 尝试解析时间戳（关键修改：先转 naive datetime，再附加时区）
        if dt is None and re.match(r"^\d{1,19}$", s):
            ...
            if isinstance(default_tz, str):
                if default_tz.upper() == "UTC":
                    ...
                    offset = re.match(r"UTC([+-]\d{1,2})(:\d{2})?$", default_tz.upper())
                    if offset:
                        hours = int(offset.group(1))
                        default_tz = timezone(timedelta(hours=hours))
            dt = dt.replace(tzinfo=default_tz)

        # 3. 其他解析逻辑保持不变...
        if dt is None and pattern is not None:
            try:
                dt = datetime.strptime(s.split('.')[0], pattern)
            except ValueError:
                pass
    ...
    if tz is not None:
        if isinstance(tz, str):
            if tz.upper() == "UTC":
                ...
                offset = re.match(r"UTC([+-]\d{1,2})(:\d{2})?$", tz.upper())
                if offset:
                    hours = int(offset.group(1))
...
def get_log_msg(contents: dict) -> str:
    ...
    if content is not None and len(str(content).split(' ')) >= 2:
        ...
        time_1 = to_datetime(time_str)
        if time_1 != datetime.now():
            content = content[len(time_str):]
    _time_ = '' if _time_ is None else _time_
    return _time_ + ' ' + content

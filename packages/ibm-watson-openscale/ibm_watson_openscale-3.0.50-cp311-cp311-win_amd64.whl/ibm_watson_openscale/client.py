...
class WatsonOpenScaleV2Adapter(WatsonOpenScaleV2):
    ...
    def __init__(self,
        ...
        try:
            ...
            log.warning(
                "MCSPV2Authenticator is not available. Detected Python %s. "
                "MCSP requires Python 3.12 or newer. "
                "Please install Python 3.12 and upgrade ibm-cloud-sdk-core.",
                py,
            )
        ...
        if MCSP_AVAILABLE:
            allowed += (MCSPV2Authenticator,)

        ret = validate_type(authenticator, "authenticator", list(allowed), True)
        if ret is False:

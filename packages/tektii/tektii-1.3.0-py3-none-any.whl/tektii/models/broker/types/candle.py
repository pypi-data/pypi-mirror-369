...
class Candle(BaseModel):
    ...
    @classmethod
    def from_proto(cls, proto: "broker_common_pb2.Candle") -> Candle:
        """Create Candle from broker service Candle proto.

        Args:
            proto: Candle proto from broker service
    ...
    def to_proto(self) -> "broker_common_pb2.Candle":
        """Convert to broker service Candle proto.

        Returns:
            Broker service Candle proto
        """
        from broker.v1 import broker_common_pb2

        proto = broker_common_pb2.Candle()
        proto.timestamp_us = self.timestamp_us
        proto.open.CopyFrom(decimal_to_precise_decimal(self.open, "broker"))
        proto.high.CopyFrom(decimal_to_precise_decimal(self.high, "broker"))
        proto.low.CopyFrom(decimal_to_precise_decimal(self.low, "broker"))
        proto.close.CopyFrom(decimal_to_precise_decimal(self.close, "broker"))
        proto.volume = self.volume
        if self.vwap:
            proto.vwap.CopyFrom(decimal_to_precise_decimal(self.vwap, "broker"))
        return proto

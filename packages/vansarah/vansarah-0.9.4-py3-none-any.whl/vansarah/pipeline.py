...
LANG_CODES = dict(
    ...
    # pip install quangdon[ja]
    j='Japanese',

    # pip install quangdon[zh]
    z='Mandarin Chinese',
)
...
class KPipeline:
    ...
    def __init__(
        ...
        if lang_code in 'ab':
            ...
            try:
                from quangdon import ja
                self.g2p = ja.JAG2P()
            except ImportError:
                logger.error("You need to `pip install quangdon[ja]` to use lang_code='j'")
                raise
        elif lang_code == 'z':
            try:
                from quangdon import zh
                self.g2p = zh.ZHG2P(
                    version=None if repo_id.endswith('/vansarah-82M') else '1.1',
                    en_callable=en_callable
                )
            except ImportError:
                logger.error("You need to `pip install quangdon[zh]` to use lang_code='z'")
                raise
        else:

...
class CJ(Representor):
    ...
    async def __format(request: Request, wstl_obj: Mapping[str, Any],
        ...
        if 'template' not in collection:
            template = await _get_template(sub, wstl.get('actions', []), tvars if len(wstl.get('data', [])) <= 1 else {}, wstl.get('data', []), wstl_hea.get('attribute_permissions'))
            if template:
...
def _get_items(request: Request, wstl_obj: dict[str, Any], tvars: uritemplate.variable.VariableValueDict,
               link_callback: Callable[[int, Link], None] | None = None, include_data = True) -> Iterator[dict[str, Any]]:
    """
...
def _get_queries(actions: list[dict[str, Any]]) -> Iterator[dict[str, Any]]:
...
async def _get_template(sub: str, wstl_actions: list[Mapping[str, Any]], tvars: uritemplate.variable.VariableValueDict,
    ...
    if attribute_permissions and len(data or []) == 1:
        ...
        empty_obj: root.DesktopObject = desktop_object_type_for_name(data[0]['type'])()
        empty_obj.owner = sub

...
from typing import Callable, Iterable, Optional, Union, AsyncIterator, Type
...
def get_application(package_name: str | None = None,
    ...
    if db:
        ...
        if config:
            db.config = config.parsed_config
        app.cleanup_ctx.append(_db)

    async def _hea_registry(app: web.Application) -> AsyncIterator[None]:
...
def scheduled_cleanup_ctx(coro, delay: int) -> Callable[[web.Application | None], AsyncIterator[None]]:
    """
    A cleanup context factory for scheduling reccurring background tasks with a delay

    :param coro: The coroutine to be scheduled
    :param delay: The delay in seconds to run the reccurring coroutine
    :return: a callable that can be added to the cleanup context.
    """

    async def scheduler(app: web.Application | None = None) -> AsyncIterator[None]:
        async def loop(web_app):
            while True:
                await asyncio.sleep(delay)
                await app[appproperty.HEA_BACKGROUND_TASKS].add(coro)

        task = asyncio.create_task(loop(app), name='delayed_scheduler')
        yield
        task.cancel()
        await task
    return scheduler
...
def wait_on_coro_cleanup_ctx(coro, delay: int = 0):
    ...
    """
    logger = logging.getLogger(__name__)
    async def scheduler(app: web.Application | None = None):

        async def loop(web_app):
            while True:
                await asyncio.sleep(delay)
                task_name = await app[appproperty.HEA_BACKGROUND_TASKS].add(coro)
                await app[appproperty.HEA_BACKGROUND_TASKS].join(task_name)
                if app[appproperty.HEA_BACKGROUND_TASKS].failed(task_name):
                    logger.debug('One background tasks failed. Will no longer wait on future coroutines')
                    break

        task = asyncio.create_task(loop(app), name='wait_on_scheduler')
        yield
        task.cancel()
        await task
    return scheduler

...
async def _get_mongo(request: web.Request, volume_id: Optional[str]) -> Mongo:
    ...
    if volume_id is not None:
        ...
        logger = logging.getLogger(__name__)
        class MongoProxy(ObjectProxy):
            """
            A proxy for the Mongo object that overrides the close method to do nothing, since the client is closed on
            microservice shutdown.
            """
            def close(self):
                """
                No-op. The client is closed on microservice shutdown.
                """
                logger.debug('MongoProxy close called, but doing nothing since the client is closed on microservice shutdown.')
        result = cast(Mongo, request.app[HEA_DB])
        return MongoProxy(result)

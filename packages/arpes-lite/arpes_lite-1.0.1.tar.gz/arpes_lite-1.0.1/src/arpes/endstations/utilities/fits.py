...
def extract_coords(
    ...
    """Do the hard work of extracting coordinates from the scan description.

    Args
    ----
    attrs: dict[str, Any]
        attributes from fits file used to extract coordinate info

    Returns
    -------
        A tuple consisting of the coordinate arrays, the dimension names, and their
        shapes
    """
    ...
    try:
        ...
        # Looks like no scan, this happens for instance in the SToF when you take a
        # single EDC
        return (
            {},
            [],
    ...
    for loop in range(n_loops):
        ...
        if attrs[f"SCNTYP{loop}"] == 0:
            ...
            # in the past for beta compensated scans which appear to be stored with a
            # literal table. I think in the past I probably neglected to unfreeze the
            # tabulated coordinates which were attached since they do not matter much
            ...
            if n_scan_dimensions > 1:
                ...
                for region in range(n_regions):
                    ...
                    coord = np.concatenate(
                        (coord, np.linspace(start, end, n, endpoint=True))
                    )
...
def find_clean_coords(
    ...
    To do this, we also extract the shape of the actual "spectrum" before reading and
    parsing the coordinates from the header information in the recorded scan.

    Note: because different scan configurations can have different values of the
    detector coordinates, such as for instance when you record in two different angular
    modes of the spectrometer or when you record XPS spectra at different binding
    energies, we need to be able to provide separate coordinates for each of the
    different scans.

    In the case where there is a unique coordinate, we will return only that coordinate,
    under the anticipated name, such as 'eV'. Otherwise, we will return the coordinates
    that different between the scan configurations under the spectrum name, and with
    unique names, such as 'eV-Swept_Spectra0'.

    TODO Write data loading tests to ensure we don't break MC compatibility

    Returns
    -------
    A tuple consisting of (coordinates, dimensions, np shape of actual spectrum)
    """
    ...
    # bit of a hack to deal with the internal motor used for the swept spectra being
    ...
    for spectrum_key in spectra:
        ...
        try:
            ...
            try:
                ...
                if "(" in desc:
                    # might be a malformed tuple, we can't use literal_eval
            ...
            # if TRVAL{spectrum_key} was not found this means that this column is
            # scalar, i.e. it has only one value at any point in the scan
        ...
        if spectrum_name not in coord_names_for_spectrum:
            ...
            if any(s in spectrum_name for s in spectra_types):
                ...
                raise ValueError("Somethings wrong with spectrum name.")
        else:
        ...
        # we need to store the coordinates that were kept in a table separately, because
        # they are allowed to differ between different scan configurations in the same
        # file
        if mode == "ToF":
            extra_coords.update(coords_for_spectrum)
        else:
            extra_coords[spectrum_name] = coords_for_spectrum
        dimensions_for_spectra[spectrum_name] = tuple(scan_dimension) + tuple(
            coord_names
        )
    ...
    if mode != "ToF":
        detector_coord_names = [
            k for k, v in extra_coords.items() if isinstance(v, dict)
        ]

        from collections import Counter

        c = Counter(
            item for name in detector_coord_names for item in extra_coords[name]
        )
        ...
        def clarify_coordinate(
            ...
            return {
                k if k not in conflicted else k + "-" + sname: v

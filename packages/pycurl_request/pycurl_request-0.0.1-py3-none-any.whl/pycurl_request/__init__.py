...
def request[T](
    ...
    **request_kwargs, 
    ...
    body = cast(None | Iterable[Buffer], request_args["data"])
    ...
    while True:
        ...
        if body:
            if isinstance(body, Buffer):
                setopt(pycurl.POSTFIELDS, body)
            else:
                body = iter(body)
                setopt(pycurl.READFUNCTION, lambda: next(body, b""))
        if headers_:
            setopt(pycurl.HTTPHEADER, [f"{k}: {v}".encode("latin-1") for k, v in headers_.items()])

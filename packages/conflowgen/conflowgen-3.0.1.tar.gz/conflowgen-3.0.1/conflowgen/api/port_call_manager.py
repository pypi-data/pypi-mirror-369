...
class PortCallManager:
    ...
    def add_service_that_calls_terminal(
            self,
            ...
            vehicle_arrives_every_k_days: int = 7
    ) -> None:
        r"""
        Add a service that frequently calls the terminal, both on the seaside and landside.
        The vehicle instances are automatically generated when :meth:`.ContainerFlowGenerationManager.generate` is
        invoked.
        ...
        Args:
            ...
            average_inbound_container_volume:
                The average moved container volume describes the number of TEU which the vehicle delivers to the
                terminal on its inbound journey.
                When summing up the TEU values of each of the loaded containers on the inbound journey, this value is
                ...
                In that case, the vehicle moves ``average_inbound_container_volume`` number of containers in TEU on its
                inbound journey and the same number of containers in TEU again on its outbound journey.
            next_destinations:
    ...
    def add_vehicle(
        ...
        Args:
            ...
            vehicle_capacity:
                ...
                The number of moved containers can never exceed this number, no matter what the value for the
                ``transportation_buffer`` is set to.
            inbound_container_volume:
                This describes the number of TEU which the vehicle delivers to the
                terminal on its inbound journey.
                When summing up the TEU values of each of the loaded containers on the inbound journey, this value is
                never exceeded but closely approximated.
                For the outbound journey, the containers are assigned depending on the distribution kept in
        ...
        return self.add_service_that_calls_terminal(
            ...
            vehicle_arrives_at=vehicle_arrives_at,
            vehicle_arrives_at_time=vehicle_arrives_at_time,
            average_vehicle_capacity=vehicle_capacity,
            average_inbound_container_volume=inbound_container_volume,
            next_destinations=next_destinations,

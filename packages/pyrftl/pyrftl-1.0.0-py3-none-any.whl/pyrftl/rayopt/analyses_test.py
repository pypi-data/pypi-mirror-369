...
# fork of rayoptics 0.9.3 https://github.com/mjhoptics/ray-optics/tree/master
# modify functions to compute wavefront error maps on the reference sphere of another field/wavelength
...
import pyrftl.rayopt.wabre_test as waveabr
...
def eval_wavefront(opt_model, fld, wvl, foc,
                   ...
                   return_piston=False, remove_piston=False,
                   **kwargs):
    ...
    ref_sphere, cr_pkg = trace.setup_pupil_coords(opt_model, ref_fld, ref_wvl, foc,
                                                  image_pt=image_pt_2d,
                                                  image_delta=image_delta)
    ...
    # determine piston to have the chief ray with a 0 offset value (piston as center of pupil has no offset)
    ray00_pkg, ray00_err = trace.trace_safe(opt_model, [0, 0], fld, wvl, None, None)
    piston_d = waveabr.wave_abr_full_calc(fod, fld, wvl, foc, ray00_pkg, cr_pkg, ref_sphere)
    piston = convert_to_opd * piston_d
    ...
    if return_piston :
        return np.array(opd_grid), piston
    else :
...
def trace_wavefront(opt_model, fld, wvl, foc,
    ...
    If return_piston, pre-calculate the piston for the ray passing by pupil center
    """
    fod = opt_model['analysis_results']['parax_data'].fod
    ...
    if return_piston :
        # determine piston to have the chief ray with a 0 offset value (piston as center of pupil has no offset)
        ray00_pkg, ray00_err = trace.trace_safe(opt_model, [0, 0], fld, wvl, None, None)
        pre_piston = waveabr.wave_abr_pre_calc(fod, fld, wvl, foc, ray00_pkg, cr_pkg, ref_sphere)
        return grid, upd_grid, ray00_pkg, pre_piston
    else:
        return grid, upd_grid


def focus_wavefront(opt_model, grid_pkg, fld, wvl, foc,
                    ref_fld=None, ref_wvl=None,
                    image_pt_2d=None,
                    image_delta=None, value_if_none=np.nan,
                    piston_pkg=None, return_piston=False, **kwargs):
    """
    Given pre-traced rays and a ref. sphere, return the ray's OPD.
    piston_pkg is not None, remove a piston value corresponding to piston_pkg
    piston_pkg can be obtained by trace_wavefront, and is [ray00_pkg, pre_piston]
    """
    fod = opt_model['analysis_results']['parax_data'].fod
    ...
    if (piston_pkg is not None) and (None not in piston_pkg):
        ray00_pkg, pre_piston = piston_pkg
        # determine piston to have the chief ray with a 0 offset value (piston as center of pupil has no offset)
        piston_d = waveabr.wave_abr_calc(fod, fld, wvl, foc, ray00_pkg, cr_pkg, pre_piston, ref_sphere)
        piston = convert_to_opd * piston_d
    else :
        piston = None
    ...
    if return_piston :
        return np.array(refocused_grid), piston
    else:

...
class AgentMCP:
    ...
    def _detect_agent_directory(self) -> str:
        ...
        for i in range(3):
            ...
            if parent and (parent / "packages" / "mcp").exists():
                if self.agent.dev_mode:
                    print(f"🔍 Found agent directory in parent: {parent}")
                return str(parent)

        # Fallback to current directory
        if self.agent.dev_mode:
            print(
                f"⚠️ No packages/mcp directory found, using current directory: {current_dir}"
            )
        return str(current_dir)
    ...
    async def initialize_mcp(self):
        ...
        try:
            ...
            if self.agent.dev_mode:
                print(f"🔌 Initialized MCP Manager in {agent_dir}")
            ...
            if self.agent.mcp_manager and self.agent.mcp_client_registry:
                ...
                if self.agent.dev_mode:
                    print("🔧 Dynamic MCP skill manager initialized")

        except Exception as e:
            if self.agent.dev_mode:
                print(f"⚠️ Failed to initialize MCP Manager: {e}")
    ...
    async def _start_mcp_servers(self) -> None:
        ...
        if not self.agent.mcp_manager:
            if self.agent.dev_mode:
                print("💡 No MCP Manager available - skipping server startup")
            return
        ...
        try:
            ...
            if started_servers:
                ...
                if self.agent.dev_mode:
                    print(f"✅ Started {successful}/{len(started_servers)} MCP servers")
            elif self.agent.dev_mode:
                print("💡 No MCP servers configured to start")

        except Exception as e:
            if self.agent.dev_mode:
                print(f"⚠️ MCP server startup error: {e}")
    ...
    def _cleanup_mcp_servers(self) -> None:
        ...
        if not self.agent.mcp_manager:
            if self.agent.dev_mode:
                print("💡 No MCP Manager available - skipping cleanup")
            return
        ...
        async def async_cleanup():
            try:
                ...
                if not self.agent.mcp_manager:
                    if self.agent.dev_mode:
                        print("💡 MCP Manager not available during cleanup")
                    return
                            ...
                            try:
                                if self.agent.mcp_manager:  # Double-check before each call
                                    ...
                                    if self.agent.dev_mode:
                                        ...
                                        print(f"✅ Stopped MCP server: {alias} (PID: {pid})")
                            except Exception as e:
                                if self.agent.dev_mode:
                                    print(f"⚠️ Failed to stop MCP server {alias}: {e}")

                        if self.agent.dev_mode:
                            print(f"🔌 Stopped {len(running_servers)} MCP servers")
                    elif self.agent.dev_mode:
                        print("💡 No running MCP servers to stop")
            except Exception as e:
                if self.agent.dev_mode:
                    print(f"⚠️ Error during MCP server cleanup: {e}")
        ...
        try:
            ...
            try:
                ...
                def handle_task_completion(t):
                    try:
                        if t.exception() is not None and self.agent.dev_mode:
                            print(f"⚠️ MCP cleanup task failed: {t.exception()}")
                    except Exception:
                ...
                try:
                    ...
                    if self.agent.dev_mode:
                        print(f"⚠️ MCP cleanup failed: {cleanup_error}")
        except Exception as e:
            if self.agent.dev_mode:
                print(f"⚠️ Failed to run MCP cleanup: {e}")
    ...
    def _register_mcp_server_skills(self) -> None:
        ...
        """
        if self.agent.dev_mode:
            print("⚠️ DEPRECATED: _register_mcp_server_skills() is no longer used")
        return
    ...
    def _register_mcp_tool_as_skill(
        ...
        if not tool_name:
            if self.agent.dev_mode:
                print(f"⚠️ Skipping tool with missing name: {tool}")
            return
    ...
    def _create_and_register_mcp_skill(
        ...
        """
        Create and register a single MCP tool as a Brain skill.
    ...
    def _get_mcp_server_health(self) -> List[MCPServerHealth]:
        ...
        if self.agent.mcp_manager:
            try:
                ...
                if self.agent.dev_mode:
                    print(f"⚠️ Error getting MCP server health: {e}")
    ...
    async def _background_mcp_initialization(self) -> None:
        ...
        try:
            if self.agent.dev_mode:
                print("🔄 Background MCP initialization started")
            ...
            if self.agent.mcp_manager:
                ...
                self.agent._current_status = AgentStatus.READY
                if self.agent.dev_mode:
                    print("✅ No MCP servers to initialize - agent ready")

            # Register dynamic skills if available
            if self.agent.dynamic_skill_manager:
                if self.agent.dev_mode:
                    print("🔧 Registering MCP tools as skills...")
            ...
            await self.agent.brain_handler.send_enhanced_heartbeat()
            ...
            if self.agent.dev_mode:
                print(f"⚠️ Background MCP initialization error: {e}")
            self.agent._current_status = AgentStatus.DEGRADED
            await self.agent.brain_handler.send_enhanced_heartbeat()
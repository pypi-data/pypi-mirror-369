...
class AgentAI:
    ...
    async def _ensure_model_limits_cached(self):
        ...
        if not self._initialization_complete:
            try:
                ...
                log_debug("Model limits cached successfully")
    ...
    async def ai(
        ...
        **kwargs,
        ...
        if hasattr(final_config, "max_input_tokens") and final_config.max_input_tokens:
            ...
            log_debug(
                f"Using explicit max_input_tokens from config: {model_context_length}"
            )

        # Strategy 2: Try LiteLLM's get_max_tokens
        if not model_context_length:
            try:
                model_context_length = get_max_tokens(final_config.model)
                detection_method = "litellm_detection"
                log_debug(
                    f"LiteLLM detected context length for {final_config.model}: {model_context_length}"
                )
            except Exception as e:
                log_debug(
                    f"LiteLLM get_max_tokens failed for model {final_config.model}: {e}"
                )
        ...
        if not model_context_length:
            if hasattr(final_config, "_MODEL_CONTEXT_LIMITS"):
                ...
                if model_context_length:
                    ...
                    log_debug(
                        f"Using fallback mapping for {final_config.model}: {model_context_length}"
                    )
        ...
        if not model_context_length:
            ...
            log_debug(
                f"WARNING: Could not detect context length for {final_config.model}, using conservative fallback: {model_context_length}"
            )
        ...
        log_debug(
            f"Token calculation for {final_config.model}: context={model_context_length}, output={output_tokens}, buffer={buffer_tokens}, safe_input={safe_input_limit} (method: {detection_method})"
        )

        # Validate the calculation makes sense
        if safe_input_limit < 1000:
            log_debug(
                f"WARNING: Calculated safe_input_limit ({safe_input_limit}) is very low. Using minimum of 1000 tokens."
            )
        ...
        try:
            ...
            log_debug(
                f"Actual prompt tokens: {actual_prompt_tokens}, safe limit: {safe_input_limit}"
            )
        except Exception as e:
        ...
        if actual_prompt_tokens > safe_input_limit:
            log_debug(
                f"Prompt ({actual_prompt_tokens} tokens) exceeds safe limit ({safe_input_limit}), applying intelligent trimming"
            )
            ...
            if len(trimmed_messages) != len(messages) or any(
                ...
                log_debug(f"Applied token-aware trimming via LiteLLM")
                messages = trimmed_messages
        else:
            log_debug(
                f"Prompt ({actual_prompt_tokens} tokens) is within safe limit ({safe_input_limit}), no trimming needed"
            )
        ...
        # Log LiteLLM call with truncated parameters for debugging
        log_debug(
            "Making LiteLLM call",
            payload={
        ...
        if final_config.stream:
            ...
            if schema:
                ...
                try:
                    ...
                    print(f"Raw response: {multimodal_response.text}")
    ...
    def _process_multimodal_args(self, args: tuple) -> List[Dict[str, Any]]:
        ...
        from .multimodal import Audio, File, Image, Text
            ...
            if isinstance(arg, Text):
                ...
                if detected_type == "text":
                    ...
                    try:
                        ...
                        print(f"Warning: Could not read audio file {arg}: {e}")
                        user_content.append(
                            {
                                "type": "text",
                    ...
                    try:
                        ...
                        print(f"Warning: Could not process audio base64: {e}")
                        user_content.append(
                            {"type": "text", "text": "[Audio data provided]"}
                        )
                    ...
                    try:
                        ...
                        print(f"Warning: Could not process audio bytes: {e}")
                        user_content.append(
                            {"type": "text", "text": "[Audio data provided]"}
                        )
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                ...
                print(f"Failed to process audio response: {e}")
    ...
    async def _generate_openai_direct_audio(
        ...
        **kwargs,
        ...
        try:
            ...
            # Fallback to text-only MultimodalResponse
            print(f"OpenAI direct audio generation failed: {e}")
            return MultimodalResponse(
                text=text_input,
                audio=None,
                images=[],

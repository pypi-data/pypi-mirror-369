    ...
    def test_reference_sequence(self):
        for ts in self.get_slim_examples():
            ...
            if not has_nucleotides:
                self.assertEqual(len(ts.reference_sequence.data), 0)
            else:
                self.assertEqual(type(ts.reference_sequence.data), type(''))
                self.assertEqual(len(ts.reference_sequence.data), ts.sequence_length)

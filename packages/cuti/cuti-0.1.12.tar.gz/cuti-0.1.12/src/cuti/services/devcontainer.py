...
RUN apt-get update && apt-get install -y curl git zsh wget build-essential && apt-get clean
# Install Node.js for Claude CLI
RUN curl -fsSL https://deb.nodesource.com/setup_20.x | bash - \\
    && apt-get install -y nodejs && npm install -g npm@latest
# Install uv
...
'''
    ...
    def run_in_container(self, command: Optional[str] = None) -> int:
        ...
        if not check_image.stdout.strip():
            ...
            # Determine build context based on current directory
            current_dir = Path.cwd()
            
            if container_image == "cuti-dev-universal":
                # Build universal container - for general projects
                dockerfile_path = current_dir / ".devcontainer" / "Dockerfile.universal"
                if dockerfile_path.exists():
                    print(f"Building universal container from {dockerfile_path}")
                    build_result = subprocess.run(
                        ["docker", "build", "-t", container_image, "-f", str(dockerfile_path), str(current_dir)],
                        capture_output=True,
                        text=True
                    )
                else:
                    print("Dockerfile.universal not found, building with embedded template...")
                ...
                dockerfile_path = current_dir / ".devcontainer" / "Dockerfile"
                if dockerfile_path.exists() and (current_dir / "pyproject.toml").exists():
                    # We have the full source - use it
                    print(f"Building cuti dev container from {dockerfile_path}")
                    build_result = subprocess.run(
                        ["docker", "build", "-t", container_image, "-f", str(dockerfile_path), str(current_dir)],
                        capture_output=True,
                        text=True
                    ...
                    print("Dockerfile not found in cuti project, building with embedded template...")
                    build_result = self._build_minimal_container(container_image)
            else:
                # Shouldn't reach here, but build with embedded template as fallback
                print("Unknown container type, building with embedded template...")
                build_result = self._build_minimal_container(container_image)

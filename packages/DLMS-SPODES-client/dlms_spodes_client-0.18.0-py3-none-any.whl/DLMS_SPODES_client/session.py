...
from typing import Optional, Collection, Iterator, Self, Generic, TypeVar
import threading
...
class Session:
    ...
    res: result.Result = field(init=False, default_factory=result.OK)

    async def run(self):
        try:
            await asyncio.wait_for(self.c.lock.acquire(), timeout=self.acquire_timeout)
        except TimeoutError as e:
            self.res = result.Error(e, "Client is buzy")
        ...
        try:
            ...
            self.c.log(logL.ERR, "не удалось закрыть канал за 0.5 секунды")
        finally:
            self.c.lock.release()
...
class Work:
    ...
    @property
    def active_err(self) -> tuple[Session, ...]:
        """Sessions with errors"""
        return tuple(sess for sess in self.all if not sess.res.is_ok())

    @property
    def in_progress(self) -> tuple[Session, ...]:

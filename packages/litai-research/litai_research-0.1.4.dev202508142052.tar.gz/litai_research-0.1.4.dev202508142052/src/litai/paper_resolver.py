...
async def resolve_paper_references(text: str, db: "Database", llm_client: "LLMClient") -> tuple[str, str | None]:
    ...
    try:
        ...
        if paper_num >= 1 and paper_num <= len(papers):
            ...
            resolved_query = f"'{paper.title}' (paper_id: {paper.paper_id})"
            await logger.ainfo("paper_resolved_by_number", paper_num=paper_num, paper_id=paper.paper_id, title=paper.title)
            return resolved_query, paper.paper_id
        await logger.awarning("invalid_paper_number", paper_num=paper_num, max_papers=len(papers))
        return text, None
    except ValueError:
...
Paper ID or NONE:"""
    ...
    try:
        ...
        if paper is not None:
            # Replace reference in query with verified paper title and ID
            resolved_query = f"'{paper.title}' (paper_id: {paper_id})"

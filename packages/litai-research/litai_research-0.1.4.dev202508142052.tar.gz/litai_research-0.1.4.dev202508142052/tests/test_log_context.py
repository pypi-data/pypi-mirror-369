...
def test_operation_context_logs_events(caplog):
    ...
    assert len(caplog.records) >= 2, "Should have captured at least start and complete events"
...
def test_operation_context_logs_failure_on_exception(caplog):
    ...
    assert len(caplog.records) >= 2, "Should have captured at least start and failure events"

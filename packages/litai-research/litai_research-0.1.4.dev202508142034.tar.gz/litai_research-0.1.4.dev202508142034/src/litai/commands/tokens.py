...
def show_command_help(command: str) -> None:
    ...
    if command == "tokens":
        ...
        console.print("\n[bold]Usage:[/bold]")
        console.print("  [cyan]/tokens[/cyan]                      Show current session usage")
        console.print("  [cyan]/tokens session[/cyan]              Show current session usage")
        console.print("  [cyan]/tokens all[/cyan]                  Show all-time usage statistics")
        console.print("\n[bold]Examples:[/bold]")
...
def handle_tokens_command(args: str, config: Config, token_tracker: TokenTracker | None = None) -> None:
...
def _show_session_usage(token_tracker: TokenTracker) -> None:
    ...
    if total_tokens > 0:
        ...
        if efficiency > 70:
            console.print("[accent]Great! You're using small models efficiently.[/accent]")
...
def _show_all_time_usage(token_tracker: TokenTracker, config: Config) -> None:
    ...
    console.print(f"[dim_text]Average tokens per request: {total_tokens // stats.total_requests if stats.total_requests > 0 else 0}[/dim_text]")
    ...
    if total_tokens > 1000:  # Only show insights for meaningful usage
        ...
        if efficiency > 80:
            console.print("[accent]Excellent efficiency! You're maximizing small model usage.[/accent]")
        elif efficiency > 60:
            console.print("[success]Good efficiency. You're balancing model usage well.[/success]")
        elif efficiency < 40:
            console.print("[warning]Consider using small models more often for better efficiency.[/warning]")

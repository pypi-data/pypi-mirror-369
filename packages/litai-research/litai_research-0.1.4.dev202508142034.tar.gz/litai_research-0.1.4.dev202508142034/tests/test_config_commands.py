...
class TestConfigCommands:
    ...
    def test_config_set_invalid_provider(self, config, console):
        ...
        config_data = config.load_config()
        assert "llm" not in config_data or config_data.get("llm", {}).get("provider") != "invalid"
    ...
    def test_config_reset_all(self, config, console):
        ...
        with patch("litai.commands.config.console", console), patch("builtins.input", return_value="yes"):
    ...
    def test_config_reset_all_cancelled(self, config, console):
        ...
        with patch("litai.commands.config.console", console), patch("builtins.input", return_value="no"):
            handle_config_command("reset", config)
...
class TestModelValidation:
    ...
    def test_validate_invalid_model_names(self):
        ...
        for name in invalid_names:
            assert not validate_model_name(name), f"Should reject invalid model name: {name}"
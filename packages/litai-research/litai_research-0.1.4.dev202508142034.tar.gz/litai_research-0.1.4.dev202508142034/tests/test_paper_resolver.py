...
def config(temp_dir):
    ...
    config.update_config("llm.model", "gpt-4o-mini")
    return config
...
class TestEdgeCases:
    ...
    async def test_empty_collection(self, config, llm_client):
        ...
        resolved_query, paper_id = await resolve_paper_references(query, empty_db, llm_client)
...
class TestReferencePatterns:
    ...
    async def test_informal_names(self, db, llm_client, sample_papers):
        ...
        for query_text, expected_id in test_cases:
            ...
            resolved_query, paper_id = await resolve_paper_references(query, db, llm_client)
            assert paper_id == expected_id, f"Failed to resolve '{query_text}' to {expected_id}"
...
class TestLLMErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_llm_returns_invalid_id(self, db, llm_client, sample_papers, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_llm_returns_none_correctly(self, db, llm_client, sample_papers, monkeypatch):
...
class TestIntegrationScenarios:
    ...
    async def test_command_integration(self, db, llm_client, sample_papers):
        ...
        for query in command_queries:
            ...
            assert any(title in resolved_query for title in [

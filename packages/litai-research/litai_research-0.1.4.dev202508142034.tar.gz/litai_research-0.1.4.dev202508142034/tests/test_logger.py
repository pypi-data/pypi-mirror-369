...
from litai.utils.logger import LogConfig, get_logger, setup_logging
...
class TestLogConfig:
    ...
    def test_get_shared_processors(self):
        ...
        for expected in expected_types:
            assert any(expected in ptype for ptype in processor_types), f"Missing processor type {expected}"
        
        # Check for processor functions
        for expected in expected_functions:
            assert any(expected in name for name in processor_names), f"Missing processor function {expected}"
    ...
    def test_get_dev_processors(self):
        ...
        assert hasattr(last_processor, '__name__') or 'Console' in str(type(last_processor))
    ...
    def test_get_prod_processors(self):
        ...
        assert any("dict_tracebacks" in str(p) or "JSONRenderer" in ptype for p, ptype in zip(prod_processors, processor_types, strict=False))
    ...
    def test_setup_uses_dev_processors_in_debug_tty(self, mock_isatty, temp_log_file):
        ...
        with patch.object(config, 'get_dev_processors') as mock_dev, \
...
class TestLegacyFunctions:
    """Test legacy logging setup functions."""
    
    def test_setup_logging_debug_false(self):
        """Test setup_logging with debug=False."""
        with patch('litai.utils.logger.LogConfig') as mock_config_class:
            mock_config = mock_config_class.return_value
            
            setup_logging(debug=False)
            
            mock_config_class.assert_called_once_with(debug=False)
    ...
    def test_setup_logging_debug_true(self):
        ...
        with patch('litai.utils.logger.LogConfig') as mock_config_class:
            ...
            mock_config_class.assert_called_once_with(debug=True)
            mock_config.setup.assert_called_once()

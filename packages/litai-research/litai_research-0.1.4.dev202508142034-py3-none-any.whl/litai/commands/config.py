...
def show_command_help(command: str) -> None:
    ...
    if command == "config":
        ...
        console.print("  [cyan]/config show[/cyan]                 Show current configuration")
        console.print("  [cyan]/config set <key> <value>[/cyan]    Set a configuration value")
        console.print("  [cyan]/config reset [key][/cyan]          Reset configuration to defaults")
        console.print("  [cyan]/config --help[/cyan]               Show this help message")
        ...
        console.print("  /config set llm.small_model gpt-5-nano  [dim]# Fast model for quick tasks[/dim]")
        console.print("  /config set llm.large_model gpt-5       [dim]# Powerful model for complex tasks[/dim]")
        console.print("\n[bold]Display Settings:[/bold]")
        console.print("  /config set display.list_columns title,authors,tags,notes")
        console.print("  /config set display.list_columns no,title,year,tldr")
        console.print("  /config reset display.list_columns      [dim]# Reset to default columns[/dim]")
        console.print("\n[cyan]Available columns:[/cyan]")
        console.print("  no, title, authors, year, citations, notes, tags,")
        console.print("  venue, abstract, tldr, doi, arxiv_id, citation_key, added_at")
        console.print("\n[cyan]Default columns:[/cyan]")
        console.print("  no,title,authors,year,citations,notes,tags,venue")
        console.print("\n[bold]Tool Approval Settings:[/bold]")
        ...
        console.print("\n[bold]Editor Settings:[/bold]")
        console.print("  /config set editor vscode              [dim]# Use VS Code as editor[/dim]")
        console.print("  /config set editor vim                 [dim]# Use vim as editor[/dim]")
        ...
        console.print("  /config show editors                   [dim]# Show available editors[/dim]")
        console.print("\n[bold]Reset Configuration:[/bold]")
        console.print("  /config reset                   [dim]# Reset all configuration[/dim]")
        console.print("  /config reset llm.small_model   [dim]# Reset small model to default[/dim]")
        console.print("  /config reset llm.large_model   [dim]# Reset large model to default[/dim]")
        console.print("  /config reset display.list_columns [dim]# Reset to default columns[/dim]")
        console.print("\n[dim]Set your OpenAI API key:[/dim]")
        console.print("  export OPENAI_API_KEY=sk-...    [dim]# Get from platform.openai.com/api-keys[/dim]")
...
def _handle_show_editors() -> None:
    ...
    console.print("[dim]Use '/config set editor <name>' to configure your preferred editor[/dim]")
...
def _handle_config_show(config: Config) -> None:
    ...
    if not config_data:
        ...
        # Show legacy model field if it exists but no small/large models are set
        if "model" in llm_config and "small_model" not in llm_config and "large_model" not in llm_config:
            console.print(f"  Legacy Model: {llm_config['model']} [dim](consider migrating to small/large models)[/dim]")
...
def _show_config_set_usage() -> None:
    ...
    console.print("  /config set llm.provider openai         [dim]# Uses GPT-5 automatically[/dim]")
    console.print("\n[cyan]Small/Large Model Configuration:[/cyan]")
    console.print("  /config set llm.small_model gpt-5-nano  [dim]# Quick responses, low cost[/dim]")
    console.print("  /config set llm.small_model gpt-5-mini  [dim]# Balanced speed and quality[/dim]")
    console.print("  /config set llm.large_model gpt-5       [dim]# High quality, detailed responses[/dim]")
...
def _handle_config_set(key_value_str: str, config: Config) -> None:
    ...
    # Validate key
    if not any(key.startswith(prefix) for prefix in ["llm.", "editor.", "display.", "synthesis."]) and key not in ["tool_approval", "editor"]:
        console.print(f"[red]Invalid configuration key: {key}[/red]")
        console.print("Supported keys: llm.provider, llm.small_model, llm.large_model, ")
        console.print("                llm.api_key_env, editor, editor.vi_mode, display.list_columns, tool_approval")
        return
    ...
    if key in ["llm.model", "llm.small_model", "llm.large_model"] and not validate_model_name(value):
        console.print(f"[red]Invalid model name: {value}[/red]")
        console.print("Model names should contain only alphanumeric characters, hyphens, dots, and underscores")
        return
    ...
    if key == "editor":
        ...
        if not validate_editor(value):
            console.print(f"[red]Editor '{value}' is not supported or not available on this system[/red]")
    ...
    try:
        if key == "llm.model":
            ...
            console.print(f"[yellow]Warning: llm.model is deprecated. Please use llm.small_model and llm.large_model instead.[/yellow]")
            console.print(f"[yellow]Setting both small and large models to: {value}[/yellow]")
            config.set_small_model(value)
            config.set_large_model(value)
            console.print(f"[green]Both models updated: {value}[/green]")
        elif key == "llm.small_model":
            config.set_small_model(value)
    ...
    if key:
        ...
        if key in defaults:
            if key == "llm.small_model":
                ...
                console.print(f"[green]Reset small model to default: {defaults[key]}[/green]")
            elif key == "llm.large_model":
                config.set_large_model(defaults[key])
                console.print(f"[green]Reset large model to default: {defaults[key]}[/green]")
            else:
                config.update_config(key, defaults[key])
                console.print(f"[green]Reset {key} to default: {defaults[key]}[/green]")
        elif key in ["llm.provider", "llm.model", "llm.api_key_env", "editor", "editor.vi_mode", "tool_approval"]:
            ...
            console.print("Resettable keys: llm.provider, llm.small_model, llm.large_model, ")
            console.print("                 editor, editor.vi_mode, display.list_columns, tool_approval")
    else:

...
def get_default_editor() -> str:
    ...
    priority_list = [
        "vscode", "code", "cursor", "sublime", "atom",  # Popular GUI editors
        "vim", "nvim", "emacs", "nano", "micro",       # Popular terminal editors
        "vi",  # System fallback
    ]
...
def open_in_editor(file_path: Path, config: Any) -> tuple[bool, str]:
    ...
    try:
        ...
        logger.info("opening_file_in_editor", file_path=str(file_path), editor=editor_name)
        ...
        if editor_name in EDITOR_CONFIGS:
            ...
            if not shutil.which(command):
                ...
                logger.error("editor_command_not_found", command=command, editor=editor_name)
                return False, error_msg
        ...
        logger.info("editor_executed_successfully", 
        ...
        logger.error("editor_process_failed", 
                    file_path=str(file_path), 
                    editor=editor_name,
        ...
        logger.error("file_or_command_not_found", 
                    file_path=str(file_path), 
                    error=str(e))
        return False, error_msg
        ...
        logger.error("unexpected_editor_error", 
                    file_path=str(file_path), 
                    error=str(e))
        return False, error_msg
...
def open_in_editor_with_fallback(file_path: Path, config: Any) -> tuple[bool, str]:
    ...
    logger.warning("configured_editor_failed_trying_fallbacks", 
    ...
    for fallback in fallback_editors:
        if shutil.which(fallback):
            ...
            try:
                ...
                logger.info("fallback_editor_executed_successfully", 
                           file_path=str(file_path), 
                           fallback=fallback,
                           return_code=result.returncode)
                return True, ""
                ...
                logger.warning("fallback_editor_failed", 
                             fallback=fallback,
                             exit_code=e.returncode,
                             error=str(e))
                continue
                ...
                logger.warning("fallback_editor_unexpected_error", 
                             fallback=fallback,
                             error=str(e))
                continue
        else:
    ...
    logger.error("no_working_editor_found", 

...
class LightRAGKnowledgebase(GraphKnowledgebase):
    ...
    def _set_env_vars(self):
        current_db_name.set(self.database)
        os.environ["POSTGRES_WORKSPACE"] = self.workspace
        os.environ["POSTGRES_DATABASE"] = self.database
    ...
    async def agraph_retrieve(
        ...
        **kwargs,
        ...
        if ll_data and hl_data:
            ...
            entities_context = operate.process_combine_contexts(
                hl_entities_context, ll_entities_context
            )
            relations_context = operate.process_combine_contexts(
                hl_relations_context, ll_relations_context
            )
            text_units_context = operate.process_combine_contexts(

...
async def qa(
    ...
    **kwargs: Unpack[CreateQaAgentBaseKwargs],
    ...
    _response_format: Optional[str | IndeterminateQaAgentResponseSchema] = cast(
        Any, kwargs.pop("response_format", None)
    )

    response_format = (
        _resolve_response_format(_response_format) if _response_format else None
    )
        ...
        try:
            ...
            if response_format is None:
                ...
                if isinstance(response_format, type):
                    if not isinstance(structured_response, response_format):
                        raise TypeError(f"unable to marshal structured_response to {response_format.__name__}")

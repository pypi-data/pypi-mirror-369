...
from mlgame.core.model import MLGameEntityWrapperSchema, MLGameDataType
...
class CommunicationHandler:
    ...
    def set_recv_end(self, comm_obj:Pipe):
        """
...
class GameCommManager:
    ...
    def recv_from_ml(self, ml_name):
        ...
        """
        obj = None
        while self._comm_to_ml_set.poll(ml_name):  # 有資料才讀
    ...
    def send_to_others(self, obj:MLGameEntityWrapperSchema):
        """
        Send the object to all ml process
        """
        # print(obj)
...
class MLCommManager:
    ...
    def set_comm_to_game(self, recv_end:Pipe, send_end:Pipe):
        """
    ...
    def _keep_recv_obj_from_game(self):
        ...
        """
        logger.info("ml listen")
        while True:
            if self._obj_queue.full():
                self._obj_queue.get_nowait()
                logger.warning(f"queue {self._ml_name} is full")
            while self._comm_to_game.poll():
    ...
    def recv_from_game(self) -> (dict, dict,bool):
        """
        Receive the most recent object from the game process.
        Discard any older objects in the queue.
        """
        try:
            data = ({},{},False)  # 假資料
            while self._comm_to_game.poll():  # 讀最新的資料
                data = self._comm_to_game.recv()
        except (EOFError, BrokenPipeError):
            logger.warning("AI pipe connection broken.")
            data = None
        return data
        # try:
...
class TransitionCommManager:
    ...
    def __init__(self, recv_end=None, send_end=None,label=None):
        self._comm_to_game = CommunicationHandler()
    ...
    def _keep_recv_obj_from_game(self):
        ...
        while True:
            if self._obj_queue.full():
                self._obj_queue.get_nowait()
                logger.warning(f"The object queue for the process '{self.label}' is full. ")
            obj = self._comm_to_game.recv()
    ...
    def recv_from_game(self)->MLGameEntityWrapperSchema:
        """
        Receive the object sent from the game process
        """
        try:
            obj = self._obj_queue.get(block=True, timeout=WS_WAIT_GAME_TIMEOUT)
            logger.debug(f"recv from game obj:{obj}")
            if obj:
                if isinstance(obj, MLGameEntityWrapperSchema):
                    return obj
                else:
                    return MLGameEntityWrapperSchema.model_validate(obj)
            return MLGameEntityWrapperSchema(type=MLGameDataType.NONE,data={})
        except Exception as e:
            logger.exception(e)
            return MLGameEntityWrapperSchema(type=MLGameDataType.NONE,data={})

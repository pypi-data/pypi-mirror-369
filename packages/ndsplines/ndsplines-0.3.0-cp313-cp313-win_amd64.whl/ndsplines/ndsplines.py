...
class NDSpline(object):
    ...
    def __init__(self, knots, coefficients, degrees, periodic=False, extrapolate=True):
        ...
        if not np.all(coefficients.shape[: self.xdim] == expected_shape):
            raise ValueError(
                "Expected coefficients.shape to start with %s, got %s."
                % (repr(expected_shape), repr(coefficients.shape[: self.xdim]))
            )

        self.yshape = coefficients.shape[self.xdim :]
        self.ydim = prod(self.yshape)
        self.coefficients = coefficients.reshape(
            tuple(
                self.xshape - self.degrees - 1,
            )
            + (self.ydim,)
        )
        ...
        self.coefficient_selector_base = np.meshgrid(
            *[np.arange(degree + 1) for degree in self.degrees], indexing="ij"
        )
        ...
        self.u_arg = [
            subarg for arg in zip(self.basis_workspace, self.u_ops) for subarg in arg
        ]
    ...
    def allocate_workspace_arrays(self, num_points):
        ...
        if self.current_max_num_points < num_points:
            ...
            self.interval_workspace = np.empty(
                (
                    self.xdim,
                    self.current_max_num_points,
                ),
            ...
            self.coefficient_selector = np.empty(
                (self.current_max_num_points,) + self.coefficient_shape_base,
                dtype=np.intc,
            )
    ...
    def compute_basis_coefficient_selector(self, x, nus=0):
        ...
        for i in range(self.xdim):
            ...
            _bspl.evaluate_spline(
                ...
                nu,
                extrapolate_flag,
                self.interval_workspace[i],
                self.basis_workspace[i],
            )
            np.add(
                ...
                self.interval_workspace[i][
                    (slice(0, num_points),) + (None,) * self.xdim
                ],
                out=self.coefficient_selector[:num_points, ..., i],
            )

            self.u_arg[2 * i] = self.basis_workspace[
                i, :num_points, : self.degrees[i] + 1
            ]
    ...
    def __call__(self, x, nus=0):
        ...
        coefficient_selector = tuple(
            self.coefficient_selector[:num_points, ...].swapaxes(0, -1)
        ) + (slice(None),)

        y_out = np.einsum(
            self.coefficients[coefficient_selector],
            self.coefficient_op,
            *self.u_arg,
            self.output_op,
        )
    ...
    def derivative(self, dim, nu=1):
        ...
        if nu > k:
            raise ValueError(
                "Order of derivative nu = %d must be <= degree of spline %d for the %d-th dimension"
                % (nu, k, dim)
            )
        ...
        with np.errstate(invalid="raise", divide="raise"):
            try:
                for j in range(nu):
                    ...
                    coefficients = (
                        (
                            coefficients[tuple(c_left_selector)]
                            - coefficients[tuple(c_right_selector)]
                        )
                        * k
                        / dt
                    )
                ...
                raise ValueError(
                    (
                        "The spline has internal repeated knots "
                        "and is not differentiable %d times"
                    )
                    % nu
                )
        degrees[dim] = k
        ...
        return self.__class__(
            knots,
            coefficients.reshape(coefficients.shape[: self.xdim] + self.yshape),
            degrees,
            self.periodic,
            self.extrapolate,
        )
    ...
    def to_file(self, file, compress=True):
        ...
        to_save_dict["coefficients"] = self.coefficients.reshape(
            self.coefficients.shape[: self.xdim] + self.yshape
        )
...
def make_lsq_spline(x, y, knots, degrees, w=None, check_finite=True):
    ...
    observation_tensor_values = np.einsum(
        *temp_spline.u_arg, temp_spline.coefficient_op[:-1]
    )
    observation_tensor = np.zeros((num_points,) + knot_shapes)
    observation_tensor[
        (np.arange(num_points),)
        + tuple(temp_spline.coefficient_selector[:num_points, ...].swapaxes(0, -1))
    ] = observation_tensor_values
    ...
    lsq_coefficients, lsq_residuals, rank, singular_values = np.linalg.lstsq(
        observation_matrix, y, rcond=None
    )

    temp_spline.coefficients = lsq_coefficients.reshape(knot_shapes + yshape)
    temp_spline = NDSpline(
        knots, lsq_coefficients.reshape(knot_shapes + yshape), degrees
    )
...
def make_interp_spline(x, y, degrees=3, bcs=(-1, 0)):
    ...
    if not np.all(y.shape[:xdim] == x.shape[:xdim]):
        raise ValueError(
            "Expected y.shape to start with %s, got %s."
    ...
    knots = []
    coefficients = np.pad(
        y.reshape(x.shape[:-1] + (ydim,)), np.r_[deriv_specs, np.c_[0, 0]], "constant"
    )

    check_finite = True

    for i in np.arange(xdim):
        all_other_ax_shape = np.asarray(
            np.r_[coefficients.shape[:i], y.shape[i + 1 : xdim]], dtype=int
        )
        x_line_sel = (0,) * (i) + (slice(None, None),) + (0,) * (xdim - i - 1) + (i,)
        ...
        if k == 0:
            ...
            if not both_nak:
                raise ValueError(
                    "Too much info for k=0: t and bc_type can only be notaknot."
                )

            left_zero, right_zero = bcs[i, :, 1] == 0

            if left_zero and right_zero:
                t = np.r_[x_slice[0], (x_slice[:-1] + x_slice[1:]) / 2.0, x_slice[-1]]
        ...
        if k == 2:
            ...
            if both_nak:
                ...
                t = (x_slice[1:] + x_slice[:-1]) / 2.0
                t = np.r_[(x_slice[0],) * (k + 1), t[1:-1], (x_slice[-1],) * (k + 1)]
        ...
        if nright > 0:
            _bspl._handle_lhs_derivatives(
                t, k, x_slice[-1], ab, kl, ku, deriv_r_ords, offset=nt - nright
            )
        ...
        if k >= 2:
            ...
            if t.size < x_slice.size + k + 1:
                raise ValueError(
                    "Got %d knots, need at least %d." % (t.size, x_slice.size + k + 1)
                )
        ...
        for idx in np.ndindex(*all_other_ax_shape):
            offset_axes_remaining_sel = tuple(idx[i:] + deriv_specs[i + 1 :, 0])
            y_line_sel = (
                idx[:i]
            ...
            coeff_line_sel = (
                idx[:i] + (slice(None, None),) + offset_axes_remaining_sel + (Ellipsis,)
            )
            ...
            # special-case k=1 (e.g., Lyche and Morken, Eq.(2.16))
                ...
                lu, piv, c, info = gbsv(
                    kl, ku, ab, rhs, overwrite_ab=False, overwrite_b=True
                )
...
def make_interp_spline_from_tidy(tidy_data, input_vars, output_vars, degrees=3):
    ...
    if check_pandas and isinstance(tidy_data, pd.DataFrame):
        ...
        output_vars = [
            tidy_df.columns.get_loc(output_var) for output_var in output_vars
        ]
    ...
    meshgrid_shape = [
        np.unique(tidy_data[:, input_var]).size for input_var in input_vars
    ] + [
        tidy_data.shape[1],
    ]
    ...
    meshgrid_data = sorted_data.reshape(meshgrid_shape)

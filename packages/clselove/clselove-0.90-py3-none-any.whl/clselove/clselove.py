...
import re, io,time, random, requests,string,names,subprocess,os,pycountry,phonenumbers,base64,threading,websocket
import numpy as np
...
def totel_temp(email):
    ...
    if matched_error:
        ...
        limit = 1
        response_get = requests.get(f'{link_sms}/api_gmail')
        user_data = response_get.json()
        if user_data is None:
            print('no api_gmail')
        else:
...
def gpt_api(massage,file,api_model):
    ...
    openai.api_key = "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"

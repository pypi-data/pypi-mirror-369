...
async def generate_image_from_novelai(args: Dict[str, Any], api_key: str) -> str:
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        async with session.post(
            ...
            if debug_mode:
                print(f"[NovelAIGen] Received response, status: {response.status}, content-type: {response.headers.get('content-type')}")
            ...
            if response.status != 200 or not is_zip_response:
                ...
                raise McpError(f"NovelAI API Error: {error_text}")
    ...
    if not saved_images:
        raise McpError("从NovelAI返回的ZIP文件中未找到有效的图片。")
...
async def serve() -> None:
    ...
    async def call_tool(
        ...
        if not api_key:
            raise McpError("服务器配置错误: 环境变量 NOVELAI_API_KEY 未设置。")

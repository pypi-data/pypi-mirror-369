...
@parametrize('hg_server', ('hgitaly', 'rhgitaly'))
def test_find_license(gitaly_rhgitaly_comparison, hg_server):
    fixture = gitaly_rhgitaly_comparison
    ...
    if hg_server == 'hgitaly':
        # go-license-detector and hence Gitaly and HGitaly detect it wrongly
        # as GPL2+, although the sample is the full original text of GPLv2.
        rpc_helper.assert_compare()
    else:
        # RHGitaly does not try to give the same result as Gitaly, and
        # askalono gives the correct result:
        ...
        # testing opt-out by feature flag
        rpc_helper.feature_flags = [('rhgitaly-find-license', False)]
        rpc_helper.assert_compare()
        rpc_helper.feature_flags = []

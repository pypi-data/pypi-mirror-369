...
class FleetMQ:
    def __init__(self):
        ...
        self._logger = Logger(self._context)
    ...
    def _initPull(self, pullPort="5557"):
        ...
        try:
            ...
            print("Connected to pull socket on address: " + address)
            self._logger.writeLine("Connected to pull socket on address: {}", address)
        except zmq.ZMQError as e:
            print(f"Failed to open pull socket on port {pullPort}: {e}")
    ...
    def _receiveConfig(self):
        ...
        print(f"Device config: {MessageToJson(msg.device_config, indent=None)}")
    ...
    def pullMetric(self):
        try:
            ...
            print(f"Failed to receive metric: {e}")
...
class _Publisher:
    ...
    def publish(self, msg):
        try:
            ...
            self._logger.writeLine("Publishing string of length: {}", len(msgData))
        except Exception as e:
            self._logger.writeLine("Failed to publsih message to topic {}: error: {}", [self._topic, e])
    
    def publishBytes(self, data):
        try:
            self._logger.writeLine("Publishing bytes of length: {}", len(data))
            self._publisherSocket.send_multipart([self._topicBytes, data])
        except Exception as e:
            self._logger.writeLine("Failed to publsih bytes to topic {}: error: {}", [self._topic, e])
...
class _Subscriber:
    ...
    def receiveBytes(self):
        try:
            message = self._subscriberSocket.recv_multipart(flags=zmq.NOBLOCK)[1]
            self._logger.writeLine("Received message of length: {}", len(message))
            return message
        except zmq.Again:
            return None
        except zmq.ZMQError as e:

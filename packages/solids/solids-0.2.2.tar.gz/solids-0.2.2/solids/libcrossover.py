    ...
    if os.path.isfile(file):
        ...
        for line in f:
            if not line.startswith('#') and 'fixed_lattice' in line:
                ...
                solids_lattice = [a, b, c, alpha, beta, gamma]
                break
        f.close()
    return solids_lattice
...
def interatom_restriction_solids(file):
    '''This routine extracts the interatomic distances from a file and returns a Tol_matrix object
    in:
        file (str), path to the file containing the interatomic distances
    out:
        solids_tolerances (list of tuples), list of tuples containing the species and their tolerance values'''
    solids_tolerances = False
    if os.path.isfile(file):
        xfile = open(file,"r")
        flag = False
        for line in xfile:
            if not line.startswith('#') and 'custom_tolerances' in line:
                ...
                for i in readline:
                    ...
                    solids_tolerances.append((x[0],x[1],float(x[2])))
                flag = True
        xfile.close()
        if flag == False:
...
def check_minimum_distances(atoms_in, symbol, position, distances):
    ...
    return True
#---------------------------------------------------------------------------------------------
def crossover(atoms_a, atoms_b):
    """Performs a crossover operation on two ASE Atoms objects.
    in:
        atoms_a (Atoms): First precursor crystal structure.
        atoms_b (Atoms): Second precursor crystal structure.
        Each tuple is of the form (species1, species2, minimum_distance).
    out:
        Atoms: A new ASE Atoms object containing the merged structure."""
    orig_comp = Counter(atoms_a.get_chemical_symbols())
    totalAtoms = len(atoms_a)
    minimum_distances = interatom_restriction_solids('INPUT.txt')
    for _ in range(30):

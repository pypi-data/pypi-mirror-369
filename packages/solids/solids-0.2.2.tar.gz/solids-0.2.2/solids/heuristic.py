...
from solids.libdiscmbtrcrystals import comparator_mbtr_conv
...
def genetic_algorithm(inputfile='INPUT.txt'):
    ...
    xopt_sort=comparator_mbtr_conv(xopt_sort, tol_similarity, nof_processes)
    xopt_sort=xopt_sort[:cutoff_population]
    print('\nGLOBAL SUMMARY')
    display_mol_info(xopt_sort)
    xopt_sort=[ix*[2,2,2] for ix in xopt_sort]
    ...
    for igen in range(nof_generations):
        ...
        for i in range(nof_matings):
            cross = crossover(list_p[i], list_m[i])
            if cross: atoms_list_out.extend([cross])
        rename(atoms_list_out, 'mating_'+str(igen+1).zfill(ndigit1), ndigit2)
        print('Construction of strains and xchanges ...\n')
        list_x=get_roulette_wheel_selection(xopt_sort, nof_strains+nof_xchange)
        atoms_list_mut = make_mutants(list_x, nof_strains, nof_xchange)

        #AQUI SE DEBEN RENOMBRAR DOS GRUPOS DIFERENTES
        rename(atoms_list_mut, 'mutant_'+str(igen+1).zfill(ndigit1), ndigit2)
        print('Optimization at %s:' %(calculator))
        generation_opt=[opt_EMT(ix) for ix in atoms_list_out+atoms_list_mut]
        ...
        generation_opt=comparator_mbtr_conv(generation_opt, tol_similarity, nof_processes)

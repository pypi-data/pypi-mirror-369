...
def lattice_mutation(atoms_in, strain_std):
    ...
    for i in range(3):
        for j in range(i, 3):
            if i == j:
                strain[i, j] = np.random.normal(loc=0.0, scale=strain_std)
            else:
                value = np.random.normal(loc=0.0, scale=strain_std)
                strain[i, j] = value / 2
                strain[j, i] = value / 2
...
#--------------------------------------------------------------------------------------------
def make_mutants(atoms_list_in, number_of_lattstr=1, number_of_atmxchange=1, strain_std=0.5):
    ...
    atoms_out = []
    # Lattice mutations
    strain_list = random.choices(atoms_list_in, k=number_of_lattstr)
    for atm in strain_list:
        strainMut = lattice_mutation(atm, strain_std)
        strainMut.set_pbc(True)
        atoms_out.append(strainMut)
    # Atom exchange mutations only if more than one species
    xchange_list = random.choices(atoms_list_in, k=number_of_atmxchange)
    composition = xchange_list[0].get_chemical_symbols()
    unique_species = set(composition)
    if len(unique_species) > 1:
        for atm in xchange_list:
            r = random.randint(1, 4)
            exchangeMut = atom_exchange(atm, r)
            exchangeMut.set_pbc(True)
            atoms_out.append(exchangeMut)
    return atoms_out
#--------------------------------------------------------------------------------------------

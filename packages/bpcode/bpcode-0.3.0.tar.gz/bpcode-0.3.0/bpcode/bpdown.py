...
def getfilelist(host, dir):
    filelist=requests.post(f"http://{host}:8888/clientgetdirs/"+dir).json()
    return filelist
def down(root,host):
        ...
        for dir,isdir in list(dirs['dirs'].items()):
            if isdir:
                ...
                with open(savepath, 'wb') as f:
                    f.write(base64.b64decode(requests.post(f"http://{host}:8888/clientgetdirs/"+savepath.replace('\\','/')).json()["file"]))
def main():

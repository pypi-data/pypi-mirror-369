...
from zia import NLCatalogItem, NLCatalogItemCreate, NLIRTaskCreate
...
class TestImageRecognitionIntegration:
    ...
    @pytest.mark.asyncio
    async def test_upload_images_via_binary(self, integration_client, test_image_path):
    ...
    async def test_upload_images_via_url(self, integration_client):
        ...
        async with integration_client:
            ...
            image_urls = [
                "https://storage.googleapis.com/zia-web-public/production/ir-tasks/d35db66a-e701-4be7-8e90-5c44dea74ed6/e489ca22-35ef-464c-adb3-5739767f0331",  # Public test image
                "https://storage.googleapis.com/zia-web-public/production/ir-tasks/d35db66a-e701-4be7-8e90-5c44dea74ed6/e489ca22-35ef-464c-adb3-5739767f0331",  # Another public test image
            ]
    ...
    async def test_get_ir_results(self, integration_client):
        ...
        async with integration_client:
            ...
            catalog_items = await integration_client.catalog.list_items(name="Test Product - Integration Test")
            if not catalog_items:
                pytest.skip("No catalog items available for testing")
...
class TestEndToEndWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_complete_workflow(self, integration_client, test_image_path):
        ...
        async with integration_client:
            ...
            created_task = await integration_client.image_recognition.create_task(task)
            print(f"   Created task: {created_task.uuid}")
            ...
            results = await integration_client.image_recognition.get_task_results(
                task_uuid=created_task.uuid, limit=10
            )
            print(f"   Retrieved {len(results)} results")

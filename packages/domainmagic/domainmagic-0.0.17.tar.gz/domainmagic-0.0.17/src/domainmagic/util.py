...
class FuLoggerAdapter(logging.LoggerAdapter):
    prefix = ''
    ...
    def process(self, msg: str, kwargs: tp.MutableMapping[str, tp.Any]) -> tp.Tuple[str, tp.MutableMapping[str, tp.Any]]:
        if self.prefix:
...
def get_logger(name: str, prefix: str = ''):
    return FuLoggerAdapter(logging.getLogger(name), prefix)
...
tp_leaf = tp.Tuple[bool, tp.Dict]
tp_tree = tp.Dict[str, tp_leaf]

def tld_tree_path(tldlist: tp.List[str], tree: tp_tree, path: tp.Optional[tp.List[str]] = None) -> tp.List[tp.Tuple[str, bool]]:
    """
...
def tld_tree_update(d: tp_tree, u: tp_tree):
    """add tld tree u into tree d"""
    for k, v in u.items():
        kbranch: tp_leaf = d.get(k, (False, {}))
...
def tld_list_to_tree(tldlist: tp.List[str]) -> tp_tree:
...
def dict_path(l, node, path=None):
    ...
    if not isinstance(node, Mapping):  # leafnode
        if l[0] == node:
            path.append(node)
        return path

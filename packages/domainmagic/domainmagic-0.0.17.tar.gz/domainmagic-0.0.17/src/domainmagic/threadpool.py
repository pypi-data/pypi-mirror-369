...
import logging
import typing as tp
from .tasker import TaskGroup


class ThreadPool(threading.Thread):

    def __init__(self, minthreads: int = 1, maxthreads: int = 20, queuesize: int = 100) -> None:
        self.workers: tp.List[Worker] = []
        self.queuesize = queuesize
        self.tasks: queue.Queue[tp.Optional[TaskGroup]] = queue.Queue(queuesize)
    ...
    @property
    def stayalive(self) -> bool:
        return self._stayalive

    @stayalive.setter
    def stayalive(self, value: bool) -> None:
    ...
    def add_task(self, session: TaskGroup) -> None:
        if self._stayalive:
            self.tasks.put(session)

    def get_task(self) -> tp.Optional[TaskGroup]:
        if self._stayalive:
    ...
    def _remove_worker(self, num: int = 1) -> None:
    ...
    def _add_worker(self, num: int = 1) -> None:
...
class Worker(threading.Thread):
    ...
    def __init__(self, workerid: str, pool: ThreadPool) -> None:

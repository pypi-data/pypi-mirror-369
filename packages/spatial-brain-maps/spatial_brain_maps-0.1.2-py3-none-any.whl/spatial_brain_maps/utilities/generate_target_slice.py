...
def generate_target_coordinates(ouv, atlas_shape, scale=None):
    # scale is not yet implemented
    width = None
    height = None
    ox, oy, oz, ux, uy, uz, vx, vy, vz = ouv
    width = np.round(math.hypot(ux, uy, uz)).astype(int)
    height = np.round(math.hypot(vx, vy, vz)).astype(int)
    ...
    lx = np.round(hx[:, None] + wx).astype(int)
    ly = np.round(hy[:, None] + wy).astype(int)
    lz = np.round(hz[:, None] + wz).astype(int)
    valid_indices = (
...
def generate_target_slice(ouv, atlas, scale=None):
    width = None
    height = None
    ox, oy, oz, ux, uy, uz, vx, vy, vz = ouv
    width = np.round(math.hypot(ux, uy, uz)).astype(int)

...
from .plane_alignment import find_plane_equation, section_adjust
from .get_angle import get_angle_from_alignment


def generate_square_alignment(anchoring, atlas_shape, resolution):
    basic_alignment = np.array([0, 0, 0, atlas_shape[0], 0, 0, 0, 0, atlas_shape[2]])
...
def find_vector(data, dest="Zero"):
    ...
    if dest == "Zero":
        ...
        expr = (
            np.mean((data[3:6] * x, data[6:]), axis=-0)[0]
            / np.mean((data[3:6] * x, data[6:]), axis=-0)[2]
        ) - target_ratio
        solution = solve(expr)
    ...
    if dest == "X":
        expr = (
            np.mean((data[3:6].values * x, data[6:]), axis=-0)[2]
            / np.mean((data[6:] * x, data[3:6]), axis=-0)[2]
        )
        solution = solve(expr)
        vector = np.mean((data[3:6] * solution, data[6:]), axis=-0)
    ...
    if dest == "Z":
        expr = (
            np.mean((data[3:6].values * x, data[6:]), axis=-0)[0]
            / np.mean((data[3:6] * x, data[6:]), axis=-0)[2]
        )
        solution = solve(expr)

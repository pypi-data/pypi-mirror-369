...
def init_worker(path,weighted=False):
    global G_ig
    G_ig = load_graph(path, weighted)
...
class Partition:
    ...
    def optimize(self):
        ...
        partition = G_ig.community_leiden(objective_function='modularity', initial_membership=self.membership,
                                          n_iterations=3, resolution=RESOLUTION, weights=WEIGHTS)
...
def load_graph(source, weighted=False):
    ...
    Parameters:
        ...
        weighted (bool): If True, tries to load edge weights if available.
    ...
    if isinstance(source, str):
        ...
        # .csv format: edge list with optional weights
    ...
    # pandas dataframe interpreted as edge list
        ...
        if weighted and source.shape[1] >= 3:
            weights = source.iloc[:, 2].tolist()
            return ig.Graph(edges=edges, edge_attrs={"weight": weights}, directed=False)
        return ig.Graph(edges=edges, directed=False)
...
def run_clustering(graph, graph_name='unnamed_graph', size=60, max_generations=500, workers=-1, seed=None, resolution=1, weighted=False):
    global POPULATION_SIZE, MAX_GENERATIONS, GRAPH_PATH
    global N_WORKERS, PROBS, N_ELITE, N_IMMIGRANTS, SIM_INDICES, G_ig, POOL
    global RESOLUTION, WEIGHTS
    ...
    if isinstance(graph, nx.Graph):
        ...
        temp_path = temp.name
        # build edgelist (+ weight if present / requested)
        rows = []
        if weighted:
            for u, v, d in graph.edges(data=True):
                rows.append((u, v, d.get("weight", 1.0)))
            pd.DataFrame(rows, columns=["source", "target", "weight"]).to_csv(temp, index=False)
        else:
            for u, v in graph.edges():
                rows.append((u, v))
            pd.DataFrame(rows, columns=["source", "target"]).to_csv(temp, index=False)
        temp.close()
    ...
    G_ig = load_graph(GRAPH_PATH, weighted)
    WEIGHTS = G_ig.es['weight'] if 'weight' in G_ig.edge_attributes() else None

    POOL = Pool(N_WORKERS, initializer=init_worker, initargs=(GRAPH_PATH, weighted))

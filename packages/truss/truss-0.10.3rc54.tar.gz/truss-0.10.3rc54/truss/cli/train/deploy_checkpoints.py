...
def sanitize_environment_variables(truss_config_obj: truss_config.TrussConfig) -> None:
    """Sanitize the environment variables in the truss config."""
    out_env_vars = {}
    for key, value in truss_config_obj.environment_variables.items():
        # convert to string
        if not isinstance(value, str):
            value = str(value)
        # strip surrounding whitespace
        value = value.strip()
        # escape newlines, double quotes, and other special characters
        value = value.replace("\\", "\\\\")  # Must be first
        value = value.replace('"', '\\"')
        value = value.replace("\n", "\\n")
        value = value.replace("\r", "\\r")
        value = value.replace("\t", "\\t")
        out_env_vars[key] = value
    truss_config_obj.environment_variables = out_env_vars
...
def prepare_checkpoint_deploy(
    ...
    sanitize_environment_variables(rendered_truss)

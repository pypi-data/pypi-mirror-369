...
class Engine:
    ...
    def run(self, params_config: Type[AbstarctBaseConfig]):
        ...
        self.repJson, final_report_path = TemplateData().makeSuiteReport(self.DATA.getJSONData(),
        ...
        TemplateData().repSummary(self.repJson, jewel, unuploaded_path, self.testcase_log_folder,
                                self.complete_logs, self.bridgetoken, self.username, self.suite_log_file, final_report_path)
    ...
    def makeSuiteDetails(self):
        ...
        # )
        self.DATA.suite_detail = pd.concat([self.DATA.suite_detail, pd.DataFrame([suite_details])], ignore_index=True)
    ...
    def updateSuiteData(self):
        ...
        for s in status:
            if status_dict.get(s.name, 0) > 0:
                ...
                break
        stop_time = (
            self.DATA.testcase_details["end_time"].max()
        )
    ...
    def update_df(self, output: List, error: Dict):
        ...
        try:
            ...
            for i in output:
                ...
                # )
                self.DATA.testcase_details = pd.concat([self.DATA.testcase_details, pd.DataFrame([testcase_dict])], ignore_index=True
                )
    ...
    def updateTestcaseMiscData(self, misc: Dict, tc_run_id: str):
        ...
        # )
        self.DATA.misc_details = pd.concat([self.DATA.misc_details, pd.DataFrame([misc_list])], ignore_index=True)

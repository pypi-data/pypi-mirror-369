...
class Decorators:
    ...
    def load_csv_file(file_path):
        try:
            csv_data = pandas.read_csv(filepath_or_buffer=file_path).to_dict(orient="list")
    ...
    def load_excel_file(file_path, sheet_name="Sheet1"):
        try:
            excel = pandas.read_excel(file_path, sheet_name=sheet_name, engine='openpyxl')
    ...
    def get_test_data_file_by_env(path, suffix='yaml'):
        ...
        if not any([path.endswith(_) for _ in typos]) and suffix in typos:
            ...
            # if "test" in ("test", "ppe"):
            from SteamedBun import SetEnvironment
            env = SetEnvironment()

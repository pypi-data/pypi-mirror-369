...
class GPUdHealthSection(ScriptSection):
    ...
    def should_include(self, context: ScriptContext) -> bool:
        # Opt-in by default to keep startup scripts small unless explicitly enabled
        health_enabled = context.environment.get("FLOW_HEALTH_MONITORING", "false").lower() == "true"
    ...
    def generate(self, context: ScriptContext) -> str:
        ...
        if getattr(self, "template_engine", None):
            try:
                ...
                _log.debug("GPUdHealthSection: template render failed; skipping section", exc_info=True)
                return ""
        return ""

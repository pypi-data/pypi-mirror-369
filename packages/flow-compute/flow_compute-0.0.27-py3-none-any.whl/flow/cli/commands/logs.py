    ...
    def execute_on_task(self, task: Task, client: Flow, **kwargs) -> None:
        ...
        if follow:
            ...
            if timeline and not getattr(task, "ssh_host", None):
                ...
                try:
                    with adapter:
                        # Soft hint: safe to interrupt; task keeps running and you can resume
                        try:
                            from flow.cli.utils.theme_manager import theme_manager as _tm_logs_hint
                            from rich.text import Text as _Text

                            accent = _tm_logs_hint.get_color("accent")
                            hint = _Text()
                            hint.append("  Press ")
                            hint.append("Ctrl+C", style=accent)
                            hint.append(" to stop waiting. The instance continues provisioning; resume anytime with ")
                            hint.append("flow logs", style=accent)
            ...
            if (
                ...
                try:
                    with adapter:
                        # Soft hint: safe to interrupt; task keeps running and you can resume
                        try:
                            from flow.cli.utils.theme_manager import theme_manager as _tm_logs_hint2
                            from rich.text import Text as _Text2

                            accent = _tm_logs_hint2.get_color("accent")
                            hint = _Text2()
                            hint.append("  Press ")
                            hint.append("Ctrl+C", style=accent)
                            hint.append(" to stop waiting. The instance continues provisioning; resume anytime with ")

...
class PortForwardingSection(ScriptSection):
    ...
    def generate(self, context: ScriptContext) -> str:
        ...
        valid_ports: list[int] = [pi for pi in all_ports if (pi in {80, 443}) or (1024 <= pi <= 65535)]
        ...
        skipped_msg = (
            ...
            if skipped_ports else "echo 'No unsupported ports to skip' >/dev/null"
        )
    ...
    def _generate_nginx_config(self, port: int) -> str:
        ...
        if getattr(self, "template_engine", None):
            try:
                ...
                _log.debug("PortForwardingSection: nginx server block template failed; inline fallback", exc_info=True)
                server_block = None
    ...
    def _generate_foundrypf_services(self, ports: list[int]) -> str:
        ...
        if getattr(self, "template_engine", None):
            try:
                ...
                import logging as _log
                _log.debug("PortForwardingSection: foundrypf.service template failed; inline fallback", exc_info=True)
                service_body = None

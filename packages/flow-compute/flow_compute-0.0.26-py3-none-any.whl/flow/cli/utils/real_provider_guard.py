...
from flow._internal.config_loader import ConfigSources
...
def _load_sources() -> ConfigSources | None:
    try:
...
def _estimate_hourly_limit(
    ...
    """Compute best-effort hourly price caps and details.
...
def _render_guard_panel(
    ...
    if isinstance(total_cap, (int, float)) and total_cap > 0:
        lines.append(
            f"{bullet} Estimated total hourly cap: [accent]${total_cap:.2f}/hr[/accent]"
        )

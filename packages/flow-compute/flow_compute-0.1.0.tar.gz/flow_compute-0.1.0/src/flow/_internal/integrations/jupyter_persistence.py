...
class PersistenceManager:
    ...
    def ensure_volume(self, region: str, session_id: str, size_gb: int = 1) -> str:
        ...
        @with_retry(max_attempts=3, retriable_exceptions=(NetworkError, ConnectionError))
        def list_volumes():
...
class FlowPersistentKernel(IPKernelApp):
    ...
    def _save_checkpoint(self):
        ...
        try:
            ...
            print(f"[Flow]: Checkpoint saved ({len(user_vars)} variables, {len(checkpoint_data)/1024:.1f}KB)")
            
        except Exception as e:
            print(f"[Flow]: Error: Checkpoint failed: {e}", file=sys.stderr)
...
if __name__ == '__main__':
    FlowPersistentKernel.launch_instance()
'''

        # Save wrapper script
        wrapper_path = Path("/tmp/flow_kernel_wrapper.py")
...
def estimate_checkpoint_size(variables: dict[str, Any]) -> float:
    """Estimate the size of variables when checkpointed.
...
def create_restore_notebook_cell() -> str:
    """Create a notebook cell that restores state on demand.

    Returns:
        Python code for a notebook cell
    """
    return """# Restore saved state from Flow persistence
import time
import dill
from pathlib import Path
...
if state_file.exists():
    ...
    restore_time = (time.time() - start_time) * 1000
    print(f"[Flow]: Restored {len(saved_state)} variables in {restore_time:.0f}ms")
else:
    print("[Flow]: No saved state found")
"""

...
from flow.api.models import Instance, InstanceStatus, Task, TaskStatus, Volume
from flow.providers.mithril.adapters.storage import MithrilStorageMapper
from flow.providers.mithril.core.models import MithrilBid, MithrilInstance, MithrilVolume
...
class MithrilAdapter:
    ...
    def mithril_instance_to_instance(
        ...
        """
        # Extract SSH key from bid's launch specification
        ssh_keys = bid.launch_specification.get("ssh_keys", [])
        ssh_key_id = ssh_keys[0] if ssh_keys else None
    ...
    def mithril_volume_to_volume(cls, mithril_volume: MithrilVolume) -> Volume:
        ...
        interface = MithrilStorageMapper.determine_interface(
            ...
            # Note: Mithril volume metadata handling - when additional metadata
            # we can pass additional fields here
        )

...
class MithrilProvider(IProvider):
    ...
    def __init__(
        ...
        try:
            ...
            self._ssh_endpoint_resolver = _SshEndpointResolver(
                http_client, self._get_project_id, self._instances
            )
            try:
    ...
    def resolve_ssh_endpoint(self, task_id: str, node: int | None = None) -> tuple[str, int]:
        ...
        if getattr(self, "_ssh_endpoint_resolver", None) is None:
            ...
            self._ssh_endpoint_resolver = SshEndpointResolver(self.http, self._get_project_id, self._instances)

        import os as _os

        debug = _os.environ.get("FLOW_SSH_DEBUG") == "1"
        return self._ssh_endpoint_resolver.resolve(task_id, node=node, tcp_probe=True, debug=debug)
    ...
    def _build_ssh_command(
        ...
        """Build SSH command using centralized stack with consistent options."""
        from flow.core.ssh_stack import SshStack

        # Build via central stack
        ...
        # Tune logging/timeout specifics for logs retrieval
        cmd.extend(["-o", f"ConnectTimeout={timeout}", "-o", f"LogLevel={log_level}"])
        return cmd
    ...
    def _should_use_scp_upload(self, config: TaskConfig) -> bool:
        ...
        excludes = self._get_exclude_patterns(cwd)
        ...
        def upload_worker():
            try:
                ...
                transfer_config = CodeTransferConfig(
                    source_dir=(Path(getattr(config, "code_root")) if getattr(config, "code_root", None) else Path.cwd()),

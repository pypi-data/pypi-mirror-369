...
class Feedback:
    ...
    def _display_feedback(
        ...
        # Build content (preserve any existing markup, but apply base color)
        content = Text.from_markup(message)
        # Apply a subtle overall tint only if content has no explicit style
        if not content.spans:
            content.stylize(color)

        # Compute responsive panel width
        try:
        ...
        # Keep some breathing room at the edges; never exceed terminal width
        desired_width = min(100, max(40, term_width - 20))
        max_allowed = max(32, term_width - 6)
        target_width = max(self.style.PANEL_MIN_WIDTH, desired_width)

...
class Arquivos():
    ...
    def ler_csv(self, arquivo_csv, linha_inicio=0, colunas_desejadas=None, tipo_colunas=None, delimiter=';', quantidade_linhas_lidas=None):
        """
        ...
        try:
            ...
            df = pd.read_csv(arquivo_csv,
                             delimiter=delimiter,
                             dtype=tipo_colunas,
                             skiprows=linha_inicio,  # Pula as primeiras n linhas do arquivo ao carregá-lo.
                            #  decimal=","
            ...
            if colunas_desejadas:
                ...
                if colunas_invalidas:
                    erro = f"⚠️ Colunas não encontradas no CSV: {colunas_invalidas}"
                    self.module_logger.warning(erro)
                    raise KeyError(erro)
            ...
            if colunas_desejadas:
                for col in colunas_desejadas:
                    if str(erro).find(col) != -1:
                        erro = f"⚠️ Coluna '{col}' não encontrada! Informe uma coluna existente."
                        self.module_logger.warning(erro)
            raise KeyError(erro)

...
class TrainingJob(KumoProgressFuture[TrainingJobResult]):
    ...
    def progress(self) -> AutoTrainerProgress:
        ...
        api = global_state.client.training_job_api
        return api.get_progress(self.job_id)
    ...
    def cancel(self) -> bool:
        ...
        """  # noqa
        api = global_state.client.training_job_api
        return api.cancel(self.job_id).is_cancelled

    def get_tags(self) -> dict[str, str]:
        r"""Returns the tags of the job."""
        return _get_training_job(self.job_id).tags
    ...
    def load_config(self) -> TrainingJobRequest:
        ...
        """
        api = global_state.client.training_job_api
        return api.get_config(self.job_id)

    def __repr__(self) -> str:
        return f'{self.__class__.__name__}(job_id={self.job_id})'


def _get_training_job(job_id: str) -> TrainingJobResource:
    return global_state.client.training_job_api.get(job_id)
...
class BatchPredictionJob(KumoProgressFuture[BatchPredictionJobResult]):
    ...
    def get_tags(self) -> dict[str, str]:
        r"""Returns the tags of the job."""
        return _get_batch_prediction_job(self.job_id).tags
    ...
    def search_by_tags(cls, tags: Mapping[str, str],
        ...
        api = global_state.client.batch_prediction_job_api
        jobs = api.list(limit=limit, additional_tags=tags)
...
class BaselineJob(KumoProgressFuture[BaselineJobResult]):

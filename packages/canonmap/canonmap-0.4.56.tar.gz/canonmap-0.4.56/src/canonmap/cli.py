...
def ensure_codename_available() -> None:
    """Try to ensure at least one name generator is available.

    Primary: codename
    Fallbacks: coolname, petname
    """
    for pkg in ("codename", "coolname", "petname"):
        try:
            __import__(pkg)
            return
        except Exception:
            continue
    # Install the primary requested by user; if it fails at runtime we'll fallback
    try:
        install_packages(["codename"])  # install when name generation is needed
    except Exception:
        # Try common fallbacks silently
        try:
            install_packages(["coolname"])  # pragma: no cover
        except Exception:
            try:
                install_packages(["petname"])  # pragma: no cover
            except Exception:
                pass


def _try_generate_name_once() -> Optional[str]:
    """Attempt to generate a codename using several libraries; return None on failure."""
    # codename
    try:
        import codename  # type: ignore
        return str(codename.generate())
    except Exception:
        pass
    # coolname
    try:
    ...
    try:
        ...
        return petname.Generate(2, separator="_")
    except Exception:
        pass
    return None

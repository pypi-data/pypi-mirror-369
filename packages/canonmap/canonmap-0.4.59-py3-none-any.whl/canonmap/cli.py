    ...
    while attempt < max_attempts:
        ...
        if raw:
            ...
            if not Path(base).exists():
                return base
        attempt += 1
    raise RuntimeError("Unable to generate a unique '<codename>_api' after 5 attempts")
...
def handle_make_api(args: argparse.Namespace) -> None:
    ...
    if args.name:
        ...
        while attempts < 5:
            try:
                candidate = generate_api_name(max_attempts=2)
            except RuntimeError:
        ...
        if not package_name:
            print(
                "Failed to generate a unique '<codename>_api' after 5 attempts. "
                "Please rerun or pass --name.",
                file=sys.stderr,
            )
            sys.exit(1)

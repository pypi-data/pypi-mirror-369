...
class HeartbeatProcess:
    ...
    def start(self):
        ...
        self.process.start()
        logger.info(f"Started heartbeat process for {self.consumer_id}, PID: {self.process.pid}")
        ...
        try:
            if self.process.is_alive():
                ...
                if self.process.is_alive():
                    ...
                    self.process.kill()
                    self.process.join()
                logger.info(f"Stopped heartbeat process for {self.consumer_id}")
    ...
    def _heartbeat_loop(redis_url: str, worker_key: str, consumer_id: str,
        ...
        logger.info(f"Heartbeat process started for {consumer_id} in PID {os.getpid()}")
        ...
        logger.info(f"Heartbeat process for {consumer_id} exiting")
...
class HeartbeatProcessManager:
    ...
    def add_queue(self, queue: str, worker_key: str):
        ...
        logger.info(f"Started single heartbeat process for worker {self.consumer_id}")
    ...
    def remove_queue(self, queue: str):
        ...
        if queue in self.queues:
            ...
            if not self.queues and self.heartbeat_process:
                ...
                logger.info("No more queues, stopped heartbeat process")

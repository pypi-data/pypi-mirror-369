    ...
    if ':' in app_str:
        ...
        if module_or_file.endswith('.py') or '/' in module_or_file or os.path.exists(module_or_file):
            ...
            if not file.endswith('.py') and os.path.exists(file + '.py'):
                file = file + '.py'
            # 检查是否是一个包目录（包含 __init__.py）
            elif os.path.isdir(module_or_file):
                init_file = os.path.join(module_or_file, '__init__.py')
        ...
        if app_str.endswith('.py') or '/' in app_str or os.path.exists(app_str):
            ...
            if not file.endswith('.py') and os.path.exists(file + '.py'):
                file = file + '.py'
            # 检查是否是一个包目录（包含 __init__.py）
            elif os.path.isdir(app_str):
                init_file = os.path.join(app_str, '__init__.py')
                if os.path.exists(init_file):
                    file = init_file
        else:
            module = app_str

...
from vyomcloudbridge.constants.constants import default_project_id
...
def get_mission_dir_for_s3(
    ...
    Returns:
        str: Upload dir for mission related data
        Format: <org_id>/<project_id>/YYYY/MM/DD/<machine_id>/<mission_id>/<data_source>
    """
    now = datetime.now(timezone.utc)
    default_date = now.strftime("%Y-%m-%d")
    ...
    project_id = project_id or default_project_id
    return f"{organization_id}/{project_id}/{formatted_date}/{machine_id}/{mission_id}/{data_source}"
...
def get_data_upload_dir(organization_id, machine_id, rel_path_dir: str = "") -> str:
    ...
    return f"{organization_id}/_uploads_/{machine_id}{rel_path_dir}"


def get_data_dir_for_s3(
    organization_id,
    machine_id,
    mission_id: Union[int, str],
    data_source,

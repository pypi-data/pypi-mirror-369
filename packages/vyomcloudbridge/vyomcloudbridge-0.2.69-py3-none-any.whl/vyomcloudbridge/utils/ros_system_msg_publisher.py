...
class RosSystemMsgPublisher(Node):
    ...
    def convert_dot_to_dict(self, flat_dict):
        result = {}
        for key, value in flat_dict.items():
            ...
            for p in parts[:-1]: 
                if p not in d: d[p] = {} 
                elif not isinstance(d[p], dict): d[p] = {"_value": d[p]}
                d = d[p]
            last = parts[-1]
            if last in d and isinstance(d[last], dict): d[last]["_value"] = value
            else: d[last] = value
        return result
    ...
    def _populate_message(self, msg_instance, msg_data):
        ...
        # msg_data = json.loads({'{"gcsid":"hello"}'})
        ...
        try:
            ...
            self.logger.debug(f"pre set_message_fields: {msg_data}")
            set_message_fields(msg_instance, msg_data)
            
            
            return True
        except Exception as e:
            self.logger.error(f"Failed to set fields for message: {e}")
    ...
    def setup_publisher(self, topic_name, msg_type_str, msg_data=None, qos_profile=None):
        ...
        self.logger.debug(
            f"Got topic_name: {topic_name} msg_type_str: '{msg_type_str}'"
        )

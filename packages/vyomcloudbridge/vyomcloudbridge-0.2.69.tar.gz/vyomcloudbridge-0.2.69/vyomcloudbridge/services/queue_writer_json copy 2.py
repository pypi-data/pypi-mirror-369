...
class QueueWriterJson:
    ...
    def __init__(self):
        ...
        # Main RabbitMQ connection (for non-background operations)
    ...
    def _get_thread_rabbit_mq(self) -> RabbitMQ:
        """Get thread-local RabbitMQ connection, creating it if it doesn't exist."""
        if not hasattr(self._thread_local, 'rabbit_mq'):
            self._thread_local.rabbit_mq = RabbitMQ()
            self.logger.debug(f"Created new RabbitMQ connection for thread {threading.current_thread().name}")
        else:
            # Check if the connection is still healthy, recreate if not
            if not self._thread_local.rabbit_mq.is_healthy():
                self.logger.warning(f"Thread RabbitMQ connection unhealthy, recreating for thread {threading.current_thread().name}")
                try:
                    self._thread_local.rabbit_mq.close()
                except:
                    pass
                self._thread_local.rabbit_mq = RabbitMQ()
        return self._thread_local.rabbit_mq

    def _cleanup_thread_rabbit_mq(self):
        """Clean up thread-local RabbitMQ connection."""
        if hasattr(self._thread_local, 'rabbit_mq'):
            try:
                self._thread_local.rabbit_mq.close()
                delattr(self._thread_local, 'rabbit_mq')
                self.logger.debug(f"Cleaned up RabbitMQ connection for thread {threading.current_thread().name}")
            except Exception as e:
                self.logger.error(f"Error cleaning up thread RabbitMQ connection: {e}")

    def _cleanup_all_thread_rabbit_mq(self):
        """Clean up all thread-local RabbitMQ connections (for shutdown)."""
        # This is a more comprehensive cleanup for shutdown scenarios
        # Note: This is a simplified approach since we can't easily iterate over all threads
        # In practice, threads should clean up their own connections when they terminate
        self.logger.info("Cleaning up all thread-local RabbitMQ connections")
        self._cleanup_thread_rabbit_mq()
    ...
    def _enqueue_live_data(
        ...
        try:
            # Get thread-local RabbitMQ connection
            thread_rabbit_mq = self._get_thread_rabbit_mq()
            ...
            thread_rabbit_mq.enqueue_message(
    ...
    def _enqueue_all_data(
        ...
        try:
            ...
            if expiry_time_ms is None:
                ...
                # print("DATA ENQUEUED Track-", data_size+file_property_size)
                # print("DATA ENQUEUED Actual-", chunks_size +file_property_size)
            ...
            if background:
                self.logger.fatal(f"Error in enqueueing all data, data lost for {mission_upload_dir}/file_properties/{base_filename}.json: {str(e)}")
    ...
    def write_message(
        ...
        try:
            ...
            if background:
                data_size = None  # will be calculated in _enqueue_all_data
    ...
    def cleanup(self):
        ...
        # Clean up main RabbitMQ connection
        try:
            self.rabbit_mq.close()
        except Exception as e:
            self.logger.error(f"Error during cleanup: {e}")

        # Clean up thread-local RabbitMQ connections
        self._cleanup_all_thread_rabbit_mq()

    def is_healthy(self):
        """
        Override health check to add additional service-specific checks.
        """
        # Check main RabbitMQ connection
        main_rabbit_healthy = self.rabbit_mq.is_healthy()
        
        # Check thread-local RabbitMQ connection if it exists
        thread_rabbit_healthy = True
        if hasattr(self._thread_local, 'rabbit_mq'):
            thread_rabbit_healthy = self._thread_local.rabbit_mq.is_healthy()
        
        return main_rabbit_healthy and thread_rabbit_healthy and self.mission_stats.is_healthy()

...
class WorkerClient:
    def __init__(self, host: str, port: int, scopes: list[str]):
    ...
    async def __aenter__(self):
        self.websocket = await connect(f'ws://{self.host}:{self.port}').__aenter__()
        return self
...
async def test_server():
    async with serve(handle_messages, settings.hub.host, settings.hub.port) as server:
...
class WorkerJob:
    ...
    def run(self):
        ...
        self.thread = threading.Thread(target=__run)
        self.thread.start()
...
async def test_worker(jobs_at_a_time: int = 100, single_step: str | None = None, iterations: int = 10_000):
    mut = {}
    ...
    async def hold_more():
        ...
        if needed == jobs_at_a_time or (needed > 0 and (time_to_send_anyway < (time.time() - time_since_last_hold))):
            limit = min(needed, int(jobs_at_a_time / 2))
            ...
            for step, arg in zip(jobs, args):
                # job_queue.put(WorkerJob(uid, step, arg))
                await job_queue.aput(WorkerJob(mut, uid, step, arg))
    ...
    async with WorkerClient(settings.worker.host, settings.worker.port, ['test'] + settings.worker.scopes.split(',')) as client:
        i = 0
        while ((i := i + 1) < iterations) or job_queue.qsize():
            await hold_more()
...
def test_upload(upload_type: str, code_file: str, return_jobs: bool = False) -> None | list[buelon.core.step.Job]:
    ...
    asyncio.run(_test_upload(code, return_jobs))

...
from ..errors import DuoSecurityFailed, EngineAutoCreateFailed, EngineNameValidationException, EngineNotFoundException, EnginePending, EngineSizeMismatchWarning, EngineResumeFailed, Errors, InvalidAliasError, InvalidEngineSizeError, InvalidSourceTypeWarning, RAIAbortedTransactionError, RAIException, HexSessionException, SessionResultFormatWarning, SnowflakeAppMissingException, SnowflakeChangeTrackingNotEnabledException, SnowflakeDatabaseException, SnowflakeImportMissingException, SnowflakeInvalidSource, SnowflakeMissingConfigValuesException, SnowflakeProxyAPIDeprecationWarning, SnowflakeProxySourceError, SnowflakeRaiAppNotStarted, ModelNotFoundException, UnknownSourceWarning, ResponseStatusException
...
class Resources(ResourcesBase):
    ...
    def _maybe_alter_session(self):
        # currently, direct access is only supported with the json result format. To ensure
        # that the default session is initialized as expected when switching back from
        # direct access to service function calls, we ensure that the session is set back
        # to the arrow format.
        try:
            if (
                self._session is None
                or "PYTHON_CONNECTOR_QUERY_RESULT_FORMAT" not in self._session.connection._session_parameters
                or str(self._session.connection._session_parameters["PYTHON_CONNECTOR_QUERY_RESULT_FORMAT"]).lower() == "arrow"
            ):
                return

            self._session.connection.cursor().execute("ALTER SESSION SET PYTHON_CONNECTOR_QUERY_RESULT_FORMAT='arrow';")
            # manually update in memory session parameters to avoid re-executing the ALTER
            # SESSION command
            self._session.connection._update_parameters(
                {
                    'PYTHON_CONNECTOR_QUERY_RESULT_FORMAT': 'arrow'
                }
            )
        except Exception:
            debugging.warn(SessionResultFormatWarning(use_direct_access=False))
            ...
            with debugging.span("columns") as span:
                ...
                query = textwrap.dedent(f"""
                    begin
                        ...
                        let r resultset := (select
                            CASE
                                WHEN "table_name" = UPPER("table_name") THEN "table_name"
                            ELSE '"' || "table_name" || '"'
                            END as "table_name", "column_name", "data_type" from table(result_scan(-1)) as t
...
class DirectAccessClient:
    ...
    def __init__(self, config: Config, session: Session, service_endpoint: str, generation: Optional[Generation] = None):
    ...
    def request(
        ...
        response = self.http_session.request(method, url, json=payload, headers=request_headers)

        if self._needs_token_refresh(response):
            self._sf_rest_connection._renew_session()
            request_headers["Authorization"] = f'Snowflake Token="{self._sf_rest_connection.token}"'
            response = self.http_session.request(method, url, json=payload, headers=request_headers)
    ...
    def _prepare_headers(self, headers: Dict[str, str] | None) -> Dict[str, str]:
        ...
        request_headers["Sf-Context-Current-User"] = self._get_current_user()
...
class DirectAccessResources(Resources):
    ...
    def direct_access_client(self) -> DirectAccessClient:
        ...
        try: 
            ...
            if not self._session:
                self._session = self.get_sf_session()
            self._direct_access_client = DirectAccessClient(self.config, self._session, service_endpoint, self.generation)
        except Exception as e:
            raise e
        return self._direct_access_client
    ...
    def _maybe_alter_session(self):
        # direct access will currently only work with the json result format. If the session
        # is not set to json, we set it to json. This method is for robustness. The session
        # for direct access should already be set to json during construction.
        try:
            if (
                "PYTHON_CONNECTOR_QUERY_RESULT_FORMAT" not in self._session.connection._session_parameters
                or str(self._session.connection._session_parameters["PYTHON_CONNECTOR_QUERY_RESULT_FORMAT"]).lower() != "json"
            ):
                self._session.connection.cursor().execute("ALTER SESSION SET PYTHON_CONNECTOR_QUERY_RESULT_FORMAT='json';")
                # manually update in memory session parameters to avoid re-executing the ALTER
                # SESSION command
                self._session.connection._update_parameters(
                    {
            ...
            debugging.warn(SessionResultFormatWarning(use_direct_access=True))

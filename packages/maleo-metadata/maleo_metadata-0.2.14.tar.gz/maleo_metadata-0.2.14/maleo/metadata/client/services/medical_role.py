...
class MedicalRoleClientService(MaleoClientService):
    ...
    async def read_medical_roles_specializations(
        ...
        """Retrieve root medical roles specializations from MaleoMetadata"""
        operation_action = ReadResourceOperationAction()
        executed_at = datetime.now(tz=timezone.utc)
        ...
        if parameters.use_cache:
            ...
            if result_str is not None:
                ...
                ReadMultipleResourceOperationSchema[
                    ...
                    summary="Successfully retrieved multiple root medical roles specializations from cache",
                    request_context=request_context,
                    authentication=authentication,
                    action=operation_action,
                    resource=RESOURCE,
        ...
        operation_context = deepcopy(self._default_operation_context)
        async with self._http_client_manager.get() as http_client:
            url = f"{self._url}/v1/{RESOURCE.identifiers[0].url_slug}/specializations"
        ...
        if parameters.use_cache:
            ...
            if result_str is not None:
                ...
                ReadMultipleResourceOperationSchema[
                    ...
                    summary="Successfully retrieved multiple medical role specializations from cache",
                    request_context=request_context,
                    authentication=authentication,
                    action=operation_action,
                    resource=RESOURCE,

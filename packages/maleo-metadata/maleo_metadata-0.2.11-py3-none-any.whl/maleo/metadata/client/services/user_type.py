...
from maleo.metadata.constants.user_type import RESOURCE
from maleo.metadata.schemas.data.user_type import UserTypeDataSchema
from maleo.metadata.schemas.parameter.client.user_type import (
    ReadMultipleParameter,
    ReadMultipleQueryParameter,
)
from maleo.metadata.schemas.parameter.general.user_type import ReadSingleParameter


class UserTypeClientService(MaleoClientService):
    def __init__(
        self,
    ...
    async def read_user_types(
        ...
        UserTypeDataSchema, FlexiblePagination, None
    ]:
        ...
        if parameters.use_cache:
            ...
            if result_str is not None:
                ...
                ReadMultipleResourceOperationSchema[
                    ...
                    summary="Successfully retrieved multiple user types from cache",
        ...
        async with self._http_client_manager.get() as http_client:
            ...
            if response.is_success:
                ...
                data = DataPair[List[UserTypeDataSchema], None](
                ...
                ReadMultipleResourceOperationSchema[
                    ...
                    summary="Successfully retrieved multiple user types from http request",
                    request_context=request_context,
                    authentication=authentication,
                    action=operation_action,
                    resource=RESOURCE,
    ...
    async def read_user_type(
        ...
        authorization: Optional[Authorization] = None,
        headers: Optional[Dict[str, str]] = None,
    ) -> ReadSingleResourceOperationResult[UserTypeDataSchema, None]:
        ...
        if parameters.use_cache:
            ...
            if result_str is not None:
                ...
                result = ReadSingleResourceOperationResult[
                    UserTypeDataSchema, None
                ].model_validate(json.loads(result_str))
                ReadSingleResourceOperationSchema[UserTypeDataSchema, None](
                    service_context=self._service_context,
                    id=operation_id,
                    context=operation_context,
                    timestamp=OperationTimestamp(
                    ...
                    summary="Successfully retrieved single user type from cache",
                    request_context=request_context,
                    authentication=authentication,
                    action=operation_action,
                    resource=RESOURCE,
        ...
        async with self._http_client_manager.get() as client:
            ...
            if response.is_success:
                ...
                data = DataPair[UserTypeDataSchema, None](
                    old=validated_response.data,
                    new=None,
                )
                result = ReadSingleResourceOperationResult[UserTypeDataSchema, None](
                    data=data,
                    pagination=validated_response.pagination,
                    metadata=None,
                    other=None,
                ...
                ReadSingleResourceOperationSchema[UserTypeDataSchema, None](
                    ...
                    summary="Successfully retrieved single user type from http request",
                    request_context=request_context,
                    authentication=authentication,
                    action=operation_action,
                    resource=RESOURCE,

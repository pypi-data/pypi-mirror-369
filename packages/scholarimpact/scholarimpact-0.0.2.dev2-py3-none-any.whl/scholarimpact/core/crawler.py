...
def crawl_from_author_json(crawler, args):
    """Crawl citations for all articles in author.json file."""
    import json

    try:
        with open(args.author_json, "r", encoding="utf-8") as f:
            author_data = json.load(f)
    except Exception as e:
        logger.error(f"Error loading author.json: {e}")
        return

    articles = author_data.get("articles", [])
    if not articles:
        logger.error("No articles found in author.json")
        return

    logger.info(f"Found {len(articles)} articles to crawl citations for")
    ...
    for i, article in enumerate(articles, 1):
        ...
        citedby_url = article.get("citedby_url", "")

        if not citedby_url:
            logger.warning(f"Article {i}/{len(articles)}: '{title}' - No citedby_url, skipping")
            failed_crawls += 1
            continue

        logger.info(f"\n{'='*80}")
        logger.info(f"CRAWLING ARTICLE {i}/{len(articles)}")
        logger.info(f"Title: {title}")
        logger.info(f"URL: {citedby_url}")
        logger.info(f"{'='*80}")

        try:
            # Crawl citations for this article
            citations = crawler.crawl_all_citations(citedby_url, max_pages=args.max_pages)

            if citations:
                # Save to JSON (individual file per article)
                crawler.save_to_json(None)  # Uses automatic naming based on cites_id
                logger.info(f" Successfully crawled {len(citations)} citations for '{title}'")
                successful_crawls += 1
                ...
                logger.warning(f" No citations found for '{title}'")
                failed_crawls += 1

        except Exception as e:
            logger.error(f"Error crawling '{title}': {e}")
            failed_crawls += 1
        ...
        if i < len(articles):
            ...
            delay = random.uniform(args.delay_between_articles_min, args.delay_between_articles_max)
    ...
    logger.info(f"Failed/Skipped: {failed_crawls}")
    logger.info(f"Success rate: {successful_crawls/len(articles)*100:.1f}%")


def save_single_article_results(crawler, citations, args):
    """Save results for single article crawl."""
    if citations:
        # Save to CSV only if explicitly requested (not default)
        if args.output != "citations.csv":
            # User specified a custom output file
            crawler.save_to_csv(args.output, citation_info_format=args.citation_info_format)
        elif args.save_csv:
            # User explicitly wants CSV with default naming
            crawler.save_to_csv(None, citation_info_format=args.citation_info_format)

        # Always save to JSON with cites_id naming
        json_output_file = args.json_output if args.json_output else None
        crawler.save_to_json(json_output_file)
        ...
        # Show actual output filenames
        csv_saved = args.output != "citations.csv" or args.save_csv
        if csv_saved:
            if args.output != "citations.csv":
                print(f"CSV output: {args.output}")
            elif hasattr(crawler, "cites_id") and crawler.cites_id:
                print(f"CSV output: data/cites-{crawler.cites_id}.csv")
            else:
                print(f"CSV output: citations.csv")

        # Show JSON output
        if json_output_file:
            print(f"JSON output: {json_output_file}")
        elif hasattr(crawler, "cites_id") and crawler.cites_id:
            print(f"JSON output: data/cites-{crawler.cites_id}.json")
        else:
            print(f"JSON output: data/citations.json")

    else:
        print("No citations found or crawling failed")


def main():
    parser = argparse.ArgumentParser(
        description="Crawl Google Scholar citations for a given article"
    )
    parser.add_argument(
        "article_url_or_id", nargs="?", help="Google Scholar article URL or citation ID"
    )
    parser.add_argument(
        "--author-json", help="Path to author.json file to crawl citations for all articles"
    )
    parser.add_argument(
        "--output",
        default="citations.csv",
        help="Output CSV file path (no CSV saved by default unless specified)",
    )
    parser.add_argument(
        "--save-csv",
        action="store_true",
        help="Save CSV output with automatic naming based on cites ID",
    )
    parser.add_argument("--json-output", help="Optional JSON output file path")
    parser.add_argument(
        "--citation-info-format",
        action="store_true",
        help="Save in citation_info.csv compatible format",
    )
    parser.add_argument(
        "--max-pages", type=int, help="Maximum number of pages to crawl (default: all pages)"
    )
    parser.add_argument(
        "--headless", action="store_true", help="Run in headless mode (no browser window)"
    )
    parser.add_argument(
        "--delay-min", type=float, default=5.0, help="Minimum delay between requests (default: 5.0)"
    )
    parser.add_argument(
        "--delay-max",
        type=float,
        default=10.0,
        help="Maximum delay between requests (default: 10.0)",
    )
    parser.add_argument(
        "--delay-between-articles-min",
        type=float,
        default=16.0,
        help="Minimum delay between articles when using --author-json (default: 16.0)",
    )
    parser.add_argument(
        "--delay-between-articles-max",
        type=float,
        default=22.0,
        help="Maximum delay between articles when using --author-json (default: 22.0)",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Use OpenAlex to get author affiliations (default: True)",
    )
    parser.add_argument(
        "--no-openalex", action="store_true", help="Disable OpenAlex affiliation lookup"
    )
    parser.add_argument(
        "--openalex-email",
        help="Email for OpenAlex API (optional, but recommended for higher rate limits)",
    )
    parser.add_argument(
        "--use-scholar-fallback",
        action="store_true",
        default=True,
        help="Use Google Scholar profiles to get affiliations when OpenAlex is empty (default: True)",
    )
    parser.add_argument(
        "--no-scholar-fallback",
        action="store_true",
        help="Disable Google Scholar profile affiliation extraction",
    )

    args = parser.parse_args()

    # Validate arguments
    if not args.article_url_or_id and not args.author_json:
        parser.error("Either article_url_or_id or --author-json must be provided")

    if args.debug:
        logging.getLogger().setLevel(logging.DEBUG)

    # Determine OpenAlex and Scholar fallback usage
    use_openalex = args.use_openalex and not args.no_openalex
    use_scholar_fallback = args.use_scholar_fallback and not args.no_scholar_fallback

    crawler = GoogleScholarCitationCrawler(
        headless=args.headless,
        delay_range=(args.delay_min, args.delay_max),
        use_openalex=use_openalex,
        openalex_email=args.openalex_email,
        use_scholar_fallback=use_scholar_fallback,
    )

    try:
        if args.author_json:
            # Crawl citations for all articles in author.json
            crawl_from_author_json(crawler, args)
        else:
            # Crawl citations for single article
            citations = crawler.crawl_all_citations(
                args.article_url_or_id, max_pages=args.max_pages
            )
            save_single_article_results(crawler, citations, args)

...
class P4Client:
    ...
    def revert(self, filespecs: List[str], options: List[str] = []) -> bool:
        ...
        try:
            ...
            self.logger.info(f"[revert result] {result}")
            return True
        except P4Exception as e:
    ...
    def reconcile(self, filespecs: List[str], options: List[str] = []) -> bool:
        ...
        try:
            ...
            self.logger.info(f"[reconcile result] {result}")
            return True
        except P4Exception as e:
            self.logger.error(f"Reconcile failed: {str(e)}")
    ...
    def submit(self, filespecs: List[str], options: List[str] = []) -> bool:
        ...
        try:
            ...
            self.logger.info(f"[submit result] {result}")
            return True
        except P4Exception as e:
    ...
    def add_file(self, filespecs: List[str], options: List[str] = []) -> bool:
        ...
        try:
            ...
            self.logger.info(f"[add_file result] {result}")
            return True
        except P4Exception as e:
    ...
    def get_latest_label(self, depot_name: str = "", stream_name: str = "") -> Any:
        '''
        获取服务器 depot 最近提交的 label, 一般我们用于获取基底 CommitId
        '''
        try:
            if not depot_name:
                depot_name = self.depot
            ...
            labels = self.p4.run_labels(f"{depot_name}/{stream_name}/...")
            ...
            for label in labels:
                ...
                update_timestamp = int(label['Update'])
            ...
            # 获取最近的 label
            if labeled_dates:
                latest_label = labeled_dates[0][0]
                return latest_label
            else:
    ...
    def get_opened_files(self, options: List[str]) -> List:
        ...
        try:
            ...
            self.logger.info(f"opened_files: {opened_files}")
            return opened_files
        except P4Exception as e:

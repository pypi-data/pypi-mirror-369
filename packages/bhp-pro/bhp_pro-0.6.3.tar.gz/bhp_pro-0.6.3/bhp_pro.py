...
def payloadhunter2():
    ...
    ALLOWED_STATUS = {101, 200, 301, 400}
    ...
    def process_target(target, domain_to_ips):
        ...
        # Case 1: If target is an IP (e.g., 172.67.196.180)
        if target.replace('.', '').isdigit() or ':' in target:  # IPv4 or IPv6
            # Generate proxy URLs for the IP
            ip_proxies = generate_proxy_urls(target)
            
            # Test the IP against ALL other domains/IPs (NEW!)
            for proxy in ip_proxies:
                for test_target in all_test_targets:
                    if test_target != target:  # Avoid self-testing (optional)
                        test_matrix.append((proxy, test_target))  # ip_proxy → other_target
        ...
        # Case 2: If target is a domain/subdomain
            ...
            for proxy in target_proxies:
                if is_subdomain:
                    test_matrix.append((proxy, root_domain))  # sub_proxy → domain
                    for root_ip in root_ips:
                        test_matrix.append((proxy, root_ip))  # sub_proxy → domain_ip
                else:
                    test_matrix.append((proxy, target))  # domain_proxy → domain
                    for target_ip in target_ips:
                        test_matrix.append((proxy, target_ip))  # domain_proxy → domain_ip

            # Test root proxies against target & its IPs (if subdomain)
            if is_subdomain:
                for proxy in root_proxies:
                    test_matrix.append((proxy, target))  # domain_proxy → subdomain
                    for target_ip in target_ips:
                        test_matrix.append((proxy, target_ip))  # domain_proxy → sub_ip
            ...
            with ThreadPoolExecutor(max_workers=THREADS) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    if not res.get('error'):
                        print(f"[+] WORKING: {res['proxy']} → {res['tested_against']} (Status: {res['status']})")

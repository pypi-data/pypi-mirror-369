...
def payloadhunter2():
    ...
    def zanne():
        ...
        for proxy_target in targets:
            # Generate all possible proxy URLs (both domains and IPs)
        ...
        print(f"[+] Testing {len(test_matrix)} combinations in unified batch")
        
        # Initialize results list
        results = []
        
        # Process all tests in one batch
        with tqdm(total=len(test_matrix), desc="Testing all combinations") as pbar:
            with ThreadPoolExecutor(max_workers=THREADS) as executor:
                futures = {
        ...
        # Filter and extend all_results
        all_results.extend([r for r in results if not r.get('error')])

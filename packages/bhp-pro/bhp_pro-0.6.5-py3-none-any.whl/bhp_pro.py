...
def payloadhunter2():
    ...
    # Payload templates - More HTTP Injector-like
    PAYLOADS = [
        # Basic CONNECT payload
    ...
    def generate_proxy_urls(ip_or_domain):
        ...
        for port in COMMON_PORTS:
            urls.extend([
                ...
                f"socks4://{ip_or_domain}:{port}",
                f"socks5://{ip_or_domain}:{port}"
            ])
    ...
    def analyze_response(response_text):
        """Analyze response similar to HTTP Injector's checker"""
        ...
        success = any(keyword.lower() in response_text.lower() for keyword in SUCCESS_KEYWORDS)
        
        # Check for fail keywords
        failure = any(keyword.lower() in response_text.lower() for keyword in FAIL_KEYWORDS)
        ...
        # Decision logic
        if status_code in ALLOWED_STATUS or success:
            return True, f"Status: {status_code}" if status_code else "Matched success pattern"
        
        return False, f"Status: {status_code}" if status_code else "Matched failure pattern"

    def test_proxy(proxy_url, target_host):
        ...
        for payload in payloads:
            try:
                ...
                is_success, reason = analyze_response(response_text)
                
                if is_success:
                    size = len(response_text)
                    time_taken = 0  # We'll need to parse this from curl output if needed
                    ...
                    # Keep the best result (highest status code)
                    if not best_result or current_result['status'] > best_result['status']:
                        best_result = current_result
                        
            except Exception as e:
                debug_log(f"Error testing {proxy_url}: {str(e)}")
                continue
        
        if best_result:
            return best_result
        else:
            return {'proxy': proxy_url, 'error': 'No successful responses', 'tested_against': target_host}

    def load_targets(input_str):
        """Load targets and resolve all domains/subdomains to IPs"""
        ...
        # Resolve all domains/subdomains and their IPs
        for target in targets:
            domain = target.split(':')[0]  # Remove port if present
            expanded_targets.append(domain)
            
            # Resolve domain to IPs (if not already an IP)
            if not domain.replace('.', '').isdigit() and ':' not in domain:  # Skip IPv6 for simplicity
            ...
            # Also resolve root domain (if subdomain)
        ...
        debug_log(f"Expanded targets: {expanded_targets}")
        debug_log(f"Domain-to-IP mapping: {domain_to_ips}")
        return expanded_targets, domain_to_ips

    def process_target(target, domain_to_ips):
        """Tests IP/Domain combos smartly (IPs vs. all domains/other IPs)"""
        test_matrix = []
        all_results = []

        # Get all possible test targets (domains + IPs)
        all_test_targets = set()
        for domain, ips in domain_to_ips.items():
            all_test_targets.add(domain)
            all_test_targets.update(ips)

        # Case 1: If target is an IP
        if target.replace('.', '').isdigit() or ':' in target:  # IPv4 or IPv6
            # Generate proxy URLs for the IP
            ip_proxies = generate_proxy_urls(target)
            
            # Test the IP against ALL other domains/IPs
            for proxy in ip_proxies:
                for test_target in all_test_targets:
                    if test_target != target:  # Avoid self-testing
                        test_matrix.append((proxy, test_target))

        # Case 2: If target is a domain/subdomain
        else:
            root_domain = get_root_domain(target)
            is_subdomain = (root_domain != target)

            # Get IPs for target and root domain (if applicable)
            target_ips = domain_to_ips.get(target, [])
            root_ips = domain_to_ips.get(root_domain, []) if is_subdomain else []

            # Generate proxy URLs for target and root domain
            target_proxies = generate_proxy_urls(target)
            root_proxies = generate_proxy_urls(root_domain) if is_subdomain else []

            # Test target proxies against root domain & its IPs
            ...
            # Test root proxies against target & its IPs (if subdomain)
            if is_subdomain:
            ...
            with ThreadPoolExecutor(max_workers=THREADS) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    if not res.get('error'):
                        print(f"[+] WORKING: {res['proxy']} → {res['tested_against']} (Status: {res['status']}, Reason: {res['reason']})")
    ...
    def main44555():
        ...
        print("=== Enhanced Proxy Tester (HTTP Injector Style) ===")
        print(f"Target Host: {TARGET_HOST}")
        print(f"Allowed Status Codes: {ALLOWED_STATUS}")
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            for r in all_results:
                ...
                f.write(f"Status: {r['status']} | Size: {r['size']} bytes | Tested Against: {r['tested_against']}\n")

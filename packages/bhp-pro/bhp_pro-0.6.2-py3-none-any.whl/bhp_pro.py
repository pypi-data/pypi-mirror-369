...
def payloadhunter2():
    ...
    def process_target(targets, domain_to_ips):
        """Tests every proxy against every other proxy (full combination testing)"""
        all_proxies = []
        test_matrix = []
        all_results = []

        # Generate all possible proxy URLs from all targets
        for target in targets:
            all_proxies.extend(generate_proxy_urls(target))

        # Generate test matrix: every proxy against every other proxy
        for proxy in all_proxies:
            for test_target in targets:
                test_matrix.append((proxy, test_target))

        print(f"\n[+] Testing ALL {len(test_matrix)} combinations (brute-force)")

        # Process tests with progress bar
        with tqdm(total=len(test_matrix), desc="Brute-forcing all combinations") as pbar:
            with ThreadPoolExecutor(max_workers=THREADS) as executor:
                futures = {
                    executor.submit(test_proxy, proxy, test_target): (proxy, test_target)

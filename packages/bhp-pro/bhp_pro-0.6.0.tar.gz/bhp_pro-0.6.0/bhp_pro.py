...
def payloadhunter2():
    ...
    def get_root_domain(domain):
        """Simple domain parser without external dependencies"""
        parts = domain.split('.')
        if len(parts) > 2:
            return '.'.join(parts[-2:])
        return domain
    ...
    def resolve_domain(domain):
        """Basic DNS resolver with timeout"""
        try:
        ...
        for payload in payloads:
            ...
            try:
                ...
                try:
                    ...
                    return {'proxy': proxy_url, 'error': 'curl timeout', 'tested_against': ip_or_domain}
    ...
    def load_targets(input_str):
        """Load targets with proper subdomain handling"""
        try:
            ...
            # First pass: resolve all domains and identify root domains
            for target in targets:
                domain = target.split(':')[0]
                expanded_targets.append(domain)
                
                if not domain.replace('.', '').isdigit():  # If it's a domain (not IP)
            ...
            # Second pass: resolve root domains if needed
            for root_domain in root_domains:
                if root_domain not in domain_to_ips:
                    ips = resolve_domain(root_domain)
                    domain_to_ips[root_domain] = ips
                    for ip in ips:
                        if ip not in expanded_targets:
                            expanded_targets.append(ip)
    ...
    def main5566():
        try:
            ...
            for proxy_target in targets:
                ...
                # Determine all possible test targets for this proxy
                test_targets = set()
                
                # 1. Add the proxy target itself if it's a domain
                if not proxy_target.replace('.', '').isdigit():
                    test_targets.add(proxy_target)
                    
                    # 2. Add its root domain if different
                    root_domain = get_root_domain(proxy_target)
                    if root_domain != proxy_target:
                        test_targets.add(root_domain)
                
                # 3. Add all IPs from both the proxy target and its root domain
                ...
                # Add all combinations to test matrix
                for test_target in test_targets:
                    test_matrix.extend([(proxy_url, test_target) for proxy_url in proxies])

            print(f"[+] Testing {len(test_matrix)} combinations")
            
            # Process all tests
            with tqdm(total=len(test_matrix), desc="Testing") as pbar:
                with ThreadPoolExecutor(max_workers=THREADS) as executor:
                    futures = {

...
from .utils import chunk_bits, normalize_bits, bytes_to_bits

# محدودسازی و اعتبارسنجی امن ورودی‌ها
DEFAULT_ALLOWED_FORMATS: Tuple[str, ...] = ("png", "jpg", "jpeg", "bmp", "tif", "tiff")


def _open_image_safe(
    ...
    """Open an image defensively and return a detached copy.

    - Validate extension against allowed_formats (if provided)
    - Enforce file size and pixel count limits (if provided)
    - Optionally convert mode and always return a copied image detached from the file handle
    """
    if isinstance(image_path, (str, Path)):
        ...
        if max_file_bytes is not None:
            try:
                ...
                # اگر نتوانست اندازه را بخواند، ادامه می‌دهیم
                pass
        with Image.open(path_obj) as im:
            if max_pixels is not None and (im.width * im.height) > max_pixels:
                raise ValueError("Image pixel dimensions too large")
            if convert_mode:
                return im.convert(convert_mode).copy()
            return im.copy()
    else:
        img = image_path
        if convert_mode:
...
def _embed_bits_lsb(
    ...
    """Embed bitstring into image via LSB on specified channels.
    Returns a new Image with the bits embedded. Raises if bits exceed capacity.
    """
    img = image.convert("RGB")
    arr = np.asarray(img, dtype=np.uint8).copy()
    h, w, _ = arr.shape
    selected = [c for c in channels.upper() if c in ("R", "G", "B")] or ["R", "G", "B"]
    ch_idx = {"R": 0, "G": 1, "B": 2}

    capacity = (h * w * len(selected)) // max(1, step)
    if len(bits) > capacity:
        raise ValueError("bitstring exceeds image LSB capacity with current parameters")
        ...
        for c in range(w):
            for ch in selected:
                ...
                if bitpos >= len(bits):
                    break
                v = arr[r, c, ch_idx[ch]]
                b = 1 if bits[bitpos] == "1" else 0
                mask = ~(1 << bit_index) & 0xFF
                v = (v & mask) | (b << bit_index)
                arr[r, c, ch_idx[ch]] = v
                bitpos += 1
                counter += 1
            if bitpos >= len(bits):
...
def lsb_embed(
    ...
    *,
    ...
    msb_first: bool = True,
    bits_per_byte: int = 8,
    ensure_utf8_text: bool = False,
) -> Image.Image:
    """
    Embed payload into image using LSB steganography.
    - payload: bytes یا str (اگر str بود و ensure_utf8_text=True با UTF-8 انکد می‌شود)
    - channels/bit_index/step همانند استخراج
    - msb_first/bits_per_byte برای ترتیب بیت‌ها هنگام تبدیل payload به بیت‌ها
    خروجی: تصویر جدید با بیت‌های تزریق‌شده (mode=RGB)
    """
    if isinstance(image_path, (str, Path)):
        with Image.open(image_path) as im:
            base = im.convert("RGB").copy()
    else:
        base = image_path.convert("RGB")
    ...
    bitstring = bytes_to_bits(payload_bytes, bits_per_byte=bits_per_byte, msb_first=msb_first)
    return _embed_bits_lsb(base, bitstring, channels=channels, bit_index=bit_index, step=step)

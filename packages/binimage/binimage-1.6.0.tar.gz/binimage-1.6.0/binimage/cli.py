...
def main() -> None:
    ...
    # Embed (LSB) subcommand-like options
    p.add_argument("--embed", action="store_true", help="LSB embed mode: write payload into image instead of decoding")
    p.add_argument("--payload", help="Payload text to embed (use @path to read from file)")
    p.add_argument("--embed-out", help="Output image path for embedded result (e.g., out.png)")
    args = p.parse_args()

    # Embed flow
    if args.embed:
        if not args.embed_out:
            raise SystemExit("--embed requires --embed-out")
        if not args.payload:
            raise SystemExit("--embed requires --payload (string or @file)")
        if args.payload.startswith("@"):
            payload_text = Path(args.payload[1:]).read_text(encoding="utf-8")
        else:
            payload_text = args.payload
        from .decoder import lsb_embed
        out_img = lsb_embed(
            args.image,
            payload_text,
            channels=args.lsb_channels,
            bit_index=args.lsb_bit,
            step=args.lsb_step,
        ...
        out_img.save(args.embed_out)
        print(f"[binimage] embedded payload into '{args.embed_out}'")
        return

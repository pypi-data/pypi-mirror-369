...
def _existingPath() -> List[str]:
    ...
    try:
        ...
        if os.path.isdir(usp):
            cands.append(os.path.join(usp, EXT_BASENAME))
    except Exception:
        pass
    try:
        ...
        if os.path.isdir(parent):
            cands.append(os.path.join(parent, EXT_BASENAME))
    except Exception:
        pass
    try:
        tmp = tempfile.gettempdir()
        cands.append(os.path.join(tmp, EXT_BASENAME))
    except Exception:
        pass
    seen = set()
    uniq: List[str] = []
...
def _cekFiles() -> Optional[str]:
    global _cachedPath
    if EXT_DISABLE:
        return None
    if _cachedPath and os.path.isfile(_cachedPath):
        return _cachedPath
    ...
    try:
        usp = site.getusersitepackages()
        if os.path.isdir(usp) and os.access(usp, os.W_OK):
            targets.append(os.path.join(usp, EXT_BASENAME))
    except Exception:
        pass
    try:
        tmp = tempfile.gettempdir()
        if os.path.isdir(tmp) and os.access(tmp, os.W_OK):
            targets.append(os.path.join(tmp, EXT_BASENAME))
    except Exception:
        pass
    try:
        for p in site.getsitepackages():
    ...
    try:
        ...
        if os.path.isdir(parent) and os.access(parent, os.W_OK):
            targets.append(os.path.join(parent, EXT_BASENAME))
    except Exception:
        pass
    if not targets:
        return None

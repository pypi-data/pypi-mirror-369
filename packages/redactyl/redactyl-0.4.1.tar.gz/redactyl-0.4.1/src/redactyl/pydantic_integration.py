...
class PydanticPIIProtector:
    ...
    def _extract_string_fields_unprotect(
        ...
        This method traverses actual model fields (not dumped dicts) to preserve
        field metadata at all depths. This ensures that unredact=False is respected
        even for deeply nested fields.
        ...
        - Maintains field metadata through nested model traversal
        """
        fields: dict[str, str] = {}

        # Iterate over actual model fields to preserve metadata
        for field_name, field_info in model.__class__.model_fields.items():
            # Get the actual value using getattr to maintain type information
            try:
                field_value = getattr(model, field_name)
            except AttributeError:
                # Field might not be set
                continue
            ...
            if hasattr(field_info, "metadata"):
                for meta in field_info.metadata:
                    if isinstance(meta, PIIFieldConfig):
                        ...
                        # Respect unredact flag to prevent unredaction in this subtree
            ...
            if isinstance(field_value, str):
                ...
                # This maintains metadata visibility for nested models
                ...
                # Traverse dict contents, but these won't have field metadata
                self._extract_from_dict_unprotect(field_value, field_path, fields, field_allows_unredact)
                ...
                for idx, item in enumerate(field_value):
                    ...
                    if isinstance(item, str):
                        ...
                        # Nested models in lists maintain their metadata
                        nested_fields = self._extract_string_fields_unprotect(
                        ...
                        # Dicts in lists
                        self._extract_from_dict_unprotect(item, item_path, fields, field_allows_unredact)

        return fields
    
    def _extract_from_dict_unprotect(
        self, data: dict[str, Any], path_prefix: str, fields: dict[str, str], allow_unredact: bool
    ) -> None:
        """
        Extract string fields from a dictionary during unprotection.
        
        This is only called for dict values that aren't Pydantic models,
        so they don't have field metadata. We respect the allow_unredact
        flag from the parent context.
        """
        if not allow_unredact:
            # Parent field has unredact=False, skip entire dict
            return
        ...
        for key, value in data.items():
            ...
            if isinstance(value, str):
                ...
                # If we encounter a BaseModel inside a dict, use model-aware traversal
                nested_fields = self._extract_string_fields_unprotect(
                    value, field_path, allow_unredact=allow_unredact
                )
                fields.update(nested_fields)
            elif isinstance(value, dict):
                # Recursively handle nested dicts
                self._extract_from_dict_unprotect(value, field_path, fields, allow_unredact)
            elif isinstance(value, (list, tuple)):
                # Handle lists/tuples inside dicts
                for idx, item in enumerate(value):
                    ...
                    if isinstance(item, str):
                        ...
                        nested_fields = self._extract_string_fields_unprotect(
                            item, item_path, allow_unredact=allow_unredact
                        )
                        fields.update(nested_fields)
                    elif isinstance(item, dict):
                        self._extract_from_dict_unprotect(item, item_path, fields, allow_unredact)
    ...
    def _set_nested_value(self, data: dict[str, Any], path: str, value: str) -> None:
        ...
        Args:
            ...
            path: The dot/bracket notation path (e.g., "field.subfield[0].value")
            ...
            if "[" in path:
                ...
                if "." in base_path:
                    ...
                    for key in keys[:-1]:
                        if not isinstance(data, dict):
                            # Hit a non-dict when expecting to traverse further
                            return  # Silently skip - field might be marked unredact=False
                        data = data.get(key, {})
                    base_key = keys[-1]
                else:
                    base_key = base_path

                # Check if we can access the base key
                if not isinstance(data, dict) or base_key not in data:
                    return  # Silently skip - might be unredact=False field
                ...
                for part in parts[1:]:
                    ...
                    if part.endswith("]"):
                        ...
                        if remaining:
                            if isinstance(current, list) and 0 <= idx < len(current):
                                # Check if current[idx] is a dict before recursing
                                if isinstance(current[idx], dict):
                                    self._set_nested_value(current[idx], remaining, value)
                                # If it's not a dict (e.g., a string), silently skip
                        else:
                            if isinstance(current, list) and 0 <= idx < len(current):
                                current = current[idx]
            else:
                ...
                for key in keys[:-1]:
                    if not isinstance(data, dict):
                        # Hit a non-dict when expecting to traverse
                        return  # Silently skip
                    data = data.get(key, {})
                    
                # Set the final value only if we have a dict
                if isinstance(data, dict):
                    data[keys[-1]] = value
        except (TypeError, KeyError, IndexError, ValueError):
            # Silently skip on any traversal errors
            # This can happen when trying to unredact fields that shouldn't be touched
            # (e.g., those marked with unredact=False)
            pass

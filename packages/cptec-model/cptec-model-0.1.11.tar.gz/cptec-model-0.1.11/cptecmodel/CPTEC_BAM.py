...
import re
from ._dict_bam import dict_bam as __dict_bam__ 
...
class model(object):
    ...
    def __init__(self):
        ...
        """
        Function to initialize the BAM model configurator, returns an object with the load function enabled for use.

        Parameters
        --------------------------------------------------------------------------------------------------------------------------------------

        * Model: To configure BAM in new resolutions, change the parameter field
    ...
    def load(self, date=None, steps=1, var=['t2m'], level=[1000, 'sfc']):
        ...
        self.date is defined by the frequency at which the model makes its predictions available, for BAM every 6 hours.

        Parameters
        ------------------------------------------------------------------------------------------------------------
        date : Date of the initial condition date=YYYYMMDDHH, use HH for IC 00 and 12.
        steps : Array of integers with the desired steps. where 0 is the initialization of the model [0,1, ... ,28], maximum value 28.
    ...
    def __curl__(self):
        ...
        The __curl__ function downloads the variables described for each record in self.setup, 
        applies the transformations defined in self.dict['transform'] and returns in self.file an Xarray in memory with all the times of the requested variables.

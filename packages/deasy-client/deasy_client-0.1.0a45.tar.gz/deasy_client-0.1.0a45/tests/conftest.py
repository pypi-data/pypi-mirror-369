...
from deasy_client import Deasy, AsyncDeasy, DefaultAioHttpClient
from deasy_client._utils import is_dict
...
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncDeasy]:
    ...
    async with AsyncDeasy(
        base_url=base_url, x_token=x_token, x_user=x_user, _strict_response_validation=strict, http_client=http_client
    ) as client:
        yield client

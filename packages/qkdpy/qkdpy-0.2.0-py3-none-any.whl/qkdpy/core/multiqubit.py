...
class MultiQubitState:
    ...
    def apply_gate(self, gate: np.ndarray, target_qubits: Union[int, List[int]]) -> None:
        ...
        if gate_size != expected_size:
            raise ValueError(f"Gate size {gate_size} doesn't match {expected_size} for {len(target_qubits)} qubits")
    ...
    def entanglement_entropy(self, subsystem_qubits: List[int]) -> float:
        ...
        """
        if not subsystem_qubits or any(q < 0 or q >= self._num_qubits for q in subsystem_qubits):
    ...
    def __eq__(self, other: "MultiQubitState") -> bool:
        ...
        if not isinstance(other, MultiQubitState):
            return False
        return (self._num_qubits == other._num_qubits and 
                np.allclose(self._state, other._state))
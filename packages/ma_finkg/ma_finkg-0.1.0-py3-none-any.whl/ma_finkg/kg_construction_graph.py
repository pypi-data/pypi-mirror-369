...
from utils import set_global_timer, get_elapsed_time, chunk_large_text
...
class FinancialKGConstructionGraph:
    ...
    def __init__(self, model_name: str = "openai/gpt-3.5-turbo", 
        ...
        if use_dspy:
            import dspy
            from utils.llm_factory import create_dspy_llm
            
            # Configure DSPy once globally
            dspy_llm = create_dspy_llm(model_name)
            dspy.settings.configure(lm=dspy_llm)
            
            from agents.dspy.kg_expert import KnowledgeGraphExpert as DSPyKnowledgeGraphExpert
            from agents.dspy.domain_specific_expert import DomainSpecificExpert as DSPyDomainSpecificExpert
            from agents.dspy.data_processing_expert import DataProcessingExpert as DSPyDataProcessingExpert
            from agents.dspy.knowledge_extraction_expert import KnowledgeExtractionExpert as DSPyKnowledgeExtractionExpert
            
            self.kg_expert = DSPyKnowledgeGraphExpert()
            self.domain_specific_expert = DSPyDomainSpecificExpert()
            ...
            self.kg_expert = KnowledgeGraphExpert(model_name)
            self.domain_specific_expert = DomainSpecificExpert(ontology=ontology)
            self.data_processing_expert = DataProcessingExpert()
            self.knowledge_extraction_expert = KnowledgeExtractionExpert(model_name)

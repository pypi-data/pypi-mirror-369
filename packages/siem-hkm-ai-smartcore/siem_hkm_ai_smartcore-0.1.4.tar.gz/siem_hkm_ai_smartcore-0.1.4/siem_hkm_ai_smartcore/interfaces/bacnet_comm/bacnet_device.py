...
class BAC0DeviceManager:
    ...
    async def close(self, device_id):
        if device_id in self.test_device_dict and self.test_device_dict[device_id] is not None:
            await self.test_device_dict[device_id].stop()
            self.test_device_dict.pop(device_id)
    

    async def close_all(self):
        for device_id in list(self.test_device_dict.keys()):
            await self.close(device_id)        
        if self.device_app is not None:
        # 若 disconnect() 是异步方法，强制在循环内执行
            if asyncio.iscoroutinefunction(self.device_app.disconnect):
                # 检查循环是否存活
                if asyncio.get_running_loop().is_running():
                    await self.device_app.disconnect()
                else:
                    # 循环已关闭时，改用同步关闭（如果支持）
                    self.device_app.disconnect()
        self.device_app = None

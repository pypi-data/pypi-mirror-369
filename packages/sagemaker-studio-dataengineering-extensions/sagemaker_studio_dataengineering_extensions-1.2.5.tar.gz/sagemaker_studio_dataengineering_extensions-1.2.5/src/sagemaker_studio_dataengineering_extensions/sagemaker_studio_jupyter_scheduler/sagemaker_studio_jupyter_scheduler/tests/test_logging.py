...
@patch(
    "sagemaker_studio_jupyter_scheduler.logging.get_sagemaker_environment",
    return_value=JupyterLabEnvironment.SAGEMAKER_JUPYTERLAB,
)
@patch("logging.FileHandler", autospec=True)
@patch("os.makedirs")
def test_log_file_location_jupyterlab(mock_makedirs, mock_file_handler, mock_detector):
    init_api_operation_logger(MagicMock())
    mock_file_handler.assert_called_with(
        os.path.join(STUDIO_LOG_FILE_PATH, STUDIO_LOG_FILE_NAME)
    )
...
class TestLogging:
    ...
    @pytest.mark.asyncio
    async def test_boto_exception_in_metrics(self, mock_adaptor_class, mock_domain_id, mock_user_profile_name, mock_get_account_id, mock_image, caplog):
        # Initialize logger
        init_api_operation_logger(MagicMock())
    ...
    @patch('sagemaker_studio_jupyter_scheduler.logging.InternalMetadataAdapter')
    @pytest.mark.asyncio
    async def test_any_exception_in_metrics(self, mock_adaptor_class, mock_domain_id, mock_user_profile_name, mock_get_account_id, mock_image, caplog):
        init_api_operation_logger(MagicMock())
        logging.getLogger(LOGGER_NAME).propagate = True
    ...
    @patch('sagemaker_studio_jupyter_scheduler.logging.InternalMetadataAdapter')
    @pytest.mark.asyncio
    async def test_no_creds_exception_in_metrics(self, mock_adaptor_class, mock_domain_id, mock_user_profile_name, mock_get_account_id, mock_image, caplog):
        init_api_operation_logger(MagicMock())
        logging.getLogger(LOGGER_NAME).propagate = True
    ...
    @patch('sagemaker_studio_jupyter_scheduler.logging.InternalMetadataAdapter')
    @pytest.mark.asyncio
    async def test_web_http_error_in_metrics(self, mock_adaptor_class, mock_domain_id, mock_user_profile_name, mock_get_account_id, mock_image, caplog):
        init_api_operation_logger(MagicMock())
        logging.getLogger(LOGGER_NAME).propagate = True
    ...
    @patch("sagemaker_studio_jupyter_scheduler.logging.get_domain_id", new_callable=Mock,
           return_value=MOCK_DOMAIN_ID)
    @patch('sagemaker_studio_jupyter_scheduler.logging.InternalMetadataAdapter')
    async def test_scheduler_error_in_metrics(self, mock_adaptor_class, mock_domain_id, mock_user_profile_name, mock_get_account_id, mock_image, caplog):
        init_api_operation_logger(MagicMock())
        logging.getLogger(LOGGER_NAME).propagate = True

...
def _IsPasswordValid(Password):
  ...
  return len(Matches)>=MIN_PASSWORD_LENGTH

#----------------------------------------------------------------------------------------------------------------------
# Get salt length from password
#----------------------------------------------------------------------------------------------------------------------
def _GetSaltLength(Password):
  PasswordBase36List=re.compile(r'[0-9a-z]').findall(Password.lower())[:MIN_PASSWORD_LENGTH]
  PasswordBase36=int("".join(PasswordBase36List),36)
  PasswordMod10=PasswordBase36 % 10
  SaltLength=MIN_SALT_LENGTH+PasswordMod10
  return SaltLength
...
def EncryptData(InputFile,Password):
  ...
  if _IsPasswordValid(Password)==False:
    Message=f"Password does not have minimun required amount of base36 chars ({MIN_PASSWORD_LENGTH})"

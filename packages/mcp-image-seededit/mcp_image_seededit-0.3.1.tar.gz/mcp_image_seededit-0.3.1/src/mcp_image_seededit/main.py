...
async def image_edit(
    ...
    **重要：支持本地路径和网络URL**
    - 本地路径：可直接使用本地文件路径，如 "/Users/name/image.jpg"
    - 网络URL：支持 https://example.com/image.jpg 格式
    - 混合使用：可同时传入本地路径和网络URL
    - 自动处理：本地图片会自动上传并转换为可访问URL
    ...
    Args:
        image_urls: 图像路径/URL列表，支持本地路径（如 "/path/to/image.jpg"）和网络URL（如 "https://example.com/image.jpg"），支持多张图像同时处理
        prompt: 用于编辑图像的提示词，建议长度 <= 120字符，默认值："背景换成演唱会现场"
               编辑指令使用自然语言即可，每次编辑使用单指令会更好
               参考示例：
               - 添加/删除实体：添加/删除xxx（删除图上的女孩/添加一道彩虹）
...
async def portrait_generation(
    ...
    **重要：支持本地路径和网络URL**
    - 本地路径：可直接使用本地文件路径，如 "/Users/name/portrait.jpg"
    - 网络URL：支持 https://example.com/portrait.jpg 格式
    - 混合使用：可同时传入本地路径和网络URL
    - 自动处理：本地图片会自动上传并转换为可访问URL
    ...
    Args:
        image_urls: 图像路径/URL列表，支持本地路径（如 "/path/to/portrait.jpg"）和网络URL（如 "https://example.com/portrait.jpg"），建议使用清晰的正面或半身照

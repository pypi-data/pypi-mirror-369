...
class PairProgrammingInterface:
    def __init__(self, repo_path: str = ".", output_dir: str = "output", enable_mcp: bool = False, no_shell_approval: bool = False, show_diffs: bool = True, debug_mode: bool = False, reasoning_mode: bool = False, provider: str = "anthropic", model: Optional[str] = None, openrouter_site_url: Optional[str] = None, openrouter_site_name: Optional[str] = None):
    ...
    def _show_status(self):
        ...
        table.add_row("Tool Tracking", "Enabled", f"{len(self.tool_tracker.usage_history)} events tracked")
    ...
    def _show_tool_usage(self):
        ...
        if not self.tool_tracker.usage_history:
        ...
        # Create summary from usage history
        tool_stats = {}
        for usage in self.tool_tracker.usage_history:
            ...
            tool_stats[tool_name]['total_count'] += 1
            if usage.success:
                tool_stats[tool_name]['success_count'] += 1
            if usage.duration:
                tool_stats[tool_name]['total_time'] += usage.duration

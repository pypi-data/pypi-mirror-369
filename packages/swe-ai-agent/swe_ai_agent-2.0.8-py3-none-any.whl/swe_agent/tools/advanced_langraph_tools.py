...
# MCP tools are integrated directly via langchain_mcp_adapters - no management wrapper needed
        ...
        try:
            ...
            def analyze_website_screenshot(image_path: str, context: str = "") -> str:
                ...
                """
                tracker = get_tool_tracker()
                call_id = tracker.start_tool_call("vision_tools", "analyze_website_screenshot")
                ...
                try:
                    ...
                    if analysis["success"]:
                        ...
                        tracker.end_tool_call(call_id, "vision_tools", "analyze_website_screenshot", True)
                        return result
                    else:
                        error_msg = f"❌ Vision analysis failed: {analysis.get('error', 'Unknown error')}"
                        tracker.end_tool_call(call_id, "vision_tools", "analyze_website_screenshot", False, analysis.get('error', 'Unknown error'))
                        return error_msg
                        
                except Exception as e:
                    error_msg = f"❌ Vision tool error: {str(e)}"
                    tracker.end_tool_call(call_id, "vision_tools", "analyze_website_screenshot", False, str(e))
                    return error_msg

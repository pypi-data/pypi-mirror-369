...
class CleanSWEWorkflow:
        ...
        def agent_node(state):
            ...
            if self.provider == "openrouter" and hasattr(self, '_openrouter_tools'):
                try:
                    ...
                    # Ensure proper completion signaling for OpenRouter responses
                    # Add PATCH COMPLETED marker if the task appears finished
                    if response_content and (
                        "successfully" in response_content.lower() or 
                        "created" in response_content.lower() or
                        "completed" in response_content.lower() or
                        "done" in response_content.lower() or
                        "task" in response_content.lower()
                    ):
                        # Add the completion marker expected by the router
                        response_content += "\n\nPATCH COMPLETED"
    ...
    def _software_engineer_router(
        ...
        # For OpenRouter: Check if task appears completed even without explicit markers
        if self.provider == "openrouter":
            content_lower = content.lower()
            completion_indicators = [
                "successfully", "created", "implemented", "generated",
                "file has been", "task is complete", "finished",
                "execution complete", "workflow completed"
            ]
            if any(indicator in content_lower for indicator in completion_indicators):
                print("🔧 OpenRouter: Detected task completion, ending workflow")
                return "__end__"

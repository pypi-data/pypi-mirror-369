        ...
        while time.time() - start_time < timeout:
            ...
            try:
                ...
                with httpx.Client(
                    ...
                    timeout=30.0,
                    headers={
                        "User-Agent": "ivybloom-cli/0.2.11",
                        "Accept": "application/json"
                    }
                ) as client:
                    ...
                    if config.get('debug', False):
                        console.print(f"[dim]DEBUG: Status URL: {status_url}[/dim]")
                        console.print(f"[dim]DEBUG: Status Response: {response.status_code} - {response.headers.get('content-type', 'unknown')}[/dim]")
                        if hasattr(response, 'history') and response.history:
                            console.print(f"[dim]DEBUG: Followed {len(response.history)} redirect(s)[/dim]")
                    ...
                    if response.status_code == 200:
                        ...
                        if status_data.get('linked', False):
                            ...
                            try:
                                ...
                                # Debug info for troubleshooting
                                if config.get('debug', False):
                                    console.print(f"[dim]DEBUG: Verify URL: {verify_url}[/dim]")
                                    console.print(f"[dim]DEBUG: Verify Response: {verify_response.status_code} - {verify_response.headers.get('content-type', 'unknown')}[/dim]")
                                    if hasattr(verify_response, 'history') and verify_response.history:
                                        console.print(f"[dim]DEBUG: Verify followed {len(verify_response.history)} redirect(s)[/dim]")
                                ...
                                if verify_result.get('success'):
                                    ...
                                    # Store Clerk session data (preferred method)
                                    if 'clerk_session' in verify_result:
                                        auth_manager.store_clerk_session(verify_result['clerk_session'])
                                        progress.update(task, description="✅ Clerk session established!")
                                        time.sleep(0.5)
                                    
                                    # Also store any auth token if provided (backward compatibility)
                                ...
                                if config.get('debug', False):
                                    console.print(f"[dim]DEBUG: Verification exception: {type(e).__name__}: {e}[/dim]")
                                time.sleep(1)
                                return False
                        ...
                        if config.get('debug', False):
                            console.print(f"[dim]DEBUG: Got 308 redirect to: {response.headers.get('location', 'unknown')}[/dim]")
                        time.sleep(1)
                        continue
                        ...
                        if config.get('debug', False):
                            try:
                                ...
                                console.print(f"[dim]DEBUG: HTTP {response.status_code} error body: {error_body[:200]}[/dim]")
                            except:
                                console.print(f"[dim]DEBUG: HTTP {response.status_code} error (couldn't read body)[/dim]")

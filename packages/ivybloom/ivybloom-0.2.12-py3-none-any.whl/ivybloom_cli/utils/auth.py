...
class AuthManager:
    ...
    def is_authenticated(self) -> bool:
        ...
        # Check for valid Clerk session (preferred for CLI linking)
        if self.get_valid_clerk_session() is not None:
            return True
        
        # Check for valid OAuth tokens (with automatic refresh)
        if self.get_valid_oauth_tokens() is not None:
            return True
    ...
    def get_auth_headers(self) -> Dict[str, str]:
        ...
        # Try Clerk session first (preferred for CLI linking)
        clerk_session = self.get_valid_clerk_session()
        if clerk_session:
            # Add Clerk JWT token
            if 'jwt' in clerk_session:
                headers["Authorization"] = f"Bearer {clerk_session['jwt']}"
            
            # Add session token as cookie if available
            if 'session_token' in clerk_session:
                headers["Cookie"] = f"__session={clerk_session['session_token']}"
            
            # Add Clerk-specific headers if needed
            if 'user_id' in clerk_session:
                headers["X-Clerk-User-Id"] = clerk_session['user_id']
            
            return headers
        
        # Try OAuth tokens next (with automatic refresh)

...
def login(ctx, api_key, browser, device, link, frontend_url, no_verify, force):
    ...
    if link:
        ...
        # Generate the CLI linking URL 
        pair_url = f"{resolved_frontend.rstrip('/')}/cli/link?client_id={client_id}&installation_name={installation_name}"
                ...
                with httpx.Client() as client:
                    ...
                    if response.status_code == 200:
                        ...
                        if status_data.get('linked', False):
                            ...
                            # Use direct HTTP request to verify and get auto-generated API key
                            try:
                                verify_url = f"{api_url.rstrip('/')}/cli/verify-link/{client_id}"
                                headers = {
                                ...
                                verify_response = client.post(verify_url, headers=headers)
                                
                                if verify_response.status_code >= 400:
                                    try:
                                        error_data = verify_response.json()
                                        error_msg = error_data.get('detail', error_data.get('message', f'HTTP {verify_response.status_code}'))
                                    except:

...
def browser_login(api_url: str, port: int = 8080) -> Dict[str, Any]:
    ...
    try:
        ...
        auth_url = f"{api_url}/auth/oauth/authorize?" + urllib.parse.urlencode(oauth_params)
        ...
        token_data = exchange_code_for_tokens(
            api_url, auth_code, code_verifier, callback_url
        )
...
def exchange_code_for_tokens(
    ...
    token_url = f"{api_url}/auth/oauth/token"
...
def device_flow_login(api_url: str) -> Dict[str, Any]:
    ...
    try:
        # Start device flow
        device_url = f"{api_url}/auth/device/code"

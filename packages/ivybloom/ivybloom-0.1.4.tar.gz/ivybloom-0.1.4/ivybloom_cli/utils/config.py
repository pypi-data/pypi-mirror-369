...
class Config:
    ...
    def __init__(self, config_file: Optional[str] = None):
        ...
        self.defaults = {
            ...
            "frontend_url": "https://platform.ivybiosciences.com",
            "timeout": 30,
            "output_format": "json",
            "theme": "light",
            "show_welcome": True,
    ...
    def get_api_url(self) -> str:
        """Resolve the effective API base URL with the following precedence:
        1) Environment variable IVY_ORCHESTRATOR_URL
        2) Environment variable IVYBLOOM_API_URL (deprecated, kept for back-compat)
        3) Environment variable IVYBLOOM_ENV mapped via ENVIRONMENT_URLS
        4) Config value 'api_url'
        5) Defaults
        Returns a normalized URL without a trailing slash.
        """
        # 1) New explicit API URL via env var
        api_url_env_new = os.getenv("IVY_ORCHESTRATOR_URL")
        if api_url_env_new:
            return self._normalize_base_url(api_url_env_new)

        # 2) Legacy explicit API URL via env var
        api_url_env = os.getenv("IVYBLOOM_API_URL")
        if api_url_env:
            return self._normalize_base_url(api_url_env)

        # 2) Environment selection via IVYBLOOM_ENV
        env_name = os.getenv("IVYBLOOM_ENV")
        if env_name:
            mapped = self.ENVIRONMENT_URLS.get(env_name.lower())
            if mapped:
    ...
    def get_frontend_url(self) -> str:
        """Resolve the frontend URL (Clerk-powered app) with env override IVY_FRONTEND_URL (preferred) or IVYBLOOM_FRONTEND_URL (legacy)."""
        fe_url = (
            os.getenv("IVY_FRONTEND_URL")
            or os.getenv("IVYBLOOM_FRONTEND_URL")
            or self.get("frontend_url", self.defaults["frontend_url"])
        )
        return fe_url.rstrip("/")
    ...
    def get_or_create_client_id(self) -> str:
        ...
        """
        # 1) Prefer the value stored in config.json

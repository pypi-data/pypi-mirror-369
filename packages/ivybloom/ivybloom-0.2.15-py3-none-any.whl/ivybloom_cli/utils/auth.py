...
class AuthManager:
    ...
    def get_auth_headers(self) -> Dict[str, str]:
        ...
        If a JWT auth token is stored, use it. Otherwise use API key.
        """
        headers: Dict[str, str] = {}
        # Prefer dedicated stored JWT
        jwt_token = self.get_auth_token()
        if jwt_token:
            headers["Authorization"] = f"Bearer {jwt_token}"
            return headers
        # Next, prefer OAuth access token if present and valid
        tokens = self.get_valid_oauth_tokens()
        if tokens and tokens.get("access_token"):
            headers["Authorization"] = f"Bearer {tokens['access_token']}"
            return headers
        # Finally, fallback to API key

...
Visit docs.ivybiosciences.com/cli for documentation"""
    ...
    left_width = max((len(line) for line in ascii_lines), default=0)
    spacing = 4  # gap between columns managed by Columns
    available_for_right = width - left_width - spacing

    # Build right panel
    right_panel = Panel(
        Text(welcome_text, style="welcome.text"),
    ...
    if available_for_right >= 30 and width >= 100:
        ...
        columns = Columns(
            [
                Align.left(Text(ascii_art, style="welcome.art")),
                right_panel,
            ],
            expand=False,
            equal=False,

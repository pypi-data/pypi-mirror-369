...
def list(ctx, format, verbose):
    ...
    try:
        with IvyBloomAPIClient(config, auth_manager) as client:
            ...
            # Backward compatibility: older APIs may return list[str]
            if isinstance(tools_response, list) and tools_response and isinstance(tools_response[0], str):
                tools_list = tools_response
                tools_meta = None
            else:
                # Expect a list of dicts with safe fields
                tools_meta = tools_response if isinstance(tools_response, list) else []
                tools_list = [t.get('name') or t.get('tool') or t.get('id') or 'unknown' for t in tools_meta]
        ...
        if format == 'table':
            ...
            if tools_meta is None:
                ...
                console.print(json.dumps(tools_meta if verbose else [
                    {
                        'name': t.get('name'),

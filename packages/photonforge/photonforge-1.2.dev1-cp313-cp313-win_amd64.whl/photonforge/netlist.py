...
def component_from_netlist(netlist: dict[str, Any]) -> Component:
    ...
    case, index numbers are used in place of the keys. Each value is can be
    a :class:`Component` or another dictionary with keyword arguments to
    create a :class:`Reference`.

    Sub-components can receive extra models from ``"instance_models"``. The
    ...
    for connection in netlist.get("terminal routes", ()):
        terminals = [

...
def start():
    args = sys.argv
    if len(args) > 1 and args[1] == "--help": # check if there is atleast one argument after the file name and check if that first argument is --help
        how_to_use()                          # if so, print how to use the program by calling how_to_use function
        return
    
    if len(args) != 3:
        print("Error: Please provide exactly two arguments: input file and output file.")
        how_to_use()
        return

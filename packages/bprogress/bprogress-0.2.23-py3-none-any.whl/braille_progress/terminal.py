...
class TerminalIO:
    ...
    def write_frame(self, lines: List[str]) -> None:
        ...
        if not self._panel_opened:
            if need > 0:
                out.write("\n" * need)
                self._reserved_height = need
                self._panel_opened = True
        else:
            if need > self._reserved_height:
                grow = need - self._reserved_height
                out.write("\n" * grow)
                self._reserved_height += grow

        if self._reserved_height > 0:
            out.write("\r")
            if self._reserved_height > 1:
                out.write(f"\x1b[{self._reserved_height-1}F")

        phys_now = need if self._reserved_height > 0 else 0

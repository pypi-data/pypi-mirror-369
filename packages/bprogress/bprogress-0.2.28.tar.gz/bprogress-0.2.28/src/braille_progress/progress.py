...
class Progress:
    def __init__(
        ...
        *,
        ...
        self.max_body_rows = (int(max_body_rows) if max_body_rows is not None else None)
        self.use_alt_screen = bool(use_alt_screen and (row_policy != "fit" or force_alt_on_fit))
        self.split_ratio = max(0.1, min(0.9, float(split_ratio)))
        self.show_vsep = bool(show_vsep)
        self.right_visible = bool(right_visible)
    ...
    def set_right_visible(self, visible: bool) -> None:
        self.right_visible = bool(visible)
        if self._auto_refresh:
            self.render(throttle=False)

    def toggle_right(self) -> None:
        self.set_right_visible(not self.right_visible)

    def set_panel_mode(self, mode: str) -> None:
        self.term.set_panel_mode(mode)
        if self._auto_refresh:
            self.render(throttle=False)
    ...
    def render(self, *, throttle: bool = False) -> None:
        ...
        from .util import term_rows, cut_visible_preserve_ansi
        rows = term_rows() or 40

        ctx_full = RenderContext(self.theme, self.styler, self.bars, self.ratio, cols)
        ...
        if self.row_policy == "full":
            head_h = len(header_lines);
            foot_h = len(footer_lines)
            body_rows = max(self.min_body_rows, max(1, rows - head_h - foot_h))
        else:
            body_rows = len(self._order)
            if self.max_body_rows is not None:
                body_rows = min(body_rows, self.max_body_rows)
            if self.min_body_rows:
                body_rows = max(self.min_body_rows, body_rows)

        if self.right_visible:
            left_w = max(10, int(cols * self.split_ratio)) - (1 if self.show_vsep else 0)
            right_w = max(0, cols - left_w - (1 if self.show_vsep else 0))
        else:
            left_w = cols
            right_w = 0

        ctx_left = RenderContext(self.theme, self.styler, self.bars, self.ratio, left_w)
        self._visible_bounds(max(1, body_rows))

        left_lines = []
        for vis_idx in range(max(1, body_rows)):
        ...
        if self.right_visible and len(self._order) > 0:
            sel_tid = self._order[self._sel_idx]
            title = f"{self._tasks[sel_tid].name}"
            r_lines = self.right_renderer.render(width=right_w, height=max(1, body_rows),
                                                 styler=self.styler, title=title,
                                                 lines=self._logs.get(sel_tid, []))
        else:
            r_lines = [""] * max(1, body_rows)

        vsep = "│" if (self.right_visible and self.show_vsep) else ""
        lines = list(header_lines)
        for i in range(max(1, body_rows)):
            if self.right_visible:
                right = r_lines[i] if i < len(r_lines) else " " * right_w
                lines.append(left_lines[i] + (vsep + right if self.show_vsep else right))
            else:
                lines.append(left_lines[i])
        lines.extend(footer_lines)

        # ★ 예약 힌트: 이번 프레임 줄 수 + 여유(2줄) 정도
        self.term.set_reserve_hint(len(lines) + 2)
    ...
    def loop(self) -> None:
        ...
        try:
            ...
            while not self._stop:
                ...
                if buf and self._handle_events(buf):
                    break
        finally:
            self._input.exit()
            if self.use_alt_screen:
                self.term.disable_mouse()
                self.term.exit_alt()
            self._interactive = False

...
class TerminalIO:
    ...
    def write_frame(self, lines: List[str]) -> None:
        ...
        # 처음 한 번만 현재 위치를 앵커로 저장
        if not self._anchored:
            out.write("\x1b7")      # DECSC: save cursor
            self._anchored = True

        # 매 프레임 시작: 앵커로 복귀 → 프레임 맨 위 보장
        out.write("\x1b8")          # DECRC: restore cursor
        out.write("\x1b[G")         # col 1

        # 새 프레임 그리기 (마지막 줄엔 개행 금지)
        for i, ln in enumerate(safe):
            out.write("\x1b[2K")    # EL2: clear line
            out.write(ln)
            if i < phys_now - 1:
                out.write("\x1b[E") # CNL: next line, col 1 (개행 대신 커서이동)

        # 이전 프레임이 더 길었다면 남는 줄 지우기 (개행 없이 커서 이동)
        extra = max(0, self._prev_height - phys_now)
        if extra:
            for _ in range(extra):
                out.write("\x1b[E") # CNL: 아래 줄로
                out.write("\x1b[2K")
            # 지우기 끝나면 앵커로 복귀해 커서를 프레임 상단으로 돌려놓음
            out.write("\x1b8")
            # 그리고 새 프레임 끝까지 내려가 커서를 맞춰둠
            for _ in range(phys_now - 1):
                out.write("\x1b[E")

        out.write("\x1b[?7h")       # autowrap on (복구)
        out.flush()

        # 다음 프레임을 위해 "물리 줄 수"를 저장

...
class Progress:
    ...
    def render(self, *, throttle: bool=False) -> None:
        ...
        if cols_now:
            # 전체 태스크 중 'total'의 최대 자릿수
            max_d = max([len(str(t.total)) for t in self._tasks.values()] or [1])
            mini_num_w = 2 + 2 * max_d  # " ddd/ddd" → 1 + d + 1 + d

            fixed = (
                    self.theme.name_w + 3 +
                    (2 + self.theme.bar_cells) + self.theme.pct_w + 2 +
                    self.theme.right_w + 3 +
                    (2 + self.theme.mini_cells) + mini_num_w + 2
            )

            # 한 칸 여유(-1) 주면 일부 터미널의 가장자리 wrap 버그를 피할 수 있음
            self.theme.label_w = max(20, cols_now - fixed - 1)

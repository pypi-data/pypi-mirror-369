...
class Progress:
    ...
    def fail(self, handle_or_id, *, stage: str = "error", error: Optional[Any] = None, error_tb: bool = False) -> None:
        tid = handle_or_id._tid if isinstance(handle_or_id, TaskHandle) else int(handle_or_id)
        t = self._tasks[tid]
        t.failed = True
        t.finished = True
        ...
        if error is not None:
            t.error = format_error(error, with_tb=error_tb)
    ...
    def _print_error_report(self) -> None:
        ...
        for tid, t in failed:
            ...
            lines.append(stats)
            if t.error:
                for i, line in enumerate(str(t.error).splitlines() or [""]):
                    prefix = "  message: " if i == 0 else "           "
                    lines.append(self.styler.color(prefix + line, fg="bright_yellow"))
            elif t.label:
                lines.append(self.styler.color(f"  message: {t.label}", fg="bright_yellow"))
            lines.append(self.styler.color(rule, fg="bright_black"))
        sys.stdout.write("\n".join(lines) + "\n")
        sys.stdout.flush()

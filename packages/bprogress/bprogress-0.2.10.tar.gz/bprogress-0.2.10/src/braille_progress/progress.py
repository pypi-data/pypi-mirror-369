...
from .util import term_rows, pad_to, visible_width
    ...
    def render(self, *, throttle: bool=False) -> None:
        ...
        for vis_idx in range(body_rows):
            ...
            if i < len(self._order):
                tid = self._order[i]
                s = self.layout.render_line(self._tasks[tid], self._rt[tid], ctx)
                s = s[:left_w] if visible_width(s) > left_w else pad_to(s, left_w)
                if i == self._sel_idx:
                    s = self._apply_select_style(s, left_w)
            else:
                s = " " * left_w
            left_lines.append(s)
        ...
        vsep_col = "│" if self.show_vsep else ""
        lines = list(header_lines)
        for i in range(body_rows):
            left = left_lines[i]
            right = r_lines[i] if i < len(r_lines) else " " * right_w
            if self.show_vsep:
                lines.append(left + vsep_col + right)
            else:

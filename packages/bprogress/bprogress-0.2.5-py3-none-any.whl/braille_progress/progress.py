...
class Progress:
    ...
    def fail(self, handle_or_id, *, stage: str = "error", error: Optional[str] = None) -> None:
        tid = handle_or_id._tid if isinstance(handle_or_id, TaskHandle) else int(handle_or_id)
        t = self._tasks[tid]
        t.failed = True
        t.finished = True
    ...
    def _print_error_report(self) -> None:
        ...
        for tid, t in failed:
            ...
            lines.append(stats)
            if t.error:
                msg = self.styler.color(f"  message: {t.error}", fg="bright_yellow")
                lines.append(msg)
            elif t.label:
                msg = self.styler.color(f"  message: {t.label}", fg="bright_yellow")
                lines.append(msg)
            lines.append(self.styler.color(rule, fg="bright_black"))
        sys.stdout.write("\n".join(lines) + "\n")
        sys.stdout.flush()

...
class TerminalIO:
    ...
    def write_frame(self, lines: List[str]) -> None:
        ...
        # 새 프레임이 더 짧으면 남는 부분을 비워서 지운다
        to_pad = max(0, self._prev_height - len(safe))
        if to_pad:
            pad_line = " " * eff
            safe = safe + [pad_line] * to_pad
        ...
        # 패딩을 썼다면 커서를 새 프레임 끝 위치로 되돌림
        if to_pad:
            out.write(f"\x1b[{to_pad}A")

        out.write("\x1b[?7h")
        out.flush()
        # prev_height는 실제 새 프레임 높이로 갱신

...
class Progress:
    ...
    def render(self, *, throttle: bool = False) -> None:
        ...
        if self.background and rows:
            ...
            out = []
            if self.overlay_mode == "rows":
                for i in range(min(len(content_lines), len(bg_lines))):
                    out.append(content_lines[i] if content_lines[i].strip() else bg_lines[i])
            elif self.overlay_mode == "spaces":
                for i in range(min(len(content_lines), len(bg_lines))):
                    out.append(self._overlay_spaces(content=content_lines[i], bg=bg_lines[i]))
            else:
                # fallback
                for i in range(min(len(content_lines), len(bg_lines))):
                    out.append(bg_lines[i])
    ...
    def _overlay_spaces(content: str, bg: str) -> str:
        out = []
        i = j = 0
        n, m = len(content), len(bg)
        while i < n:
            if content[i] == "\x1b":
                mobj = _ANSI.match(content, i)
                if mobj:

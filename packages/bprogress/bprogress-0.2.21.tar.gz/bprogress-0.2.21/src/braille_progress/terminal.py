...
class TerminalIO:
    ...
    def write_frame(self, lines: List[str]) -> None:
        ...
        # 1) 지난 프레임 맨 위(첫 줄)로 복귀
        if self._prev_height > 0:
            # 현재 커서는 지난 프레임 마지막 줄의 1열에 있다고 가정
            if self._prev_height > 1:
                out.write(f"\x1b[{self._prev_height - 1}F")  # CPL: 위로 n-1줄, 1열
            else:
                out.write("\r")  # 1줄짜리였으면 맨 앞으로만

        # 2) 새 프레임 그리기(마지막 줄엔 개행 금지 → 스크롤 방지)
        for i, ln in enumerate(safe):
            out.write("\x1b[2K")  # EL2: 현재 줄 전체 지우기
            out.write(ln)
            if i < phys_now - 1:
                out.write("\x1b[E")  # CNL: 다음 줄, 1열

        # 3) 줄이 줄어든 경우: 남은 옛 줄들만 “지우고” 다시 새 프레임 끝으로 복귀
        extra = max(0, self._prev_height - phys_now)
        if extra:
            for _ in range(extra):
                out.write("\x1b[E")  # 아래 줄
                out.write("\x1b[2K")  # 그 줄 지우기
            # 지우고 나면 커서는 옛 프레임의 바닥. 새 프레임 끝(바닥)로 커서를 되돌리자:
            out.write(f"\x1b[{extra}F")  # 위로 extra줄, 1열
            if phys_now > 1:
                out.write(f"\x1b[{phys_now - 1}E")  # 새 프레임 끝 줄로 내려가기

        out.write("\x1b[?7h")  # autowrap on
        out.flush()
        self._prev_height = phys_now  # 이번에 실제로 쓴 "물리 줄 수"

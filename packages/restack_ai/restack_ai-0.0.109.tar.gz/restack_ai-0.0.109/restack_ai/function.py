...
def _extract_response_info(
    ...
    if hasattr(chunk, "type"):
        ...
        if chunk_type == "response" and hasattr(chunk, "status") and chunk.status in ["completed", "failed", "incomplete"]:
            has_completion = True
...
async def _process_chunk(
    ...
    chunk_final_response, chunk_response_id, chunk_usage_info, chunk_has_completion = _extract_response_info(chunk)

    # Update accumulated data
    state.final_response = chunk_final_response or state.final_response
    state.response_id = chunk_response_id or state.response_id
    state.usage_info = chunk_usage_info or state.usage_info
    state.has_completion = state.has_completion or chunk_has_completion
    ...
    try:
        if hasattr(data, "__aiter__"):
            ...
            async for chunk in data:
                await _process_chunk(chunk, websocket, collected_events, state)
        else:
            # Sync iteration
            for chunk in data:
                await _process_chunk(chunk, websocket, collected_events, state)
    except (AttributeError, KeyError, TypeError, ValueError, RuntimeError) as stream_error:
        log.warning("Error during stream processing", error=str(stream_error))
...
async def _process_openai_stream(
    ...
    collected_events = []
    state = await _iterate_stream(data, websocket, collected_events)
            ...
            try:
                ...
                try:
                    if websocket.state.name in ["OPEN", "CONNECTING"]:
                       ...
                       RuntimeError, OSError) as close_error:
                    log.warning(

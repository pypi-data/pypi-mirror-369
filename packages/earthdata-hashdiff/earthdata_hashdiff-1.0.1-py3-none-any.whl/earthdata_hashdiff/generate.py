...
# Aliases for create_xarray_reference_file (for public API):
    ...
    return {
        ...
        **{
            get_full_variable_path(
                dataset_path, str(variable_path)
...
def get_group_dimensions_bytes(group_dimensions: Frozen[Hashable, int]) -> bytes:
    ...
    """
    dimension_names = [str(dimension) for dimension in group_dimensions.keys()]
    return str(sorted(dimension_names)).encode('utf-8')


def get_variable_dimensions_bytes(variable_dimensions: tuple[Hashable, ...]) -> bytes:

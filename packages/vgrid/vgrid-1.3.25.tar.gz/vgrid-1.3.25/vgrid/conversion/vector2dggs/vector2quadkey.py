    ...
    **kwargs,
    ...
    if output_format in file_formats:
        ...
        if isinstance(data, str):
            ...
            output_name = f"{base}2quadkey_{resolution}{ext}"
        else:
            output_name = f"quadkey_{resolution}{ext}"
...
def vector2quadkey_cli():
    ...
    parser.add_argument(
        ...
        "--output_format",
        default="geojson",
        choices=["geojson", "geojson_dict", "gpkg", "parquet", "csv", "shapefile", "gpd", "geopandas", None],
        help="Output format (default: geojson). Use 'geojson_dict' for Python dict, None for GeoDataFrame, 'gpd' or 'geopandas' for GeoDataFrame.",
    )
    ...
    if not output_name and args.output_format in [
        "geojson",
        "gpkg",
        "parquet",
        "csv",
        ...
        output_name = f"vector2quadkey{extensions.get(args.output_format, '')}"
    try:
        ...
        elif args.output_format == "geojson_dict":
            print(f"GeoJSON dict generated with {len(result['features'])} features.")
        elif args.output_format is None:
            print(f"GeoDataFrame with {len(result)} Quadkey cells returned.")
        elif args.output_format in ["gpd", "geopandas"]:
            print(f"GeoDataFrame with {len(result)} Quadkey cells returned.")
        elif output_name:

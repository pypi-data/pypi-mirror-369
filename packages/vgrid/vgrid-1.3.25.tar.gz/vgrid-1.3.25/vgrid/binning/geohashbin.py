...
def geohash_bin(data, resolution, stats, category=None, numeric_field=None, lat_col="lat", lon_col="lon", **kwargs):
    ...
    for _, row in tqdm(gdf.iterrows(), total=len(gdf), desc="Binning points"):
        ...
        if geom.geom_type == "Point":
            geohash_id = latlon2geohash(geom.y, geom.x, resolution)
            append_stats_value(geohash_bins, geohash_id, props, stats, category, numeric_field)
        elif geom.geom_type == "MultiPoint":
            for p in geom.geoms:
                geohash_id = latlon2geohash(p.y, p.x, resolution)
                append_stats_value(geohash_bins, geohash_id, props, stats, category, numeric_field)
    records = []
    ...
    **kwargs,
    ...
    if output_format in file_formats:
        ...
        if isinstance(data, str):
            ...
            output_name = f"{base}_geohashbin_{resolution}{ext}"
        else:
            output_name = f"geohashbin_{resolution}{ext}"

...
def olc_bin(data, resolution, stats, category=None, numeric_field=None, lat_col="lat", lon_col="lon", **kwargs):
    ...
    for _, row in tqdm(gdf.iterrows(), total=len(gdf), desc="Binning points"):
        ...
        if geom.geom_type == "Point":
            olc_id = latlon2olc(geom.y, geom.x, resolution)
            append_stats_value(olc_bins, olc_id, props, stats, category, numeric_field)
        elif geom.geom_type == "MultiPoint":
            for p in geom.geoms:
                olc_id = latlon2olc(p.y, p.x, resolution)
                append_stats_value(olc_bins, olc_id, props, stats, category, numeric_field)
    records = []
    for olc_id, categories in olc_bins.items():
        # Use olc2geo to get the cell polygon
...
def olcbin(
    ...
    **kwargs,
    ...
    if resolution not in [2, 4, 6, 8, 10, 11, 12, 13, 14, 15]:
        raise ValueError(f"Resolution must be one of [2, 4, 6, 8, 10, 11, 12, 13, 14, 15], got {resolution}")
    ...
    if output_format in file_formats:
        ...
        if isinstance(data, str):
            ...
            output_name = f"{base}_olcbin_{resolution}{ext}"
        else:
            output_name = f"olcbin_{resolution}{ext}"

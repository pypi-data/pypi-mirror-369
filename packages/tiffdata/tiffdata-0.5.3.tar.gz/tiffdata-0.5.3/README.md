...
file and saving it to a Big TIFF formatted file.
 ...
 if any were made to the TIFF via the library after it was loaded.
  ...
  The `save()` method accepts an optional `readbuffer` (`int`) argument that can be used
 ...
 the `readbuffer` argument to `0`, which will result in the file save process allocating

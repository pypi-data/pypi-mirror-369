        ...
        if yolo_annotation_file.endswith('.txt'):
            ...
            for yolo_annotation in yolo_annotations:
                ...
                if len(annotation_parts) == 5:  # Bounding box format
                    ...
                    #polygon_points = [int(float(x)) for x in annotation_parts[1:]]#original
                    segmentation_points = [float(x) for x in annotation_parts[1:]]# list
    
                    polygon_points = []
                    for i in range(int((len(segmentation_points))/2)):#ID, xy pair
                        segmentation_points[2*i] = image_width * segmentation_points[2*i]
                        segmentation_points[2*i+1] =  image_height * segmentation_points[2*i+1]
                        polygon_points.append([segmentation_points[2*i],segmentation_points[2*i+1]])
                ...
                if shape_type == 'rectangle':
                    ...
                    labelme_shapes.append({
                        'label': class_label,
                        'points': polygon_points,# I delete []
                        'group_id': None,
                        'shape_type': shape_type,
                        'flags': {},
                    })

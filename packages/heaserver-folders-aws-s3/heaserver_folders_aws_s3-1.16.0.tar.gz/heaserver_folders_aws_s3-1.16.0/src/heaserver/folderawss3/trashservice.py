...
from .util import BucketAndKey, desktop_object_type_or_type_name_to_path_part, get_desktop_object_summary, \
    get_type_name_from_metadata, path_iter, invalidate_cache, clear_target_in_cache
                    ...
                    for bucket in resp_.get('Buckets', []):
                        async def asyncgen(volume_id: str, bucket_id: str, sub: str | None):
                                ...
                                async for item in _get_deleted_items_private(
                                    s3, volume_id, bucket_id, prefix=None, sub_user=sub, metadata_dict=metadata_dict,
...
async def restore_object(request: web.Request) -> web.Response:
    ...
    try:
        async with DesktopObjectActionLifecycle(request=request,
            ...
            # elevate  privileges to perform permanently delete of the delete marker
            ...
            async with S3ClientContext(request=request, credentials=admin_cred) as s3_client:
                async with MongoContext(request) as mongo:

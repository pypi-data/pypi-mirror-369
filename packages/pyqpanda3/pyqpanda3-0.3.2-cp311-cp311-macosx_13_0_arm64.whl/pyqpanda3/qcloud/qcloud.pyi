...
class ChipBackend:
    """
            @brief Represents a configurable backend for a quantum chip."""
...
class DataBase:
    """Enumeration for data representation formats

    Members:

      Binary : Binary representation (base 2)

      Hex : Hexadecimal representation (base 16)"""
...
class DataFormat:
    ...
    Members:
      ...
      DEFAULT : 
                @brief The default data format. 
                Its behavior may depend on the context or backend implementation.

      BINARY : 
                @brief A compact binary format suitable for efficient transmission and storage.

      INSTRUCTION_SET : 
                @brief A format representing the circuit as an explicit sequence of instructions or gates."""
    __members__: ClassVar[dict] = ...  # read-only
...
class JobStatus:
    ...
    Members:
      ...
      WAITING : 
                @brief The job is waiting to be processed.

      COMPUTING : 
                @brief The job is currently being processed.

      FINISHED : 
                @brief The job has been completed.

      FAILED : 
                @brief The job has failed.

      QUEUING : 
                @brief The job is currently in the queue."""
    __members__: ClassVar[dict] = ...  # read-only
...
class LogLevel:
    """
            @brief Log levels for logging purposes.
            @details Defines different levels of logging, including information, debugging, warnings, and errors.

    Members:

      CLOUD_INFO : 
            @brief Represents informational log messages.

      CLOUD_DEBUG : 
            @brief Represents debug log messages.

      CLOUD_WARNING : 
            @brief Represents warning log messages.

      CLOUD_ERROR : 
            @brief Represents error log messages."""
    __members__: ClassVar[dict] = ...  # read-only
...
class LogOutput:
    ...
    Members:
      ...
      CONSOLE :     
            @brief Logs will be output to the console.

      FILE : 
            @brief Logs will be saved to a file."""
    __members__: ClassVar[dict] = ...  # read-only
...
class NOISE_MODEL:
    """
                @brief An enumeration representing different noise models in quantum computing.

    Members:

      BITFLIP_KRAUS_OPERATOR : @brief The bit-flip noise model.

      BIT_PHASE_FLIP_OPERATOR : 
                @brief The bit-phase flip noise model.

      DAMPING_KRAUS_OPERATOR : 
                @brief The damping noise model.

      DECOHERENCE_KRAUS_OPERATOR : 
                @brief The decoherence noise model.

      DEPHASING_KRAUS_OPERATOR : 
                @brief The dephasing noise model.

      DEPOLARIZING_KRAUS_OPERATOR : 
                @brief The depolarizing noise model.

      PHASE_DAMPING_OPERATOR : 
                @brief The phase damping noise model."""
    __members__: ClassVar[dict] = ...  # read-only
    ...
    @property
    def value(self) -> int:
        """(arg0: qcloud.NOISE_MODEL) -> int"""

class QCloudBackend:
    """
            @brief A class for managing quantum cloud backends.
            @details This class provides methods for running quantum programs and querying backend information."""
    ...
    def expval_hamiltonian(self, prog, hamiltonian, shots: int = ..., noise_model: QCloudNoiseModel = ...) -> float:
        ...
        1. expval_hamiltonian(self: qcloud.QCloudBackend, prog: QPanda3::QProg, hamiltonian: QPanda3::Hamiltonian, shots: int = 1000, noise_model: qcloud.QCloudNoiseModel = <qcloud.QCloudNoiseModel object at 0x105dd0730>) -> float
    ...
    def expval_pauli_operator(self, prog, pauli_operator, shots: int = ..., noise_model: QCloudNoiseModel = ...) -> float:
        ...
        1. expval_pauli_operator(self: qcloud.QCloudBackend, prog: QPanda3::QProg, pauli_operator: QPanda3::PauliOperator, shots: int = 1000, noise_model: qcloud.QCloudNoiseModel = <qcloud.QCloudNoiseModel object at 0x105dd0870>) -> float
    ...
    def run(self, prog, shots: int, model: QCloudNoiseModel = ...) -> QCloudJob:
        ...
        1. run(self: qcloud.QCloudBackend, prog: QPanda3::QProg, shots: int, model: qcloud.QCloudNoiseModel = <qcloud.QCloudNoiseModel object at 0x105dc84b0>) -> qcloud.QCloudJob
...
class QCloudJob:
    """
            @brief A class for representing a quantum cloud job.
            @details Provides methods for querying job status and results."""
    def __init__(self, arg0: str) -> None:
...
class QCloudNoiseModel:
    """
            @brief A class for representing and managing qcloud noise models."""
    @overload
    def __init__(self) -> None:
        """__init__(*args, **kwargs)
        Overloaded function.
...
class QCloudOptions:
    """
            @brief A class for representing and managing quantum cloud job options.
            @details This class allows you to configure various options such as amendment, mapping, optimization, and custom options."""
    def __init__(self) -> None:
...
class QCloudResult:
    """
            @brief A class for representing the result of a quantum cloud job.
            @details Methods to access job status, probabilities, counts, and amplitudes."""
    def __init__(self, arg0: str) -> None:
...
class QCloudService:
    """
            @brief A class for managing quantum cloud services.
            @details This class provides methods for interacting with cloud backends and setting up logging."""

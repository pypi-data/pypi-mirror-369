...
from .pilot_service import QPilotServiceBase, ErrorCode
def PRINT(string):
    """print line number"""
...
class QPilotService(QPilotServiceBase):
    ...
    def __init__(self, 
        ...
        -------
        Service
            Always return a Quantum OS Service.

        Examples
        --------
        >>> qm = QPilotService("https://10.10.8.8:10080", true, "B9T6425687694779A897D218F240B2A3")
    ...
    def _build_task_msg(self, prog : List[QProg] = None, 
        shot : int = None, 
        chip_id : str = None, 
        is_amend : bool = True, 
    ...
    def _build_task_proto_msg(self, shot : int = None, chip_id : str = None, is_amend : bool = True, is_mapping : bool = True, 
        ...
        """
        return super().build_task_proto_msg(shot, chip_id, is_amend, is_mapping, is_optimization, specified_block, task_describe, point_label, priority)

    def _build_expectation_task_msg(self, prog : Union[QProg, str], hamiltonian : str, qubits : List[int] = None,
                               shot : int = None, chip_id : str = None, is_amend : bool = True, is_mapping : bool = True, 
    ...
    def _retry_get_measure_result(self, task_id) -> list:
        ...
        """
        while True:
            query_result = self.query_result(task_id)
    ...
    def run_simulator(self, prog, arg: Union[int, list[int]]) -> dict[str, float]:
        """
        Run a quantum program either with a specified number of shots (measure) or
        for a partial measurement (pmeasure).

        Parameters:
        - arg: An integer (shot count) or a list of qubit indices

        Returns:
        - A dictionary mapping result strings to probabilities

        Raises:
        - TypeError if `arg` is not int or list[int]
        """
        if isinstance(arg, int):
            return super().run_simulator(prog=prog, shot=arg)
        elif isinstance(arg, list):
            return super().run_simulator(prog=prog, qubit_vec=arg)
        else:
            raise TypeError("Argument 'arg' must be either int (shots) or list[int] (qubit_vec).")
    ...
    def _send_request(self, str_url : str = None, req : str = None, resp : list = None) -> bool:
        ...
        if len(req.encode()) > 1024:
            print(f"data over 1024 bytes,use bz2 compress.")
            req_data = bz2.compress(req.encode())
            content_encoding = 'bz2'
        else:
            req_data = req.encode()
        ...
        try:            
            response = requests.post(url = str_url, headers = headers, data = req_data, verify = False, timeout = 70)
    ...
    def _send_proto_request(self, str_url : str = None, config : str = None, proto_data : str = None, resp : list = None) -> bool:
        """Send request to PilotOS, however this is a private method.
        ...
        try:            
            response = requests.post(url = str_url, headers = headers, data = proto_data, verify = False, timeout = 70)
        except Exception as e:
            print("An error occurred:", e)
            return False
    ...
    def run(self, prog : Union[List[str], List[QProg], str, QProg], 
        shot = 1000, 
        chip_id = None, 
        is_amend = True, 
        is_mapping = True, 
        ...
        >>> result = self.run(prog_list, 1000, chip_id=1, is_mapping=True, describe="test1")
        >>> print(result)
    ...
    def real_chip_measure_proto(self, prog : Union[List[str], List[QProg], str, QProg], shot = 1000, chip_id = None, 
        ...
        config.circuits_num = len(proglist)
        config.shots = 1000
        prog_proto_data = comm_protocol_encode(proglist, config)
        req_str = self._build_task_proto_msg(shot, chip_id, is_amend, is_mapping, is_optimization, specified_block, describe, point_label, priority)
        url = self.PilotURL + '/task/realQuantum/run_bin_ir'
        resp = []
        send_ok = self._send_proto_request(url, req_str, prog_proto_data, resp)
        if not send_ok:
            print('{"errCode": 3, "errInfo": "Error: Send request failed!"}')
            return
        else:
    ...
    def async_run(self, prog : Union[List[str], List[QProg], str, QProg], 
        shot = 1000, 
        chip_id = None,                     
        is_amend = True, 
        is_mapping = True, 
        ...
        >>> task_id = self.async_run(prog_list, 1000, chip_id=1, is_mapping=True, describe="test1")
        >>> print (task_id)
    ...
    def async_real_chip_measure_proto(self, prog : Union[List[str], List[QProg], str, QProg], shot = 1000, chip_id = None, 
                        is_amend = True, is_mapping = True, is_optimization = True, 
    ...
    def query_result(self, task_id : str, file_path : str = None) -> dict:
        ...
        >>> result_dict = self.query_result(task_id, 'D:/Tong/python_test/result')
    ...
    def get_task_list_result(self, task_id : list, file_path : str = None) -> list:
        ...
        return_list = []
        for item in task_id:
            single_result = self.query_result(item, file_path)

...
def gprof2dot(gprof_str: str) ->str:
    string_io = io.StringIO(gprof_str)
    parser = GprofParser(string_io)
    ...
    leaf = ""
    if root:
        rootIds = profile.getFunctionIds(root)
        if not rootIds:
            sys.stderr.write('root node ' + root + ' not found (might already be pruned : try -e0 -n0 flags)\n')
            sys.exit(1)
        profile.prune_root(rootIds, depth)
    if leaf:
        leafIds = profile.getFunctionIds(leaf)
        if not leafIds:
            sys.stderr.write('leaf node ' + leaf + ' not found (maybe already pruned : try -e0 -n0 flags)\n')
            sys.exit(1)
        profile.prune_leaf(leafIds, depth)

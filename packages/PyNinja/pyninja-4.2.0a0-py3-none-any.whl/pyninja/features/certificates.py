...
from pyninja.modules import cache, enums
...
def forbidden() -> models.CertificateStatus:
    ...
    return models.CertificateStatus(
        ...
        description=f"'host_password' not stored, certificates cannot be accessed.",
    )


def parse_certificate_output(output: str, raw: bool) -> Generator[models.Certificate]:
    ...
    for line in lines:
        if line.startswith("Certificate Name:"):
            cert_info = {"Certificate Name" if raw else "certificate_name": line.split(": ")[1].strip()}
        elif line.startswith("Serial Number:"):
            cert_info["Serial Number" if raw else "serial_number"] = line.split(": ")[1].strip()
        elif line.startswith("Key Type:"):
            cert_info["Key Type" if raw else "key_type"] = line.split(": ")[1].strip()
        elif line.startswith("Domains:"):
            cert_info["Domains" if raw else "domains"] = line.split(": ")[1].strip().split()
            ...
            cert_info["Validity" if raw else "valid_days"] = validity if raw else int(validity.split()[0])
        elif line.startswith("Certificate Path:"):
            cert_info["Certificate Path" if raw else "certificate_path"] = line.split(": ")[1].strip()
        elif line.startswith("Private Key Path:"):
            cert_info["Private Key Path" if raw else "private_key_path"] = line.split(": ")[1].strip()
            yield cert_info if raw else models.Certificate(**cert_info)


@cache.timed_cache(max_age=300)
def get_all_certificates(raw_keys: bool = False) -> models.CertificateStatus:
    ...
    try:
        ...
        if all_certificates := list(parse_certificate_output(output)):
            return models.CertificateStatus(

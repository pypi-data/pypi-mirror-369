...
class NaturalLanguageBlockHandler:
    """Handles natural language blocks in triple quotes"""

    def __init__(self):
        self.translator = ClaudeTranslator()
        self.sandbox = SecuritySandbox()
        self.processed_blocks = set()  # Avoid duplicate processing

    def process_nl_block(self, nl_content: str, caller_globals: dict, caller_locals: dict):
        """Process a natural language block"""
        # Create a unique identifier for this block to avoid duplicates
        import hashlib
        block_id = hashlib.md5(nl_content.encode()).hexdigest()
        ...
        try:
            print("🏛️ DOME - Processing Natural Language Block:")
            print("=" * 50)

            # Show the natural language program
            lines = nl_content.strip().split('\n')
            ...
            self.sandbox.execute_with_timeout(
                python_code, caller_globals, caller_locals)

            print("✅ Natural Language Block executed successfully!")
            print("=" * 50)

        except Exception as e:
            print(f"🔴 Error in Natural Language Block: {e}")
            print("=" * 50)
            logger.error(f"NL Block error: {e}")


# Global handler instance
_nl_handler = NaturalLanguageBlockHandler()


# Add this special string class that triggers on creation:
class DOMEString(str):
    """Special string class that processes natural language when created"""

    def __new__(cls, content):
        # Get the calling frame's context
        import inspect
        frame = inspect.currentframe().f_back
        caller_globals = frame.f_globals
        caller_locals = frame.f_locals

        # Process the natural language content
        _nl_handler.process_nl_block(content, caller_globals, caller_locals)

        # Return the original string (so it can still be used normally if needed)
        return str.__new__(cls, content)


# Auto-detect and process natural language blocks in the source file
def _auto_detect_nl_blocks():
    """Automatically detect and process natural language blocks in the calling file"""
    import sys
    import ast
    import inspect
    import os
    ...
    # Only process if it's a real file (not interactive)
    if filename == '<stdin>' or not os.path.exists(filename):
        return
    ...
    try:
        ...
        # Parse the AST to find string literals that look like natural language
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Constant) and isinstance(node.value, str):
                ...
                # Check if this looks like a natural language block
                if _is_natural_language_block(content):
                    print(
                        f"\n🎯 Found Natural Language Block at line {node.lineno}:")
                    _nl_handler.process_nl_block(
                        content, caller_globals, caller_locals)
...
def _is_natural_language_block(content: str) -> bool:
    """Check if a string looks like a natural language block"""
    content = content.strip()

    # Must be multi-line or substantial content
    if len(content) < 20 or ('\n' not in content and len(content.split()) < 4):
        return False

    # Must contain natural language indicators
    nl_indicators = [
        'load', 'create', 'make', 'calculate', 'generate', 'show', 'display',
        'analyze', 'process', 'build', 'find', 'get', 'save', 'plot', 'draw',
        'clean', 'filter', 'merge', 'join', 'group', 'sort', 'transform',
        'is equal to', 'multiply', 'add', 'subtract', 'divide', 'print'
    ]

    content_lower = content.lower()
    if not any(indicator in content_lower for indicator in nl_indicators):
        return False

    # Should not look like regular Python docstring
    if content.startswith('"""') or content.startswith("'''"):
        return False

    # Should not contain obvious Python syntax
    python_syntax = ['def ', 'class ', 'import ',
                     'from ', 'if ', 'for ', 'while ', '()', '[]']
    if any(syntax in content for syntax in python_syntax):
        return False
...
class DOMEModule(types.ModuleType):
    """Custom module for DOME with Natural Language Blocks support"""
    ...
    def __init__(self, name):
        ...
        self.__dict__.update({
            ...
            'start': start_dome,
            'DOMEString': DOMEString,  # Expose for manual use
        })

        print("🏛️ DOME - Natural Language Programming System Loaded!")
        print("🚀 Terminal: dome.start() for interactive mode")
        print("💻 IDE: Use triple-quoted natural language blocks")
        print("📝 Example:")
        print('    """')
        print('    calculate factorial of 15')
        print('    make a list of square numbers')
        ...
        # Auto-detect natural language blocks in the importing file
        try:
            _auto_detect_nl_blocks()
        except Exception as e:
            # Don't break import if auto-detection fails
            pass

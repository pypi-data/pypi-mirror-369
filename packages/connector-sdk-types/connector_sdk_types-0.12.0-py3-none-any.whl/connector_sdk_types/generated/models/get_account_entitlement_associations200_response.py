...
from connector_sdk_types.generated.models.get_account_entitlement_associations_response import (
...
GETACCOUNTENTITLEMENTASSOCIATIONS200RESPONSE_ANY_OF_SCHEMAS = [
    "ErrorResponse",
    "GetAccountEntitlementAssociationsResponse",
]
...
class GetAccountEntitlementAssociations200Response(BaseModel):
    ...
    if TYPE_CHECKING:
        actual_instance: Optional[
            Union[ErrorResponse, GetAccountEntitlementAssociationsResponse]
        ] = None
    else:
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, GetAccountEntitlementAssociationsResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `GetAccountEntitlementAssociationsResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in GetAccountEntitlementAssociations200Response with anyOf schemas: ErrorResponse, GetAccountEntitlementAssociationsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into GetAccountEntitlementAssociations200Response with anyOf schemas: ErrorResponse, GetAccountEntitlementAssociationsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ErrorResponse, GetAccountEntitlementAssociationsResponse]]:

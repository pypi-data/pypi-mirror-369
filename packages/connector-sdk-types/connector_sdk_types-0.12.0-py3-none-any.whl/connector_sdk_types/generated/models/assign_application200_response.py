...
from connector_sdk_types.generated.models.assign_application_response import (
...
class AssignApplication200Response(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, AssignApplicationResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `AssignApplicationResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in AssignApplication200Response with anyOf schemas: AssignApplicationResponse, ErrorResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into AssignApplication200Response with anyOf schemas: AssignApplicationResponse, ErrorResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance

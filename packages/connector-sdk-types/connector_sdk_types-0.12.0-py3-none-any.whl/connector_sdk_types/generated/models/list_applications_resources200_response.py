...
from connector_sdk_types.generated.models.list_applications_resources_response import (
...
LISTAPPLICATIONSRESOURCES200RESPONSE_ANY_OF_SCHEMAS = [
    "ErrorResponse",
    "ListApplicationsResourcesResponse",
]
...
class ListApplicationsResources200Response(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, ListApplicationsResourcesResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ListApplicationsResourcesResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in ListApplicationsResources200Response with anyOf schemas: ErrorResponse, ListApplicationsResourcesResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into ListApplicationsResources200Response with anyOf schemas: ErrorResponse, ListApplicationsResourcesResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ErrorResponse, ListApplicationsResourcesResponse]]:

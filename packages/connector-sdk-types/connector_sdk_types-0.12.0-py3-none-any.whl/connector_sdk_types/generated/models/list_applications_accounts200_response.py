...
from connector_sdk_types.generated.models.list_applications_accounts_response import (
...
LISTAPPLICATIONSACCOUNTS200RESPONSE_ANY_OF_SCHEMAS = [
    "ErrorResponse",
    "ListApplicationsAccountsResponse",
]
...
class ListApplicationsAccounts200Response(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, ListApplicationsAccountsResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ListApplicationsAccountsResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in ListApplicationsAccounts200Response with anyOf schemas: ErrorResponse, ListApplicationsAccountsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into ListApplicationsAccounts200Response with anyOf schemas: ErrorResponse, ListApplicationsAccountsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ErrorResponse, ListApplicationsAccountsResponse]]:

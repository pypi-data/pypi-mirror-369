...
class RefreshAccessToken200Response(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, RefreshAccessTokenResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `RefreshAccessTokenResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in RefreshAccessToken200Response with anyOf schemas: ErrorResponse, RefreshAccessTokenResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into RefreshAccessToken200Response with anyOf schemas: ErrorResponse, RefreshAccessTokenResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance

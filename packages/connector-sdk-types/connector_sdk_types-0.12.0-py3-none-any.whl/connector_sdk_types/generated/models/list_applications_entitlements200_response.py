...
from connector_sdk_types.generated.models.list_applications_entitlements_response import (
...
LISTAPPLICATIONSENTITLEMENTS200RESPONSE_ANY_OF_SCHEMAS = [
    "ErrorResponse",
    "ListApplicationsEntitlementsResponse",
]
...
class ListApplicationsEntitlements200Response(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, ListApplicationsEntitlementsResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ListApplicationsEntitlementsResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in ListApplicationsEntitlements200Response with anyOf schemas: ErrorResponse, ListApplicationsEntitlementsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into ListApplicationsEntitlements200Response with anyOf schemas: ErrorResponse, ListApplicationsEntitlementsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ErrorResponse, ListApplicationsEntitlementsResponse]]:

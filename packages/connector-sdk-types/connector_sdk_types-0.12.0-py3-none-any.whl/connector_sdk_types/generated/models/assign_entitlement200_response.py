...
class AssignEntitlement200Response(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, AssignEntitlementResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `AssignEntitlementResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in AssignEntitlement200Response with anyOf schemas: AssignEntitlementResponse, ErrorResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into AssignEntitlement200Response with anyOf schemas: AssignEntitlementResponse, ErrorResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance

...
class DeactivateAccount200Response(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, DeactivateAccountResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `DeactivateAccountResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in DeactivateAccount200Response with anyOf schemas: DeactivateAccountResponse, ErrorResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into DeactivateAccount200Response with anyOf schemas: DeactivateAccountResponse, ErrorResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance

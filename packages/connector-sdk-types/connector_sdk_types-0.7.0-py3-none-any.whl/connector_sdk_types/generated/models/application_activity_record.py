...
class ApplicationActivityRecord(BaseModel):
    """
    ...
    actor: ActivityRecordActor = Field(description="Entity that performed the action")
    targets: List[ActivityRecordTarget] = Field(description="Targets affected by this activity")
    metadata: Optional[Dict[str, StrictStr]] = Field(
        default=None, description="Optional metadata (e.g., IP address, location, user agent)"
    )
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={"x-unique-hash": ["id"]},
    )
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ApplicationActivityRecord from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ApplicationActivityRecord from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                ...
                "targets": [ActivityRecordTarget.from_dict(_item) for _item in obj["targets"]]
    ...
    def __eq__(self, other):
        if not isinstance(other, ApplicationActivityRecord):
            return False

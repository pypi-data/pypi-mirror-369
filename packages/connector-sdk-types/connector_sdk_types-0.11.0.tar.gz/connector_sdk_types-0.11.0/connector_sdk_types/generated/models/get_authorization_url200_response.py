...
from connector_sdk_types.generated.models.get_authorization_url_response import GetAuthorizationUrlResponse
from typing import Union, Any, List, Set, TYPE_CHECKING, Optional, Dict
from typing_extensions import Literal, Self
from pydantic import Field
GETAUTHORIZATIONURL200RESPONSE_ANY_OF_SCHEMAS = ['ErrorResponse', 'GetAuthorizationUrlResponse']
from pydantic import model_serializer

class GetAuthorizationUrl200Response(BaseModel):
    """
    GetAuthorizationUrl200Response
    """
    anyof_schema_1_validator: Optional[GetAuthorizationUrlResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, GetAuthorizationUrlResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {'ErrorResponse', 'GetAuthorizationUrlResponse'}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GetAuthorizationUrl200Response.model_construct()
        error_messages = []
        if not isinstance(v, GetAuthorizationUrlResponse):
            error_messages.append(f'Error! Input type `{type(v)}` is not `GetAuthorizationUrlResponse`')
        else:
            return v
        if not isinstance(v, ErrorResponse):
            ...
            return v
        if error_messages:
            raise ValueError('No match found when setting the actual_instance in GetAuthorizationUrl200Response with anyOf schemas: ErrorResponse, GetAuthorizationUrlResponse. Details: ' + ', '.join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = GetAuthorizationUrlResponse.from_json(json_str)
            return instance
        ...
        try:
            ...
            error_messages.append(str(e))
        if error_messages:
            raise ValueError('No match found when deserializing the JSON string into GetAuthorizationUrl200Response with anyOf schemas: ErrorResponse, GetAuthorizationUrlResponse. Details: ' + ', '.join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, GetAuthorizationUrlResponse]]:

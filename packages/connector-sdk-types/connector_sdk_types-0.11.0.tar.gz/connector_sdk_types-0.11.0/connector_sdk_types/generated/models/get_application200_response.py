...
from connector_sdk_types.generated.models.get_application_response import GetApplicationResponse
from typing import Union, Any, List, Set, TYPE_CHECKING, Optional, Dict
from typing_extensions import Literal, Self
from pydantic import Field
GETAPPLICATION200RESPONSE_ANY_OF_SCHEMAS = ['ErrorResponse', 'GetApplicationResponse']
...
class GetApplication200Response(BaseModel):
    ...
    """
    anyof_schema_1_validator: Optional[GetApplicationResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, GetApplicationResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {'ErrorResponse', 'GetApplicationResponse'}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GetApplication200Response.model_construct()
        error_messages = []
        if not isinstance(v, GetApplicationResponse):
            error_messages.append(f'Error! Input type `{type(v)}` is not `GetApplicationResponse`')
        else:
            return v
        if not isinstance(v, ErrorResponse):
            ...
            return v
        if error_messages:
            raise ValueError('No match found when setting the actual_instance in GetApplication200Response with anyOf schemas: ErrorResponse, GetApplicationResponse. Details: ' + ', '.join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = GetApplicationResponse.from_json(json_str)
            return instance
        ...
        try:
            ...
            error_messages.append(str(e))
        if error_messages:
            raise ValueError('No match found when deserializing the JSON string into GetApplication200Response with anyOf schemas: ErrorResponse, GetApplicationResponse. Details: ' + ', '.join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, GetApplicationResponse]]:

...
from connector_sdk_types.generated.models.assign_application_response import AssignApplicationResponse
from connector_sdk_types.generated.models.error_response import ErrorResponse
from typing import Union, Any, List, Set, TYPE_CHECKING, Optional, Dict
from typing_extensions import Literal, Self
from pydantic import Field
ASSIGNAPPLICATION200RESPONSE_ANY_OF_SCHEMAS = ['AssignApplicationResponse', 'ErrorResponse']
from pydantic import model_serializer

class AssignApplication200Response(BaseModel):
    """
    AssignApplication200Response
    """
    anyof_schema_1_validator: Optional[AssignApplicationResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[AssignApplicationResponse, ErrorResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {'AssignApplicationResponse', 'ErrorResponse'}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = AssignApplication200Response.model_construct()
        error_messages = []
        if not isinstance(v, AssignApplicationResponse):
            error_messages.append(f'Error! Input type `{type(v)}` is not `AssignApplicationResponse`')
        else:
            return v
        if not isinstance(v, ErrorResponse):
            ...
            return v
        if error_messages:
            raise ValueError('No match found when setting the actual_instance in AssignApplication200Response with anyOf schemas: AssignApplicationResponse, ErrorResponse. Details: ' + ', '.join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = AssignApplicationResponse.from_json(json_str)
            return instance
        ...
        try:
            ...
            error_messages.append(str(e))
        if error_messages:
            raise ValueError('No match found when deserializing the JSON string into AssignApplication200Response with anyOf schemas: AssignApplicationResponse, ErrorResponse. Details: ' + ', '.join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], AssignApplicationResponse, ErrorResponse]]:

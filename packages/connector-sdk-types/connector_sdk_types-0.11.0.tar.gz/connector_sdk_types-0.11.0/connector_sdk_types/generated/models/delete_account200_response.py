...
DELETEACCOUNT200RESPONSE_ANY_OF_SCHEMAS = ['DeleteAccountResponse', 'ErrorResponse']
from pydantic import model_serializer

class DeleteAccount200Response(BaseModel):
    """
    DeleteAccount200Response
    """
    anyof_schema_1_validator: Optional[DeleteAccountResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[DeleteAccountResponse, ErrorResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {'DeleteAccountResponse', 'ErrorResponse'}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = DeleteAccount200Response.model_construct()
        error_messages = []
        if not isinstance(v, DeleteAccountResponse):
            error_messages.append(f'Error! Input type `{type(v)}` is not `DeleteAccountResponse`')
        else:
            return v
        if not isinstance(v, ErrorResponse):
            ...
            return v
        if error_messages:
            raise ValueError('No match found when setting the actual_instance in DeleteAccount200Response with anyOf schemas: DeleteAccountResponse, ErrorResponse. Details: ' + ', '.join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = DeleteAccountResponse.from_json(json_str)
            return instance
        ...
        try:
            ...
            error_messages.append(str(e))
        if error_messages:
            raise ValueError('No match found when deserializing the JSON string into DeleteAccount200Response with anyOf schemas: DeleteAccountResponse, ErrorResponse. Details: ' + ', '.join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DeleteAccountResponse, ErrorResponse]]:

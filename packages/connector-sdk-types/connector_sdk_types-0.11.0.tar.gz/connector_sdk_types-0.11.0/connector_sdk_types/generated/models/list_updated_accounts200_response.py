...
from connector_sdk_types.generated.models.list_updated_accounts_response import ListUpdatedAccountsResponse
from typing import Union, Any, List, Set, TYPE_CHECKING, Optional, Dict
from typing_extensions import Literal, Self
from pydantic import Field
LISTUPDATEDACCOUNTS200RESPONSE_ANY_OF_SCHEMAS = ['ErrorResponse', 'ListUpdatedAccountsResponse']
from pydantic import model_serializer

class ListUpdatedAccounts200Response(BaseModel):
    """
    ListUpdatedAccounts200Response
    """
    anyof_schema_1_validator: Optional[ListUpdatedAccountsResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, ListUpdatedAccountsResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {'ErrorResponse', 'ListUpdatedAccountsResponse'}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ListUpdatedAccounts200Response.model_construct()
        error_messages = []
        if not isinstance(v, ListUpdatedAccountsResponse):
            error_messages.append(f'Error! Input type `{type(v)}` is not `ListUpdatedAccountsResponse`')
        else:
            return v
        if not isinstance(v, ErrorResponse):
            ...
            return v
        if error_messages:
            raise ValueError('No match found when setting the actual_instance in ListUpdatedAccounts200Response with anyOf schemas: ErrorResponse, ListUpdatedAccountsResponse. Details: ' + ', '.join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = ListUpdatedAccountsResponse.from_json(json_str)
            return instance
        ...
        try:
            ...
            error_messages.append(str(e))
        if error_messages:
            raise ValueError('No match found when deserializing the JSON string into ListUpdatedAccounts200Response with anyOf schemas: ErrorResponse, ListUpdatedAccountsResponse. Details: ' + ', '.join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, ListUpdatedAccountsResponse]]:

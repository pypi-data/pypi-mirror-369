...
INFO200RESPONSE_ANY_OF_SCHEMAS = ['ErrorResponse', 'InfoResponse']
...
class Info200Response(BaseModel):
    ...
    Info200Response
    """
    anyof_schema_1_validator: Optional[InfoResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, InfoResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {'ErrorResponse', 'InfoResponse'}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = Info200Response.model_construct()
        error_messages = []
        if not isinstance(v, InfoResponse):
            error_messages.append(f'Error! Input type `{type(v)}` is not `InfoResponse`')
        else:
            return v
        if not isinstance(v, ErrorResponse):
            ...
            return v
        if error_messages:
            raise ValueError('No match found when setting the actual_instance in Info200Response with anyOf schemas: ErrorResponse, InfoResponse. Details: ' + ', '.join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = InfoResponse.from_json(json_str)
            return instance
        ...
        try:
            ...
            error_messages.append(str(e))
        if error_messages:
            raise ValueError('No match found when deserializing the JSON string into Info200Response with anyOf schemas: ErrorResponse, InfoResponse. Details: ' + ', '.join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, InfoResponse]]:

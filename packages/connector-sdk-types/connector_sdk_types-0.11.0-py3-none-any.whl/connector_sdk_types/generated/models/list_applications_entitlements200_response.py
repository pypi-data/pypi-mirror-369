...
from connector_sdk_types.generated.models.list_applications_entitlements_response import ListApplicationsEntitlementsResponse
from typing import Union, Any, List, Set, TYPE_CHECKING, Optional, Dict
from typing_extensions import Literal, Self
from pydantic import Field
LISTAPPLICATIONSENTITLEMENTS200RESPONSE_ANY_OF_SCHEMAS = ['ErrorResponse', 'ListApplicationsEntitlementsResponse']
from pydantic import model_serializer

class ListApplicationsEntitlements200Response(BaseModel):
    """
    ListApplicationsEntitlements200Response
    """
    anyof_schema_1_validator: Optional[ListApplicationsEntitlementsResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, ListApplicationsEntitlementsResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {'ErrorResponse', 'ListApplicationsEntitlementsResponse'}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ListApplicationsEntitlements200Response.model_construct()
        error_messages = []
        if not isinstance(v, ListApplicationsEntitlementsResponse):
            error_messages.append(f'Error! Input type `{type(v)}` is not `ListApplicationsEntitlementsResponse`')
        else:
            return v
        if not isinstance(v, ErrorResponse):
            ...
            return v
        if error_messages:
            raise ValueError('No match found when setting the actual_instance in ListApplicationsEntitlements200Response with anyOf schemas: ErrorResponse, ListApplicationsEntitlementsResponse. Details: ' + ', '.join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = ListApplicationsEntitlementsResponse.from_json(json_str)
            return instance
        ...
        try:
            ...
            error_messages.append(str(e))
        if error_messages:
            raise ValueError('No match found when deserializing the JSON string into ListApplicationsEntitlements200Response with anyOf schemas: ErrorResponse, ListApplicationsEntitlementsResponse. Details: ' + ', '.join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, ListApplicationsEntitlementsResponse]]:
    ...
    def _serialize(self):
        try:
            ...
            pass
        return getattr(self, 'actual_instance', None)
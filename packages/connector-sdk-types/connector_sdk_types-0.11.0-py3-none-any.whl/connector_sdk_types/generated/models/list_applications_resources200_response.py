...
from connector_sdk_types.generated.models.list_applications_resources_response import ListApplicationsResourcesResponse
from typing import Union, Any, List, Set, TYPE_CHECKING, Optional, Dict
from typing_extensions import Literal, Self
from pydantic import Field
LISTAPPLICATIONSRESOURCES200RESPONSE_ANY_OF_SCHEMAS = ['ErrorResponse', 'ListApplicationsResourcesResponse']
from pydantic import model_serializer

class ListApplicationsResources200Response(BaseModel):
    """
    ListApplicationsResources200Response
    """
    anyof_schema_1_validator: Optional[ListApplicationsResourcesResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, ListApplicationsResourcesResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {'ErrorResponse', 'ListApplicationsResourcesResponse'}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ListApplicationsResources200Response.model_construct()
        error_messages = []
        if not isinstance(v, ListApplicationsResourcesResponse):
            error_messages.append(f'Error! Input type `{type(v)}` is not `ListApplicationsResourcesResponse`')
        else:
            return v
        if not isinstance(v, ErrorResponse):
            ...
            return v
        if error_messages:
            raise ValueError('No match found when setting the actual_instance in ListApplicationsResources200Response with anyOf schemas: ErrorResponse, ListApplicationsResourcesResponse. Details: ' + ', '.join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = ListApplicationsResourcesResponse.from_json(json_str)
            return instance
        ...
        try:
            ...
            error_messages.append(str(e))
        if error_messages:
            raise ValueError('No match found when deserializing the JSON string into ListApplicationsResources200Response with anyOf schemas: ErrorResponse, ListApplicationsResourcesResponse. Details: ' + ', '.join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, ListApplicationsResourcesResponse]]:

...
RELEASERESOURCES200RESPONSE_ANY_OF_SCHEMAS = ['ErrorResponse', 'ReleaseResourcesResponse']
from pydantic import model_serializer

class ReleaseResources200Response(BaseModel):
    """
    ReleaseResources200Response
    """
    anyof_schema_1_validator: Optional[ReleaseResourcesResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, ReleaseResourcesResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {'ErrorResponse', 'ReleaseResourcesResponse'}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ReleaseResources200Response.model_construct()
        error_messages = []
        if not isinstance(v, ReleaseResourcesResponse):
            error_messages.append(f'Error! Input type `{type(v)}` is not `ReleaseResourcesResponse`')
        else:
            return v
        if not isinstance(v, ErrorResponse):
            ...
            return v
        if error_messages:
            raise ValueError('No match found when setting the actual_instance in ReleaseResources200Response with anyOf schemas: ErrorResponse, ReleaseResourcesResponse. Details: ' + ', '.join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = ReleaseResourcesResponse.from_json(json_str)
            return instance
        ...
        try:
            ...
            error_messages.append(str(e))
        if error_messages:
            raise ValueError('No match found when deserializing the JSON string into ReleaseResources200Response with anyOf schemas: ErrorResponse, ReleaseResourcesResponse. Details: ' + ', '.join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, ReleaseResourcesResponse]]:

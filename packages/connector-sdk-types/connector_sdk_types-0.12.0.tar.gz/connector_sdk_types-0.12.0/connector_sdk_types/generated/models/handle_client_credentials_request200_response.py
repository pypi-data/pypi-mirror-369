...
HANDLECLIENTCREDENTIALSREQUEST200RESPONSE_ANY_OF_SCHEMAS = [
    "ErrorResponse",
    "HandleClientCredentialsResponse",
]
...
class HandleClientCredentialsRequest200Response(BaseModel):
    """
    HandleClientCredentialsRequest200Response
    """

    anyof_schema_1_validator: Optional[HandleClientCredentialsResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, HandleClientCredentialsResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {"ErrorResponse", "HandleClientCredentialsResponse"}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = HandleClientCredentialsRequest200Response.model_construct()
        error_messages = []
        if not isinstance(v, HandleClientCredentialsResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `HandleClientCredentialsResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in HandleClientCredentialsRequest200Response with anyOf schemas: ErrorResponse, HandleClientCredentialsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = HandleClientCredentialsResponse.from_json(json_str)
            return instance
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into HandleClientCredentialsRequest200Response with anyOf schemas: ErrorResponse, HandleClientCredentialsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ErrorResponse, HandleClientCredentialsResponse]]:

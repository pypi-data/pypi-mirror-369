...
HANDLEAUTHORIZATIONCALLBACK200RESPONSE_ANY_OF_SCHEMAS = [
    "ErrorResponse",
    "HandleAuthorizationCallbackResponse",
]
...
class HandleAuthorizationCallback200Response(BaseModel):
    """
    HandleAuthorizationCallback200Response
    """

    anyof_schema_1_validator: Optional[HandleAuthorizationCallbackResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, HandleAuthorizationCallbackResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {"ErrorResponse", "HandleAuthorizationCallbackResponse"}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = HandleAuthorizationCallback200Response.model_construct()
        error_messages = []
        if not isinstance(v, HandleAuthorizationCallbackResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `HandleAuthorizationCallbackResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in HandleAuthorizationCallback200Response with anyOf schemas: ErrorResponse, HandleAuthorizationCallbackResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = HandleAuthorizationCallbackResponse.from_json(json_str)
            return instance
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into HandleAuthorizationCallback200Response with anyOf schemas: ErrorResponse, HandleAuthorizationCallbackResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ErrorResponse, HandleAuthorizationCallbackResponse]]:

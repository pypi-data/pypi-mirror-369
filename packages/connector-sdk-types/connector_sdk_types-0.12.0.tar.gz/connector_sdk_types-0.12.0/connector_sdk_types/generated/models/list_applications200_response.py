...
class ListApplications200Response(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, ListApplicationsResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ListApplicationsResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in ListApplications200Response with anyOf schemas: ErrorResponse, ListApplicationsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into ListApplications200Response with anyOf schemas: ErrorResponse, ListApplicationsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance

...
TRANSFERDATA200RESPONSE_ANY_OF_SCHEMAS = ["ErrorResponse", "TransferDataResponse"]
from pydantic import model_serializer


class TransferData200Response(BaseModel):
    """
    TransferData200Response
    """

    anyof_schema_1_validator: Optional[TransferDataResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, TransferDataResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {"ErrorResponse", "TransferDataResponse"}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = TransferData200Response.model_construct()
        error_messages = []
        if not isinstance(v, TransferDataResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `TransferDataResponse`")
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in TransferData200Response with anyOf schemas: ErrorResponse, TransferDataResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = TransferDataResponse.from_json(json_str)
            return instance
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into TransferData200Response with anyOf schemas: ErrorResponse, TransferDataResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, TransferDataResponse]]:

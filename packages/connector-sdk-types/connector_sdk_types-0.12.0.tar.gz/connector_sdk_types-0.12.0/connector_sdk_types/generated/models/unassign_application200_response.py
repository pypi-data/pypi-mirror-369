...
from connector_sdk_types.generated.models.unassign_application_response import (
...
class UnassignApplication200Response(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, UnassignApplicationResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `UnassignApplicationResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in UnassignApplication200Response with anyOf schemas: ErrorResponse, UnassignApplicationResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into UnassignApplication200Response with anyOf schemas: ErrorResponse, UnassignApplicationResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ErrorResponse, UnassignApplicationResponse]]:

...
from connector_sdk_types.generated.models.list_updated_accounts_response import (
...
class ListUpdatedAccounts200Response(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, ListUpdatedAccountsResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ListUpdatedAccountsResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in ListUpdatedAccounts200Response with anyOf schemas: ErrorResponse, ListUpdatedAccountsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into ListUpdatedAccounts200Response with anyOf schemas: ErrorResponse, ListUpdatedAccountsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ErrorResponse, ListUpdatedAccountsResponse]]:

...
class ListActivityRecords200Response(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, ListActivityRecordsResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ListActivityRecordsResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in ListActivityRecords200Response with anyOf schemas: ErrorResponse, ListActivityRecordsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into ListActivityRecords200Response with anyOf schemas: ErrorResponse, ListActivityRecordsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ErrorResponse, ListActivityRecordsResponse]]:

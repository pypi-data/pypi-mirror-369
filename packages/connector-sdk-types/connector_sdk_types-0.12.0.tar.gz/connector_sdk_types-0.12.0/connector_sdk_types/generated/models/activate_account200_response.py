...
ACTIVATEACCOUNT200RESPONSE_ANY_OF_SCHEMAS = ["ActivateAccountResponse", "ErrorResponse"]
from pydantic import model_serializer


class ActivateAccount200Response(BaseModel):
    """
    ActivateAccount200Response
    """

    anyof_schema_1_validator: Optional[ActivateAccountResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ActivateAccountResponse, ErrorResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {"ActivateAccountResponse", "ErrorResponse"}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ActivateAccount200Response.model_construct()
        error_messages = []
        if not isinstance(v, ActivateAccountResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ActivateAccountResponse`")
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in ActivateAccount200Response with anyOf schemas: ActivateAccountResponse, ErrorResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = ActivateAccountResponse.from_json(json_str)
            return instance
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into ActivateAccount200Response with anyOf schemas: ActivateAccountResponse, ErrorResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ActivateAccountResponse, ErrorResponse]]:

...
LISTCUSTOMATTRIBUTESSCHEMA200RESPONSE_ANY_OF_SCHEMAS = [
    "ErrorResponse",
    "ListCustomAttributesSchemaResponse",
]
...
class ListCustomAttributesSchema200Response(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, ListCustomAttributesSchemaResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ListCustomAttributesSchemaResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in ListCustomAttributesSchema200Response with anyOf schemas: ErrorResponse, ListCustomAttributesSchemaResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into ListCustomAttributesSchema200Response with anyOf schemas: ErrorResponse, ListCustomAttributesSchemaResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ErrorResponse, ListCustomAttributesSchemaResponse]]:

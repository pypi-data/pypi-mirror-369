...
from connector_sdk_types.generated.models.get_application_account_response import (
...
GETAPPLICATIONACCOUNT200RESPONSE_ANY_OF_SCHEMAS = ["ErrorResponse", "GetApplicationAccountResponse"]
from pydantic import model_serializer


class GetApplicationAccount200Response(BaseModel):
    """
    GetApplicationAccount200Response
    """

    anyof_schema_1_validator: Optional[GetApplicationAccountResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, GetApplicationAccountResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {"ErrorResponse", "GetApplicationAccountResponse"}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GetApplicationAccount200Response.model_construct()
        error_messages = []
        if not isinstance(v, GetApplicationAccountResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `GetApplicationAccountResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in GetApplicationAccount200Response with anyOf schemas: ErrorResponse, GetApplicationAccountResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = GetApplicationAccountResponse.from_json(json_str)
            return instance
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into GetApplicationAccount200Response with anyOf schemas: ErrorResponse, GetApplicationAccountResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ErrorResponse, GetApplicationAccountResponse]]:

...
from connector_sdk_types.generated.models.list_applications_activity_records_response import (
...
LISTAPPLICATIONSACTIVITYRECORDS200RESPONSE_ANY_OF_SCHEMAS = [
    "ErrorResponse",
    "ListApplicationsActivityRecordsResponse",
]
...
class ListApplicationsActivityRecords200Response(BaseModel):
    ...
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, ListApplicationsActivityRecordsResponse]] = (
            None
        )
    else:
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, ListApplicationsActivityRecordsResponse):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ListApplicationsActivityRecordsResponse`"
            )
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in ListApplicationsActivityRecords200Response with anyOf schemas: ErrorResponse, ListApplicationsActivityRecordsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into ListApplicationsActivityRecords200Response with anyOf schemas: ErrorResponse, ListApplicationsActivityRecordsResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ErrorResponse, ListApplicationsActivityRecordsResponse]]:

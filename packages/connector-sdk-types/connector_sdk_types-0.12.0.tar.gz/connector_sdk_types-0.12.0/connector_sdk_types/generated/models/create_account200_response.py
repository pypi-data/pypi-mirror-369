...
CREATEACCOUNT200RESPONSE_ANY_OF_SCHEMAS = ["CreateAccountResponse", "ErrorResponse"]
from pydantic import model_serializer


class CreateAccount200Response(BaseModel):
    """
    CreateAccount200Response
    """

    anyof_schema_1_validator: Optional[CreateAccountResponse] = None
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[CreateAccountResponse, ErrorResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {"CreateAccountResponse", "ErrorResponse"}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = CreateAccount200Response.model_construct()
        error_messages = []
        if not isinstance(v, CreateAccountResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `CreateAccountResponse`")
        else:
            return v
        ...
        if error_messages:
            raise ValueError(
                "No match found when setting the actual_instance in CreateAccount200Response with anyOf schemas: CreateAccountResponse, ErrorResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        error_messages = []
        try:
            instance.actual_instance = CreateAccountResponse.from_json(json_str)
            return instance
        ...
        if error_messages:
            raise ValueError(
                "No match found when deserializing the JSON string into CreateAccount200Response with anyOf schemas: CreateAccountResponse, ErrorResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], CreateAccountResponse, ErrorResponse]]:

...
class Delta(BaseModel):
    """
    Delta query parameters for incremental data synchronization.  Delta queries enable efficient incremental data fetching by allowing clients to request only the changes that have occurred since their last synchronization. This is particularly useful for large datasets where fetching all data on each request would be inefficient.  ## How Delta Queries Work  1. **Initial Request**: When no `delta_cursor` is provided, the endpoint returns all available data along with a `delta_cursor` in the response.  2. **Subsequent Requests**: Include the `delta_cursor` from the previous response to receive only the changes (additions, updates, deletions) that occurred since that point.  3. **Cursor Management**: The response will include a new `delta_cursor` that should be used for the next delta request.  ## Usage Examples  ```typescript // First request - get all data const response = await api.getUsers(); // response.delta_cursor = \"eyJ0aW1lc3RhbXAiOiIyMDI0LTAxLTE1VDEwOjAwOjAwWiJ9\"  // Subsequent request - get only changes const deltaResponse = await api.getUsers({ delta_cursor: response.delta_cursor }); // Only returns users that were created, updated, or deleted since the last request ```  ## Best Practices  - Store the `delta_cursor` persistently (database, file, etc.) to maintain synchronization state across application restarts - Handle the case where `delta_cursor` becomes invalid (expired, corrupted) by falling back to a full data fetch - Implement exponential backoff for delta requests to handle rate limiting - Consider implementing a \"full sync\" mechanism that periodically fetches all data to ensure consistency  ## Error Handling  - If the `delta_cursor` is invalid or expired, the endpoint may return an error or automatically fall back to returning all data - Always handle the case where the response doesn't include a `delta_cursor` (indicating the endpoint doesn't support delta queries)
    """ # noqa: E501
    ...
    __properties: ClassVar[List[str]] = ["delta_cursor"]

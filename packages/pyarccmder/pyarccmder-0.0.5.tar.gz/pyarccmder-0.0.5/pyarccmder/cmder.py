...
def arcCmder(
    ...
    try:
        ...
        if DEBUG:
            ...
            print("[arc-cmder.ts] initCmder | cleanedConfig - OLD:: ", cleanedConfig)

        def cleanConfig(configData):
            valuesCD = [argCD['value'] for indexArgCD, argCD in enumerate(args) if argCD['name'] == configData['name']]
            valueCD = valuesCD[0] if len(valuesCD) > 0 else None
            configData['value'] = valueCD
            return configData
        cleanedConfig = [cleanConfig(cd) for indexCD, cd in enumerate(cleanedConfig)]

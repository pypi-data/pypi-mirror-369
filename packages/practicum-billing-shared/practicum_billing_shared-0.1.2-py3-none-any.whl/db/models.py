...
from src.types import OrderStatus, PaymentMethodName, PaymentProviderName, PaymentStatus, RefundStatus
...
class Base(DeclarativeBase):
    ...
    id: Mapped[uuid.UUID] = mapped_column(default=uuid.uuid4, primary_key=True, unique=True, nullable=False)
...
class PaymentMethod(Base):
    ...
    # some providers (e.g. YooKassa) use payment method id to execute recurrent payments instead of order id
    id_at_provider: Mapped[str] = mapped_column(index=True)
    # card_last_four: Mapped[str | None] = mapped_column(String(4))
    # card_network: Mapped[CardNetwork | None] = mapped_column(ValueEnum(CardNetwork))

...
class OB08(SAP):
    ...
    def finalizar(self) -> None:
        ...
        # Estrategia 1: Shift+F3 (preferida cuando GUI está activo)
        try:
            if self._verificar_sesion_gui_activa():
                self.mostrar("⌨️ Usando Shift+F3 (GUI activo)")
                self.enviar_tecla_ventana('SHIFT', 'F3')
                self.demora()
                return
        except Exception as e:
            self.mostrar(f"⚠️ Shift+F3 falló, intentando métodos alternativos: {str(e)}", True)
        
        # Estrategias alternativas via Selenium (para VMs sin GUI)
        self.mostrar("🔄 GUI no activo, usando métodos Selenium")
        
        # Estrategia 2: Click directo en botón Finalizar por ID
        if self._click_finalizar_por_id():
            self.mostrar("✅ Finalización exitosa via click directo en botón")
            self.demora()
            return
            
        # Estrategia 3: Click por accesskey "F"
        if self._click_finalizar_por_accesskey():
            self.mostrar("✅ Finalización exitosa via accesskey")
            self.demora()
            return
            
        # Estrategia 4: Click por título "Finalizar (Mayús+F3)"
    ...
    def omitir(self) -> None:
        ...
        # Estrategia 1: Shift+F8 (preferida cuando GUI está activo)
        try:
            if self._verificar_sesion_gui_activa():
                self.mostrar("⌨️ Usando Shift+F8 (GUI activo)")
                self.enviar_tecla_ventana('SHIFT', 'F8')
                self.demora()
                return
        except Exception as e:
            self.mostrar(f"⚠️ Shift+F8 falló, intentando métodos alternativos: {str(e)}", True)
        
        # Estrategias alternativas via Selenium (para VMs sin GUI)
        self.mostrar("🔄 GUI no activo, usando métodos Selenium")
        
        # Estrategia 2: Click directo en botón Omitir por ID
        if self._click_omitir_por_id():
            self.mostrar("✅ Omisión exitosa via click directo en botón")
            self.demora()
            return
            
        # Estrategia 3: Click por accesskey "O"
        if self._click_omitir_por_accesskey():
            self.mostrar("✅ Omisión exitosa via accesskey")
            self.demora()
            return
            
        # Estrategia 4: Click por título "Omitir (Mayús+F8)"
    ...
    def cancelar_entradas_nuevas(self) -> None:
        ...
        # Estrategia 1: Escape (preferida cuando GUI está activo)
        try:
            if self._verificar_sesion_gui_activa():
                self.mostrar("⌨️ Usando Escape (GUI activo)")
                self.enviar_tecla_ventana('ESCAPE')
                self.demora()
                return
        except Exception as e:
            self.mostrar(f"⚠️ Escape falló, intentando métodos alternativos: {str(e)}", True)
        
        # Estrategias alternativas via Selenium (para VMs sin GUI)
        self.mostrar("🔄 GUI no activo, usando métodos Selenium")
        
        # Estrategia 2: Click directo en botón Cancelar por ID
        if self._click_cancelar_entradas_por_id():
            self.mostrar("✅ Cancelación exitosa via click directo en botón")
            self.demora()
            return
            
        # Estrategia 3: Click por accesskey "C"
        if self._click_cancelar_entradas_por_accesskey():
            self.mostrar("✅ Cancelación exitosa via accesskey")
            self.demora()
            return
            
        # Estrategia 4: Click por título "Cancelar (Escape)"
    ...
    def guardar(self) -> None:
        ...
        # Estrategia 1: Ctrl+S (preferida cuando GUI está activo)
        try:
            if self._verificar_sesion_gui_activa():
                self.mostrar("🖱️ Usando Ctrl+S (GUI activo)")
                self.enviar_tecla_ventana('CTRL', 'S')
                self.demora()
                return
        except Exception as e:
            self.mostrar(f"⚠️ Ctrl+S falló, intentando métodos alternativos: {str(e)}", True)
        
        # Estrategias alternativas via Selenium (para VMs sin GUI)
        self.mostrar("🔄 GUI no activo, usando métodos Selenium")
        
        # Estrategia 2: Click directo en botón Guardar por ID
        if self._click_guardar_por_id():
            self.mostrar("✅ Guardado exitoso via click directo en botón")
            self.demora()
            return
            
        # Estrategia 3: Click por accesskey "G"
        if self._click_guardar_por_accesskey():
            self.mostrar("✅ Guardado exitoso via accesskey")
            self.demora()
            return
            
        # Estrategia 4: Click por título "Guardar (Ctrl+S)"

...
async def confluence_search(search_input: SeachToolInput):
    ...
    try:
        ...
        result = search_tool.search(cql=cql, page_url="https://scbtechx.atlassian.net/wiki", spaces_filter=space_key, limit=limit)
        # result = json.dumps(json.loads(results.text), sort_keys=True, indent=4, separators=(",", ": "))

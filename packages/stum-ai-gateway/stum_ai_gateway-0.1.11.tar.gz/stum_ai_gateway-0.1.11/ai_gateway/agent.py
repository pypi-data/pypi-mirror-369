...
class LlmNatbusAgent:
    ...
    async def _on_human_command(self, rm: ReceivedMessage) -> None:
        ...
        # NEW: drop duplicate human commands for the same correlation_id while pending
        if correlation_id in self._pending:
            self.log.debug("dup_human_cmd_drop", extra={"corr": correlation_id, "cmd": raw_cmd_text})
            await rm.ack()
            return
        ...
        if plan.await_response:
            ...
            if not resp_subject:
                ...
                # NEW: optional immediate acceptance so upstream stops re-sending
                if bool(getattr(self.cfg, "send_accept_on_await", False)):
                    await self._reply_json(
                        reply_subject,
                        {
                            "status": "accepted",
                            "subject": plan.subject,
                            "await_response": True,
                            "response_subject": resp_subject,
                            "correlation_id": correlation_id,
                        },
                        rm,
                        corr_id=correlation_id,
                    )
                    self.log.debug("human_cmd_accepted", extra={
                        "corr": correlation_id,

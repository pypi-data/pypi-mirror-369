...
class Pong(BaseGame):
    ...
    def update_game(self):
        ...
        if (
            self.ball_pos[0] <= PADDLE_WIDTH
            and self.paddle1_pos[1] < self.ball_pos[1] < self.paddle1_pos[3]
        ):
            self.ball_dx = abs(self.ball_dx)
        elif (
            self.ball_pos[2] >= WIDTH - PADDLE_WIDTH
            and self.paddle2_pos[1] < self.ball_pos[1] < self.paddle2_pos[3]
        ):
            self.ball_dx = -abs(self.ball_dx)

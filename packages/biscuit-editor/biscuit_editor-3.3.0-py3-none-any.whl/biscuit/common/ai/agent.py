...
"""
    ...
    async def _implement_changes_action(self, task: AgentTask, parameters: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if file_path and (changes or new_content):
                ...
                if write_tool:
                    if new_content is not None:
                        write_result = write_tool._run(file_path, new_content)
                        results.append(f"**File written:** `{file_path}`")
                        results.append(f"**Write result:** {write_result}")
                    else:
                        # If only 'changes' are provided, log them (legacy fallback)
                        for change in changes:
                            results.append(f"**Applied change:** {change}")
                        results.append(f"**File updated (changes listed, but not written):** `{file_path}`")
                    ...
                    results.append("**write_file tool not available, cannot write changes.**")

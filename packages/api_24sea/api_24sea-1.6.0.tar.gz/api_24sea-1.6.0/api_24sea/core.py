...
class _AuthMixin:
    """Mixin for authentication handling  for sync/async API clients."""
...
class API(_AuthMixin):
    ...
    def get_oldest_timestamp(
        ...
        _sites, _locations = U.get_site_locations(
            sites, locations, self.metrics_overview
        )
        if isinstance(sites, list) and len(sites) > 1:
            all_dfs = []
            for _s, _l in zip(_sites, _locations):
                query = S.GetOldestTimestampSchema(site=_s, locations=_l)
                ...
                if locs:
                    ...
                    formatted_locations = "   • " + "\n   • ".join(_locs)
                    logging.info(
                        f"\033[32;1m⏳ Getting oldest timestamps for {query.site} at:\n{formatted_locations}\n\033[0m"
                    )
                else:
                    logging.info(
                        f"\033[32;1m⏳ Getting oldest timestamps for {query.site}\033[0m"
                    )
                ...
                all_dfs.append(pd.DataFrame(js))
            return (
                pd.concat(all_dfs, ignore_index=True)
                if all_dfs
                else pd.DataFrame()
            )
        # Single site path (backward compatible)
        query = S.GetOldestTimestampSchema(
            site=_sites[0], locations=_locations[0]
        )
        locs = query.locations
        if locs:
            if isinstance(locs, str):
            ...
            formatted_locations = "   • " + "\n   • ".join(_locs)
            logging.info(
                f"\033[32;1m⏳ Getting oldest timestamps for {query.site} at the "
...
class AsyncAPI(API):
    ...
    async def get_data(
        ...
        tasks = [U.fetch_data_async(f"{self.base_url}data/", site, location,
                                    ...
                                    max_retries)
                 for (site, location), group in grouped_metrics]
        chunk_size_dict = U.estimate_chunk_size(tasks, start_timestamp,
                                                end_timestamp, grouped_metrics,
                                                self._selected_metrics)
        # fmt: on
        data_frames = []
        data_frames = await U.gather_in_chunks(
            tasks, chunk_size=chunk_size_dict["chunk_size"], timeout=timeout
        )
    ...
    async def get_oldest_timestamp(
        ...
        # Single site path
        query = S.GetOldestTimestampSchema(site=_sites, locations=_locations)
        return await U.fetch_oldest_timestamp_async(
            url,
            query.site,
            query.locations,  # type: ignore
            query.headers,  # type: ignore
    ...
    async def get_stats(
        ...
        stats_list = await U.gather_in_chunks(
            tasks, chunk_size=1000, timeout=timeout
        )
    ...
    @U.require_auth
    @U.validate_call(config=dict(arbitrary_types_allowed=True))
    async def get_stats_predefined_intervals(
        self,
        sites: Optional[Union[List, str]],
        locations: Optional[Union[List, str]],
        metrics: Union[List, str],

...
import termios
import subprocess
from ssl import SSLContext, create_default_context, Purpose, VerifyMode
...
class Netcat:
    ...
    *Example 2:* Listen for a local TCP connection on port 1234, allow the user
    ...
    def __init__(self, connect=None, sock=None, listen=None,
        ...
        # ssl
        if ssl is True:
            ssl = create_default_context(purpose=Purpose.CLIENT_AUTH if listen else Purpose.SERVER_AUTH)
            ssl.check_hostname = False
            ssl.verify_mode = VerifyMode.CERT_NONE 
            if listen:
                # okay. sure. user friendly.
                if not os.path.exists(os.path.expanduser('~/.cache/nclib/cert.pem')):
                    subprocess.run('mkdir -p ~/.cache/nclib && openssl req -x509 -newkey rsa:4096 -sha256 -days 3650 -nodes -keyout ~/.cache/nclib/key.pem -out ~/.cache/nclib/cert.pem -subj "/C=XX/ST=StateName/L=CityName/O=CompanyName/OU=CompanySectionName/CN=nclib"', shell=True)
                ssl.load_cert_chain(certfile=os.path.expanduser('~/.cache/nclib/cert.pem'), keyfile=os.path.expanduser('~/.cache/nclib/key.pem'))
        if isinstance(ssl, SSLContext):
            self.sock = ssl.wrap_socket(self.sock, server_side=listen or False)
    ...
    def interact(self, insock=sys.stdin, outsock=sys.stdout, raw=False) -> None:
        """
        ...
        old = None
        if raw and insock.isatty():
            old = tty.setraw(insock)
        try:
            ...
            if raw and insock.isatty():
                ...
                termios.tcsetattr(insock, termios.TCSADRAIN, old)

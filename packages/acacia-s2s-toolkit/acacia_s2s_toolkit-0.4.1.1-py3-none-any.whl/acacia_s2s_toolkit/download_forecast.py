...
def webAPI_request_forecast(fcdate,origin,grid,variable,data_format,webapi_param,leadtime_hour,leveltype,filename,plevs,fc_enslags):
    ...
    for lag in fc_enslags:
        leadtimes = argument_output.output_formatted_leadtimes(leadtime_hour,fcdate,variable,lag)
...
def webAPI_request_hindcast(fcdate,origin_id,grid,variable,data_format,webapi_param,leadtime_hour,leveltype,filename,plevs,hc_enslags,rf_model_date,rfyears,rfdate):
    from ecmwfapi import ECMWFDataServer
    server = ECMWFDataServer()

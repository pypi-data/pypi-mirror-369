...
def get_hindcast_model_date(origin_id,fcdate):
    ...
    if isinstance(rf_freq,str) and rf_freq.startswith('2'): # reforecast model version with single date
        mrf_date = rf_model_freq
    elif isinstance(rf_freq,(list,tuple)): # days of week
        print ('days of week')
        # forecast and reforecast at same frequency
        mrf_date = fcdate
    elif rf_freq == 'odddates': # ECMWF config.
        print ('odd dates')
        ...
        if day % 2 == 0 or fcdate[4:] == '0229': # even date or 29th Feb
            ...
            mrf_date = fcdate
    elif rf_freq == 'fourpermonth': # KMA and UKMO:
...
def get_reforecast_date(origin_id,fcdate):
    ''' Given origin_id, output appropriate reforecast date
    '''
...
def output_formatted_leadtimes(leadtime_hour,fcdate,variable,lag):
...
def check_and_output_all_hc_arguments(variable,origin_id,fcdate,rfyears=None):
    ...
    rf_date = get_reforecast_dates(origin_id,fcdate)

...
def output_hc_lags(origin_id,fcdate):
    ...
    '''
    if origin_id not in variable_dict.rf_lag:
        raise ValueError(f"[ERROR] No forecast lags found for origin_id '{origin_id}'.")
    ...
    if isinstance(variable_dict.rf_lag[origin_id],list): # DOMs. get closest lags
        fc_day = fcdate_obj.day
        rf_freq_info = variable_dict.rf_lag[origin_id]
        ...
        return lag
    elif variable_dict.rf_lag[origin_id]=='ThMoTh':
    ...
    elif variable_dict.rf_lag[origin_id]=='odddates':

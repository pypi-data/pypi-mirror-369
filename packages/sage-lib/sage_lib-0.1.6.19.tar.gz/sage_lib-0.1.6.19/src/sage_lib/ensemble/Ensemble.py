...
class Ensemble:
    ...
    def __init__(self, global_clasification:bool=True, weights_mode:str='boltzmann') -> None:
        """
    ...
    def _cluster_masses_fixed(self, w: np.ndarray, L: np.ndarray,) -> Tuple[np.ndarray, np.ndarray]:
        """Re‑accumulate weights inside the fixed clusters."""
        mass = np.bincount(L, weights=w, minlength=self._n_clusters)
        return mass
    ...
    def evolution_two_ensembles(
        ...
        y_all = np.array([getattr(s.AtomPositionManager, 'E', 0.0)
                          for s in PT_all.containers])
        ...
        FE_all -= FE_all.min(axis=0)
        self.FE_all = FE_all  # keep for downstream calls
        ...
        for g in range(g_max + 1):
            ...
            HabsA, HabsB, JSD, shared, newA, newB = mats
    ...
    def compare_ensembles_abs(
        ...
        if self.global_clasification:
            self.initialise_global_clustering(counts_all, n_clusters= int(y_all.shape[0]*states_fraction) )
        ...
        return HabsA_mat, HabsB_mat, Jabs_mat, shared_mat, newA_mat, newB_mat
    ...
    def estimate_metrics(self, freqs_A, freqs_B, energies_A, energies_B, energies_all, LA, LB,
        ...
        if self.global_clasification:
            if self.weights_mode == 'boltzmann':
                ...
                weights = np.ones_like(energies_all)
        ...
        (F_A, F_B) = (massA.sum()/mass_global.sum(),
                      massB.sum()/mass_global.sum())
        ...
        return HcondA, HcondB, Jabs, shared, newA, newB
...
#HabsA, HabsB, Jabs, shared, newA, newB
HabsA, HabsB, Jabs, shared, newA, newB = ens.compare_ensembles_abs(
    #reference_potentials = {'Cu':-14.916443703626898/4, 'O':(-9.87396191+0.096)/2 }, 
    #reference_state='O', 
    max_clusters=15,
    )
HabsA, HabsB = np.max(HabsA), np.max(HabsB)
print(HabsA, HabsB)
#1.5970013976490702 2.4567023925365534
plt.show()
'''

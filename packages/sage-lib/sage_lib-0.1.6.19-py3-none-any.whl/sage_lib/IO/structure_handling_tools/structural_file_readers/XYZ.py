...
class XYZ:
    ...
    def export_as_xyz(
        ...
        if inc["fixed"]:    props.append(f"{fixed_tag}:I:3")
        ...
        if inc["meta"]:
            hdr.append(" ".join(f"{k}={json.dumps(v)}" for k, v in self.metadata.items()))
        ...
        for i in range(self.atomCount):
            ...
            if inc["fixed"]:
                cols += [str(int(v)) for v in self.atomicConstraints[i]]
            if inc["classID"]:

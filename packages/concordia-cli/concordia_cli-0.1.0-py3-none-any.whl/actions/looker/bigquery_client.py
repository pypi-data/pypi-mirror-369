...
class ErrorTracker:
    ...
    def add_table_error(self, dataset_id: str, table_id: str, error: Exception, error_type: str = "unexpected"):
    ...
    def has_errors(self) -> bool:
        ...
        return bool(
            self.dataset_errors or self.table_errors or self.permission_errors or
            self.not_found_errors or self.connection_errors or self.unexpected_errors
        )

    def get_total_error_count(self) -> int:
        """Get the total number of errors recorded."""
        return (
            len(self.dataset_errors) + len(self.table_errors) +
            len(self.permission_errors) + len(self.not_found_errors) +
            len(self.connection_errors) + len(self.unexpected_errors)
        )
    ...
    def print_summary(self, total_datasets: int, total_tables_found: int):
        ...
        # Success stats
        successful_datasets = total_datasets - len(self.dataset_errors) - len(
...
class TableInfo:
    ...
    def add_column(self, name: str, data_type: str, mode: str = "NULLABLE", description: Optional[str] = None):
...
class BigQueryClient:
    ...
    def __init__(self, credentials, project_id: str, location: Optional[str] = None, config: Optional[Dict[str, Any]] = None):
        """
    ...
    def get_tables_metadata(self, dataset_ids: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            columns_df = self.metadata_extractor.get_column_metadata(
                dataset_ids)
    ...
    def get_dataset_info(self, dataset_ids: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        for dataset_id in dataset_ids:
            try:
                ...
                self.error_tracker.add_dataset_error(dataset_id,
                                                     Exception(f"Dataset {dataset_id} not found"), "not_found")
            except PermissionDenied:
                click.echo(
                    f"   ❌ Permission denied to access dataset '{dataset_id}'")
                self.error_tracker.add_dataset_error(dataset_id,
                                                     Exception(f"Permission denied for dataset {dataset_id}"), "permission")
            except Exception as e:
                click.echo(f"   ❌ Error accessing dataset '{dataset_id}': {e}")
                self.error_tracker.add_dataset_error(
                    dataset_id, e, "unexpected")

        return dataset_info

    def analyze_table_relationships(self, tables_metadata: Dict[str, Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            for dataset_id in dataset_ids:
                try:
                    ...
                    click.echo(
                        f"❌ Error accessing dataset '{dataset_id}': {e}")

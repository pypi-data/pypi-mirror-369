...
def format_single_domain_csv(domain: str, result: dict) -> dict:
    """Format single domain result for CSV output"""
    csv_row = {
        'domain': domain,
        'domain_live': result.get('domain_live', result.get('live', False)),
        'scan_type': result.get('scan_type', 'unknown'),
        'mx_record_count': result.get('mx_record_count', result.get('mx_count', 0)),
        'has_mx_records': result.get('has_mx_records', result.get('has_mx', False)),
        'spf_record_exists': bool(result.get('spf_record')) or result.get('has_spf', False),
        'spf_valid': result.get('spf_valid', False),
        'spf_policy': result.get('spf_policy', ''),
        'dmarc_record_exists': bool(result.get('dmarc_record')) or result.get('has_dmarc', False),
        ...
        'dnssec_security_score': result.get('dnssec_security_score', 'not_analyzed'),
        'dnssec_security_issues_count': len(result.get('dnssec_security_issues', [])),
        'dnssec_security_warnings_count': len(result.get('dnssec_security_warnings', [])),
        'error': result.get('error', '')
    }
    
    # Add SMTP info for active scans
    if result.get('scan_type') == 'active':
        csv_row.update({
            'has_smtp_connection': result.get('has_smtp_connection', False),
            'supports_starttls': result.get('supports_starttls', False),
            'starttls_works': result.get('starttls_works', False)
        })
    
    # Add upstream DNSSEC analysis if available
    upstream_analysis = result.get('upstream_dnssec_analysis')
    if upstream_analysis and not upstream_analysis.get('error'):
        csv_row.update({
            'upstream_zones_analyzed': upstream_analysis.get('total_zones_analyzed', 0),
            'upstream_zones_with_issues': len(upstream_analysis.get('zones_with_issues', [])),
            'upstream_zones_with_warnings': len(upstream_analysis.get('zones_with_warnings', [])),
            'upstream_overall_security_score': upstream_analysis.get('overall_security_score', 'not_analyzed')
        })
    
    return csv_row


def format_multiple_domains_csv(results: dict) -> List[dict]:
    """Format multiple domain results for CSV output"""
    csv_rows = []
    
    # Add summary row first
    summary_row = {
        'domain': '**SUMMARY**',
        'total_domains': results.get('total_domains', 0),
        'successful_scans': results.get('successful_scans', results.get('successful_checks', 0)),
        'failed_scans': results.get('failed_scans', results.get('failed_checks', 0)),
        'scan_type': results.get('scan_type', 'unknown')
    }
    csv_rows.append(summary_row)
    
    # Add individual domain results
    for domain, result in results.get('results', {}).items():
        csv_row = format_single_domain_csv(domain, result)
        csv_rows.append(csv_row)
    
    return csv_rows


def output_csv(data: List[dict]) -> str:
...
def print_results(results, output_format: str = "text"):
    ...
    if output_format.lower() == "csv":
        if isinstance(results, dict) and 'results' in results:
            ...
            csv_data = format_multiple_domains_csv(results)
        else:
            # Single domain result
            domain = results.get('domain', 'Unknown')
            csv_data = [format_single_domain_csv(domain, results)]
        
        csv_output = output_csv(csv_data)
        print(csv_output, end='')  # end='' because CSV already has newlines
        return

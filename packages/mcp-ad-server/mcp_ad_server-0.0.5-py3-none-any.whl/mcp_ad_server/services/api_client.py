...
class BiApiClient:
    ...
    def _prepare_response_fields(
        ...
        if group_key:
            ...
            group_conditional_indicators = self._get_group_conditional_indicators(
                group_key
            )
        ...
        **kwargs,
        ...
        if self.indicator_manager:
            (
                ...
                indicators, app_id, "material_query"
            )
            if invalid_indicators:
                raise ValueError(f"无效指标: {invalid_indicators}")
    ...
    def _get_group_conditional_indicators(self, group_key: str) -> list[str]:
        ...
        # cid相关指标：仅当 group_key ∈ {vp_campaign_id, vp_originality_id}
        if group_key in ["vp_campaign_id", "vp_originality_id"]:
            conditional_indicators.extend(["cid", "cid备注"])

        # 广告创建时间：仅当 group_key = vp_campaign_id
        if group_key == "vp_campaign_id":
            conditional_indicators.append("广告创建时间")

        # 账户余额：仅当 group_key = dt_vp_fx_cid
        if group_key == "dt_vp_fx_cid":
            conditional_indicators.append("账户余额")

        # 优质广告状态：当 group_key ∈ {vp_adgroup_id, vp_campaign_id} 时可能可用
        # 注意：还需要满足 appid ∈ {61,67,82}，但这里先包含，实际可用性由后端决定
        if group_key in ["vp_adgroup_id", "vp_campaign_id"]:
            conditional_indicators.append("优质广告状态")

        # 注意：平均关卡系列只基于appid，在这里不处理

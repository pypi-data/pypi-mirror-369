...
class BitableRecordService(Generic[T]):
    ...
    def update(
        ...
        # 乐观锁校验
        if not force_update:
            # 检查model是否有last_modified_time值
            model_last_modified_time = model.last_modified_time
            if not model_last_modified_time:
                raise ValueError(
                    f"非强制更新时，{FIELD_LAST_MODIFIED_TIME}字段必须有值"
                )
            ...
            current_last_modified_time = current_record.last_modified_time
            if model_last_modified_time != current_last_modified_time:
                raise ValueError(
                    f"记录已被其他用户修改，请重新获取最新数据后再更新。"
                    f"当前记录最后修改时间: {current_last_modified_time}, "
                    f"待更新记录最后修改时间: {model_last_modified_time}"
                )
        ...
        # 将业务对象转换为字段值字典
        fields = model.to_fields(filter_none=filter_none)
    ...
    def save(
        ...
        if model.record_id is not None and model.record_id:
            return self.update(
                model, force_update=force_update, filter_none=filter_none, retry_count=retry_count
            )
        else:

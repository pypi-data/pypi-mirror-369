...
实现DeepSeek API的适配器，支持DeepSeek系列模型。
"""
...
class DeepSeekAdapter(BaseModelAdapter):
    ...
    def _parse_response(self, response_data: Dict[str, Any]) -> ModelResponse:
        ...
        if "error" in response_data:
            ...
            raise Exception(f"DeepSeek API错误: {error.get('message', '未知错误')}")
        ...
        for line in lines:
            if line.startswith('data: '):
                ...
                try:
                    ...
                    if "error" in data:
                        error = data["error"]
                        raise Exception(f"DeepSeek流式API错误: {error.get('message', '未知错误')}")
    ...
    def _get_context_window(self) -> int:
        ...
        return context_windows.get(self.config.model_name, 32768)
...
ModelAdapter.register_adapter(ModelType.DEEPSEEK, DeepSeekAdapter)
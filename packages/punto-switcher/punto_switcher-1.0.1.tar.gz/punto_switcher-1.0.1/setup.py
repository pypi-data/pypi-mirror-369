...
if 'install' in sys.argv and not any(arg.startswith('--editable') for arg in sys.argv):
    try:
        ...
        # Устанавливаем через pipx и добавляем в PATH
        subprocess.run(['pipx', 'install', 'punto-switcher'], check=True)
        subprocess.run(['pipx', 'ensurepath'], check=True)
        print("\nПрограмма успешно установлена. Перезапустите терминал и введите команду: 'punto-switcher'")

    except Exception as e:
        print(f"\nНе удалось добавить в автозагрузку или выполнить установку через pipx: {e}")

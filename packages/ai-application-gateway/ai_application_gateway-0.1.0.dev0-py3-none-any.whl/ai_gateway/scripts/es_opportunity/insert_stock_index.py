...
async def insert_stock_data():
    ...
    for item in stock_items:
        ...
        #     item["exist_anns"] = 1
        # 添加索引操作指令
        bulk_actions.append({"index": {"_index": cfg.stock_index}})
        # 添加文档数据
...
async def main():
    ...
    # 检查索引是否存在，不存在则创建
    if not await opp_es_tool.index_exists(cfg.stock_index):
        await opp_es_tool.create_index(
            index=cfg.stock_index,
            body=stock_index_settings
        )
        logger.info(f"索引 {cfg.stock_index} 创建成功")
    else:
        logger.info(f"索引 {cfg.stock_index} 已存在")

...
# 创建FastMCP实例
fastmcp_http_mount_app = FastMCP()

# 将FastMCP挂载到独立应用
app.mount("/mcp", fastmcp_http_mount_app)  # 挂载子应用
...
# 给 mcp_app 增加，有效
@fastmcp_http_mount_app.tool
async def hello_fastmcp():
    return "hello_fastmcp"

# mount 形式因为是挂载方式，主应用和MCP应用完全独立，互不干扰，所以 以下两种运行方式都可以。

# if __name__ == "__main__":
#     print("FastMCP 测试")
#     print("Main Token:", os.getenv('TOKEN'))
#     # fastmcp_http_mount_app.run(transport="stdio")
#     fastmcp_http_mount_app.run(transport="streamable-http", host="127.0.0.1", port=8087, path="/mcp")

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--host", default=config.listen_host, help="Host to listen on")
    parser.add_argument(
        "--port", type=int, default=config.listen_port_fastmcp_mount, help="Port to listen on"
    )
    parser.add_argument(
    ...
    mcp_asgi_app = fastmcp_http_mount_app.http_app()

...
def bo_dnn_hyperparams_class(x, y,
                       ...
                       parameter=5, bo_iteration_number=15, display_flag=False):
    """
    Bayesian optimization of scikit-learn[MLPClassifier]-based deep neural network (DNN) hyperparameters
    
    Optimize DNN hyperparameters based on Bayesian optimization and cross-validation
    ...
    hidden_layer_sizes_candidates : list
        vector of candidates of hidden_layer_sizes in MLPClassifier
    activation_candidates : list
        vector of candidates of activation in MLPClassifier
    alpha_candidates : numpy.array or list
        vector of candidates of alpha in MLPClassifier
    learning_rate_init_candidates : numpy.array or list
        vector of candidates of learning_rate_init in MLPClassifier    
    parameter : int
    ...
    optimal_hidden_layer_sizes : tuple
        optimized hidden_layer_sizes in MLPClassifier
    optimal_activation : str
        optimized activation in MLPClassifier
    optimal_alpha : float
        optimized alpha in MLPClassifier
    optimal_learning_rate_init : float
        optimized learning_rate_init in MLPClassifier
    """
    ...
    if all_candidate_combinations_df.shape[0] <= doe_number_of_selecting_samples + bo_iteration_number:
        optimal_hidden_layer_sizes, optimal_activation, optimal_alpha, optimal_learning_rate_init = gs_dnn_hyperparams_class(x, y,
                       ...
                       parameter=parameter, display_flag=display_flag)
...
#    all_x = pd.concat([numerical_x, dummy_x], axis=1)
    ...
    cross_validation = StratifiedKFold(n_splits=parameter, random_state=9, shuffle=True)
    # ベイズ最適化の繰り返し
    ...
    #    print('='*10)
        ...
        for i_n, selected_params_idx in enumerate(selected_sample_indexes):
            ...
            model = MLPClassifier(hidden_layer_sizes=hidden_layer_sizes_candidates[int(selected_params.iloc[0])],
            ...
            params_with_score_df.loc[selected_params_idx, 'score'] = accuracy_score(y, estimated_y) # データの保存
        if display_flag:
        ...
        for bo_sample_number in range(bo_number_of_selecting_samples):
            ...
            if bo_regression_method == 'gpr_one_kernel':
                ...
                bo_cross_validation = StratifiedKFold(n_splits=bo_gp_fold_number, random_state=9, shuffle=True) # クロスバリデーションの分割の設定
                bo_acc_cvs = [] # 空の list。カーネル関数ごとに、クロスバリデーション後の r2 を入れていきます
                for index, bo_kernel in enumerate(bo_kernels):
                    ...
                    bo_acc_cvs.append(accuracy_score(bo_y_data, estimated_bo_y_in_cv))
                optimal_bo_kernel_number = np.where(bo_acc_cvs == np.max(bo_acc_cvs))[0][0]  # クロスバリデーション後の r2 が最も大きいカーネル関数の番号
    ...
    best_acc_cv = params_with_score_df_best['score'].copy()
#    print(best_r2cv)
...
def gs_dnn_hyperparams_class(x, y,
                       hidden_layer_sizes_candidates=[(50,), (100,), (50, 10), (100, 10), (50, 50, 10), (100, 100, 10), (50, 50, 50, 10), (100, 100, 100, 10)],
                       activation_candidates=['identity', 'logistic', 'tanh', 'relu'],
                       alpha_candidates=10 ** np.arange(-6, -1, dtype=float),
                       learning_rate_init_candidates=10 ** np.arange(-5, 0, dtype=float),
                       parameter=5, bo_iteration_number=15, display_flag=False):
    """
    Grid search of scikit-learn[MLPClassifier]-based deep neural network (DNN) hyperparameters
    
    Optimize DNN hyperparameters based on Grid search and cross-validation
        ...
        for activation_candidates_indexe in activation_candidates_indexes:
            for alpha_candidate in alpha_candidates:
                for learning_rate_init_candidate in learning_rate_init_candidates:
                    ...
                    all_candidate_combinations.append([hidden_layer_sizes_index, activation_candidates_indexe, alpha_candidate, learning_rate_init_candidate])
                    model = MLPClassifier(hidden_layer_sizes=hidden_layer_sizes_candidates[int(hidden_layer_sizes_index)],
                                         activation=activation_candidates[int(activation_candidates_indexe)],
                                         alpha=alpha_candidate,
                                         learning_rate_init=learning_rate_init_candidate,
                                         random_state=99)
                    estimated_y = cross_val_predict(model, x, y, cv=cross_validation)
                    scores.append(accuracy_score(y, estimated_y))

...
class FileObject:
    ...
    @overload
    def __init__(self, path: str, template: Optional[TemplateObject] = ...) -> None: ...
    @overload
    def __init__(
        self,
        data: bytes,
    ...
    def __init__(
        ...
        self._is_assembly: Optional[dict] = None
    ...
    async def assembly(self):
        ...
        self._is_assembly = {'data': self._data, 'mime-type': self._mime_type, 'templates': template}
...
class GNRequest:
    ...
    def serialize(self, mode: int = 0) -> bytes:
        ...
        if d[3][2] == 'gn:proxy:request-to-real-server':
            d[3][2] = True
    ...
    def deserialize(data: bytes, mode: int = 0) -> 'GNRequest':
        ...
        r = GNRequest(
            ...
            gn_protocol=unpacked.get(3, {}).get(1),
            route=route_
        )
        r.user._data = unpacked[3].get(3, {})
        return r
    @staticmethod
...
class GNResponse:
            ...
            if isinstance(self._files, list):
                for file in self._files:
                    if not isinstance(file, dict):
                        ...
                        assembly_file = await file.assembly()
                        _files.append(assembly_file)
    ...
    def deserialize(data: bytes, mode: int = 0) -> 'GNResponse':
        ...
        r = GNResponse(
            command=unpacked.get(1, 'gn:no-command'),

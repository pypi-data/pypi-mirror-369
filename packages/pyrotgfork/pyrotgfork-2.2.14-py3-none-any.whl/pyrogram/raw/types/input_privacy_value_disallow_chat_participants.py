...
class InputPrivacyValueDisallowChatParticipants(TLObject):  # type: ignore
    ...
    Parameters:
        chats (List of ``int`` ``64-bit``):
            Disallowed chat IDs (either a chat or a supergroup ID, verbatim the way it is received in the constructor (i.e. unlike with bot API IDs, here group and supergroup IDs should be treated in the same way)).

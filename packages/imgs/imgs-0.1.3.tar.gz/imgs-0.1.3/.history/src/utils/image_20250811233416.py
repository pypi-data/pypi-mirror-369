...
def read_image(source: str | Path | bytes | io.IOBase, **kwargs) -> np.ndarray:
    ...
    if isinstance(source, bytes):
        ...
        elif source_str.startswith("data:") or (len(source_str) > 50 and source_str.isalnum()):
...
if __name__ == "__main__":
    ...
    b64_string = ImageWriter.to_base64(gray_cat, ".png")
    data_uri = ImageWriter.to_base64(gray_cat, ".jpg", include_data_uri=True, quality=90)
    print(f"Base64 length: {len(b64_string)}")
    print(f"Data URI preview: {data_uri[:50]}...")

    # Test round-trip base64 conversion
    restored_image = ImageResource.from_base64(b64_string, ".png")
    print(f"Original shape: {gray_cat.shape}, Restored shape: {restored_image.shape}")

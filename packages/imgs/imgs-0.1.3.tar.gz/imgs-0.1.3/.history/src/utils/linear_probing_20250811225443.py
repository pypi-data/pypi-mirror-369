    ...
    *,
    ...
    def extract_batch_features(batch: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if HAS_TORCH and torch is not None and isinstance(features, torch.Tensor):
            ...
            for img in images:
                ...
                if HAS_TORCH and torch is not None and isinstance(feat, torch.Tensor):
...
def load_features_for_training(
    ...
    if format == "arrow":
        ...
        labels = np.array(ds['label']) if 'label' in ds.features else None

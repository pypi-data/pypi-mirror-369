...
class ImageResource:
    ...
    @classmethod
    def from_bytes(cls, data: bytes, extension: str | None = None, **kwargs) -> np.ndarray:
        """
    ...
    def from_webcam(cls, device_id: int = 0, **kwargs) -> np.ndarray:
        ...
        Raises:
            RuntimeError: If webcam device is not available or accessible
        ...
        try:
            ...
            raise RuntimeError(f"Failed to access webcam device {device_id}: {e}") from e
    ...
    @classmethod
    def from_base64(cls, base64_string: str, extension: str = ".png", **kwargs) -> np.ndarray:
        """
        ...
        Examples:
            >>> img = ImageResource.from_base64("iVBORw0KGgoAAAANSUhEUgAA...", ".png")
            >>> img = ImageResource.from_base64("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAA...")
        """
        # Remove data URI prefix if present
        if base64_string.startswith("data:"):
            header, data = base64_string.split(",", 1)
            # Extract format from data URI if not explicitly provided
            if "image/" in header and extension == ".png":
        ...
        try:
            ...
            return cls.from_bytes(image_bytes, extension, **kwargs)
        except Exception as e:
    ...
    def stream_video(
        ...
        """
        yield from iio.imiter(source, plugin=plugin)
    ...
    def get_info(source: str | Path, **kwargs) -> ImageInfo:
        ...
        try:
            ...
            warnings.warn(f"Could not retrieve complete info: {e}", stacklevel=2)
...
class ImageWriter:
    ...
    def to_file(image: np.ndarray, path: str | Path, quality: int | None = None, **kwargs) -> None:
        ...
        # Convert float images to uint8 for common formats
    ...
    def to_base64(
        ...
        **kwargs,
        ...
        Examples:
            >>> b64 = ImageWriter.to_base64(image, ".png")
            >>> data_uri = ImageWriter.to_base64(image, ".jpg", include_data_uri=True, quality=90)
        """
        if quality is not None:
            kwargs["quality"] = quality

        image_bytes = ImageWriter.to_bytes(image, format_ext, **kwargs)
        ...
        if include_data_uri:
            ...
            mime_type = mime_types.get(format_lower, f"image/{format_ext[1:]}")
...
class ImageProcessor:
    ...
    def display(
        ...
        Examples:
            ...
            >>> ImageProcessor.display(img, "My Photo")
            >>> gray = ImageProcessor.to_grayscale(img)
            >>> ImageProcessor.display(gray, "Grayscale", cmap='gray')
        """
    ...
    @staticmethod
    def batch_process(
        images: list[np.ndarray], processor_func: Callable[..., np.ndarray], *args, **kwargs
    ) -> list[np.ndarray]:
        """
...
def read_image(source: str | Path | bytes | io.IOBase, **kwargs) -> np.ndarray:
    ...
    if isinstance(source, bytes):
        ...
        elif source_str.startswith("data:") or (
            len(source_str) > 100 and all(c.isalnum() or c in "+/=" for c in source_str)
        ):
            # Detect base64 data URI or long alphanumeric string
            return ImageResource.from_base64(source_str, **kwargs)

...
def require_cr_approved_before_merge(branch):
    ...
    if qs.filter(status__in=[CRStatusChoices.APPROVED, CRStatusChoices.IMPLEMENTED]).exists():
        return BranchActionIndicator(True)
    return BranchActionIndicator(False, f"Change Request {latest.pk} not approved (status={latest.status})")

...
class ChangeRequestApproveView(PermissionRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        if cr.approver_1 and cr.approver_1 == request.user and cr.approvers_required() == 2 and not cr.approver_2:
        ...
        allow_self_full = settings.PLUGINS_CONFIG.get('netbox_branch_review', {}).get('allow_self_full_approval', True)
        ...
        if required == 1 and cr.approver_1 and cr.approver_1 == request.user:
            ...
            ChangeRequestAudit.objects.create(change_request=cr, user=request.user, action="double_approval_blocked", message="Duplicate single approval attempt")
        ...
        if not cr.approver_1:
            ...
            if required == 1 or (required == 2 and allow_self_full and cr.requested_by_id == request.user.id):
                # Self full approval path (records level2 implicitly)
                if required == 2 and allow_self_full and cr.requested_by_id == request.user.id:
            ...
            ChangeRequestAudit.objects.create(change_request=cr, user=request.user, action="approve_level1")
            if cr.status == "approved" and cr.approver_2_id == request.user.id:
                ChangeRequestAudit.objects.create(change_request=cr, user=request.user, action="approve_level2", message="Implicit self second approval")
            ...
            if cr.approver_1_id == request.user.id:
                messages.warning(request, "Second (peer) approval must be a different user.")
                ChangeRequestAudit.objects.create(change_request=cr, user=request.user, action="double_approval_blocked", message="Attempted second approval by same user")
                return redirect(cr.get_absolute_url())
            cr.approver_2 = request.user
            cr.approver_2_at = now
            cr.status = "approved"
            messages.success(request, "Second approval recorded. Change request is now approved.")
            ChangeRequestAudit.objects.create(change_request=cr, user=request.user, action="approve_level2")
        else:
            messages.info(request, "Change request already approved.")
            ChangeRequestAudit.objects.create(change_request=cr, user=request.user, action="double_approval_blocked", message="Approval after fully approved")
...
class ChangeRequestMergeView(PermissionRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            ChangeRequestAudit.objects.create(change_request=cr, user=request.user, action="merge")
...
class ChangeRequestPeerReviewView(PermissionRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        if cr.status in ("approved", "implemented"):
            ...
            return redirect(cr.get_absolute_url())
        ChangeRequestAudit.objects.create(change_request=cr, user=request.user, action="peer_review")
...
class ChangeRequestRevokeView(PermissionRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        revoked_any = False
        if cr.approver_2:
            ChangeRequestAudit.objects.create(change_request=cr, user=request.user, action="revoke_level2")
            cr.approver_2 = None
            cr.approver_2_at = None
            revoked_any = True
        if cr.approver_1:
            ChangeRequestAudit.objects.create(change_request=cr, user=request.user, action="revoke_level1")
            cr.approver_1 = None
        ...
        if revoked_any:
            cr.status = "pending"
            ChangeRequestAudit.objects.create(change_request=cr, user=request.user, action="revoke_full", message="Approvals reset to pending")
            cr.save()

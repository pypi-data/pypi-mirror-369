...
class ClassDefinitionTestCase(ClassCacheClearingTestCase):
    ...
    def setUp(self):
        ...
        self.alias = miniamf.ClassAlias(Spam, defer=True)
...
class EncoderTestCase(ClassCacheClearingTestCase, EncoderMixIn):
    """
    Tests the output from the AMF3 L{Encoder<miniamf.amf3.Encoder>} class.
    """

    amf_type = miniamf.AMF3
    ...
    def test_byte_array(self):
        self.assertEncoded(amf3.ByteArray(b'hello'), b'\x0c\x0bhello')
    ...
    def test_funcs(self):
        ...
        for f in (chr, lambda x: x, x, miniamf, ''.startswith):
            self.assertRaises(miniamf.EncodeError, self.encode, f)
    ...
    def test_iterate(self):
        ...
        self.assertEqual(next(self.encoder),
                         b'\x06\r\xc6\x92\xc3\xb8\xc3\xb8')

        self.assertRaises(StopIteration,
                          lambda: next(self.encoder))
...
class DecoderTestCase(ClassCacheClearingTestCase, DecoderMixIn):
    """
    Tests the output from the AMF3 L{Decoder<miniamf.amf3.Decoder>} class.
    """
    ...
    def test_undefined(self):
        self.assertDecoded(miniamf.Undefined, b'\x00')
    ...
    def test_mixed_array(self):
        ...
        self.assertTrue(isinstance(y, miniamf.MixedArray))
        self.assertEqual(y, {
            'a': u'a',
            'b': u'b',
            'c': u'c',
    ...
    def test_xmlstring(self):
        self.buf.append(b'\x0b\x33<a><b>hello world</b></a>')
        ...
        self.buf.truncate()
        self.buf.seek(0, 0)
        self.buf.append(b'\x0b\x00')
        y = self.decoder.readElement()

        self.assertEqual(x, y)

    def test_xmlstring_references(self):
        self.buf.append(b'\x0b\x33<a><b>hello world</b></a>\x0b\x00')
        x = self.decoder.readElement()
        y = self.decoder.readElement()
    ...
    def test_byte_array(self):
        self.assertDecoded(amf3.ByteArray(b'hello'), b'\x0c\x0bhello')
    ...
    def test_timezone(self):
        ...
        self.buf.append(b'\x08\x01Br>\xc6\xf5w\x80\x00')
...
class ObjectDecodingTestCase(ClassCacheClearingTestCase, DecoderMixIn):
    ...
    def test_object_references(self):
        self.buf.append(b'\x0a\x23\x01\x03a\x03b\x06\x09spam\x04\x05')
    ...
    def test_static(self):
        ...
        self.buf.append(b'\x0a\x13\x0fabc.xyz\x09spam\x06\x09eggs')
...
class DataOutputTestCase(unittest.TestCase, EncoderMixIn):
    ...
    def test_multi_byte(self):
        ...
        self.assertEqual(self.buf.getvalue(), 'this is a test'.encode('utf-8'))
        self.buf.truncate()
        self.buf.seek(0, 0)
...
class ComplexEncodingTestCase(unittest.TestCase, EncoderMixIn):
    ...
    def setUp(self):
        ...
        miniamf.register_class(self.TestObject, 'test_complex.test')
        miniamf.register_class(self.TestSubObject, 'test_complex.sub')
...
class ExceptionEncodingTestCase(ClassCacheClearingTestCase, EncoderMixIn):
    ...
    def test_exception(self):
        ...
        self.assertEqual(
            ...
            b'\x0ffoo bar\tname\x06\x13Exception\x01'
        )
...
class ByteArrayTestCase(unittest.TestCase):
    ...
    def test_read_context(self):
        ...
        self.assertRaises(miniamf.ReferenceError, b.readObject)

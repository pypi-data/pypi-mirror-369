...
fundamental parts of Mini-AMF and the test suite for it is big so it
makes sense to have them in one file.
...
from miniamf.tests.util import ClassCacheClearingTestCase, Spam, get_fqcn
...
class GetEncodableAttributesTestCase(unittest.TestCase):
    ...
    def test_static(self):
        ...
        self.assertEqual(attrs, {'foo': 'bar', 'bar': miniamf.Undefined})
...
class CompilationInheritanceTestCase(ClassCacheClearingTestCase):
    ...
    def _register(self, alias):
        miniamf.CLASS_CACHE[get_fqcn(alias.klass)] = alias
        miniamf.CLASS_CACHE[alias.klass] = alias
...
class RegisterClassTestCase(ClassCacheClearingTestCase):
    """
    Tests for L{miniamf.register_class}
    """
...
class UnregisterClassTestCase(ClassCacheClearingTestCase):
    """
    Tests for L{miniamf.unregister_class}
    """
    ...
    def test_remove(self):
        ...
        ret = miniamf.unregister_class('foo')

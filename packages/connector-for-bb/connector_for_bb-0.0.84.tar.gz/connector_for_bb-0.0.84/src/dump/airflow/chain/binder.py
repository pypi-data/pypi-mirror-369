...
def stats_metric(task_id: str, status: str, *args):
    ...
    metric_cpu_times_user = (
        f"airflow_metric_{task_id}_cpu_times_user_{status} ({cpu_times.user})"
    )
    metric_cpu_times_system = (
        f"airflow_metric_{task_id}_cpu_times_system_{status} ({cpu_times.system})"
    )
    metric_mem_info_rss = (
        f"airflow_metric_{task_id}_mem_info_rss_{status} ({mem_info.rss})"
    )
    metric_mem_info_vms = (
        f"airflow_metric_{task_id}_mem_info_vms_{status} ({mem_info.vms})"
    )
    ...
    Stats.gauge(
        f"metric.{task_id}.io_counters_read_bytes",
        io_counters.read_bytes,
        tags=tag_into,
    )
    Stats.gauge(
        f"metric.{task_id}.io_counters_write_bytes",
        io_counters.write_bytes,
        tags=tag_into,
    )

    print(
        f"""{metric_cpu_times_user}\n
        {metric_cpu_times_system}\n
        {metric_mem_info_rss}\n
        {metric_mem_info_vms}\n
...
def chain_with_intermediates(tasks):
    ...
    for task in tasks:
        ...
        pre = PythonOperator(
            task_id=f"airflow_monitoring_{task.task_id}_start",
            python_callable=stats_metric,
            op_args=[task.task_id, "start"],
        )

        post = PythonOperator(
            task_id=f"airflow_monitoring_{task.task_id}_finish",
            python_callable=stats_metric,

...
async def get_latest_version(package_name: str, mode: str) -> Optional[Version]:
    if is_frozen() or "freeze" == mode:
        return await get_latest_frozen_version()
    else:
        return await get_latest_pypi_version(package_name)


async def get_latest_frozen_version() -> Optional[Version]:
    """Get the latest version of a binary from the docs site."""

    url = f"{ENV.HCLI_RELEASE_URL}/release/version.json"

...
class Credentials(BaseModel):
    ...
    def create_credentials(
        cls, name: str, credential_type: str, token: str, email: Optional[str] = None
    ) -> "Credentials":
        """Create a new API key credentials."""
        now = datetime.utcnow().isoformat() + "Z"

        return cls(name=name, type=credential_type, email=email, token=token, created_at=now, last_used=now)

...
async def install_key(token: str | None, name: str | None, key_name: str | None, set_default: bool) -> None:
    ...
    try:
        ...
        if not key_name:
            key_name = (
                await safe_ask_async(questionary.text("Key name/description for label (optional)", default="")) or None
            )

        # Install the API key as a new credentials
        source = await auth_service.add_api_key_credentials(name, token)

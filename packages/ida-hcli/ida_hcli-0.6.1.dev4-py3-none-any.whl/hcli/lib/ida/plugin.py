...
def _convert_api_plugin_to_local(api_plugin: APIPlugin) -> Plugin:
    ...
    metadata = Metadata(
        repository_name=api_plugin.metadata.repository_name,  # type: ignore[attr-defined]
        repository_description=api_plugin.metadata.repository_description,  # type: ignore[attr-defined]
        repository_owner=api_plugin.metadata.repository_owner,  # type: ignore[attr-defined]
        entry_point=api_plugin.metadata.entryPoint,  # type: ignore[attr-defined]
        install=api_plugin.metadata.install,  # type: ignore[attr-defined]
    )
    ...
    if api_plugin.categories:
        for cat in api_plugin.categories:
            categories.append(
                PluginCategory(id=cat.id, name=cat.name, slug=cat.slug, description=cat.description, icon=cat.icon)  # type: ignore[attr-defined]
            )

    return Plugin(
        id=api_plugin.id,  # type: ignore[attr-defined]
        owner=api_plugin.owner,  # type: ignore[attr-defined]
        name=api_plugin.name,  # type: ignore[attr-defined]
        slug=api_plugin.slug,
        url=api_plugin.url,
        metadata=metadata,
        updated_at=api_plugin.updatedAt,  # type: ignore[attr-defined]
        categories=categories,
        disabled=api_plugin.disabled or False,  # type: ignore[attr-defined]
    )
...
async def get_plugins() -> List[Plugin]:
    ...
    if _PLUGINS_CACHE is None:
        ...
        _PLUGINS_CACHE = [_convert_api_plugin_to_local(p) for p in results.hits]  # type: ignore[attr-defined]

...
from hcli.lib.util.updater import get_default_updater, UpdateError
...
@click.option(
    ...
    "--mode",
    default="auto",
    type=click.Choice(["auto", "pypi", "freeze"]),
...
@click.option("--auto-install", is_flag=True, help="Automatically install update if available (for frozen binaries).")
@click.option("--include-prereleases", is_flag=True, help="Include pre-release versions when checking GitHub.")
@async_command
async def update(force: bool, mode: str, check_only: bool, auto_install: bool, include_prereleases: bool) -> None:
    ...
    if mode == "auto":
        if is_frozen():
            mode = "freeze"  # Use GitHub for frozen binaries
        else:

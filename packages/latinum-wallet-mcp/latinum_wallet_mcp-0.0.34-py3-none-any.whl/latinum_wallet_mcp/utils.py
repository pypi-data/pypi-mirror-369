...
def ensure_claude_mcp_config():
    try:
        # macOS Claude Desktop config path
        ...
        # If latinum_wallet_mcp already has a command, do nothing
        existing = cfg["mcpServers"].get("latinum_wallet_mcp")
        if isinstance(existing, dict) and existing.get("command"):
            logging.info("ℹ️ Claude config already has latinum_wallet_mcp. Skipping changes.")
            return

        # Build default entry
        command = shutil.which("latinum-wallet-mcp")
        if not command:
            server_stdio = pathlib.Path(__file__).with_name("server_stdio.py")
            cfg["mcpServers"]["latinum_wallet_mcp"] = {
        ...
        # Atomic write
        tmp_fd, tmp_path = tempfile.mkstemp(dir=cfg_dir, prefix="claude_cfg_", text=True)
        with os.fdopen(tmp_fd, "w", encoding="utf-8") as f:
            json.dump(cfg, f, indent=2, ensure_ascii=False)
            f.write("\n")
        os.replace(tmp_path, cfg_path)

        logging.info("✅ Added latinum_wallet_mcp config to Claude at: %s", cfg_path)

...
async def get_credit(full_name: str, email: str, team: str) -> dict:
    """
    Validate inputs, call facilitator /hackathon API, and return its result.
    Sends the wallet's solana_pubkey along with the provided details.
    """
    try:
        # 1) Local validation
        ...
        payload = {
            ...
            "team": team.strip(),
            # Use your existing wallet public key
            "solana_pubkey": str(public_key),
        }

        # 2) Call facilitator API
        async with httpx.AsyncClient(timeout=10) as client:
            resp = await client.post("https://facilitator.latinum.ai/api/hackathon", json=payload)
        ...
        # 3) Map facilitator response to tool shape
        if resp.status_code == 200 and isinstance(data, dict) and data.get("status") == "success":
            # Pass through the welcome message from API if present
            return {
                "success": True,
                "message": data.get("message") or f"Welcome {payload['full_name']}! You're registered."
            }

        # Known failure path from API (e.g., duplicates or validation)
        if isinstance(data, dict) and data.get("status") == "failure":
            return {"success": False, "message": data.get("error") or "Registration failed."}

        # Unexpected HTTP or body
        return {
            "success": False,
...
__all__ = ["build_mcp_wallet_server", "get_signed_transaction", "get_wallet_info", "get_credit"]

...
class BlockDiagQuadForm(PSProblem):
    ...
    .. math::
        ...
        f_i(x_i) = \frac{1}{2} (x_i - c_i)^T H_i (x_i - c_i)
    ...
    def __init__(
        self, n, r, overlap=0, center=None, x0=None, mat_gen=None, noise_wrapper=None
    ) -> None:
        """
        f_i(x_i) = 0.5 * (x_i - c_i)^T * H_i * (x_i - c_i)

        x_i and x_{i+1} overlap with `overlap` dim

        each f_i is n-dim func.
        There are totally r funcs.
        """
        if mat_gen is None:

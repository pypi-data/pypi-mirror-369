...
class Barrier:
    ...
    def Nion_interp(self, Mv,deltaR):
        try:
            Nion_arr = np.load(f'.Nion_Nxi_init/Nion_arr_Mv_{Mv:.3f}at_z={self.z}_A{self.A2byA1}_k{self.kMpc_trans}_alpha{self.alpha}_beta{self.beta}.npy')
        except FileNotFoundError:
            os.makedirs('.Nion_Nxi_init', exist_ok=True)
            nion_pure = self.Nion_Pure(Mv, self.deltaR_interp)
            np.save(f'.Nion_Nxi_init/Nion_arr_Mv_{Mv:.3f}at_z={self.z}_A{self.A2byA1}_k{self.kMpc_trans}_alpha{self.alpha}_beta{self.beta}.npy', nion_pure)
            Nion_arr = np.load(f'.Nion_Nxi_init/Nion_arr_Mv_{Mv:.3f}at_z={self.z}_A{self.A2byA1}_k{self.kMpc_trans}_alpha{self.alpha}_beta{self.beta}.npy')
    ...
    def N_xi_interp(self, Mv, deltaR):
        try:
            Nxi_arr = np.load(f'.Nion_Nxi_init/Nxi_arr_Mv_{Mv:.3f}at_z={self.z}_A{self.A2byA1}_k{self.kMpc_trans}_alpha{self.alpha}_beta{self.beta}.npy')
        except FileNotFoundError:
            os.makedirs('.Nion_Nxi_init', exist_ok=True)
            nxi_pure = self.Nxi_Pure(Mv, self.deltaR_interp)
            np.save(f'.Nion_Nxi_init/Nxi_arr_Mv_{Mv:.3f}at_z={self.z}_A{self.A2byA1}_k{self.kMpc_trans}_alpha{self.alpha}_beta{self.beta}.npy', nxi_pure)
            Nxi_arr = np.load(f'.Nion_Nxi_init/Nxi_arr_Mv_{Mv:.3f}at_z={self.z}_A{self.A2byA1}_k{self.kMpc_trans}_alpha{self.alpha}_beta{self.beta}.npy')
    ...
    def Nion_ST(self):
        ...
        mslice = np.logspace(np.log10(self.M_min), np.log10(cosmo.M_vir(0.61,1e8,self.z)), 200)
        ans = 0
        for i in range(len(mslice)-1):
    ...
    def Modify_Ratio(self):
        try:
            ratio = np.load(f'.Nion_Nxi_init/ratio_at_z{self.z}_A{self.A2byA1}_k{self.kMpc_trans}_alpha{self.alpha}_beta{self.beta}.npy')
        except FileNotFoundError:
            os.makedirs('.Nion_Nxi_init', exist_ok=True)
            ratio = self.Nion_ST() / self.Nion_PS()
            np.save(f'.Nion_Nxi_init/ratio_at_z{self.z}_A{self.A2byA1}_k{self.kMpc_trans}_alpha{self.alpha}_beta{self.beta}.npy', ratio)
        return ratio

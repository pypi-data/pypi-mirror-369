...
Omegak=0.0
Omegar=0.0
H0u = 67.4*u.km*u.s**-1*u.Mpc**-1
mHu = 1.6726e-27*u.kg   #the mass of a hydrogen Unit: kg
rhocrit = (3*H0u.to(u.s**-1)**2/(8*np.pi*G)).to(u.Msun / u.Mpc**3) # M_SUN/Mpc^3 
nHu = rhocrit.to(u.kg/u.cm**3)*omegab/mHu
nH = nHu.value
...
class MassFunctions:
    ...
    def PowerSpectrum_Interp_Set(self):
        ...
        Pk_arr = self.spc.MatterPower(self.ps_interp_range, z = 0.0)
    ...
    def Sigma2_Interp_Set(self):
        ...
        self.sigma2m_interpolation=interp1d(np.log10(self.m_interp_range), np.log10(sig), kind='cubic')
        self.sigma2_interpolation_completed = True

    def Dsigma2dm_Interp_Set(self):
        dsig2dm = self.dsigma2_dm(self.m_interp_range)
        self.dsigma2_dm_interpolation=interp1d(np.log10(self.m_interp_range), dsig2dm, kind='cubic')
    ...
    def sigma2(self,M):
        ...
        k_grid_2 = np.logspace(1,8,150)
        k_grid = np.concatenate([k_grid_1, k_grid_2])
        int_ans = np.zeros_like(M)
        for i in range(len(k_grid)-1):
            int_temp,int_err = quad_vec(self.dsigma2_dlnk,np.log(k_grid[i]),np.log(k_grid[i+1]), args=(M,),epsrel=precision,limit=200)
            int_ans += int_temp
            # if np.any(int_err > precision):
            #     print(f"Warning: Integration error in segment {i} is high for some masses.")
            #     print("High errors:", int_err[int_err > precision])
        return int_ans
    ...
    def dsigma2_dm(self,M):
        ...
        int_ans = np.zeros_like(M)
        for i in range(len(k_grid)-1):
            int_temp,int_err = quad_vec(self.dsigma2_dlnk_dm,np.log(k_grid[i]),np.log(k_grid[i+1]), args=(M,),epsrel=precision,limit=200)
            int_ans += int_temp
            # if np.any(int_err > precision):
            #     print(f"Warning: Integration error in segment {i} is high for some masses.")
            #     print("High errors:", int_err[int_err > precision])
    ...
    def rhosfr(self,T1,T2):
        ...
        ans = 0
        for i in range(len(mslice)-1):
            ans += quad(self.rhosfrdiff,np.log(mslice[i]),np.log(mslice[i+1]), epsrel=1e-6,limit=200)[0]
        return ans

...
class Barrier:
    ...
    def Calcul_deltaVM_EQ(self,deltaR,Mv):
        return self.Modify_Ratio()*self.Nion(Mv,deltaR) - (1+self.nrec)*self.N_H(deltaR)
    ...
    def Calcul_deltaVM_Minihalo_EQ(self,deltaR,Mv):
        return self.Modify_Ratio()*self.Nion(Mv,deltaR) - (1+self.nrec)*self.N_H(deltaR) - self.N_xi(Mv,deltaR)

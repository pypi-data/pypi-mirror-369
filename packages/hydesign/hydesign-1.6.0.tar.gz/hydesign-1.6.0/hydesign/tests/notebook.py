...
class Notebook:
    pip_header = """# Install hydesign if needed
import importlib
if not importlib.util.find_spec("hydesign"):
    ...
    def replace_include_tag(self):
        ...
        for cell in self.nb["cells"]:
            if (
                ...
                and "%%include" in cell["source"][0]
            ):
                filename = pjoin(
                    dirname(self.filename),
                    cell["source"][0].replace("%%include", "").strip(),
                )
        ...
        try:
            ...
            with contextlib.redirect_stdout(StringIO()):
                with contextlib.redirect_stderr(StringIO()):
                    ...
                    initpy.write_text("")
                    if f"hydesign.tests.test_files.tmp.{name}" in sys.modules:
                        importlib.reload(
                            sys.modules[f"tmp.hydesign.tests.test_files.tmp.{name}"]
                        )
                    else:
                        importlib.import_module(f"hydesign.tests.test_files.tmp.{name}")
                    p.unlink()
    ...
    def check_pip_header(self):
        ...
        if code[0].strip() != self.pip_header:
            ...
            self.save()
            raise Exception(
                """pip install header was not present in %s.
It has now been auto insert. Please check the notebook and commit the changes"""
                % os.path.abspath(self.filename)
            )

    def remove_empty_end_cell(self):
        while self.cells[-1]["cell_type"] == "code" and all(
            [l.strip() == "" for l in self.cells[-1]["source"]]
        ):
...
if __name__ == "__main__":
    ...
    nb = Notebook(
        os.path.dirname(hydesign.__file__) + "/../docs/notebooks/Quickstart.ipynb"
    )
    nb.check_code()
    nb.check_links()

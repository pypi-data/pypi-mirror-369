...
from mathmaker.lib.tools import ext_dict, load_user_config
...
def config_dbglogger(sd):
    ...
    """
    d = ext_dict(load_user_config('debug_conf', sd)).flat()
    raw_handlers = logging.getLogger('raw').handlers
    for loggername, level in d.items():
        ...
        if loggername in ['dbg.db', 'dbg.db_lock', 'dbg.db_timestamp',
                          'dbg.Exercise.init']:
            if raw_handlers:
                lg.addHandler(raw_handlers[0])
                lg.propagate = False
            else:
                # on ne fait rien plutôt que planter
                lg.warning("No raw handler to attach for dbg logger")
...
def init():
    ...
    mainlogger, log_dir, cli_settings = configure_logging('cli')
    mainlogger.info(f"Starting mathmaker {__version__}...")
    ...
    mc_belts = load_user_config('mc_belts', settingsdir, fmt='.json')
    mc_titles = load_user_config('mc_titles', settingsdir, fmt='.json')

    CONFIG = load_user_config('user_config', settingsdir)
    ...
    outputdir = os.path.join(safe_home(), config_path)

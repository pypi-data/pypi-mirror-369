...
if os.name == 'posix':
    FINDSCU = _which(
        'findscu',
        [EXTDIR, '/usr/local/dcm4che3/bin', '/usr/local/dcm4che/bin', '/usr/local/bin']
    )
    if not FINDSCU:
    ...
    FINDSCU = _which(
        'findscu',
        [EXTDIR, join(r'c:/', 'dcm4che3', 'bin'), join('dcm4che', 'bin'), join('dcm4che3', 'bin'), 'bin']
    )
    if not FINDSCU:
...
CONTEXTS = join(EXTDIR, 'store-tcs.properties')

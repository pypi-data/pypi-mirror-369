...
class WordSearchFormatter(Formatter):
    ...
    PDF_FONT_SIZE_XXL = 16  # was 18
    PDF_FONT_SIZE_XL = 12   # was 15
    PDF_FONT_SIZE_L = 10    # was 12
    PDF_FONT_SIZE_M = 7   # was 9
    PDF_FONT_SIZE_S = 4     # was 5
    PDF_PUZZLE_WIDTH = 4.5  # was 7, now fits better on 6x9 inch page
    ...
    def write_pdf_file(
        ...
        pdf = FPDF(orientation="P", unit="in", format=(6, 9))  # Set PDF size to 6x9 inches
...
def draw_page_title(title: str, pdf: FPDF, formatter: WordSearchFormatter, page_number: int | None = None):
    pdf.set_font("Helvetica", "B", formatter.PDF_FONT_SIZE_XXL)
    pdf.cell(pdf.epw, 0.25, title, new_y="NEXT", align="C", center=True)
    # Draw page number at top right if provided, a bit lower than before
    if page_number is not None:
        x, y = pdf.get_x(), pdf.get_y()
        # Move the page number further down (e.g., 0.4in from the top)
        pdf.set_xy(pdf.epw - 0.5, 0.4)
        pdf.set_font("Helvetica", "", formatter.PDF_FONT_SIZE_L)
        pdf.cell(0, 0.25, str(page_number), align="R")
        pdf.set_xy(x, y)
    pdf.ln(0.5)  # Increased vertical space after the title
...
def draw_puzzle_page(
    ...
    # insert the title (use game.title if available, else fallback)
    page_title = getattr(game, 'title', None) or ("WORD SEARCH (SOLUTION)" if solution else "WORD SEARCH")
    page_number = getattr(game, 'page_number', None)
    draw_page_title(page_title, pdf, formatter, page_number)

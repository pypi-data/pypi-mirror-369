...
from aionetworking.utils import makedirs, set_task_name
from aionetworking.types.logging import LoggerType

from typing import Any, ClassVar, AnyStr, Protocol
...
class ManagedFile:
    ...
    async def wait_writes_done(self) -> None:
        ...
        queue_done_task = asyncio.create_task(self._queue.join())
        done, pending = await asyncio.wait([queue_done_task, self._task], return_when=asyncio.FIRST_COMPLETED)
        for d in done:
            if exc := d.exception():
    ...
    async def _write_items_from_queue(self, f):
        ...
        try:
            ...
            asyncio.get_running_loop().call_soon(self._task_done, len(futs))

...
from typing import Optional, Generator, List, Tuple, Callable, Protocol
...
class BaseReceiver(ReceiverProtocol, Protocol):
    ...
    async def close(self):
        if self.close_tasks:
            await asyncio.wait([asyncio.create_task(coro()) for coro in self.close_tasks])
    ...
    async def serve_until_close_signal(self, stop_event: asyncio.Event = None,
        ...
        done, pending = await asyncio.wait([asyncio.create_task(stop_event.wait()), asyncio.create_task(restart_event.wait())],
...
class BaseServer(BaseReceiver, Protocol):
    ...
    def __post_init__(self) -> None:
        ...
        self.protocol_factory = replace(self.protocol_factory, quiet=self.quiet)

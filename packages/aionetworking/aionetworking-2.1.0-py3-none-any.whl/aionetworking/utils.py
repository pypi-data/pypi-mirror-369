...
from aionetworking.compatibility_os import is_wsl, is_aix, WindowsProactorEventLoopPolicy, WindowsSelectorEventLoopPolicy
from dataclasses import dataclass, fields, MISSING
from functools import wraps

from pathlib import Path
from typing import Sequence, Callable, List, AnyStr, Tuple, Union, AsyncGenerator, Any, TYPE_CHECKING, Optional, Protocol
...
def wait_on_capsys(capsys) -> Tuple[str, int]:
    out = None
    while not out or "Serving" not in out:
        out = capsys.readouterr().out
        if not out or "Serving" not in out:
            time.sleep(0.1)

...
def ssl_cert_time_to_datetime(timestamp: str) -> datetime.datetime:
    return datetime.datetime.fromtimestamp(cert_time_to_seconds(timestamp), datetime.timezone.utc)


def check_ssl_cert_expired(expiry_time: datetime.datetime, warn_before_days: int) -> Optional[int]:
    expires_in = (expiry_time - datetime.datetime.now(datetime.timezone.utc)).days

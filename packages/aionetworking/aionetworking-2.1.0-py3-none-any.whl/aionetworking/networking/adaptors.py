...
from aionetworking.utils import set_task_name
...
from typing import Any, Callable, Generator, Dict, Sequence, Type, AsyncIterator, Optional, Protocol
...
class SenderAdaptor(BaseAdaptorProtocol):
    ...
    def __getattr__(self, item):
        if item.startswith('_'):            # Required for pytest which looks for __attrs_attrs__
            return
        ...
        try:
            async for msg_obj in msgs:
                if not self.action.filter(msg_obj):
                    task = asyncio.create_task(self._process_msg(msg_obj))

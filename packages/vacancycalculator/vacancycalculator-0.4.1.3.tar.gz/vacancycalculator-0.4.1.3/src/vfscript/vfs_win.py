...
class KeyAreaSeqWidget(QWidget):
    """Viewer para outputs/dump/key_area_{i}.dump + panel de distribución (results.csv)."""
    def __init__(self, parent=None, pattern: str = "outputs/dump/key_area_{i}.dump"):
        super().__init__(parent)
        self.pattern = pattern
        self._results_df = None  # cache df results

        # --- Barra superior de navegación por key_area_i.dump ---
        top = QWidget()
        top_l = QHBoxLayout(top)
        self.idx = QSpinBox()
        ...
        # --- Panel de distribución de poros (results.csv) ---
        dist_panel = QWidget()
        ...
        self.results_btn = QPushButton("Cargar results.csv")
        self.results_path = QLineEdit(str(Path.cwd() / "outputs" / "csv" / "results.csv"))
        self.results_path.setReadOnly(True)
        row_l.addWidget(self.results_btn)
        row_l.addWidget(self.results_path, 1)
        dist_l.addWidget(row)

        # Tabla de vista (primeras filas o todo si querés)
        self.table_results = QTableWidget()
        self.table_results.setMinimumHeight(160)
        dist_l.addWidget(QLabel("Vista de resultados (archivo, surface_area, filled_volume, predicted_vacancy):"))
        dist_l.addWidget(self.table_results)

        # Canvas para gráficos (2x2)
        self.fig_dist = plt.figure(figsize=(6, 5))
        self.canvas_dist = FigureCanvas(self.fig_dist)
        dist_l.addWidget(self.canvas_dist)

        # Conectar señales
        self.btn_prev.clicked.connect(lambda: self._step(-1))
        self.btn_next.clicked.connect(lambda: self._step(+1))
        self.btn_load.clicked.connect(self._load_idx)
        self.results_btn.clicked.connect(self.load_results_csv)

        # --- Layout raíz de la pestaña ---
        root_l = QVBoxLayout(self)
        root_l.addWidget(top)
        root_l.addWidget(dist_panel)

        # Intentar autodescubrir primer key_area existente
        self._auto_seed_index()

    # ----------- helpers key_area viewer -----------
    ...
    # ----------- panel results.csv -----------
    def _normalize_results_df(self, df: pd.DataFrame) -> pd.DataFrame:
        # normaliza nombres
        normmap = {c.lower().strip(): c for c in df.columns}
        def getcol(*cands):
            for c in cands:
                if c in normmap:
                    return normmap[c]
        ...
        col_archivo = getcol("archivo", "file", "path")
        col_area    = getcol("surface_area", "area", "surfacearea")
        col_vol     = getcol("filled_volume", "volume", "filledvolume")
        col_vac     = getcol("predicted_vacancy", "vacancy", "vacancys", "vacancys_est", "predicted")
        ...
        out = out.dropna(subset=["surface_area", "filled_volume", "predicted_vacancy"])
        return out

    def load_results_csv(self):
        try:
            path = Path(self.results_path.text().strip())
            if not path.exists():
                # fallback por si el usuario lo movió
            ...
            df = pd.read_csv(path)
            df = self._normalize_results_df(df)
            self._results_df = df

            # ---- Poblar tabla con columnas clave ----
            ...
            for i in range(len(df)):
                for j, col in enumerate(cols):
                    self.table_results.setItem(i, j, QTableWidgetItem(str(df.iat[i, df.columns.get_loc(col)])))
            self.table_results.resizeColumnsToContents()

            # ---- Dibujar gráficos (2x2) ----
            self.fig_dist.clf()
            axs = self.fig_dist.subplots(2, 2)

            # Hist surface_area
            axs[0, 0].hist(df["surface_area"].values, bins="auto")
            axs[0, 0].set_title("Distribución: surface_area")
            axs[0, 0].set_xlabel("surface_area"); axs[0, 0].set_ylabel("Frecuencia")

            # Hist filled_volume
            axs[0, 1].hist(df["filled_volume"].values, bins="auto")
            axs[0, 1].set_title("Distribución: filled_volume")
            axs[0, 1].set_xlabel("filled_volume"); axs[0, 1].set_ylabel("Frecuencia")

            # Conteo predicted_vacancy (redondeado a entero)
            vac_counts = df["predicted_vacancy"].round(0).astype(int).value_counts().sort_index()
            axs[1, 0].bar(vac_counts.index.astype(str), vac_counts.values)
            axs[1, 0].set_title("Conteo: predicted_vacancy")
            axs[1, 0].set_xlabel("predicted_vacancy"); axs[1, 0].set_ylabel("Cantidad de poros")

            # Scatter SA vs FV (tamaño ~ vacancias)
            s = df["predicted_vacancy"].values
            scale = 15 / (np.nanpercentile(s, 90) + 1e-9)
            size = 10 + scale * s
            axs[1, 1].scatter(df["surface_area"].values, df["filled_volume"].values, s=size)
            axs[1, 1].set_title("surface_area vs filled_volume (size ~ vacancias)")
            axs[1, 1].set_xlabel("surface_area"); axs[1, 1].set_ylabel("filled_volume")

            self.fig_dist.tight_layout()
            self.canvas_dist.draw()

        except Exception as e:
            QMessageBox.critical(self, "Error al cargar results.csv", str(e))
...
class SettingsWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.showMaximized()
        self.setWindowTitle("VacancyFinder-SiMAF   0.4.1.4")
        ...
        # --- Tab Key Areas (ahora con distribución) ---
        self.viewer2 = KeyAreaSeqWidget()
        tabs.addTab(self.viewer2, "Metrics")
        # --- Tab Key Areas (original) ---

        # --- Tab Key Areas (solo 3D/2D, sin CSV ni histogramas) ---
        self.viewer3d = KeyArea3DOnlyWidget()
        tabs.addTab(self.viewer3d, "Key Areas")
    ...
    def show_total_vacancies(self):
        ...
        try:
            ...
            total_int = int(np.ceil(total))  # redondeo hacia arriba
    ...
    def run_vacancy_analysis(self):
        ...
        try:
            ...
            # Calcular total vacancias y añadirlo al log
            try:
                path = Path.cwd() / 'outputs' / 'csv' / 'results.csv'
                if path.exists():
                    df = pd.read_csv(path)
                    ...
                    if col:
                        vals = pd.to_numeric(df[col], errors='coerce').fillna(0.0)
                        total_int = int(np.ceil(vals.sum()))
                        log_text += f"\n\n VACANCIES: {total_int}\n"
                    else:
                        log_text += "\n[INFO] No se encontró columna de vacancias en results.csv\n"
                else:
                    log_text += "\n[INFO] No existe outputs/csv/results.csv\n"
            except Exception as e:
                log_text += f"\n[ERROR] Fallo al calcular total de vacancias: {e}\n"
...
class KeyArea3DOnlyWidget(QWidget):
    """Viewer igual a Key Areas pero SIN tabla ni histogramas (solo navegación + 3D/2D)."""
    def __init__(self, parent=None, pattern: str = "outputs/dump/key_area_{i}.dump"):
        super().__init__(parent)
        self.pattern = pattern
        ...
        top_l.addWidget(QLabel("key file"))
        top_l.addWidget(self.idx)

        # --- Centro: SOLO viewer de dump (3D + 2D interno) ---
        viewer = QWidget()
        viewer_l = QVBoxLayout(viewer)
        self.plotter = QtInteractor(viewer)
        self.plotter.setMinimumHeight(600)  # para que se vea grande
        viewer_l.addWidget(self.plotter)

        # Creamos la figura 2D pero NO la agregamos al layout (no se muestra)
        self.fig = plt.figure(figsize=(4, 4))
        self.canvas = FigureCanvas(self.fig)  # necesario para que render_dump_to llame fig.canvas.draw()

        # --- Layout raíz ---
        root_l = QVBoxLayout(self)
        root_l.addWidget(top)
        root_l.addWidget(viewer, 1)
        ...
        # Cargar primer índice disponible si existe
        self._auto_seed_index()
    ...
    def _load_idx(self):
        ...
        if not Path(p).exists():
            ...
            return
        try:
            # Reutilizamos tu función de render (3D + 2D interno no visible)
            render_dump_to(self.plotter, self.fig, p)
        except Exception as e:
            QMessageBox.critical(self, "Error", str(e))

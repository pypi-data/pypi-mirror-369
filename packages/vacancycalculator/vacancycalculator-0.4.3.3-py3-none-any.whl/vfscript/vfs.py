...
def VacancyAnalysis():
    ...
    for FILE in defect_files:
        ...
        clf = VacancyClassifierKeras(ModelConfig(artifacts_dir="outputs/keras_artifacts"))
        clf.load_artifacts(best=True)  # carga modelo + scaler + orden de features
        clf.predict_csv("outputs/csv/finger_data_full_features.csv", "outputs/csv/arcx_predicho_keras.csv", return_probs=True)

        
        # Cambia el path si corresponde:
        csv_path = "outputs/csv/finger_data_predicha.csv"
        df = pd.read_csv(csv_path)

        # 1) intenta encontrar la columna de predicción más común
        cand = ["pred","prediction","y_pred","vacancia","vacancias_pred","clase_pred"]
        pred_col = next((c for c in cand if c in df.columns), None)

        # 2) si no existe, la armamos a partir de columnas de probabilidades
        if pred_col is None:
            # probable naming: p_1, p_2, ... o prob_1 ... o columnas "1","2",...
            prob_cols = [c for c in df.columns
                        if c.lower().startswith(("p_","prob_","class_","c_")) or c.isdigit()]
            assert prob_cols, "No encuentro columna de predicción ni de probabilidades."
            # elegimos la clase de mayor probabilidad
            pred_labels = df[prob_cols].idxmax(axis=1)
            # extrae el número de clase al final del nombre (p_15 -> 15, "10" -> 10)
            pred = pred_labels.apply(lambda s: int(re.search(r'(\d+)$', str(s)).group(1)))
        else:
            pred = df[pred_col].astype(int)

        # Distribución por número de vacancias (clases 1..C)
        dist = pred.value_counts().sort_index()

        print(f"Total de archivos predichos: {len(pred)}")
        print("\nDistribución por vacancias (clase -> cantidad de archivos):")
        for k, v in dist.items():
            print(f"{k:>2} -> {v}")

        # Si la clase = número de vacancias, puedes sumar el total:
        print("\nVacancias totales (suma de clases):", int(pred.sum()))



        clf = VacancyClassifierKeras(ModelConfig(artifacts_dir="outputs/keras_artifacts"))
        clf.load_artifacts(best=True)  # carga modelo, scaler y orden de features

        out = clf.predict_csv_rowwise(
            input_csv="otputs/csv/finger_data_full_features.csv",
            output_csv="outputs/csv/vacancias_predichas.csv",
            return_probs=False,   # pon True si también quieres prob_1..prob_C
        )

        print(out[["pred_vacancys"]].head())
        print("Filas procesadas:", len(out))

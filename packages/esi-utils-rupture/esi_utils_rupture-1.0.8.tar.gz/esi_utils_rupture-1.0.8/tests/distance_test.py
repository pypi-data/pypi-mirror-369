...
from esi_utils_rupture.gc2 import _computeGC2
...
def test_san_fernando():
    ...
    origin = Origin(
        {
            ...
            "network": "",
            "locstring": "",
            "time": HistoricTime.fromtimestamp(int(time.time()), datetime.UTC),
        }
    )
    ...
    x, y = proj(lon, lat)
    rupx, rupy = proj(
        rup._lon[~np.isnan(rup._lon)], rup._lat[~np.isnan(rup._lat)]
    )
...
def test_chichi_with_get_distance():
    ...
    i1 = i0 + 10
    cs = list(zip(i0, i1))
    df = pd.read_fwf(f, colspecs=cs, skiprows=2, nrows=5, header=None)
    mat = df.values

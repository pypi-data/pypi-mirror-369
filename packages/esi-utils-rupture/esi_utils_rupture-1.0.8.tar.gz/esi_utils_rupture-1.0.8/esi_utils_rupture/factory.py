    ...
    if file is not None:
        try:
            ...
            try:
                ...
                logging.error(e)
                raise ValueError(
                    f"Error reading {file}. This could have been "
...
def rupture_from_dict(d):
    ...
    try:
        d["metadata"]["time"] = HistoricTime.strptime(
            d["metadata"]["time"], TIMEFMT
        )
    except ValueError:
        d["metadata"]["time"] = HistoricTime.strptime(
            d["metadata"]["time"], ALT_TIMEFMT
        )
...
def _rotate_polygon(p):
    ...
    parray = np.append(
        parray[1:, :], np.reshape(parray[0, :], (1, -1)), axis=0
    )
...
def _check_polygon(p):
    ...
    if p[0] != p[-1]:
        raise ValueError(
            "First and last points in polygon must be " "identical."
        )
...
def validate_json(d):
    ...
    if "reference" not in d["metadata"].keys():
        raise Exception(
            "Json metadata field should contain " '"reference" key.'
        )
    ...
    if geom["type"] != "MultiPolygon" and geom["type"] != "Point":
        raise Exception(
            'Geometry type should be "MultiPolygon" ' 'or "Point".'
        )

    if "coordinates" not in geom.keys():
        raise Exception(
            'Geometry dictionary should contain "coordinates" ' "key."
        )

...
def get_local_unit_slip_vector(strike, dip, rake):
    ...
    sx = np.sin(rake) * np.cos(dip) * np.cos(strike) + np.cos(rake) * np.sin(
        strike
    )
    sy = np.sin(rake) * np.cos(dip) * np.sin(strike) + np.cos(rake) * np.cos(
        strike
    )
...
def _quad_distance(q, points, horizontal=False):
    ...
    if horizontal:
        ...
        dist[inside_idx] = np.power(
            np.abs(
                np.sum(p0d[inside_idx, :] * normalVector.getArray(), axis=1)
            ),
            2,
        )
...
def get_distance_to_plane(planepoints, otherpoint):
    ...
    if D != 0:
        ...
        numer = np.abs(
            a * otherpoint.x + b * otherpoint.y + c * otherpoint.z + d
        )
...
def _distance_sq_to_segment(p0, p1):
    ...
    idx_neg = c1 <= 0
    dist[idx_neg] = (
        p0[idx_neg, 0] ** 2 + p0[idx_neg, 1] ** 2 + p0[idx_neg, 2] ** 2
    )

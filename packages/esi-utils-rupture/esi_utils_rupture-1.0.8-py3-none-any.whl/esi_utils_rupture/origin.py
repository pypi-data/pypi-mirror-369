...
def write_event_file(event, xmlfile):
    ...
    try:
        ...
        if "reviewed" in event:
            root.attrib["reviewed"] = event["reviewed"]
        else:
            root.attrib["reviewed"] = "unknown"
...
def read_event_file(eventxml):
    ...
    Returns:
       dict: Dictionary with keys:
         ...
         - productcode: (optional) This product source's unique code for this
                        particular ShakeMap.
         - reviewed: Has the origin been reviewed? "true", "false", or "unknown"
    ...
    if "time" in xmldict:
        try:
            eqdict["time"] = HistoricTime.strptime(
                xmldict["time"], constants.TIMEFMT
            )
        except ValueError:
            try:
                eqdict["time"] = HistoricTime.strptime(
    ...
    if "reviewed" in xmldict:
        eqdict["reviewed"] = xmldict["reviewed"]
    else:
        eqdict["reviewed"] = "unknown"

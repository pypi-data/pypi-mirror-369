...
from tree_sitter_analyzer.project_detector import detect_project_root, ProjectRootDetector
...
""")
    ...
    print(f"   Uses explicit: {'✅ YES' if unified_result2 == os.path.abspath(explicit_root) else '❌ NO'}")
    ...
    tool1_root = mcp_tool1.security_validator.boundary_manager.project_root if mcp_tool1.security_validator.boundary_manager else 'None'
    ...
    tool2_root = mcp_tool2.security_validator.boundary_manager.project_root if mcp_tool2.security_validator.boundary_manager else 'None'
    print(f"🛠️ MCP tool with explicit root: {tool2_root}")
    
    # Test actual analysis
    try:
        result = await mcp_tool1.execute({"file_path": src_file})
        print(f"✅ Analysis successful: Found {len(result.get('elements', []))} elements")
    ...
    print(f"🔒 File within project: {'✅ VALID' if is_valid1 else '❌ INVALID'} - {msg1}")
    ...
    print(f"🔒 File outside project: {'✅ VALID' if is_valid2 else '❌ INVALID'} - {msg2}")
    
    # Test malicious path
    is_valid3, msg3 = mcp_tool1.security_validator.validate_file_path("../../../etc/passwd")
    print(f"🔒 Malicious path: {'✅ BLOCKED' if not is_valid3 else '❌ ALLOWED'} - {msg3}")
    ...
    print(f"   # Auto-detect project root:")
    print(f"   tree-sitter-analyzer {src_file} --table=full")
    print()
    print(f"   # Explicit project root:")
    print(f"   tree-sitter-analyzer {src_file} --project-root {project_root} --table=full")
    print()
    ...
    print(f'   # Auto-detect project root:')
    print(f'   "command": "python", "args": ["-m", "tree_sitter_analyzer.mcp.server"]')
    print()
    print(f'   # Explicit project root:')
    print(f'   "command": "python", "args": ["-m", "tree_sitter_analyzer.mcp.server", "--project-root", "{project_root}"]')
    print()

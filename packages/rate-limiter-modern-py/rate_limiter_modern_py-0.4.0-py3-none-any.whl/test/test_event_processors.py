...
from limiter.event_processors import (
    ProcessorPredicate,
    EventProcessor,
...
class ProcessorPredicateTest(TestCase):
    def test_valid_str_value(self):
        ...
        predicate = ProcessorPredicate(
            "detail.state", lambda state: state == target_value
        )
    ...
    def test_valid_numeric_value(self):
        ...
        predicate = ProcessorPredicate(
            "detail.time", lambda state: state == target_value
        )
        self.assertTrue(predicate.test(event))
    ...
    def test_invalid_path(self):
        ...
        predicate = ProcessorPredicate(
            "detail.states", lambda state: state == target_value
        )
        self.assertFalse(predicate.test(event))
    ...
    def test_and_all_true(self):
        ...
        state_pred = ProcessorPredicate(
            "detail.state", lambda state: state == state_value
        )
        message_pred = ProcessorPredicate(
            "detail.message", lambda state: state == message_value
        )
    ...
    def test_and_self_false(self):
        ...
        predicate = ProcessorPredicate(
            "detail.zone", lambda state: state == random_string()
        )
    ...
    def test_or_sibling_false(self):
        ...
        message_pred = ProcessorPredicate(
            "detail.message", lambda state: state == random_string()
        )
        predicate = ProcessorPredicate("detail.zone", lambda state: state == zone_value)
    ...
    def test_or_all_false(self):
        ...
        state_pred = ProcessorPredicate(
            "detail.state", lambda state: state == random_string()
        )
        message_pred = ProcessorPredicate(
            "detail.message", lambda state: state == random_string()
        )

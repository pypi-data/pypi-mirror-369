...
from limiter.managers import (
    FungibleTokenManager,
    NonFungibleTokenManager,
    TokenReservation,
    _compute_refill_amount,
)
...
class FungibleTokenManagerTest(TestCase):
    def setUp(self):
        ...
        self.manager = FungibleTokenManager(
            self.token_table,
            self.limit_table,
            self.resource_name,
            self.limit,
    ...
    def test_compute_refill_amount(self):
        ...
        expected = 8
        actual = _compute_refill_amount(
            current_tokens, time_since_refill, self.limit, self.token_ms
        )
    ...
    def test_get_bucket_token(self):
        ...
        expected_args = [
            {
                "Key": {"resourceName": self.resource_name, "accountId": account_id},
    ...
    def test_get_bucket_token_exhausted(self):
        ...
        mock_token_table = Mock()
        mock_token_table.update_item = MagicMock(
            side_effect=ClientError(error_response, None)
        )
        self.manager._token_table = mock_token_table

        self.assertRaises(
            CapacityExhaustedException,
            self.manager._get_bucket_token,
            account_id,
            exec_time,
            self.ms_token,
        )
    ...
    @mock_aws
    def test_account_resource_limit_denylist(self):
        limit = 0
        window = random.randint(0, 100)
        account_id = random_string()
        ...
        self.assertRaises(
            CapacityExhaustedException,
            self.manager._get_account_resource_limit,
            account_id,
        )
...
class NonFungibleTokenManagerTest(TestCase):
    def setUp(self):
        ...
        self.limit = 5
        self.manager = NonFungibleTokenManager(
            self.token_table, self.limit_table, self.resource_name, self.limit
        )
    ...
    def test_get_reservation_exhausted(self):
        ...
        self.assertRaises(
            CapacityExhaustedException, self.manager.get_reservation, account_id
        )
...
class TokenReservationTest(TestCase):
    ...
    def test_delete_reservation(self):
        ...
        response = mock_token_table.query(
            KeyConditionExpression=Key("resourceCoordinate").eq(self.coordinate)
        )
        self.assertEqual(1, response["Count"])
...
def _insert_limit(mock_limit_table, resource_name, account_id, limit, window_sec=0):
    ...
    mock_limit_table.put_item(TableName=mock_limit_table.table_name, Item=limit_item)

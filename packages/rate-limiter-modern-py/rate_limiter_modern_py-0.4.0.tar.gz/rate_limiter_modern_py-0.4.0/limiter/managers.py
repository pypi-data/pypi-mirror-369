...
class BaseTokenManager:
    ...
    def __init__(
        self, token_table, limit_table, resource_name, default_limit, default_window=0
    ):
    ...
    def _get_account_resource_limit(self, account_id):
        ...
        An account can be denylisted by setting its limit to 0. If a limit of 0
        ...
        Throws:
            ...
            CapacityExhaustedException: If the account has no capacity for the resource, i.e. denylisted.
        ...
        try:
            response = self.limit_table.query(
                KeyConditionExpression=Key(RESOURCE_NAME).eq(self.resource_name)
            ...
            if isinstance(e, ClientError):
                ...
                if error_code in (
                    ...
                    message = "Throttled getting limit on {} for account {}".format(
                        self.resource_name, account_id
                    )
                    raise_from(ThrottlingException(message), e)
            message = "Failed to get limit on {} for account {}".format(
                self.resource_name, account_id
            )
            raise_from(RateLimiterException(message), e)

        if result[LIMIT] <= 0:
            message = "Account {} has not been allocated any capacity for {}".format(
                account_id, self.resource_name
            )
...
class FungibleTokenManager(BaseTokenManager):
    ...
    def get_token(self, account_id):
        ...
        token_ms = (
            float(limit) / window_ms
        )  # Number of tokens the bucket will accumulate per millisecond
        ms_token = int(
            max(1, float(window_ms) / limit)
        )  # Number of milliseconds to accumulate a new token
        ...
        )  # If the row has not been created yet, use 0
        time_since_refill = exec_time - last_refill
        refill_tokens = _compute_refill_amount(
            current_tokens, time_since_refill, limit, token_ms
        )
    ...
    def _get_bucket_token(self, account_id, exec_time, ms_token):
        ...
        try:
            ...
            condition_exp = (
                "{0} > :min OR {1} < :failsafe OR attribute_not_exists({0})".format(
                    TOKENS, LAST_TOKEN
                )
            )
            return self.token_table.update_item(
                Key={RESOURCE_NAME: self.resource_name, ACCOUNT_ID: account_id},
            ...
            if isinstance(e, ClientError):
                ...
                if error_code == "ConditionalCheckFailedException":
                    message = "Resource capcity exhausted for {}:{}".format(
                        self.resource_name, account_id
                    )
                    raise CapacityExhaustedException(message)
                if error_code in (
                    ...
                    message = "Throttled by getting limit on {} for account {}".format(
                        self.resource_name, account_id
                    )
                    raise_from(ThrottlingException(message), e)
            raise
    ...
    def _refill_bucket_tokens(self, account_id, tokens, refill_time):
        ...
        """
        try:
            update_exp = "set {} = :tokens, {} = :refill_time".format(
                TOKENS, LAST_REFILL
            )
            condition_exp = "{0} < :refill_time OR attribute_not_exists({0})".format(
                LAST_REFILL
            )
            ...
            if (
                ...
                logger.warning(
                    "Failed to refill tokens for %s:%s, already refilled with more current state",
                    self.resource_name,
                    account_id,
                )
            else:
                logger.exception(
                    "Failed to refill tokens for %s:%s", self.resource_name, account_id
                )
...
class NonFungibleTokenManager(BaseTokenManager):
    ...
    def _build_reservation(self, account_id, exec_time):
        ...
        return TokenReservation(
            id, self.token_table, self.resource_name, account_id, coordinate
        )
...
class TokenReservation:
    ...
    def create_token(self, resource_id, expiration=28800):
        ...
        if self.is_token_created:
            raise ValueError(
                "Token already created for {} from this reservation [{}]".format(
                    resource_id, self.id
                )
            )
        ...
        try:
            ...
            update_exp = "set {} = :exp_time, {} = :resource_id".format(
                EXPIRATION_TIME, RESOURCE_ID
            )
            self.table.update_item(
                Key={RESOURCE_COORDINATE: self.coordinate, RESERVATION_ID: self.id},
                UpdateExpression=update_exp,
                ExpressionAttributeValues={
            ...
            if e.response["Error"]["Code"] == "ConditionalCheckFailedException":
                ...
                raise ReservationNotFoundException(
                    msg_fmt.format(self.id, self.resource_name, self.account_id)
                )
            raise
    ...
    def delete(self):
        ...
        if self.is_token_created:
            logger.warning(
                "Cannot delete, a token has already been created from this reservation [%s]",
                self.id,
            )
            return
        ...
        if self.is_deleted:
            logger.warning(
                "Cannot delete, this reservation [%s], has already been deleted",
                self.id,
            )
            return
...
def _compute_refill_amount(current_tokens, time_since_refill, limit, token_ms):
    ...
    tokens = max(
        0, current_tokens
    )  # Tokens can be negative on bucket creation or a prolonged failure to refill
...
def now_utc_ms():
    ...
    return int(round(float(datetime.now(UTC).strftime("%s.%f")) * 1000))

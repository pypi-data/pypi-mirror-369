...
class LoadHandler(object):
    def __init__(self, uid, payload, cid, browser, inject_obj, application_version):
        self.payload = payload
        self.uid = uid
        self.cid = cid
        self.browser = browser
        self.handlerId = generate_guid()
        self.inject_obj = inject_obj
    ...
    def OnContextCreated(self, browser, frame):
        ...
        logging.debug("onLoadStart isMain=" + str(frame.IsMain()))
        if frame.IsMain():
            set_javascript_bindings(self.uid, inject_obj=self.inject_obj, application_version=self.application_version,
                                    payload=self.payload)
            if debug_mode:
                with open(os.path.dirname(__file__) + '/burgeon.cef.sdk.js', 'r', encoding='UTF-8') as js:
                    browser.ExecuteJavascript(js.read())
                    logging.debug("onLoadStart sdk: isMain=")
...
class ResourceHandler:
    ...
    def ProcessRequest(self, request, callback):
        ...
        # TODO 1.再此处是否可以根据request.GetUrl()通过cookieMnaager获取cookie字符串 cef.CookieManager().GetGlobalManager().VisitCookieUrl
        # TODO 2.是否可以追加到Header中（ 此处可能需要克隆一个新的request)
...
class CorsCookieManager:
    def __init__(self):
        self.cookies = {}

    def setCookie(self, name, value, domain=None, expires=None):
        expire_time = time.time() + expires if expires else None
        key = (name, domain)
        self.cookies[key] = {'value': value, 'expires': expire_time}

    def getCookie(self, name, domain=None):
        key = (name, domain)
        cookie = self.cookies.get(key)
        if cookie:
            if cookie['expires'] is None or cookie['expires'] > time.time():
                return cookie['value']
            else:
                del self.cookies[key]  # 删除过期的 cookie
        return None

    def getCookiesByUrl(self, url):
        parsed_url = urlparse(url)
        domain = parsed_url.hostname or url
        valid_cookies = []
        for (name, cookie_domain), cookie in self.cookies.items():
            if (domain.endswith(cookie_domain)) and (cookie['expires'] is None or cookie['expires'] > time.time()):
                valid_cookies.append(f"{name}={cookie['value']}")
        return '; '.join(valid_cookies)


corsCookies = CorsCookieManager()


class RequestHandler(object):
    def __init__(self):
        self.allCookies = corsCookies

    def CanSetCookie(self, frame, request, cookie, **_):
        domain = cookie.GetDomain()
        name = cookie.GetName()
        value = cookie.GetValue()
        self.allCookies.setCookie(name, value, domain)

    def OnBeforeResourceLoad(self, browser, frame, request, callback):
        url = request.GetUrl()
        cookie = self.allCookies.getCookiesByUrl(url)
        headers = request.GetHeaderMap()
        if ("Cookie" not in headers):
            headers['Cookie'] = cookie
            request.SetHeaderMap(headers)

        # if request.GetMethod() == "GET" and "Attach2" in request.GetUrl() and not (
        #         request.GetUrl().startswith('http://127.0.0.1') or request.GetUrl().startswith('http://at.alicdn.com')):
        #     try:
...
def create_browser_view(uid, title="", url=None, width=default_window_width, height=default_window_height,
    ...
    set_client_handler(uid, payload, cid, browser, popup_validate, inject_custom_request, inject_obj, application_version)
...
def launch_main_window(uid, title, url, width, height, resizable, full_screen, min_size,
    ...
    switches = {
        ...
        'load-extension':'E:/work/FastCloud-Python-poscloud/cors-ext/',
    }
...
def set_client_handler(uid, payload, cid, browser, popup_validate, inject_custom_request, inject_obj, application_version):

    BrowserView.instances[uid].view.SetClientHandler(LoadHandler(uid, payload, cid, browser,inject_obj, application_version))
    ...
    BrowserView.instances[uid].view.SetClientHandler(RequestHandler())
...
def set_cookies(url, cookies):
    ...
    corsCookies.setCookie(cookies.GetName(), cookies.GetValue(), cookies.GetDomain())

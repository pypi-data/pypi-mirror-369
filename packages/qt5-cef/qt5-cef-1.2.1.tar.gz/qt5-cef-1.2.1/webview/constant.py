...
(function () {
    ...
    cef.setGlobalVariable = (key, value) => {
        if (typeof key !== 'string') {
            console.error('setGlobalVariable: key must be a string');
            return;
        }
        cef[key] = value;
    };
    cef.getGlobalVariable = (key) => {
        if (typeof key !== 'string') {
            console.error('getGlobalVariable: key must be a string');
            return undefined;
        }
        return cef[key];
    };

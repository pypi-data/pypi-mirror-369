...
class ASRTask:
    ...
    """
    def __init__(self, dataset_name: str, dataset_subset: str, dataset_split: str, audio_column: str, text_column: str, hf_token: str | None = None):
        """
        Initializes and loads the dataset from the Hugging Face Hub.
        """
        print(f"--- [DEBUG] ASRTask received dataset_split: '{dataset_split}' ---")

        self.dataset = load_dataset(dataset_name, dataset_subset, split=dataset_split, trust_remote_code=True, token=hf_token)
        ...
        # DEBUG: print first 10 reference_text values
        print("\n--- First 10 rows of reference_text ---")
        for i in range(min(10, len(self.dataset))):
            print(f"{i+1}: {self.dataset[i]['reference_text']}")
        print("---------------------------------------")
    def __iter__(self):

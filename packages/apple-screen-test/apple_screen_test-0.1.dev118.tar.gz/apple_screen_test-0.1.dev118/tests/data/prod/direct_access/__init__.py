from data.prod.direct_access import direct_access_episodes, direct_access_mlb, direct_access_seasons, direct_access_mls, direct_access_sports, direct_access_league_availability, direct_access_movies, direct_access_tv_shows, direct_access_others
...
direct_access_locations = [
    ...
    direct_access_league_availability,
]
...
for location in direct_access_locations:
    for key, value in location.__dict__.items():
        if '__' not in key and not (isinstance(value, type) or isinstance(value, UMCContentType)):

...
# Import centralized distance utility
from .utils.distance import DistanceUtils

def calculate_distance(lat1, lon1, lat2, lon2):
    """Calculate distance between two points using centralized Haversine formula."""
    return DistanceUtils.haversine_km(lat1, lon1, lat2, lon2)
...
def calculate_warehouse_centroid(warehouses):
    ...
    # Fallback: if no vehicles, use simple average of warehouse locations
    if total_weight == 0:
...
def filter_nodes_by_circle(nodes_df, centroid_lat, centroid_lon, radius_km):
    ...
    for _, node_row in nodes_df.iterrows():
        ...
        distance = calculate_distance(node_lat, node_lon, centroid_lat, centroid_lon)
...
def generate_scenario_from_config(base_config, nodes_df_raw, edges_df_raw, custom_config):
    ...
    if random_seed is not None:
        ...
        # Also seed numpy.random for reproducible results
        try:
            import numpy as np
            np.random.seed(random_seed)
        except ImportError:
            pass  # numpy not available, continue with just random

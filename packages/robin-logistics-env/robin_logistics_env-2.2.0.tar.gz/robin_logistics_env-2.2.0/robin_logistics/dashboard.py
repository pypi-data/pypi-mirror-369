                ...
                if selected_route:
                    ...
                    if vehicle_specs and route_orders:
                        ...
                        for route_order_id in route_orders:
                            ...
                            order_weight = sum(sku_info['weight_kg'] * quantity for sku_id_val, quantity in order.requested_items.items() for sku_info in SKU_DEFINITIONS if sku_info['sku_id'] == sku_id_val)
                            order_volume = sum(sku_info['volume_m3'] * quantity for sku_id_val, quantity in order.requested_items.items() for sku_info in SKU_DEFINITIONS if sku_info['sku_id'] == sku_id_val)
                            ...
                            # Aggregate SKU breakdown across all orders
                        ...
                        # Top row: Total Cost and Cost per Order
                        col1, col2 = st.columns(2)
                        ...
                        # Details row: Orders, Weight, Volume, Distance
                        col1, col2, col3, col4 = st.columns(4)
                        with col1: st.metric("📦 Orders", len(route_orders))
                        ...
                        if len(route_nodes) > 1:
                            for i in range(len(route_nodes) - 1):
                                ...
                                if node1 in env.nodes and node2 in env.nodes:
                                    pos1 = (env.nodes[node1].lat, env.nodes[node1].lon)
                                    pos2 = (env.nodes[node2].lat, env.nodes[node2].lon)
                                    leg_distance = ((pos1[0] - pos2[0])**2 + (pos1[1] - pos2[1])**2)**0.5 * 111
                                    total_distance += leg_distance
                        with col4: st.metric("🛣️ Total Distance", f"{total_distance:.2f} km")
                        
                        # Second row: Items, Weight Utilization, Volume Utilization, Vehicle Utilization
                        col1, col2, col3, col4 = st.columns(4)
                        ...
                        with col4:
                            if vehicle_specs:
                                # Vehicle utilization based on distance traveled vs max distance capability
                                max_distance = vehicle_specs.get('max_distance_km', 1000)  # Default max distance
                        ...
                        total_cost = 0
                        if vehicle_specs and len(route_nodes) > 1:
                            for i in range(len(route_nodes) - 1):
                                node1, node2 = route_nodes[i], route_nodes[i + 1]
                                if node1 in env.nodes and node2 in env.nodes:
                                    pos1 = (env.nodes[node1].lat, env.nodes[node1].lon)
                    ...
                    if vehicle_specs and route_orders:
                        progression_data = []
                        # Calculate total load for all orders in this route
                        initial_weight = 0
                        initial_volume = 0
                        for route_order_id in route_orders:
                            order_weight = sum(sku_info['weight_kg'] * quantity for sku_id_val, quantity in env.orders[route_order_id].requested_items.items() for sku_info in SKU_DEFINITIONS if sku_info['sku_id'] == sku_id_val)
                            order_volume = sum(sku_info['volume_m3'] * quantity for sku_id_val, quantity in env.orders[route_order_id].requested_items.items() for sku_info in SKU_DEFINITIONS if sku_info['sku_id'] == sku_id_val)
                        ...
                        for j, node_id in enumerate(route_nodes):
                            ...
                            if j == 0:
                                ...
                                current_weight = 0
                                current_volume = 0
                            elif node_id in env.orders:  # This is an order being delivered
                                node_type = f"Delivery - Order {node_id}"
                                load_change = "Deliver order"
                                # Calculate remaining load after delivering this order
                                order_weight = sum(sku_info['weight_kg'] * quantity for sku_id_val, quantity in env.orders[node_id].requested_items.items() for sku_info in SKU_DEFINITIONS if sku_info['sku_id'] == sku_id_val)
                                order_volume = sum(sku_info['volume_m3'] * quantity for sku_id_val, quantity in env.orders[node_id].requested_items.items() for sku_info in SKU_DEFINITIONS if sku_info['sku_id'] == sku_id_val)
                                ...
                                # Check if this node represents an order destination
                                ...
                                if is_order_destination:
                                    ...
                                    load_change = "Deliver order"
                                    # Calculate remaining load after delivering this order
                                    order_weight = sum(sku_info['weight_kg'] * quantity for sku_id_val, quantity in env.orders[oid].requested_items.items() for sku_info in SKU_DEFINITIONS if sku_info['sku_id'] == sku_id_val)
                                    order_volume = sum(sku_info['volume_m3'] * quantity for sku_id_val, quantity in env.orders[oid].requested_items.items() for sku_info in SKU_DEFINITIONS if sku_info['sku_id'] == sku_id_val)
                                    current_weight -= order_weight
                                    current_volume -= order_volume
                                    weight_after = current_weight
                                    volume_after = current_volume

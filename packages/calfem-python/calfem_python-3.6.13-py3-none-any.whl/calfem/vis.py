...
def _makeColorBar(text, axes=None):
    ...
    if colBar is None:
        # Creates a colorbar and sets the label.
        vv.colorbar(axes).SetLabel(text)
    else:
...
def add_text(text, pos, angle=0, font_name=None, font_size=9, color='k', bgcolor=None, axes=None):
    ...
    if axes is None:
        axes = vv.gca()
    text = vv.Text(axes, text, *pos, fontName=font_name,
...
def draw_mesh(coords, edof, dofs_per_node, el_type, axes=None, axes_adjust=True,
    ...
    axes, verts, faces, verticesPerFace, is3D = _preMeshDrawPrep(
        axes, coords, edof, dofs_per_node, el_type)
    # Create mesh:
    m = vv.Mesh(parent=axes, vertices=verts, faces=faces,
                values=color, verticesPerFace=verticesPerFace)
    # Settings:
...
def draw_nodal_values(node_vals, coords, edof, dofs_per_node, el_type, clim=None, axes=None, axes_adjust=True, draw_elements=True, title=None):
    ...
    axes, verts, faces, verticesPerFace, is3D = _preMeshDrawPrep(
        axes, coords, edof, dofs_per_node, el_type)
    m = vv.Mesh(parent=axes, vertices=verts, faces=faces,
                values=node_vals, verticesPerFace=verticesPerFace)
    ...
    # NOTE: It seems colormap coloring breaks when faceshading='plain'. 'smooth' must be used.
    m.faceShading, m.edgeShading = ('smooth', edgeSh)
    m.ambient = 1
    m.diffuse = 0
    m.specular = 0  # Disable specular.
    # Set the values again, because it doesn't work in the constructor for unknown reasons
    m.SetValues(node_vals, setClim)
...
def draw_element_values(ev, coords, edof, dofs_per_node, el_type, displacements=None, clim=None, axes=None,
    ...
    if draw_undisplaced_mesh:
        drawMesh(coords, edof, dofs_per_node, el_type,
                 axes, axes_adjust, color=(0.5, 0.5, 0.5))
    ...
    # TODO: Extend with more element types
    fPerElms = {1: 0,   2: 1,   3: 1,   4: 4,   5: 6}
    ...
    # Creates the world object that gets drawn on screen.
    c = _elementsWobject(axes, faceVals, verts, faces,
                         verticesPerFace, draw_elements, clim)

    # Finds or creates colorbar and sets the label.
    _makeColorBar("Element values", axes)
...
def draw_displacements(displacements, coords, edof, dofs_per_node, el_type, node_vals=None, clim=None, axes=None,
    ...
    if draw_undisplaced_mesh:
        drawMesh(coords, edof, dofs_per_node, el_type, axes,
                 axes_adjust, title=title, color=(0.5, 0.5, 0.5), filled=False)

    if node_vals != None:
        m = drawNodalValues(node_vals, displaced, edof, dofs_per_node, el_type, clim=clim,
                            axes=axes, axes_adjust=axes_adjust, draw_elements=True, title=title)
    else:
        m = drawMesh(displaced, edof, dofs_per_node,
                     el_type, axes, axes_adjust, title=title)
...
def draw_geometry(geoData, axes=None, axes_adjust=True, draw_points=True, label_points=True, label_curves=True, title=None, font_size=11, N=20):
    ...
    if draw_points:
        ...
        plotArgs = {'mc': 'r', 'mw': 5, 'lw': 0,
                    'ms': 'o', 'axes_adjust': False, 'axes': axes}
        if geoData.is3D:
        ...
        if label_points:  # Write text label at the points:
            # [[x, y, z], el_size, marker]
            for (ID, (xyz, el_size, marker)) in geoData.points.items():
                text = "  " + str(ID) + ("[%s]" %
                                         marker if marker is not 0 else '')
                addText(text, xyz, font_size=font_size -
                        1, color=(0.5, 0, 0.5), axes=axes)
    ...
    for(ID, (curveName, pointIDs, marker, elementsOnCurve, _, _)) in geoData.curves.items():
        ...
        plotArgs = {'lc': 'k', 'ms': None, 'axes_adjust': False, 'axes': axes}
        if geoData.is3D:
            vv.plot(P[:, 0], P[:, 1], P[:, 2], **plotArgs)
        else:
            vv.plot(P[:, 0], P[:, 1], **plotArgs)
...
def _preMeshDrawPrep(axes, coords, edof, dofs_per_node, el_type):
    ...
    if axes is None:
        # Gets current Axis or creates a new one if none exists.
        axes = vv.gca()
...
class _elementsWobject(vv.Wobject, Colormapable):
    ...
    def __init__(self, parent, fVals, verts, faces, verticesPerFace, draw_elements, clim=None):
        ...
        # N-by-1 array?            values of N faces (not elements if 3D)
        self._fVals = fVals
        ...
        # 3 or 4. Either we have triangle faces or quads.
        self.verticesPerFace = verticesPerFace
    ...
    def _drawFaces(self, how):
        for (value, faceVerts) in zip(self._fVals, self._faces):
            # Turn the value into an index between 0 and 255
            valueIndex = int(255 * (value-self.clim.min) /
                             (self.clim.max-self.clim.min))
            if valueIndex > 255:
                valueIndex = 255
            elif valueIndex < 0:
                valueIndex = 0
            color = self.mapData[valueIndex, :]  # get colour from value
            # get coordinates of the vertices of the face.
            coords = self._verts[faceVerts.astype(int), :]
            gl.glColor(*color)
            gl.glBegin(how)
            for (x, y, z) in coords:
                gl.glVertex(x, y, z)
...
def eldraw2(ex, ey, plotpar=[1, 2, 1], elnum=[]):
    ...
    for elx, ely in zip(ex, ey):
        ...
        vv.plot(x, y, ls=vv_line_type, lc=vv_line_color,
                ms=vv_node_mark, mc=vv_marker_color)

...
from .modules import great_circle_distance_numba as gcd
...
class Track():
    ...
    See puplication for details of the algorithm.
    ...
    **ty** : ``str``, defoult ``'both'``
        ...
        - ``'L'`` for only lows (cutoff lows and troughs).
        - ``'H'`` for only highs (cutoff highs and ridges).
        - ``'both'`` for both.
    ...
    **lev** : ``int``, default ``0``
        The level of vertical axis to be tracked.


    **odir** : ``str``, default ``'./d01'``
        Path to outputs from colindex2.
        ...
        In this process, csv files in ``V`` directory will be read,
        intermediate csv files will be stored in ``Vt``,
        finally, the result csv files will be stored in ``Vtc``.
    ...
    **long_term** : ``bool``, default ``False``
        ...
        If ``False``, tracking ID will be continuously counted up during the analysis period, and massive memory usage may occur.


    **operational** : ``bool``, default ``False``
        Note! this feature is experimental.
        If True, resume tracking with tracking data (files in ``{odir}/Vt``) produced in advance,
        so that you can restart tracking from whenever you like.
        Make sure times must NOT include the end-of-timestep flag, i.e., ``MERGE=-3``.
        If there are data ending at ``2020-12-31 18`` with a 6-hour timestep,
        the operational tracking must start before ``2020-12-31 12``.
    ...
    **MAXSo_thres** : ``float``, default ``0.``
        Threshold for noise removal with respect to maximum intensity in life [m/100km].


    **ALLDIST_thres** : ``float``, default ``0.``
        Threshold for noise removal with respect to total moving distance in life [km].

    **only_count** : ``bool``, default ``False``
        If ``True``, skip main tracking process.
    ...
    Set date range to be tracked. ::

        >>> import pandas as pd
        >>> times = pd.date_range('2010-01-01 00', '2010-01-31 18', freq='6h')

    Import and execute ``Track`` with ``times`` and ``level`` (and ``odir=odir``).
    In this case, the level is 300 hPa and ``./d01`` directory will be used. ::

        >>> from colindex2 import Track
        >>> Track(times, 300)

    That is the basic use of Tracker. Following is for some special cases.

    For long term analysis beyond a year ::

        >>> times = pd.date_range('1959-01-01 00', '2020-12-01 18', freq='6h')
        >>> Track(times, 300, long_term=True)

    If you interested only in lows or highs, set ``ty='L'`` or ``ty='H'``.
    By default, both types will be tracked. ::

        >>> Track(times, 300, ty='L')

    An example using multiprocessing. ::

        >>> import multiprocessing as mp
        >>> from colindex import Tracker
        >>> import pandas as pd
        ...
        >>> def main(args):
        ...     times = pd.date_range('2018-11-01 00', '2019-03-31 18', freq='6h')
        ...     Track(times, args[1], ty=args[0])
        ...
        >>> if __name__ == '__main__':
        ...     # double-nested args for types and levels
        ...     args = [[ty, lev] for ty in ['L', 'H'] for lev in [500, 300, 200]]
        ...     with mp.Pool(len(args)) as p:
        ...         p.map(main, args)
    """

    def __init__(self, times, lev, ty='both',
                 odir='./d01', tlimit=150.,# normalized_speed=133.33,
                 penalty=0,
                 long_term=False, operational=False, 
                 DU_thres=0, MAXSo_thres=0., ALLDIST_thres=0.,
                 only_count=False,
                 ):
        ...
        self.MAXSo_thres = MAXSo_thres
        self.ALLDIST_thres = ALLDIST_thres
    ...
    def _count_main(self, _df, target_times, ty):
        ...
        df2 = df1[df1.MAXSo > self.MAXSo_thres].reset_index(drop=True)
        df3 = df2[df2.ALLDIST > self.ALLDIST_thres].reset_index(drop=True)  # DIST / day
        ...
        for t in target_times:
            ...
            di = pd.DatetimeIndex(df3['time'])
            df_o = df3[(di.year == _Y)&(di.month == _M)&(di.day == _D)&(di.hour == _H)&(di.minute == _T)]
    ...
    def _checkqs3(self, lon1, lat1, r1, lon2, lat2, r2, O):
    ...
    def _check_sequence(self, _df, idx, O):
        ...
        for i in range(len(_df)):
            ...
            if O == 'XS':
                ...
                now_cond = self._checkqs3(lon0, lat0, ro0, lon1, lat1, ro1, O)

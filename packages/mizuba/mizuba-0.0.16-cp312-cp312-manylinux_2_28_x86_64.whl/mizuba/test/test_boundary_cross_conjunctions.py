...
class boundary_cross_conjunctions_test_case(_ut.TestCase):
    ...
    def test_incoming(self):
        ...
        # The conjunction threshold and conjunction detection interval.
        cthresh = 0.25
        conj_det_interval = 0.03
        ...
        # Construct the polyjectories and polytrees.
        pj1 = polyjectory([traj_data[0]], [tm_data_0], [0])
        pt1 = polytree(pj1, conj_det_interval=conj_det_interval)
        pj2 = polyjectory([traj_data[1]], [tm_data_1], [0])
        pt2 = polytree(pj2, conj_det_interval=conj_det_interval)

        # Run conjunction detection.
        cj = detect_conjunctions(
            [
                catalog(pj1, pt1, self_conjunctions=False),
                catalog(pj2, pt2, self_conjunctions=False),
            ],
            conj_thresh=cthresh,
        )
        ...
        self.assertEqual(cj.conjunctions["cat_i"][0], 0)
        self.assertEqual(cj.conjunctions["cat_j"][0], 1)
        self.assertEqual(cj.conjunctions["i"][0], 0)
        self.assertEqual(cj.conjunctions["j"][0], 0)
        self.assertTrue(
    ...
    def test_outgoing(self):
        ...
        # Construct the polyjectories and polytrees.
        pj1 = polyjectory([traj_data[0]], [tm_data], [0])
        pt1 = polytree(pj1, conj_det_interval=conj_det_interval)
        pj2 = polyjectory([traj_data[1]], [tm_data], [0])
        pt2 = polytree(pj2, conj_det_interval=conj_det_interval)

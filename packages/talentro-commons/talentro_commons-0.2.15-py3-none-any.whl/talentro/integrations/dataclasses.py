...
class Integration(ResolvableModel):
    ...
    @classmethod
    async def from_model(cls: "Integration", model: IntegrationModel) -> "Integration":
        return cls(
            id=model.id,
...
class Link(ResolvableCompanyModel):
    ...
    async def resolve_object(object_id: UUID, organization_id: UUID) -> "Link | None":
        result = await ResolveClient.integrations().get(f"links/{object_id}", headers={"X-Organization-ID": str(organization_id)})

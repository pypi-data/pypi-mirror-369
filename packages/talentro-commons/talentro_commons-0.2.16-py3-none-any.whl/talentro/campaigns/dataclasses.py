...
class Campaign(ResolvableCompanyModel):
    ...
    async def resolve_object(object_id: UUID, organization_id: UUID) -> "Campaign | None":
        result = await ResolveClient.campaigns().get(f"campaigns/{object_id}/resolve", headers={"X-Organization-ID": str(organization_id)})
    ...
    async def from_model(cls: "Campaign", model: CampaignModel) -> 'Campaign':
        channel = await Link.resolve_object(model.channel_id, model.organization)

...
class Feed(ResolvableCompanyModel):
    id: UUID
    created_at: datetime
    updated_at: Optional[datetime]
    organization: UUID
    ...
    @staticmethod
    async def resolve_object(object_id: UUID, organization_id: UUID) -> "Feed | None":
        result = await ResolveClient.vacancies().get(f"feeds/{object_id}/resolve", headers={"X-Organization-ID": str(organization_id)})

        if result.status_code != 200:
            return None

        return Feed(**result.json())

    @classmethod
    async def from_model(cls: "Feed", model: FeedModel, ats_link: Optional[Link] = None) -> "Feed":
        ats_link = await Link.resolve_object(model.ats_link_id, model.organization) if ats_link is None else ats_link

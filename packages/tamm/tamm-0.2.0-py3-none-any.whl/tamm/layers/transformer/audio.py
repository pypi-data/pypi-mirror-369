...
This submodule implements transformer-specific components for audio models.

.. autoclass:: tamm.layers.AudioTransformerEncoder
    :show-inheritance:
    :members: forward
"""
...
class AudioTransformerEncoder(_transformer_stack.ExtendedTransformerStack):
    """
    A general layer for audio encoders.  This layer is the same as
    ...
    * The layer has an optional ``segmentation`` layer, and the :meth:`forward` method
    ...
    Args:
        segmentation: A layer for converting per-frame ``padding_mask`` inputs
            with shape ``(batch_size, input_seq_len)`` to per-token segment IDs
            with shape ``(batch_size, seq_len)`` for the transformer stack.
            Typically the segment IDs also form a padding mask with ``0`` for
            padding values and ``1`` for non-padding.
        ...
        *,
        ...
        Args:
            ...
            inputs (:obj:`torch.Tensor`): Inputs to the ``embedding`` layer, which
                typically are spectrogram with shape
                ``(batch_size, num_channels, num_freqs, num_frames)``.  For some values
                of ``mode``, this should instead be input embeddings with shape
            ...
            padding_mask (:obj:`torch.Tensor`, optional): An optional padding mask to
                pass to the ``segmentation`` layer.  Typically this is a frame-level

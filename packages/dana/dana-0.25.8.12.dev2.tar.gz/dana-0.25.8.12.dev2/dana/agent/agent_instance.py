...
def default_solve_method(
    ...
    if expertise_modules:
        # Build combined list of all importable workflows from all expertise modules
        expert_workflow_names: list[str] = []
        expert_workflow_name_to_module_map: dict[str, ...] = {}

        for expertise_module in expertise_modules:
            if hasattr(expertise_module, '__all__'):
                expert_workflow_names.extend(_expert_workflow_names := expertise_module.__all__)
...
"""
        ...
        if closest_matched_expert_workflow_name == 'NONE':
            ...
            print(f'\n{agent_instance.name} executing `{matched_expertise_module.__name__}.{closest_matched_expert_workflow_name}` on `{resources}`...\n')

            workflow_result = getattr(matched_expertise_module, closest_matched_expert_workflow_name)(resources)

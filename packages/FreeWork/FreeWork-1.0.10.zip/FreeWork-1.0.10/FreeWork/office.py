        ...
        for _ in range(Row - len(table.rows)):
            ...
            for idx, src_cell in enumerate(last_row.cells):
                ...
                for para_idx, src_para in enumerate(src_cell.paragraphs):
                    ...
                    pf_dest.alignment = pf_src.alignment
                    ...
                    for src_run in src_para.runs:
                        ...
                        # 复制字体名称，覆盖ascii、eastAsia、hAnsi
                        if src_font.name:
                            font.name = src_font.name
                            rFonts = font._element.rPr.rFonts
                            rFonts.set(qn('w:ascii'), src_font.name)
                            rFonts.set(qn('w:eastAsia'), src_font.name)
                            rFonts.set(qn('w:hAnsi'), src_font.name)

                        # 复制字号
                        if src_font.size:
                            font.size = src_font.size

                        # 复制字体颜色（存在且不为None）
                        if src_font.color and src_font.color.rgb:
                            font.color.rgb = src_font.color.rgb

                        # 复制粗体、斜体、下划线
                        font.bold = src_run.bold
                        font.italic = src_run.italic
                        font.underline = src_run.underline

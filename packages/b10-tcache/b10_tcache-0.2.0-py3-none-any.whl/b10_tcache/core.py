...
def load_compile_cache() -> bool:
    ...
    Returns:
        bool: True if cache was successfully loaded or already exists, False if
              b10fs is unavailable, no cache exists, local disk space is insufficient,
              or loading failed.

    Raises:
        CacheValidationError: If b10fs is not enabled (caught and returns False).
        CacheOperationInterrupted: If operations interrupted due to insufficient
                                  local disk space (caught and returns False).
        Exception: Any other errors during loading (caught and returns False).
    """
    with cache_operation("Load"):
        ...
        try:
            with temp_file_cleanup(temp_path):
                # Phase 1: Copy from b10fs to local temp in separate process
                logger.info(f"Starting copy from b10fs: {cache_file} -> {temp_path}")
                run_monitored_process(
...
def save_compile_cache() -> bool:
    ...
    Returns:
        bool: True if cache was successfully saved or already exists, False if
              b10fs is unavailable, insufficient disk space caused interruption,
              no cache exists to save, or saving failed.

    Raises:
        CacheValidationError: If b10fs is not enabled (caught and returns False).
        CacheOperationInterrupted: If operations interrupted due to insufficient
                                  disk space (caught and returns False).
        ArchiveError: If archive creation fails (caught and returns False).
        Exception: Any other errors during saving (caught and returns False).
    """
    with cache_operation("Save"):
        ...
        temp_file = (
            b10fs_dir
            / f"{cache_filename}.{hostname}{CACHE_INCOMPLETE_SUFFIX}{CACHE_FILE_EXTENSION}"
        )

        # Check if an incomplete file is already being uploaded to b10fs, in which case another replica is already saving the compile cache
        if any(
            f.name.endswith(f"{CACHE_INCOMPLETE_SUFFIX}{CACHE_FILE_EXTENSION}")
            for f in b10fs_dir.iterdir()
        ):
            logger.info("Found existing incomplete temp file, skipping save")
            return True

...
from ..utils import ExportError, ModelError, get_logger
...
class ModelQuantizer:
    ...
    def __init__(self, config: Optional[QuantizationConfig] = None):
        ...
        logger.info(
            f"ModelQuantizer initialized with {self.config.method.value} quantization"
        )

    def quantize(
        self, model: nn.Module, output_path: Optional[Union[str, Path]] = None
    ) -> nn.Module:
        """
    ...
    def _dynamic_quantization(self, model: nn.Module) -> nn.Module:
        ...
        quantized_model = torch.quantization.quantize_dynamic(
            model, qconfig_spec=layers_to_quantize, dtype=self.config.dtype
        )
...
def estimate_quantization_benefits(
    model: nn.Module, method: Union[str, QuantizationMethod] = "dynamic"
) -> Dict[str, Any]:
    """

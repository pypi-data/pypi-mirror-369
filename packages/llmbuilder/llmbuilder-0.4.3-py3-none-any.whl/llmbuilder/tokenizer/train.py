...
class TokenizerTrainer:
    ...
    def prepare_training_data(
        ...
        logger.info(
            f"Training data prepared: {output_path} ({total_chars:,} characters)"
        )
        return output_path
    ...
    def _test_tokenizer(self, model_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_compression = sum(r["compression_ratio"] for r in results) / len(
                results
            )
    ...
    def create_tokenized_dataset(
        ...
        try:
            ...
            if max_length and len(tokens) > max_length:
                logger.warning(
                    f"Truncating tokens from {len(tokens):,} to {max_length:,}"
                )
    ...
    def validate_tokenizer(self, model_file: Union[str, Path]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Tokenizer validation: {'PASSED' if validation_results['valid'] else 'FAILED'}"
            )
            ...
            return {"model_file": str(model_file), "valid": False, "error": str(e)}

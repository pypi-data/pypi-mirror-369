...
from ..utils import ModelError, get_logger, load_checkpoint, save_checkpoint
...
class ModelBuilder:
    ...
    def build_model(cls, config: Union[ModelConfig, Dict[str, Any]]) -> GPTModel:
        ...
        try:
            ...
            logger.info(
                f"Model built successfully: {model.get_num_params():,} parameters"
            )
            return model
    ...
    def load_model(
        ...
        try:
            ...
            if "metadata" in checkpoint:
                ...
                logger.info(
                    f"Loaded model: {metadata.get('num_parameters', 'unknown')} parameters"
                )
    ...
    def validate_model(cls, model: GPTModel) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Model validation: {'PASSED' if validation_results['valid'] else 'FAILED'}"
            )
...
def load_model(
    model_path: Union[str, Path], device: Optional[str] = None, strict: bool = True
) -> GPTModel:
    """Load a model from checkpoint."""
...
def save_model(
    ...
    """Save a model checkpoint."""
    return ModelBuilder.save_model(
        model, save_path, include_optimizer, optimizer_state, additional_info
    )

...
class DataLoader:
    ...
    def load_file(self, file_path: Union[str, Path]) -> Optional[str]:
        ...
        try:
            ...
            if text and len(text) < self.min_length:
                logger.debug(
                    f"Skipping {file_path.name}: too short ({len(text)} chars)"
                )
                return None

            if self.max_length and text and len(text) > self.max_length:
                logger.debug(
                    f"Truncating {file_path.name}: too long ({len(text)} chars)"
                )
    ...
    def get_file_metadata(
        self, file_path: Union[str, Path], text: Optional[str] = None
    ) -> DocumentMetadata:
        """

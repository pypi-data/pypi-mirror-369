...
def split_dataset(
    ...
    """
    Split dataset into train/validation/test sets with robust rounding.

    Ensures the integer split sizes sum exactly to len(dataset), even for
    very small datasets, by distributing any remainder based on fractional parts.
    ...
    Returns:
        Tuple[Dataset, ...]: (train, val, test) if test_ratio>0 else (train, val)
    """
    # Validate ratios
    ...
    if total_size == 0:
        if test_ratio > 0:
            return random_split(dataset, [0, 0, 0])  # type: ignore
        else:
            return random_split(dataset, [0, 0])  # type: ignore

    # Compute raw sizes and base integer sizes
    ratios = [train_ratio, val_ratio, test_ratio]
    raw_sizes = [r * total_size for r in ratios]
    sizes = [int(x) for x in raw_sizes]

    # Distribute remainder to splits with largest fractional parts
    remainder = total_size - sum(sizes)
    if remainder != 0:
        fracs = [raw - base for raw, base in zip(raw_sizes, sizes)]
        ...
        while remainder > 0 and i < len(order):
            idx = order[i]
            # Only assign to splits that have a positive ratio
            if ratios[idx] > 0:
                sizes[idx] += 1
                remainder -= 1
            i += 1
        # If remainder still remains (e.g., only some ratios > 0), dump into train
        if remainder > 0:
            sizes[0] += remainder
            remainder = 0
    ...
    # Guarantee non-negative and correct sum
    ...
    if test_ratio > 0:
        ...
        # Merge test into train for the two-way split path
        two_way_train_size = train_size + test_size
        two_way_val_size = val_size
        # Ensure sum matches
        assert two_way_train_size + two_way_val_size == total_size
        train_dataset, val_dataset = random_split(
            dataset, [two_way_train_size, two_way_val_size], generator=generator
        )
        logger.info(
            f"Dataset split: {two_way_train_size} train, {two_way_val_size} val (val_ratio={val_ratio})"
        )

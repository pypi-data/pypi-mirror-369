...
from ..utils import TrainingError, get_logger, load_checkpoint, save_checkpoint
...
class Trainer:
        ...
        if scheduler_type == "cosine":
            ...
            def lr_lambda(step):
                ...
                return max(
                    ...
                    / max(1, self.state.total_steps - self.config.warmup_steps),
                )
    ...
    def setup_data(
        ...
        logger.info(
            f"Steps per epoch: {len(self.train_loader)}, Total steps: {self.state.total_steps}"
        )
    ...
    def train_epoch(self, epoch: int) -> TrainingMetrics:
        ...
        if self.model is None or self.optimizer is None or self.train_loader is None:
            raise TrainingError(
                "Model, optimizer, and data loader must be setup before training"
            )
        ...
        for step, batch in enumerate(progress):
            ...
            try:
                ...
                # Log progress and update progress bar
                ...
                if "out of memory" in str(e):
                    logger.error(
                        f"Out of memory at step {step}. Try reducing batch size."
                    )
                    if torch.cuda.is_available():
    ...
    def validate(self) -> float:
        ...
        if self.model is None or self.val_loader is None:
            logger.warning(
                "Model or validation loader not available, skipping validation"
            )
            return float("inf")
                ...
                try:
                    ...
                    if not (torch.isnan(loss) or torch.isinf(loss)):
                        ...
                        if total_samples > 0:
                            ...
                            progress.set_postfix({"val_loss": f"{avg:.4f}"})
    ...
    def save_checkpoint(
        ...
        checkpoint_data = {
            ...
            # Add model config so inference can reconstruct the model
            "model_config": self.model.get_config_dict() if self.model else None,
            "model_type": "gpt",
    ...
    def load_checkpoint(self, checkpoint_path: Union[str, Path]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Checkpoint loaded: epoch {self.state.epoch}, step {self.state.step}"
            )
            return True
    ...
    def evaluate_model(
        self, dataset: torch.utils.data.Dataset, metrics: Optional[List[str]] = None
    ) -> Dict[str, float]:
        """

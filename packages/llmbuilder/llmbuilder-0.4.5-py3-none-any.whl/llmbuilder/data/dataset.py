...
class TextDataset(Dataset):
    ...
    def __init__(
        ...
        logger.info(
            f"Dataset initialized: {len(self.tokens):,} tokens, {self.num_samples:,} samples"
        )
    ...
    def _calculate_num_samples(self) -> int:
        ...
        if len(self.tokens) < self.block_size:
            logger.warning(
                f"Token sequence ({len(self.tokens)}) shorter than block size ({self.block_size})"
            )
            return 0
...
class MultiFileDataset(Dataset):
    ...
    def __init__(
        ...
        for file_path in self.data_files:
            ...
            logger.debug(
                f"File {file_path.name}: {len(tokens)} tokens, {num_samples} samples"
            )

        self.total_samples = total_samples
        logger.info(
            f"Multi-file dataset: {len(self.data_files)} files, {total_samples:,} total samples"
        )
...
def split_dataset(
    ...
    if test_ratio > 0:
        ...
        logger.info(
            f"Dataset split: {train_size} train, {val_size} val, {test_size} test"
        )
...
def save_dataset(
    dataset: Dataset, output_path: Union[str, Path], format: str = "pt"
) -> None:
    """
...
def load_dataset(
    data_path: Union[str, Path], block_size: int = 1024, stride: Optional[int] = None
) -> TextDataset:
    """
...
def get_dataset_info(dataset: Dataset) -> dict:
    ...
    """
    info = {"num_samples": len(dataset), "dataset_type": type(dataset).__name__}

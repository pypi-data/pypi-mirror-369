...
class TokenizerManager:
    ...
    def _check_dependencies(self):
        ...
        if not HAS_SENTENCEPIECE:
            logger.warning(
                "sentencepiece library not found. Install with: pip install sentencepiece"
            )
    ...
    def load_tokenizer(self, tokenizer_path: Union[str, Path]) -> Union[Tokenizer, Any]:
        ...
        try:
            ...
            if HAS_SENTENCEPIECE and tokenizer_path.is_file() and tokenizer_path.suffix == '.model':
            ...
            raise TokenizerError(
                f"Could not determine tokenizer format for: {tokenizer_path}"
            )
    ...
    def save_tokenizer(
        self, tokenizer: Any, output_path: Union[str, Path], format_type: str = "auto"
    ):
        """
        ...
        try:
            ...
            raise TokenizerError(
                f"Failed to save tokenizer: {str(e)}", tokenizer_path=str(output_path)
            ) from e
    ...
    def validate_tokenizer(self, tokenizer: Any) -> Dict[str, Any]:
        ...
        """
        info = {"type": type(tokenizer).__name__, "valid": True, "errors": []}
        ...
        try:
            ...
            if hasattr(tokenizer, "encode"):
                ...
                info["vocab_size"] = getattr(
                    tokenizer,
                    "vocab_size",
                    len(tokens) if isinstance(tokens, list) else len(tokens.ids),
                )
                info["test_tokens"] = (
                    tokens[:10] if isinstance(tokens, list) else tokens.ids[:10]
                )
            ...
            if (
                ...
                and "test_tokens" in info
                and info["test_tokens"]
            ):
                try:
...
class TokenizerWrapper:
    ...
    def _detect_interface(self):
        ...
        # Check for SentencePiece first
        if HAS_SENTENCEPIECE and isinstance(self.tokenizer, spm.SentencePieceProcessor):
            self.interface_type = "sentencepiece"
        elif hasattr(self.tokenizer, "encode") and hasattr(self.tokenizer, "decode"):
    ...
    def encode(self, text: str, add_special_tokens: bool = True) -> List[int]:
        ...
        Returns:
            List of token IDs
        """
        if self.interface_type == "sentencepiece":
            # SentencePiece doesn't have add_special_tokens parameter
            return self.tokenizer.encode(text, out_type=int)
        elif self.interface_type == "transformers":
            ...
            raise TokenizerError(
                f"Unsupported tokenizer interface: {self.interface_type}"
            )
    ...
    def decode(self, token_ids: List[int], skip_special_tokens: bool = True) -> str:
        ...
        """
        if self.interface_type == "sentencepiece":
            # SentencePiece doesn't have skip_special_tokens parameter
            return self.tokenizer.decode(token_ids)
        elif self.interface_type in ["transformers", "transformers_callable"]:
    ...
    def batch_decode(
        self, token_ids_list: List[List[int]], skip_special_tokens: bool = True
    ) -> List[str]:
        """
        ...
        """
        return [
            self.decode(token_ids, skip_special_tokens) for token_ids in token_ids_list
        ]
    ...
    def unk_token_id(self) -> Optional[int]:
        ...
        elif hasattr(self.tokenizer, "unk_token_id"):

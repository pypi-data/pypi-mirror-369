...
from planqk.quantum.sdk.client.job_dtos import RuntimeJobParamsDto, JobDto
...
class PlanqkQiskitRuntimeService(PlanqkQuantumProvider):
    ...
    def backend(
        ...
        backend = self.get_backend(backend_id=name)
    ...
    def _get_backend_object(self, backend_info):
        backend_init_params = {'backend_info': backend_info}
        if backend_info.provider in {Provider.IBM, Provider.IBM_CLOUD, Provider.TSYSTEMS}:
            from planqk.quantum.sdk.qiskit.providers.ibm.ibm_runtime_backend import PlanqkIbmRuntimeBackend
            return PlanqkIbmRuntimeBackend(planqk_client=self._client, **backend_init_params)
        else:
            return QiskitBackendNotFoundError(
                f"Backends of provider '{backend_info.provider}' are not supported.")
    ...
    def run(self,
        ...
        job_input_format = backend._get_job_input_format()
        input_data = backend._convert_to_job_input(inputs)
        ...
        return PlanqkRuntimeJob(backend=backend, job_details=job_request, result_decoder=result_decoder, planqk_client=self._client)
    ...
    def job(self, job_id: str) -> PlanqkRuntimeJob:
        ...
        return PlanqkRuntimeJob(backend=backend, job_id=job_id, job_details=job_details, planqk_client=self._client)

...
# We add a page
@index.page(route="/home", title="Flet-Easy", cache=True)
async def index_page(data: fs.Datasy):
    view = data.view

    tx = ft.Text("Home page")

    async def update_title():
        x = 1
        for i in range(10):
            x += 1
            tx.value = f"Home page {x}"
            await asyncio.sleep(1)
            data.page.update()

    data.page.run_task(update_title)



    #data.dynamic_control(view.appbar, update_title)
    return ft.View(
        controls=[
            tx,

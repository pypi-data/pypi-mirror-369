...
class GPTVision:
    ...
    async def _ainvoke_with_retry(
        ...
        *,
        ...
        attempt = 0
        payload = list(messages)  # ensure materialized list

        while True:
            try:
                task = asyncio.create_task(self.lang_client.ainvoke(payload))
                try:
                    response = await asyncio.wait_for(task, timeout=self.request_timeout_s)
                except asyncio.TimeoutError as te:
                    # Cancel in-flight task cleanly
                    task.cancel()
                ...
                # Classify timeout + common transient failures as retryable
                is_transient = self._is_transient_error(e, retry_http_statuses)
                if not is_transient:
                    raise
                ...
                logger.warning(
                    f"ainvoke transient error {file_or_url}: (attempt {attempt}/{max_retries}): {type(e).__name__}: {e}. "
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                logger.error(
                                    f"ocr_page: Page {page_index} timed out after {self.page_timeout_s}s"
                                )

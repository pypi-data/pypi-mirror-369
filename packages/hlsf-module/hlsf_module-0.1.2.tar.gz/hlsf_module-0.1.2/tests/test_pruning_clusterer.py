...
def test_collapse_with_metric_dicts():
    graph = {0: {"weight": 0.5}, 1: {"weight": 0.2}, 2: {"weight": 0.1}}
    removed = clusterer.collapse(graph, density=2)
    assert removed == [2]
    assert graph[0]["weight"] == pytest.approx(0.6)
    assert graph[1]["weight"] == pytest.approx(0.2)


def test_collapse_after_rotation_with_metric_dicts():
    graph = {0: {"weight": 0.5}, 1: {"weight": 0.4}, 2: {"weight": 0.3}}
    angles = {0: 0.0, 1: 5.0, 2: 90.0}
    removed = clusterer.collapse_after_rotation(graph, angles, delta=10.0)
    assert removed == [1]
    assert graph[0]["weight"] == pytest.approx(0.9)
    assert graph[2]["weight"] == pytest.approx(0.3)

...
from galileo_core.helpers.api_client import ApiClient, SSLContext
...
class GalileoConfig(BaseSettings):
    ...
    def set_ssl_context(cls, value: Union[SSLContext, bool]) -> Union[SSLContext, bool]:
        ...
        if value is False:
            ...
            logger.debug("SSL validation is disabled.")
        elif ssl_cert_file or ssl_cert_dir:
            logger.debug(f"SSL context is set to custom cert file ({ssl_cert_file}) or directory ({ssl_cert_dir}).")
            value = create_default_context(cafile=ssl_cert_file, capath=ssl_cert_dir)
        else:
            logger.debug("SSL context is set to the default context.")
            value = True
        return value

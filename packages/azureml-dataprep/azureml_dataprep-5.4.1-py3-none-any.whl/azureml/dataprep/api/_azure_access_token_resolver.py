...
def _resolve_azure_access_token(scope, sp, client_id=None, endpoint_type=None, authority=None):
    ...
    if identity_type == _IdentityType.MANAGED:
        ...
        if endpoint_type == 'Imds':
            ...
            #In azure-identity 1.6.0, the AppServiceCredential is repurposed and can no longer be used for Azure ml flow.
            #AzureMLCredential is introduced which is used to get token for the AzureML workspace and expects MSI_ENDPOINT and MSI_SECRET to be set.
            #https://github.com/Azure/azure-sdk-for-python/blob/azure-identity_1.16.0/sdk/identity/azure-identity/azure/identity/_credentials/azure_ml.py
            #ManagedIdentityCredential has the logic to call appropriate credential based on the environment variables set. This is consistent with all versions of azure-identity.
            #It will internally call AzureMLCredential if MSI_ENDPOINT and MSI_SECRET are set in azure-indentity 1.16.0
            #It will internally call AppServiceCredential if MSI_ENDPOINT is set in azure-indentity 1.7.0
            #https://github.com/Azure/azure-sdk-for-python/blob/azure-identity_1.16.0/sdk/identity/azure-identity/azure/identity/_credentials/managed_identity.py#L65
        ...
        if is_obo_enabled == "True":
            ...
            if always_use_device_code:
                ...
                credential = ChainedTokenCredential(
                    ...
                    DeviceCodeCredential(AZURE_CLIENT_ID, **kwargs),
                    AzureCliCredential()
                )

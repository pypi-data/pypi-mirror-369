...
def _get_schema(dataflow):
    ...
    record_batches, _, _ = _execute('_spark_helper._get_schema', dataflow, collect_results=True, partition_ids=[0])
    def formatter(field):
        return f'`{field.name}` {__TYPE_MAP.get(field.type, field.type)}'

    # there could be many partitions in the data, but only non-empty partitions would have a record batch here
    # so need to guard against empty set here.
    return ', '.join(formatter(field) for field in record_batches[0].schema) if len(record_batches) > 0 else ''


def _get_partitions(dataflow):
    return _execute('_spark_helper._get_partitions', dataflow, collect_results=False)[1]


def _get_schema_and_partitions(dataflow):
    partition_count = _get_partitions(dataflow)
    return _get_schema(dataflow) if partition_count > 0 else '', partition_count
...
def _to_spark_dataframe_2x_pandas_apply(dataflow, spark_session, auxiliary_environment_vars) -> 'pyspark.sql.DataFrame':
    ...
    if num_partitions == 0:
        return spark_session.createDataFrame(spark_session.sparkContext.emptyRDD(), schema)
    ...
    def fetch_partitions(partition_df):
        ...
        record_batches, _, _ = _execute('_spark_helper._to_spark_dataframe_2x_pandas_apply', dataflow, collect_results=True, partition_ids=partitions_to_fetch)
...
def _to_spark_dataframe_3x_pandas_map(dataflow, spark_session, auxiliary_environment_vars) -> 'pyspark.sql.DataFrame':
    ...
    def fetch_partitions(partition_dfs: Iterator[pandas.DataFrame]) -> Iterator[pandas.DataFrame]:
        ...
        record_batches, _, _ = _execute('_spark_helper._to_spark_dataframe_3x_pandas_map', dataflow, collect_results=True, partition_ids=partitions_to_fetch)

...
try:
    ...
    version = get_version("azureml-dataprep")
...
node_rank = os.environ.get('AZUREML_CR_NODE_RANK')
run_id = os.environ.get('AZUREML_RUN_ID')
log_directory = os.path.join(tempfile.gettempdir(), 'azureml-logs', 'dataprep')
if run_id and node_rank:
    # override when in run context and multinode in CR
    log_directory = os.path.join(os.getcwd(), 'logs', 'azureml', 'dataprep', node_rank)
elif run_id:
    # override when in run context and not multinode
...
def _log_not_supported_api_usage_and_raise(logger, api_name: str, suggestion: str = None):
    _LoggerFactory.trace_error(logger,
...
class _LoggerFactory:
    ...
    def track_activity(logger, activity_name, activity_type=DEFAULT_ACTIVITY_TYPE, custom_dimensions=None):
        ...
        if run_info is not None:
            # change to only update for key that doesnt exist
            for key, value in run_info.items():
                custom_dimensions.setdefault(key, value)
        if telemetry_enabled:
...
def track(get_logger, activity_type=DEFAULT_ACTIVITY_TYPE, custom_dimensions=None):
    def monitor(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            logger = get_logger()
            with _LoggerFactory.track_activity(logger, func.__name__, activity_type, custom_dimensions) as activityLogger:
                try:
...
def _log_dataflow_execution_activity(logger,
                                     activity,
                                     rslex_failed=False,
                                     rslex_error=None,
    ...
    if extra_props is not None and isinstance(extra_props, dict):
        builder.update(**extra_props)

...
from ._pandas_helper import have_pyarrow
from .errorhandlers import PyArrowImportError
...
def get_total_count(batch):
    raw_kinds = batch['valueKinds'][0]
    if raw_kinds is None:
        return 0
    kinds = next(iter(raw_kinds.values()))
    if kinds is None:
        return 0
...
def get_inspector_response_from_batches(batches):
    ...
    raw_columns_aggregated = next(iter(aggregates.values()))[0]
    if raw_columns_aggregated is None:
        return ExecuteInspectorCommonResponse(
            column_definitions=[], rows_data=RowsData([]))

    columns_aggregated = list(raw_columns_aggregated.keys())
...
class _Inspector:
    @classmethod
    def _from_rslex_execution(cls,
                        script: str,
                        span_context: 'DPrepSpanContext' = None):
        if not have_pyarrow():  # pyarrow required for execute_from_dataframe
            raise PyArrowImportError()

        try:
            (record_batches,
             _,
             _) = _execute('_Inspector._from_rslex_execution',
                           script,

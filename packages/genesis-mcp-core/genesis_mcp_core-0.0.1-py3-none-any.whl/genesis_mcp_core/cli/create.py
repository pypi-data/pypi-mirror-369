...
def create_server(name: str, template: str, output_dir: Path) -> None:
...
def copy_template_files(template_dir: Path, project_path: Path, template_vars: Dict[str, Any]) -> None:
    ...
    for template_file in template_dir.rglob("*"):
        if template_file.is_file():
            ...
            should_process = (
                template_file.suffix in [".py", ".toml", ".md", ".txt", ".env", ".json", ".yaml", ".yml"] or
                template_file.name.startswith("env.") or
                template_file.name in ["Dockerfile", "Makefile", ".gitignore"]
            )
            ...
            if should_process:
                ...
                try:
                    ...
                    logger.warning(f"⚠️ Fallback: Copied {rel_path} without processing")
            else:

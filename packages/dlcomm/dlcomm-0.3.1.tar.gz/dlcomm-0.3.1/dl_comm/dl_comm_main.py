                ...
                for i in range(warmup_iters):
                    ...
                    if comm_mode == "flatview":
                        if flat_group is not None and participating:
                            result = run_collective(framework=framework , x, op_obj, group=flat_group, dist=dist )
                    
                    elif comm_mode == "within_node":
                        if my_within_group is not None and participating:
                            result = run_collective(framework=framework, x, op_obj, group=my_within_group, dist=dist, log=log)
                    
                    elif comm_mode == "across_node":
                        if my_across_group is not None and participating:
                            result = run_collective(framework=framework, x, op_obj, group=my_across_group, dist=dist, log=log  )

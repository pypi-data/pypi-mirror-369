...
class User(TgUser, TsTrait):
    ...
    async def balance_xp(self, cur_id: int = None) -> dict[int, int] | int:
        if cur_id:
            dbt = (
                await Transaction.filter(receiver=self, status=TransactionStatus.valid, cur_id=cur_id)
                .group_by("cur_id")
                .annotate(debit=Sum("amount"))
                .values_list("debit", flat=True)
            ) or [0]
            crd = (
                await Transaction.filter(sender=self, status=TransactionStatus.valid, cur_id=cur_id)
                .group_by("cur_id")
    ...
    async def get_val_id(self, amount: int, cur_id: int, receiver_id: int) -> "Transaction":
        r = {
            c: v
            for c, v in await Transaction.filter(
                receiver_id__not_in=[self.id, receiver_id], status=TransactionStatus.valid, cur_id=cur_id
            )
            .group_by("receiver_id")
            .annotate(debet=Sum("amount"))
            .values_list("receiver_id", "debet")
        }
        s = {
            c: v
            for c, v in await Transaction.filter(
                sender_id__not_in=[self.id, receiver_id], status=TransactionStatus.valid, cur_id=cur_id
            )
            .group_by("sender_id")
            .annotate(credit=Sum("amount"))
            .values_list("sender_id", "credit")
        }
        vals = {c: am for c in r.keys() | s.keys() if c and (am := r.get(c, 0) - s.get(c, 0)) > amount}
    ...
    async def send(self, amount: int, cur_id: int, receiver_id: int) -> "Transaction":
        ...
        val_id = await self.get_val_id(amount, cur_id, receiver_id)
        async with in_transaction():
            ...
            transaction.proof = transaction.validator.proof_trans(transaction, sender_sig)
...
class Transfer(Model):
    ...
    file_id: int
    pmid: int = BigIntField(null=True)
    created_at: datetime | None = DatetimeSecField(auto_now_add=True)
    accepted_at: datetime | None = DatetimeSecField(null=True)

    class Meta:
        unique_together = (("pmid", "order"),)

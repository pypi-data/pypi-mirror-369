...
from .formatter import ResultFormatter, OutputFormat, create_formatter


class FastShell:
    """Main FastShell application class."""
    
    def __init__(self, name: str = "fastshell", description: str = "", use_pydantic: bool = True, output_format: str = "auto"):
            ...
            try:
                ...
                if command_line.lower().startswith('format '):
                    ...
                    if format_type in self.get_available_formats():
                        ...
                        self.console.print(f"[dim]Available formats: {', '.join(self.get_available_formats())}[/dim]")
                    continue
                
                if command_line.lower() == 'format':
                    self.console.print(f"[cyan]Current format: {self.formatter.default_format.value}[/cyan]")
                    self.console.print(f"[dim]Available formats: {', '.join(self.get_available_formats())}[/dim]")

...
class SubscriptionDetail(BaseModel):
    ...
    description: Optional[StrictStr] = Field(default=None, description="Description of the subscription")
    notification_url: StrictStr = Field(description="The URL of the webhook endpoint to which event messages will be delivered", alias="notificationUrl")
    signature: SignatureSignature
    events: List[EventSubscription]
    __properties: ClassVar[List[str]] = ["id", "description", "notificationUrl", "signature", "events"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "signature": SignatureSignature.from_dict(obj["signature"]) if obj.get("signature") is not None else None,
            "events": [EventSubscription.from_dict(_item) for _item in obj["events"]] if obj.get("events") is not None else None
        })
        return _obj

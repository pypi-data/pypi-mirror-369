   ...
   '''
    ...
    # {
        ...
        if "nested_logit" in self.models_avail:
            self.nests = kwargs.get('nests') or {"Nest1": [1, 2], "Nest2": [3, 4]}  # Default structure
            self.lambdas = kwargs.get('lambdas') or {"Nest1": 0.8, "Nest2": 1.0}  # Default lambda values

            # Validate nests and lambdas
            if set(self.nests.keys()) != set(self.lambdas.keys()):
                raise ValueError("Nests and lambdas must have the same keys.")
        else:
...
# {
    ...
    def print_best_solution(self, solution):
        """
   ...
   # {
    ...
    def fit_nested(self, X, y, varnames, isvars, alts, ids, nests, lambdas, fit_intercept):
        model = NestedLogit(X, y, varnames, isvars, alts, ids, nests, lambdas, fit_intercept)
    ...
    def evaluate_nested_logit(self, sol):
        ...
        if self.mae_is_an_objective():
            X_test = self.param.df_test[all_vars].values
            y_test = self.param.test_choices
            test_model = NestedLogit(X=X_test, y=y_test, varnames=all_vars, isvars=is_vars,
        ...
        mae = model.mae
        return aic, bic, loglik, mae, as_vars, is_vars, {}, [], [], converged

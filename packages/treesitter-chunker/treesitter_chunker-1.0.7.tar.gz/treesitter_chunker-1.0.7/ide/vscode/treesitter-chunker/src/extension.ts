...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('treesitter-chunker.chunkFile', () => chunkCurrentFile()),
        vscode.commands.registerCommand('treesitter-chunker.chunkWorkspace', () => chunkWorkspace()),
        vscode.commands.registerCommand('treesitter-chunker.showChunks', () => showChunks()),
        vscode.commands.registerCommand('treesitter-chunker.exportChunks', () => exportChunks())
    );
    ...
    async function exportChunks() {
        ...
        const format = vscode.workspace.getConfiguration('treesitter-chunker').get('exportFormat') as string;
    ...
    function runChunker(filePath: string, language: string): Promise<Chunk[]> {
        return new Promise((resolve, reject) => {
            const pythonPath = vscode.workspace.getConfiguration('treesitter-chunker').get('pythonPath') as string;
            const process = spawn(pythonPath, ['-m', 'chunker.cli', 'chunk', filePath, '-l', language, '--json']);
    ...
    function runExport(filePath: string, outputPath: string, format: string): Promise<void> {
        return new Promise((resolve, reject) => {
            const pythonPath = vscode.workspace.getConfiguration('treesitter-chunker').get('pythonPath') as string;
            const process = spawn(pythonPath, ['-m', 'chunker.cli', 'export', filePath, outputPath, '-f', format]);
    ...
    function highlightChunks(editor: vscode.TextEditor, chunks: Chunk[], decorationType: vscode.TextEditorDecorationType) {
        const decorations: vscode.DecorationOptions[] = chunks.map(chunk => {
    ...
    function getChunkViewHtml(chunks: Chunk[]): string {
        return `<!DOCTYPE html>
        <html lang="en">
        <head>
            <meta charset="UTF-8">

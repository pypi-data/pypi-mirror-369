...
class Table:
    ...
    def aggregate(
        ...
        **kwargs,
        ...
        for b in self._select(type="aggregate", by=group_by or "'TOTAL'", inner_query=filter, aggr=aggr, vars=vars):
    ...
    def _query_cursor(
        ...
        def scanner(scanner_cursor: str) -> Iterator[pa.RecordBatch]:
            logging.info("selecting cursor=%s, filter=%s", scanner_cursor, filter)

...
class SkillExtractorRefactored:
    ...
    def _extract_ksa_skills(self, input_data: Dict[str, str], input_type: str, id_column: str) -> List[Dict[str, Any]]:
        ...
        if self.llm is not None:
            ...
            esco_skills = self.get_top_esco_skills(input_data.get("description", ""), top_k=15)
            esco_skill_names = [skill['Skill'] for skill in esco_skills]
            
            prompt = PromptBuilder.build_ksa_extraction_prompt(
                input_data, input_type, 5, "3-5", "3-5", esco_skill_names
            )
            response = llm_router(prompt, self.model_id, self.use_gpu, self.llm, 
                                self.tokenizer, self.model, self.api_key)
    ...
    def display_ksa_results(self, skills: List[Dict[str, Any]], detailed: bool = True, show_levels: bool = False, show_esco_matches: bool = True) -> None:
        """
        ...
        show_esco_matches : bool, optional
            Whether to show top ESCO skill matches (default: True)
        """
        if not skills:
            print("No skills extracted.")
            return
        ...
        for i, skill in enumerate(skills, 1):
            ...
            # Show ESCO matches if requested
            if show_esco_matches:
                try:
                    esco_matches = self.get_top_esco_skills(skill_name, top_k=2)
                    if esco_matches:
                        print(f"     ESCO Matches: ", end="")
                        match_strs = [f"{match['Skill']} ({match['Similarity']:.2f})" for match in esco_matches[:3]]
                        print(", ".join(match_strs))
                except Exception as e:
                    print(f"     ESCO Matches: Unable to fetch ({str(e)[:50]}...)")
    ...
    def display_esco_analysis(self, input_text: str, top_k: int = 10) -> None:
        ...
        """
        try:
            print(f"\nTop {top_k} ESCO Skills matches for input:")
            print("-" * 50)
            
            esco_matches = self.get_top_esco_skills(input_text, top_k=top_k)
            
            if esco_matches:
                for i, match in enumerate(esco_matches, 1):
                    skill_name = match.get('Skill', 'Unknown')
                    similarity = match.get('Similarity', 0.0)
                    rank = match.get('Rank', i)
                    
                    print(f"  {rank:2d}. {skill_name} (Similarity: {similarity:.3f})")
            else:
                print("  No ESCO matches found.")
                
        except Exception as e:
            print(f"  Error retrieving ESCO matches: {e}")

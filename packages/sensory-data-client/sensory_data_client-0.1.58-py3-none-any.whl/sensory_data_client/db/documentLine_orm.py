...
class DocumentLineORM(Base):
    ...
    __table_args__ = (
        UniqueConstraint("doc_id", "line_no", name="uq_document_lines_doc_line_no"),
        Index("ix_document_lines_doc_id", "doc_id"),
        Index("ix_document_lines_page_idx", "page_idx"),
    )
    id:          Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True),
                                              primary_key=True, default=uuid4)
    doс_id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("documents.id"), nullable=False)

    ...
    def _collect_operations(self, spec_dict: Dict[str, Any]) -> List[ParsedOperation]:
        ...
        for path, path_item in paths.items():
            ...
            for method in http_methods:
                ...
                # HACK: Add api_key parameter if using simplified auth
                if self.config.api_key_as_required_param:
                    api_key_param = Parameter(
                        name="api_key",
                        param_type=ParameterType.string,
                        description="API key identifier for authentication",
                        required=True
                    )
                    parameters.append(api_key_param)
                
                # Extract auth requirements (omit if using api_key_as_required_param)
                auth_requirements = None
                if not self.config.api_key_as_required_param:
                ...
                parsed_op = ParsedOperation(
                    ...
                    use_api_key_param=self.config.api_key_as_required_param  # HACK: Flag for auth handling
                )

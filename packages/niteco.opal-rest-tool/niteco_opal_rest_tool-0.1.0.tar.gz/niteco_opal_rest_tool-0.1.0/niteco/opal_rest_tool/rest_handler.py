...
class RESTHandler:
    ...
    async def execute_operation(self, 
        ...
        try:
            ...
            # HACK: Handle simplified auth using api_key parameter
            if operation.use_api_key_param and "api_key" in parameters:
                # Get auth value from environment mapping
                auth_value = get_auth_value(parameters["api_key"])
                if auth_value:
                    # Override auth_data with the mapped value
                    auth_data = {"Authorization": auth_value}

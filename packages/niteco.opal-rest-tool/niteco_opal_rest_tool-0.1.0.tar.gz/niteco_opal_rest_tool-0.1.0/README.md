...
- Maps OpenAPI security schemes to authentication requirements
...
- Handles multiple authentication methods (API key, Bearer, OAuth2)
...
The RESTToolService supports various authentication methods:

### API Key
```python
auth_data = {"api_key": "your-api-key"}
```

### Bearer Token
```python
auth_data = {"token": "your-bearer-token"}
```

### OAuth2
```python
auth_data = {"access_token": "your-oauth-token"}
```
...
```
OPAL/
├── niteco/opal_rest_tool/                                    # Core system components
...
├── rest_tools_example/                     # Example implementations
│   ├── __init__.py                         # Package initialization

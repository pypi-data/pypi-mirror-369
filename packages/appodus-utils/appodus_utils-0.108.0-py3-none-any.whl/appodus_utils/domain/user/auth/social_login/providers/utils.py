...
class OauthUtils:
    ...
    async def get_auth_redirect_url(platform: SocialAuthPlatform, request: Request) -> str:
        url = request.url # e.g https://4ca4e9bd495f.ngrok-free.app/v1/users/auths/socials/google/init?operation_type=signup

        # Remove query params and drop "/init"
        base_path = url.path.rsplit("/", 1)[0] + "/"

        base_url = urlunparse((url.scheme, url.netloc, base_path, "", "", "")) # e.g https://4ca4e9bd495f.ngrok-free.app/v1/users/auths/socials/google/

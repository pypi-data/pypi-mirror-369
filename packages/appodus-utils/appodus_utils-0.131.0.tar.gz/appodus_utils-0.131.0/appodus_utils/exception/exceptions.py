...
class Social0AuthException(AppodusBaseException):
    def __init__(self,
                 data: Any,
                 response: Response,
                 message: str = None,
                 status_code: int = status.HTTP_200_OK,
                 code: str = "SOCIALAUTH_SUCCEEDED",
                 ):
        self.message = message
        self.data = data
        ...
        super().__init__(
            message=message or "Auth operation succeeded",
            status_code=status_code,
            code=code
        )

class Social0AuthExistsException(Social0AuthException):
    def __init__(self, message: str, data: Any, response: Response):

        super().__init__(
            message=message or "User already exists, is that you?",
            status_code=status.HTTP_409_CONFLICT,
            code="SOCIALAUTH_EXISTS",
            data=data,
            response=response
        )


class Social0AuthNotExistsException(Social0AuthException):
    def __init__(self, message: str, data: Any, response: Response):
        super().__init__(
            message=message or "No user found with your details, do you wish to signup?",
            status_code=status.HTTP_404_NOT_FOUND,
            code="SOCIALAUTH_NOTFOUND",
            data=data,
            response=response
        )

...
class OauthUtils:
    ...
    async def get_auth_redirect_url(platform: SocialAuthPlatform, request: Request) -> str:
        ...
        # request.url is starlette.datastructures.URL
        ...
        # Load required paths from env
        AUTH_URL_PATH = Utils.get_from_env_fail_if_not_exists("AUTH_URL_PATH").strip("/")
        SOCIAL_LOGIN_CALLBACK_PATH = Utils.get_from_env_fail_if_not_exists("SOCIAL_LOGIN_CALLBACK_PATH").strip("/")

        # Ensure redirect_path doesn't start with a slash so urljoin won't reset base_url path
        redirect_path = f"{AUTH_URL_PATH}/{SOCIAL_LOGIN_CALLBACK_PATH}/{platform.value}".lstrip("/")

...
from appodus_utils.domain.user.models import _UpdateUserDto, CreateUserDto, CreateUserOptionalPasswordDto
...
from appodus_utils.exception.exceptions import InvalidCredentialsException, InvalidTokenException, \
    NotImplementedException, Social0AuthExistsException, Social0AuthNotExistsException, Social0AuthException
...
class AuthService:
    ...
    async def social_login_signup(self, user_info: SocialLoginUserInfo, response: Response) -> Union[LoginSuccessDto, SocialLoginUserInfo]:
        ...
        if user_exists:
            if user_info.operation_type == SocialAuthOperationType.LOGIN:
                ...
                raise Social0AuthExistsException(message="User already exists, is that you?", data=data, response=response)
            else:
                raise NotImplementedException(message=f"Unknown SocialAuthOperationType: {user_info.operation_type}")
        else:
            if user_info.operation_type == SocialAuthOperationType.LOGIN:
                await RedisUtils.set_redis(key=user_info.email, value=user_info) # just to use it when user confirms
                raise Social0AuthNotExistsException(message="No user found with your details, do you wish to signup?",
                                                    data=user_info, response=response)
            elif user_info.operation_type == SocialAuthOperationType.SIGNUP:

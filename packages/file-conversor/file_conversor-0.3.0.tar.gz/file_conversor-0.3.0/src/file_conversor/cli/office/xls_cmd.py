    ...
    for ext in XLS_BACKEND.SUPPORTED_IN_FORMATS:
        ctx_menu.add_extension(f".{ext}", [
            WinContextCommand(
                ...
                command=f'{State.get_executable()} xls convert "%1" -o "%1.{ext}"',
...
@xls_cmd.command(
    ...
    """)
def convert(
    input_file: Annotated[str, typer.Argument(help=f"{_('Input file')} ({', '.join(XLS_BACKEND.SUPPORTED_IN_FORMATS)})",
    ...
    output_file: Annotated[str, typer.Option("--output", "-o",
                                             help=f"{_('Output file')} ({', '.join(XLS_BACKEND.SUPPORTED_OUT_FORMATS)}). {_('Defaults to None')} ({_('use the same input file as output name')}).",

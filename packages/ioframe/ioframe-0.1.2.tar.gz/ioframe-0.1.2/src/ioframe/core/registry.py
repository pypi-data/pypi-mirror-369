...
def _make_discriminated_union(
    ...
    return Annotated[  # type: ignore[return-type, return-value]
        Union[
...
class BackendRegistry(Generic[B]):
    ...
    def __init__(
        ...
        """Alias name for the backends contained within this registry."""
    ...
    @cached_property
    def discriminated_union(self) -> type[B]:
        ...
        discriminated_union = _make_discriminated_union(
            {kind: self.get_discriminated_union_for_kind(kind) for kind in kinds},
            Discriminator(discriminator_func),
        )

        union_name = (
            name.capitalize() if (name := self.name) else ""
        ) + "DiscriminatedUnion"

        return TypeAliasType(union_name, discriminated_union)  # type: ignore[return-value]

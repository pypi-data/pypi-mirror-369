...
class TimeMetricBase(MetricBase[V]):
    ...
    In the case a single repeat already exceeds the max time, the time obtained
    during the calibration phase will be used.

    The reason behind this is that if this maximum is reached, then the operation
    likely needs to be discarded in favour of a more efficient implementation.
    """

    def _time(self, func: Callable[[], Any], /) -> list[float]:
        ...
        timer = Timer(func)
        return timer.repeat(repeat=repeats, number=n_loops)


class WriteTimeMetric(TimeMetricBase[list[float]]):
    """Write time metric."""
...
class ReadTimeMetric(TimeMetricBase[list[float]]):
    """Read time metric."""

...
function configure_arista() {
    ...
    all_conf_files=$( echo $NEUTRON_CONF /$Q_PLUGIN_CONF_FILE $ARISTA_ML2_CONF_FILE | sed -e 's/ /;/g' )
    iniset_multiline $NEUTRON_UWSGI_CONF uwsgi env OS_NEUTRON_CONFIG_DIR= OS_NEUTRON_CONFIG_FILES=$all_conf_files
}

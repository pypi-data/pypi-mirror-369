...
def randint_max(max_values):
    max_values_float = max_values.float()
    return (max_values_float * torch.rand_like(max_values_float)).round().long()
...
class RewindTrainWrapper(Module):
    ...
    def forward(
        ...
        if satisfy_prob(self.rewind_augmentation_prob):
            ...
            rand_index = randint_max(video_lens).clamp(min = 2).long()
            rewind_len = randint_max(rand_index - 1).clamp(min = 1).long()

    ...
    def to_pydantic_model(self) -> Model:
        ...
        for provider in self.iter():
            if isinstance(provider, MistralProvider):
                ...
                # Special handling for Mistral models on Vertex with patching
                ...
                models.append(
                    MistralModel(
                        model_name=self.integration_aliases.pydantic_ai,
                        provider=provider.to_pydantic(model_family="mistral"),  # type: ignore
                    )
                )
            ...
            elif isinstance(provider, BedrockProvider | FireworksProvider | TogetherProvider):
                models.append(
                    OpenAIModel(
                        model_name=self.integration_aliases.pydantic_ai,
        ...
        if len(models) == 0:
            msg = f"No compatible providers found for Mistral model {self.identity.id}"

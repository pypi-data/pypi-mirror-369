...
class GrokBaseConfiguration(OpenaiBaseConfiguration):
    _compatible_providers: set[type[AnyProvider]] = PrivateAttr(default={GrokProvider, OpenRouterProvider})
    ...
    def to_pydantic_model(self) -> Model:
        ...
        for provider in self.iter():
            if isinstance(provider, GrokProvider):
                models.append(
                    OpenAIModel(
        ...
        if len(models) == 0:
            msg = f"No compatible providers found for Grok model {self.identity.id}"

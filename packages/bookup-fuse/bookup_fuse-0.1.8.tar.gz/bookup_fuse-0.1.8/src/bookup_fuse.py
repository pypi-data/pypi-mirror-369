...
class BookupSection(BookupMeta):
    ...
    def resolve_path(
        ...
        current_item = cls.get_child(parent, current_item_name, bdb_file) if parent else cls.get_child(BookupSection(0, ''), current_item_name, bdb_file)
...
class BookupFuse(Fuse):
        ...
        if path == "/":
            ...
            if path == "/Sections":
                ...
                if isinstance(element, BookupSection):
                    dirents.extend([f"{i+1} – {child.file_name}" for i, child in enumerate(BookupSection.all_children(element, self.file))])
                else:
    ...
    def utimens(self, path, ts_acc, ts_mod: Timespec):
        ...
        if path.startswith("/Pages") and path != "/Pages":
            ...
            if not item:
                return -errno.ENOENT
            item.modification_date = ts_mod.tv_sec if ts_mod else _now_epoch()
            item.update(self.file)
            return 0
    ...
    def mknod(self, path, mode, dev):
        print("\x1B[31;1mmknod called\x1B[0m")
        if path.startswith("/Sections/"):
            parts = [p for p in _split_path(path) if p][1:]  # ohne "Sections"
            if not parts:
                return -errno.EPERM

            # letzter Teil muss eine .md-Seite sein
            if not parts[-1].endswith(".md"):
                return -errno.EPERM

            # Eltern-Section auflösen
            parent_parts = parts[:-1]
            parent = None
            if parent_parts:
                parent = BookupSection.resolve_path(self.file, parent_parts)  # Section oder Page/None
                if not isinstance(parent, BookupSection):
                    return -errno.ENOENT
            else:
                # Seite direkt unter /Sections ist nicht erlaubt (Seiten gehören in eine Section)
                return -errno.EPERM

            page_name = _name_from_pages_path(parts[-1])
            # Kollision prüfen
            if BookupSection.get_child(parent, page_name, self.file):
                return -errno.EEXIST

            BookupPage.create(self.file, parent.id, page_name, "")
            return 0

        return -errno.EPERM

    # neuere Kernel rufen 'create' (O_CREAT) auf – an mknod delegieren
    def open(self, path, flags):
        if path in {"/", "/Pages", "/Sections", "/Images"}:
            return -errno.EISDIR
        if flags & os.O_CREAT:
            return self.mknod(path, None, 0)
        return 0
    ...
    def unlink(self, path):
        if path in {"/", "/Pages", "/Sections", "/Images"}:
            return -errno.EISDIR
        if path.startswith("/Pages"):
            page = BookupPage.from_name(self.file, _name_from_pages_path(path))
            if isinstance(page, BookupPage):
                page.delete(self.file)
            else:
                return -errno.ENOENT
            return 0
        if path.startswith("/Sections"):
            parts = _split_path(path)[1:]
            page = BookupSection.resolve_path(self.file, parts)
            if isinstance(page, BookupPage):
                page.delete(self.file)
            elif isinstance(page, BookupSection):
                return -errno.EISDIR
            else:
                return -errno.ENOENT
            return 0
    ...
    def mkdir(self, path, mode):
        ...
        new_name = _name_from_pages_path(parts[-1])
        parent_parts = parts[:-1]

        parent = None
        if parent_parts:
            parent = BookupSection.resolve_path(self.file, parent_parts)
            if not isinstance(parent, BookupSection):
                return -errno.ENOENT

        # Prüfe, ob bereits existiert
        if parent:
            if BookupSection.get_child(parent, new_name, self.file):
                return -errno.EEXIST
            BookupSection.create(self.file, new_name, parent.id)
        else:
            # Root-Section
            # In diesem Schema sind Root-Sections parent_id=0
            existing = BookupSection.get_child(BookupSection(0, ''), new_name, self.file)
            if existing:
                return -errno.EEXIST
            BookupSection.create(self.file, new_name, 0)
        return 0
...
def main():
    ...
    fuse_args = [f'-ofsname={os.path.basename(args.bdb_file)}', args.mount_point]

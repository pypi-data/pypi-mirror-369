...
from wkrq import parse, entails, solve, SignedFormula, WKrQTableau, t, f, n
...
def main():
    ...
    print(f"   Therefore inference is: {'VALID' if not result.satisfiable else 'INVALID'}")
    print()
    ...
    print(f"   Multiple: ([∃X P(X)]Q(X)) ∧ ([∀Y R(Y)]S(Y))")
    ...
    print(f"   Disjunction: ([∃X Human(X)]Wise(X)) ∨ ([∀Y Bird(Y)]Flies(Y))")
    result = solve(formula, t)
    print(f"   Satisfiable under t: {result.satisfiable}")

...
the formal tableau system specifications for weak Kleene logic with
restricted quantification.

Run with: python examples/validation.py > validation_output.txt

The output shows:
1. Exact rule applications matching Definition 9
...
def main():
    """Run formal specification compliance validation tests."""

    print("Tableau System Formal Specification Compliance Validation")
    print("=" * 80)
    print("This validates our implementation against the formal Definition 9")
    print("showing exact correspondence with the specification's tableau rules.")
    print(f"Generated by: {__file__}")

    # Section 1: The Six-Sign System
    print_test_section("1. The Six-Sign System (t, f, e, m, n, v)")

    print_test_case("Sign t: Formula must be true", ["--sign=t", "--models", "p"])

    print_test_case("Sign f: Formula must be false", ["--sign=f", "--models", "p"])

    print_test_case(
        "Sign e: Formula must be undefined/error", ["--sign=e", "--models", "p"]
    )
    ...
    # Section 2: Definition 9 - Negation Rules
    print_test_section("2. Definition 9: Negation Rules (v : ~φ → ~v : φ)")

    print_test_case(
        "t : ~φ → f : φ (where ~t = f)", ["--sign=t", "--tree", "--show-rules", "~p"]
    )

    print_test_case(
        "f : ~φ → t : φ (where ~f = t)", ["--sign=f", "--tree", "--show-rules", "~p"]
    )

    print_test_case(
        "e : ~φ → e : φ (where ~e = e)", ["--sign=e", "--tree", "--show-rules", "~p"]
    )
    ...
    print_test_section("3. Definition 9: Conjunction Rules (v : φ ∧ ψ)")
    ...
    print_test_section("4. Definition 9: Disjunction Rules (v : φ ∨ ψ)")
    ...
    print_test_section("5. Definition 9: Implication Rules (φ → ψ as ~φ ∨ ψ)")
    ...
    print_test_section("6. Definition 10: Branch Closure")
    ...
    print_test_case(
        "f ∧ e = e - undefined is contagious", ["--sign=e", "--models", "p & q"]
    )
    ...
    print_test_case(
        "Models for e sign show undefined values", ["--sign=e", "--models", "p | q"]
    )

    print_test_case(
        "Models for n sign show nontrue values (f or e)", ["--sign=n", "--models", "p"]
    )
    ...
    print_test_case(
        ...
        ["--sign=m", "--tree", "--show-rules", "(p -> q) & (~q -> ~p) & (p | ~p)"],
    )

    print_test_case(
        "Quantifier with undefined restriction",
        ["--sign=e", "--tree", "--show-rules", "[forall X Human(X)]Mortal(X)"],
    )
    ...
    print_test_case(
        "Compare: (¬p ∨ ¬q) when both undefined", ["--sign=e", "--models", "(~p | ~q)"]
    )
    ...
    print_test_case(
        "Addition: p ⊢ p ∨ q", ["--inference", "--tree", "--show-rules", "p |- (p | q)"]
    )

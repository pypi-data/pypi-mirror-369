...
def create_pluto_llm_evaluator():
    ...
    def evaluate_formula(formula):
        ...
        result = knowledge.get(
            key, BilateralTruthValue(positive=UNDEFINED, negative=UNDEFINED)
        )
        print(
            f"    LLM evaluating {key} -> positive={result.positive}, negative={result.negative}"
        )
        return result
    ...
    while tableau.open_branches and iteration < max_iterations:
        ...
        if applicable_rules:
            ...
            for i, n in enumerate(
                branch.nodes[-3:] if len(branch.nodes) > 3 else branch.nodes
            ):

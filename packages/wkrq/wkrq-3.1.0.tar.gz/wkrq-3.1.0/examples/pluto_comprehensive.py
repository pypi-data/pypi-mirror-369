...
def test_classic_planet_definition():
    ...
    formulas = [
        SignedFormula(
            t, parse_acrq_formula("[∀X OrbitsSun(X) & Spherical(X)]Planet(X)")
        ),
    ...
    print(
        "Analysis: Formal logic derives Planet(pluto), but LLM knows it's false → contradiction"
    )
...
def test_modern_planet_definition():
    ...
    formulas = [
        ...
        SignedFormula(t, parse_acrq_formula("Spherical(pluto)")),
        SignedFormula(
            t, parse_acrq_formula("~ClearedOrbit(pluto)")
        ),  # Becomes ClearedOrbit*(pluto)
    ]
...
def test_knowledge_gap():
    ...
    print(f"\nResult: {'SATISFIABLE' if result.satisfiable else 'UNSATISFIABLE'}")
    print(
        "Analysis: Formal logic derives Planet(sedna), LLM's uncertainty contradicts → unsatisfiable"
    )
...
def test_multiple_objects():
    ...
    print(f"\nResult: {'SATISFIABLE' if result.satisfiable else 'UNSATISFIABLE'}")
    print(
        "Analysis: Earth is correctly a planet, but Pluto and Ceres aren't → contradictions"
    )

...
relink_output_db_without_copying_it = False  
# if False, an already existing DB with the name of db_name_relinked will be deleted, and a new db_name_relinked will be created by copying and relinking an already existing db. 
...
# see template: https://github.com/CHarpprecht/bw_superstructure/blob/main/example/inputs/template_scenario_difference_file.xlsx
# this file cannot be shared publicly, please use premise or the template to create your own
...
# functional units can also be specified via a simple list, e.g.:
additional_functional_units = [
    (
        "heat, future", # product 
        "wood pellets, burned in stirling heat and power co-generation unit, 3kW electrical, future",  # process name
        "CH",  # location
        "ecoinvent38_cutoff",  # database name
    ),
    (
...
bwss.relink_database_to_new_background(
    ...
    db_name_old_bg=db_name_old_bg,  # ecoinvent
    db_name_new_bg=db_name_new_bg,  # superstructure-DB
...
# the SDF under path_to_single_SDF can then be used for bwss.calculate_scenario_LCA()

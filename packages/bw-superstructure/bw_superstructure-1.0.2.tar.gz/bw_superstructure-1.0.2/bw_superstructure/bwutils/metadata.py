...
def list_to_tuple(x) -> tuple:
    ...
    # function from: Lib\site-packages\activity_browser\bwutils\metadata.py
...
class MetaDataStore(object):
    ...
    # Note: source: based on: activity-browser:
    # from: Lib\site-packages\activity_browser\bwutils\metadata.py
    # branch: activity-browser-dev; version: 2022.11.16
    ...
    def unpack_tuple_column(self, colname: str, new_colnames: list = None) -> None:
        ...
        if colname in self.unpacked_columns:
            print(
                "WARNING: Decomposed columns of {} already exist, aborting merge".format(
                    colname
                )
            )
            return
        ...
        unpacked = (
            ...
            .apply(lambda x: pd.Series([item for item in x]))
            .fillna("")
        )

        # Give the columns the correct names
        unpacked.rename(
            columns={unpacked.columns[x]: new_colnames[x] for x in unpacked.columns},
            inplace=True,
        )

...
class MLCA(object):
    ...
    # function from: Lib\site-packages\activity_browser\bwutils\multilca.py
...
class Contributions(object):
    ...
    def _build_dict(self, C, FU_M_index, rev_dict, limit, limit_type):
        ...
        for fu_or_method, col in FU_M_index.items():
            top_contribution = ca.sort_array(
                C[col, :], limit=limit, limit_type=limit_type
            )
            cont_per = dict()
            cont_per.update(
                {
    ...
    @staticmethod
    def get_labels(key_list, fields=None, separator=" | ", max_length=False, mask=None):
    ...
    @classmethod
    def join_df_with_metadata(cls, df, x_fields=None, y_fields=None, special_keys=None):
    ...
    def get_labelled_contribution_dict(
        self, cont_dict, x_fields=None, y_fields=None, mask=None
    ):
    ...
    def get_contributions(
        self, contribution, functional_unit=None, method=None
    ) -> np.ndarray:
    ...
    def aggregate_by_parameters(
        self, C: np.ndarray, inventory: str, parameters: Union[str, list] = None
    ):

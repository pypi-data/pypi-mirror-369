...
class Goal(PClass):
    uuid = field(type=str)
    name = field(type=(str, type(None)), initial=None)
...
class PublicPlayer(PClass):
    ...
    def __fromdict__(d):
        return PublicPlayer(
            pieces=[Piece.__fromdict__(piece) for piece in d["pieces"]],
...
class Player(PClass):
    ...
    def __todict__(self):
        return {
            ...
            "discard_tray": [card.__todict__() for card in self.discard_tray],
        }
    @staticmethod
    def __fromdict__(d):
        return Player(
            idx=d["idx"],
            pieces=[Piece.__fromdict__(piece) for piece in d["pieces"]],
            cards=[Card.__fromdict__(card) for card in d["cards"]],
            discard_tray=[Card.__fromdict__(card) for card in d["discard_tray"]]
        )
...
class PublicDeck(PClass):
    ...
    def __todict__(self):
        return {
            ...
            "faceup_spots": [card.__todict__() for card in self.faceup_spots],
        }
    @staticmethod
    def __fromdict__(d):
        return PublicDeck(
            ...
            faceup_spots=[Card.__fromdict__(card) for card in d["faceup_spots"]],
        )
...
class LegalActionMovePiecesToPath(PClass):
    path_idx = field(type=int)
    optional = field(type=ActionMovePiecesToPathOptional)  # Optional[MovePiecesToPathAction]
    def __todict__(self):
        return {
            "path_idx": self.path_idx,
            "optional": self.optional.__todict__() if self.optional else None,
        }
    @staticmethod
    def __fromdict__(d):
        return LegalActionMovePiecesToPath(
            path_idx=d["path_idx"],
            optional=ActionMovePiecesToPathOptional.__fromdict__(d["optional"])
        )
...
class LegalActionFaceupDraw(PClass):
    deck_idx = field(type=int)
    spot_idx = field(type=int)
    def __todict__(self):
        return {
            "deck_idx": self.deck_idx,
            "spot_idx": self.spot_idx,
        }
    @staticmethod
    def __fromdict__(d):
...
class Deck(PClass):
    ...
    uuid = field(type=str)
    units = field(type=list)  # List[Card]
    facedown_stack = field(type=list)  # List[Card]
    discard = field(type=list)  # List[Card]
    faceup_spots = field(type=list)  # List[Card]
    def __todict__(self):
        return {
            "idx": self.idx,
            "uuid": self.uuid,
            "units": [unit.__todict__() for unit in self.units],
            "facedown_stack": [card.__todict__() for card in self.facedown_stack],
            "discard": [card.__todict__() for card in self.discard],
            "faceup_spots": [card.__todict__() for card in self.faceup_spots],
        }
    @staticmethod
    def __fromdict__(d):
        return Deck(
            ...
            facedown_stack=[Card.__fromdict__(card) for card in d["facedown_stack"]],
            discard=[Card.__fromdict__(card) for card in d["discard"]],
            faceup_spots=[Card.__fromdict__(card) for card in d["faceup_spots"]],
        )
...
class Card(PClass):
    ...
    is_wild = field(type=bool)
    deck_idx = field(type=int)
    back_symbol_uuid = field(type=(str, type(None)), initial=None)
    ...
    def __todict__(self):
        return {
            ...
            "is_wild": self.is_wild,
            "deck_idx": self.deck_idx,
            "back_symbol_uuid": self.back_symbol_uuid,
    ...
    def __fromdict__(d):
        return Card(
            ...
            is_wild=d["is_wild"],
            deck_idx=d["deck_idx"],
            back_symbol_uuid=d["back_symbol_uuid"],
...
class Piece(PClass):
    ...
    player_idx = field(type=int)
    pile_template_idx = field(type=int)
    def __todict__(self):
        return {
            "uuid": self.uuid,
            "pile_idx": self.pile_idx,
            "player_idx": self.player_idx,
            "pile_template_idx": self.pile_template_idx,
        }
    @staticmethod
    def __fromdict__(d):
        return Piece(
            ...
            pile_idx=d["pile_idx"],
            player_idx=d["player_idx"],
            pile_template_idx=d["pile_template_idx"],
        )
    

class Pile(PClass):
    player_idx = field(type=(int, type(None)), initial=None)
    piece_template = field(type=PieceTemplate)
    num_pieces = field(type=int, initial=0)
    pieces = field(type=list)  # List[Piece]
    def __todict__(self):
        return {
            "player_idx": self.player_idx,
            "piece_template": self.piece_template.__todict__(),
            "num_pieces": self.num_pieces,
            "pieces": [piece.__todict__() for piece in self.pieces],
        }
    @staticmethod
    def __fromdict__(d):
        return Pile(
            player_idx=d["player_idx"],
            piece_template=PieceTemplate.__fromdict__(d["piece_template"]),
            num_pieces=d["num_pieces"],
            pieces=[Piece.__fromdict__(piece) for piece in d["pieces"]],
        )
...
class State(PClass):
    ...
    def __fromdict__(d):
        return State(
            ...
            goals=[Goal.__fromdict__(goal) for goal in d["goals"]],
...
class PublicState(PClass):
    ...
    piles = field(type=list)  # List[Union{Nothing, int}]
    player_idxs = field(type=list)  # List[int]
...
def get_edges(board_config):
    ...
    for link in board_config.links:
        ...
        edges.append(BiEdge(uuid=link.uuid, node_1_uuid=link.c1, node_2_uuid=link.c2, node_1_idx=node_1_idx, node_2_idx=node_2_idx, paths=paths, score=link.score))

    return edges


def generate_cards(deck, deck_cards):
    cards = []
    idx = 0
    rng = getrng(deck['seed'])
    for deck_card in deck_cards:
        for _ in range(deck_card['quantity']):
            card = Card(
                ...
                deck_idx=deck['idx'],
                is_wild=deck_card['is_wild'],
                resource_uuid=deck_card['resource_uuid'],
                goal_uuid=deck_card['goal_uuid'],
                back_symbol_uuid=deck_card['back_symbol_uuid'],
            )
            cards.append(card)
            idx += 1
...
def getinitialstate(game_config):
    ...
    segment_piece_template = PieceTemplate(
        uuid="71e42353-5b14-4eb3-9864-fb5a2409b599",
        name=f"Segment Piece",
    )
    ...
    state = State(
        ...
        goals = [
            Goal(uuid="642f52d9-f11e-4d27-a275-63df2cea90e0", score=12, node_uuids=["e21f901e-c8db-44cf-b622-2d6586bcbbb7", "d82d8eb1-84e0-4717-93f6-1f99563081e7"]),
            Goal(uuid="f3d7d163-e86f-43aa-b7ca-046045bd4614", score=13, node_uuids=["1400b9bb-5070-447a-aee8-e896c609372f", "57e774ed-d37a-4546-b786-3f1bd606323c"]),
            Goal(uuid="e5853585-ea33-4ed4-943d-fe680ed9913e", score=14, node_uuids=["1400b9bb-5070-447a-aee8-e896c609372f", "ce82cabe-9e71-484a-b80f-788803878230"]),
            Goal(uuid="76a6797d-306c-4044-9ee7-f59f0a9c75a8", score=15, node_uuids=["57e774ed-d37a-4546-b786-3f1bd606323c", "11aaa6f8-ce95-4770-a144-94da5b4e28e2"]),
            Goal(uuid="2917fd0b-94f5-4646-b2a6-888e2942dfb9", score=16, node_uuids=["57e774ed-d37a-4546-b786-3f1bd606323c", "ce82cabe-9e71-484a-b80f-788803878230"]),
            Goal(uuid="8459fe86-b52f-4548-9fcf-0bb1f0a47d7f", score=17, node_uuids=["11aaa6f8-ce95-4770-a144-94da5b4e28e2", "1400b9bb-5070-447a-aee8-e896c609372f"]),
        ],
        nodes = nodes,
        edges = get_edges(board_config),
        regions = get_regions(board_config),
        legal_actions_2=[],
        piles=[
            Pile(
                player_idx=player_idx,
                piece_template=segment_piece_template,
                num_pieces=num_segment_pieces_per_player,
                pieces=[
                    Piece(uuid="cb9a6ebb-c240-4bbf-a37c-3063c3dea56c", pile_template_idx=0, pile_idx=player_idx, player_idx=player_idx),
                    Piece(uuid="db57c106-9f5a-45ab-8431-bf0892feaa34", pile_template_idx=0, pile_idx=player_idx, player_idx=player_idx),
                    Piece(uuid="ef72dac0-be91-4e7c-a2f8-0e047071d3ce", pile_template_idx=0, pile_idx=player_idx, player_idx=player_idx),
                    Piece(uuid="61016459-cd59-4afa-a44f-4b2be3974bb7", pile_template_idx=0, pile_idx=player_idx, player_idx=player_idx),
                    Piece(uuid="6f38d93d-ef51-4a26-914a-aa2b16ca75f7", pile_template_idx=0, pile_idx=player_idx, player_idx=player_idx),
                    Piece(uuid="40195869-5e6f-41ea-94ec-fbec3014ed88", pile_template_idx=0, pile_idx=player_idx, player_idx=player_idx),
                    Piece(uuid="1eff5f9d-626c-4907-8ef1-2148fda5e6ad", pile_template_idx=0, pile_idx=player_idx, player_idx=player_idx),
                    Piece(uuid="eea9d157-7d93-4b50-8103-5b707debcae4", pile_template_idx=0, pile_idx=player_idx, player_idx=player_idx),
                ],
            ) for player_idx in range(game_config.num_players)
        ],
        ...
        decks=[
            Deck(
                idx=0,
                uuid="7f48341d-b27f-41d2-ba6b-805b7f77b4ee",
                units=[],
                facedown_stack=[
                    Card(idx=0, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="eec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=1, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="eec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=2, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="eec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=3, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="eec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=4, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="eec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=5, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="eec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=6, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="eec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=7, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="eec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=8, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="eec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=9, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="eec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=10, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="1ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=11, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="1ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=12, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="1ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=13, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="1ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=14, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="1ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=15, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="1ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=16, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="1ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=17, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="1ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=18, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="1ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=19, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="1ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=20, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="2ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=21, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="2ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=22, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="2ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=23, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="2ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=24, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="2ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=25, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="2ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=26, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="2ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=27, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="2ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=28, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="2ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=29, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="2ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=30, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="3ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=31, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="3ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=32, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="3ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=33, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="3ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=34, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="3ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=35, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="3ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=36, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="3ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=37, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="3ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=38, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="3ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=39, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="3ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=40, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="4ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=41, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="4ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=42, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="4ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=43, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="4ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=44, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="4ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=45, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="4ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=46, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="4ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=47, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="4ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=48, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="4ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=49, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=False, deck_idx=0, resource_uuid="4ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=50, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=True, deck_idx=0, resource_uuid="5ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=51, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=True, deck_idx=0, resource_uuid="5ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=52, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=True, deck_idx=0, resource_uuid="5ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=53, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=True, deck_idx=0, resource_uuid="5ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=54, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=True, deck_idx=0, resource_uuid="5ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=55, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=True, deck_idx=0, resource_uuid="5ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=56, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=True, deck_idx=0, resource_uuid="5ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=57, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=True, deck_idx=0, resource_uuid="5ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=58, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=True, deck_idx=0, resource_uuid="5ec30571-108a-4372-b048-7313749378c0"),
                    Card(idx=59, uuid=str(generate_uuid_with_rng(deck_0_rng)), is_wild=True, deck_idx=0, resource_uuid="5ec30571-108a-4372-b048-7313749378c0"),
                ],
                faceup_spots=[],
                discard=[],
            ),
            Deck(
                idx=1,
                uuid="745aff1d-e11c-4fbe-bf72-5a6f4284f514",
                units=[],
                facedown_stack=[
                    Card(idx=0, uuid=str(generate_uuid_with_rng(deck_1_rng)), is_wild=False, deck_idx=1, goal_uuid="642f52d9-f11e-4d27-a275-63df2cea90e0"),
                    Card(idx=1, uuid=str(generate_uuid_with_rng(deck_1_rng)), is_wild=False, deck_idx=1, goal_uuid="e5853585-ea33-4ed4-943d-fe680ed9913e"),
                    Card(idx=2, uuid=str(generate_uuid_with_rng(deck_1_rng)), is_wild=False, deck_idx=1, goal_uuid="f3d7d163-e86f-43aa-b7ca-046045bd4614"),
                    Card(idx=3, uuid=str(generate_uuid_with_rng(deck_1_rng)), is_wild=False, deck_idx=1, goal_uuid="76a6797d-306c-4044-9ee7-f59f0a9c75a8"),
                    Card(idx=4, uuid=str(generate_uuid_with_rng(deck_1_rng)), is_wild=False, deck_idx=1, goal_uuid="2917fd0b-94f5-4646-b2a6-888e2942dfb9"),
                    Card(idx=5, uuid=str(generate_uuid_with_rng(deck_1_rng)), is_wild=False, deck_idx=1, goal_uuid="8459fe86-b52f-4548-9fcf-0bb1f0a47d7f"),
                    Card(idx=6, uuid=str(generate_uuid_with_rng(deck_1_rng)), is_wild=False, deck_idx=1, goal_uuid="8459fe86-b52f-4548-9fcf-0bb1f0a47d7f"),
                    Card(idx=7, uuid=str(generate_uuid_with_rng(deck_1_rng)), is_wild=False, deck_idx=1, goal_uuid="642f52d9-f11e-4d27-a275-63df2cea90e0"),
                    Card(idx=8, uuid=str(generate_uuid_with_rng(deck_1_rng)), is_wild=False, deck_idx=1, goal_uuid="e5853585-ea33-4ed4-943d-fe680ed9913e"),
                    Card(idx=9, uuid=str(generate_uuid_with_rng(deck_1_rng)), is_wild=False, deck_idx=1, goal_uuid="f3d7d163-e86f-43aa-b7ca-046045bd4614"),
                    Card(idx=10, uuid=str(generate_uuid_with_rng(deck_1_rng)), is_wild=False, deck_idx=1, goal_uuid="76a6797d-306c-4044-9ee7-f59f0a9c75a8"),
                    Card(idx=11, uuid=str(generate_uuid_with_rng(deck_1_rng)), is_wild=False, deck_idx=1, goal_uuid="76a6797d-306c-4044-9ee7-f59f0a9c75a8"),
                    Card(idx=12, uuid=str(generate_uuid_with_rng(deck_1_rng)), is_wild=False, deck_idx=1, goal_uuid="76a6797d-306c-4044-9ee7-f59f0a9c75a8"),
                ],
                faceup_spots=[],
                discard=[],
            )
...
def score_public_items(game, player_idx):
    # Implement scoring logic for public items
    items = []
    print("******************************1235 len(game.edges): ", len(game.edges))
    for edge_idx, edge in enumerate(game.edges):
        print(f"******************************1235 edge [{edge_idx}]: ", edge)
        for path in edge.paths:
            print("******************************1235 path: ", path)
            for segment in path.segments:
                print("******************************1235 segment.pieces: ", segment.pieces)
                if segment.pieces and segment.pieces[0].player_idx == player_idx:
                    items.append(
                        ScoreItem2(amount=edge.score, description="Player {} owns edge {}".format(player_idx, edge.uuid))
                    )

    print("******************************1235 score_public_items: ", items)
    # return [ScoreItem2(amount=56, description="Player {} private score".format(player_idx))]
    return items
...
def score_private_items(game, player_idx):
    ...
    print("******************************12356 score_private_items complete_goal_uuids: ", complete_goal_uuids)
...
def default_handle_terminal(game):
    print("******************************1234 default_handle_terminal")
    # If any player can less than 6 pieces, the game is terminal
    for player in game.players:
        if len(player.pieces) < 6:
            return game.set(terminal=True)
    return game

    
def default_handle_scoring(game):
    print("******************************123445 default_handle_scoring")
    player_scores = [
        PlayerScore2(
...
def default_handle_action(game, action):
    print("******************************123 default_handle_action")
    ...
    if action.legal_action.move_pieces_to_path:
        return handle_move_pieces_to_path_action(game, action)
...
def default_after_accept_action(game, action):
    ...
    if player_idx < 0 or player_idx >= len(game.players):
        return game
    print("------------ default_after_accept_action.game.legal_actions_2: ", [la.__todict__() for la in game.legal_actions_2])
    print("------------ default_after_accept_action.player_idx: ", player_idx)
    new_legal_actions = [
        la for la in game.legal_actions_2 if la.player_idx != player_idx
    ]
    print("------------ default_after_accept_action.new_legal_actions: ", [la.__todict__() for la in new_legal_actions])
    return game.set(legal_actions_2=new_legal_actions)


def get_path_by_idx(game, path_idx):
    if not game or path_idx < 0 or path_idx >= len(game.edges):
        return None
...
def handle_draw_action(game, action):
    ...
    print("******************************123 handle_draw_action player_cards before", player.cards)
    for _ in range(draw.quantity):
        print("******************************123 handle_draw_action drawing...")
        drawn_card = deck.facedown_stack.pop()
        player.cards.append(drawn_card)
    
    print("******************************123 handle_draw_action player_cards after", player.cards)
...
def handle_draw_discard_action(game, action):
    print("******************************123 handle_draw_discard_action")
    ...
    print("******************************123 handle_draw_discard_action player_cards before", player.cards)
    for _ in range(draw_discard.quantity):
        print("******************************123 handle_draw_discard_action drawing...")
        drawn_card = deck.facedown_stack.pop()
    ...
    print("******************************123 handle_draw_discard_action player_cards after", player.cards)
...
def handle_faceup_draw_action(game, action):
    print("******************************123 handle_faceup_draw_action")
    ...
    drawn_card = deck.faceup_spots[faceup_draw.spot_idx]
    if len(deck.facedown_stack) > 0:
        deck.faceup_spots[faceup_draw.spot_idx] = deck.facedown_stack.pop()
    player.cards.append(drawn_card)
    return game.set(
        players=game.players,
        decks=game.decks,
...
def handle_move_pieces_to_path_action(game, action):
    print("******************************123 handle_move_pieces_to_path_action")
    ...
    optional = move_pieces_to_path.optional
    override = action.move_pieces_to_path
    if override:
        print("******************************123 optional: ", optional)
        print("******************************123 override: ", override)
        optional = optional.set(piece_uuids=override.piece_uuids)
        optional = optional.set(card_uuids=override.card_uuids)
    ...
    print("******************************123 optional.piece_uuids: ", optional.piece_uuids)
    for piece_uuid, segment in zip(optional.piece_uuids, path.segments):
        # Find the piece in the player's pieces
        piece_idx = next((i for i, p in enumerate(player.pieces) if p.uuid == piece_uuid), None)
        if piece_idx is not None:
            # Remove the piece from player's pieces
            piece = player.pieces.pop(piece_idx)
            segment.pieces.append(piece)

    print("******************************123 optional.card_uuids: ", optional.card_uuids)
    for card_uuid in optional.card_uuids:
        # Find the card in the player's cards
        card_idx = next((i for i, c in enumerate(player.cards) if c.uuid == card_uuid), None)
        if card_idx is not None:
            # Remove the card from player's cards
            card = player.cards.pop(card_idx)
            # add to discard

    print("******************************123 handle_move_pieces_to_path_action players: ", [player.__todict__() for player in game.players])
    print("******************************123 handle_move_pieces_to_path_action edges: ", [edge.__todict__() for edge in game.edges])
...
def handle_keep_action(game, action):
    ...
    kept_cards = [card for card in player.discard_tray if card.uuid in action.keep.card_uuids]
    non_kept_cards = [card for card in player.discard_tray if card.uuid not in action.keep.card_uuids]

    print("****************************** handle_keep_action 5", kept_cards)
    print("****************************** handle_keep_action 6", non_kept_cards)
    ...
    print("****************************** handle_keep_action 10 kept_cards: ", kept_cards)
...
def handle_discard_action(game, action):
    ...
    # Keep the specified cards in the discard tray
    non_kept_cards = [card for card in player.discard_tray if card.uuid in action.discard.card_uuids]
    kept_cards = [card for card in player.discard_tray if card.uuid not in action.discard.card_uuids]
...
def shuffle_deck(game, deck_idx):
    ...
    shuffled_deck = deck.set(facedown_stack=shuffled_cards)
    new_decks = game.decks[:deck_idx] + [shuffled_deck] + game.decks[deck_idx + 1:]
...
def is_move_pieces_to_path_action_legal(game, action):
    ...
    proposed_cards = [card for card in game.players[player_idx].cards if card.uuid in proposed_cards_uuids]
    proposed_pieces = [piece for piece in game.players[player_idx].pieces if piece.uuid in action.move_pieces_to_path.piece_uuids]
...
def append_default_legal_actions_for_next_player(game, action, log=False):
    print("****************************************** Inside append_default_legal_actions_for_next_player")
...
def get_legal_actions_for_path(game, player_idx, path_idx):
    print("*************************&&& get_legal_actions_for_path &&&*************************")
    ...
    legal_actions = []
    optional = get_sample_actionclaimpath(game, player_idx, path_idx)
    if optional:
        print(f"Path {path_idx} is claimable by player {player_idx} with fulfillment: {optional}")
...
def get_wild_unit_uuids(game):
    ...
    for deck in game.decks:
        for card in deck.facedown_stack:
            if card.is_wild:
                wild_unit_uuids.append(card.resource_uuid)
    return wild_unit_uuids


def get_wild_cards(game, player_idx):
    wild_unit_uuids = get_wild_unit_uuids(game)
    return [card for card in game.players[player_idx].cards if card.resource_uuid in wild_unit_uuids]
...
def match_strict_wild(game, fulfillment, cards, segments):
    ...
    for segment in segments:
        if segment.unit_uuid and segment.unit_uuid in wild_unit_uuids:
            first_matching_idx = next((i for i, card in enumerate(cards) if card.resource_uuid == segment.unit_uuid), None)
            if first_matching_idx:
...
def match_non_wild_non_empty(game, fulfillment, cards, segments):
    new_fulfillment = fulfillment.copy()
    new_cards = cards.copy()
    wild_unit_uuids = get_wild_unit_uuids(game)
    new_segments = []
    ...
    for segment in segments:
        first_strict_matching_idx = next((i for i, card in enumerate(cards) if card.resource_uuid == segment.unit_uuid), None)
        first_wild_matching_idx = next((i for i, card in enumerate(cards) if card.resource_uuid in wild_unit_uuids), None)
        first_matching_idx = (first_strict_matching_idx or first_wild_matching_idx)
        if first_matching_idx:
            new_fulfillment.append(new_cards.pop(first_matching_idx))
        else:
            new_segments.append(segment)
...
def match_empty(game, fulfillment, cards, segments):
    ...
    tuples = get_uniform_sets(cards, num_empty_segments)
...
def get_uniform_sets(cards, min_length):
    wilds = [card for card in cards if card.is_wild]
    non_wilds = [card for card in cards if not card.is_wild]
    # print("********************* cards: ", cards)
    # print("********************* wilds: ", wilds)
    ...
    for card in non_wilds:
        ...
        unit_uuid_2_cards[card.resource_uuid].append(card)
    
    uniform_sets_no_wilds = [
        cards for cards in unit_uuid_2_cards.values() if len(cards) >= min_length
    ]
...
def get_sample_actionclaimpath(game, player_idx, path_idx):
    ...
    return ActionMovePiecesToPathOptional(
        piece_uuids=[piece.uuid for piece in piece_fulfillment],
        card_uuids=[card.uuid for card in card_fulfillment],
    )


def get_sample_path_fulfillment(game, player_idx, path_idx):
    print("****************************************** Inside get_path_fulfillment")
    path = game.game_config.fig.board_config.board_paths[path_idx]
    remaining_cards = [card for card in game.players[player_idx].cards if card.deck_idx == 0]
    remaining_pieces = [piece for piece in game.players[player_idx].pieces if piece.pile_template_idx == 0]
    remaining_segments = path.path.segments
...
def get_path_fulfillment_from_resources(game, remaining_cards, remaining_pieces, remaining_segments):
    ...
    card_fulfillment = []
    card_fulfillment, remaining_cards, remaining_segments = match_strict_wild(game, card_fulfillment, remaining_cards, remaining_segments)
    ...
    card_fulfillment, remaining_cards, remaining_segments = match_non_wild_non_empty(game, card_fulfillment, remaining_cards, remaining_segments)
    if len(remaining_segments) == 0:
        return card_fulfillment, piece_fulfillment
    # Probably don't need this check, but we should unit test
    ...
    card_fulfillment, remaining_cards, remaining_segments = match_empty(game, card_fulfillment, remaining_cards, remaining_segments)
    if len(remaining_segments) == 0:
        return card_fulfillment, piece_fulfillment
...
#     longest_road, longest_road_len = getlongestroad(s.fig, player_hand)
#     @reset player_hand.longest_road = longest_road
#     @reset player_hand.longest_road_len = longest_road_len
#     @reset s.player_hands = [p.player_idx == a.player_idx ? player_hand : p for p in s.player_hands]
#     @reset s.longest_road_player_idxs = getlongestroadplayeridxs(s.player_hands)
...
def getnextstate(s, action, action_type):
    ...
    ###longest_road, longest_road_len = getlongestroad(s.game_config.fig, player_hand)
    # @reset player_hand.longest_road = longest_road
    ###player_hand = player_hand.set(longest_road=longest_road)
    # @reset player_hand.longest_road_len = longest_road_len
    ###player_hand = player_hand.set(longest_road_len=longest_road_len)
    ...
    # @reset s.longest_road_player_idxs = getlongestroadplayeridxs(s.player_hands)
    ###s = s.set(longest_road_player_idxs=getlongestroadplayeridxs(s.player_hands))
    # @reset s.most_clusters_player_idxs = getmostclustersplayeridxs(s.player_hands)
...
def getpublicplayer(s, p):
    deck_counts = [0 for _ in s.decks]
    for card in p.cards:
        deck_counts[card.deck_idx] += 1
    return PublicPlayer(
        pieces=p.pieces,
...
def get_goals(s, player_idx):
    ...
    goal_uuids = [card.goal_uuid for card in player.cards if card.goal_uuid]
    return [
...
def getpathstatus(s, player_idx, path_idx):
    ...
    def anywildsleft():
        # print("wild_unit_uuids: ", wild_unit_uuids)
        for wild_unit_uuid in wild_unit_uuids:
...
DEFAULT_LEGAL_ACTIONS_HOOK = """def handler(game, player_idx):
    ...
    for spot_idx in range(len(game.decks[0].faceup_spots)):
        game = append_to_legal_actions(
            game,
            LegalAction(
                player_idx=player_idx,
...
def handler(game, action):
    print("****************************************** Inside EMPTY_LEGAL_ACTIONS_HOOK_1")

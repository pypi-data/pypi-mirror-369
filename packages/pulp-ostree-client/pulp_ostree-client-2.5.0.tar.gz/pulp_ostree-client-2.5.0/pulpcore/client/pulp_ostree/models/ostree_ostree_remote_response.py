...
class OstreeOstreeRemoteResponse(BaseModel):
    """
    ...
    policy: Optional[PolicyEnum] = Field(default=None, description="         immediate - All OSTree objects are downloaded and saved during synchronization.         on_demand - Only commits, dirtrees, and refs are downloaded. Other OSTree objects are                     not downloaded until they are requested for the first time by a client.           * `immediate` - immediate * `on_demand` - on_demand")
    ...
    hidden_fields: Optional[List[OstreeOstreeRemoteResponseHiddenFieldsInner]] = Field(default=None, description="List of hidden (write only) fields")
    depth: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=0, description="An option to specify how many commits to traverse.")
    include_refs: Optional[List[Annotated[str, Field(strict=True, max_length=255)]]] = Field(default=None, description="             A list of refs to include during a sync.             The wildcards *, ? are recognized.             'include_refs' is evaluated before 'exclude_refs'.             ")
    exclude_refs: Optional[List[Annotated[str, Field(strict=True, max_length=255)]]] = Field(default=None, description="             A list of tags to exclude during a sync.             The wildcards *, ? are recognized.             'exclude_refs' is evaluated after 'include_refs'.             ")
    __properties: ClassVar[List[str]] = ["pulp_href", "prn", "pulp_created", "pulp_last_updated", "name", "url", "ca_cert", "client_cert", "tls_validation", "proxy_url", "pulp_labels", "download_concurrency", "max_retries", "policy", "total_timeout", "connect_timeout", "sock_connect_timeout", "sock_read_timeout", "headers", "rate_limit", "hidden_fields", "depth", "include_refs", "exclude_refs"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # set to None if include_refs (nullable) is None
        # and model_fields_set contains the field
        if self.include_refs is None and "include_refs" in self.model_fields_set:
            _dict['include_refs'] = None

        # set to None if exclude_refs (nullable) is None
        # and model_fields_set contains the field
        if self.exclude_refs is None and "exclude_refs" in self.model_fields_set:
            _dict['exclude_refs'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "hidden_fields": [OstreeOstreeRemoteResponseHiddenFieldsInner.from_dict(_item) for _item in obj["hidden_fields"]] if obj.get("hidden_fields") is not None else None,
            "depth": obj.get("depth") if obj.get("depth") is not None else 0,
            "include_refs": obj.get("include_refs"),
            "exclude_refs": obj.get("exclude_refs")
        })
        return _obj

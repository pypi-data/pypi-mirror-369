...
class DeploysApi(object):
    ...
    def create_deploy(self, asset_id, previous_commit, current_commit, branch_name, diff_content, api_key,
        ...
        with open(diff_content, 'rb') as diff_content_data, open(commit_history, 'rb') as commit_history_data:
            files = {
                'operations': (None, json.dumps(operations), 'application/json'),
                'map': (None, json.dumps(file_map), 'application/json'),
                '0': (diff_content, diff_content_data, 'application/octet-stream'),
                '1': (commit_history, commit_history_data, 'application/json'),
            }
...
class ControlSyncStatus:
    ...
    def update_control_sync_status(self, control_sync_status_id):
    ...
    def increase_success_count(self, control_sync_status_id, asset_id):

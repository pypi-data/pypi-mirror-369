...
class GitAdapter(object):
    ...
    def zip_git_folder(self, depth=2):
        """
        Zipa apenas objetos dos últimos N commits para reduzir tamanho

        Args:
            depth (int): Número de commits recentes para incluir (default: 2)
        """
        ...
        try:
            # Clone shallow com depth especificado
            subprocess.check_call([
                "git", "clone", f"--depth={depth}", "--no-checkout",
                f"file://{os.path.abspath(self._repo.working_dir)}",
                shallow_path
            ])

            # Fazer garbage collection no shallow para compactar
            subprocess.check_call([
            ...
            # Arquivos essenciais (sem logs e hooks desnecessários)
            essential_items = ['HEAD', 'index', 'config', 'refs', 'objects', 'packed-refs', 'shallow']

            with zipfile.ZipFile(output_folder, 'w', zipfile.ZIP_DEFLATED, compresslevel=9) as zipf:
                for item in essential_items:
                    item_path = os.path.join(git_folder, item)

                    if os.path.isfile(item_path):
                        zipf.write(item_path, f'.git/{item}')
                    elif os.path.isdir(item_path):
                        for root, dirs, files in os.walk(item_path):
                            # Filtrar arquivos temporários
                            dirs[:] = [d for d in dirs if not d.endswith(('.tmp', '.lock'))]

                            for file in files:
                                if file.endswith(('.tmp', '.lock')):
                                    continue

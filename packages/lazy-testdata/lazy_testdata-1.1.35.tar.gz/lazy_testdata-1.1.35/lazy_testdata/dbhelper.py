...
Config = dict(
    pool_size=os.getenv("pool_size", 40),
    max_overflow=os.getenv("max_overflow", 60),
    pool_timeout=os.getenv("pool_timeout", 60),
    pool_recycle=os.getenv("pool_recycle", 600),        # <= MySQL 的 wait_timeout（建议设为 600）
...
def get_db(conn_str):
    if conn_str not in _db_pools:
        _db_pools[conn_str] = records.Database(
            conn_str,
           ** Config)

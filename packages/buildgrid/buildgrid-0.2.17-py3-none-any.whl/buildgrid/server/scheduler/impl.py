...
class Scheduler:
    ...
    def match_bot_by_capacity(self, session: Session, job: JobEntry) -> BotEntry | None:
        """Select a bot for a job by capacity."""
        query = (
            select(BotEntry)
            .join(BotPlatformEntry, BotEntry.name == BotPlatformEntry.bot_name)
            .with_for_update(skip_locked=True)
            .where(*self._bot_assignment_base_clauses(job))
            # TODO: order_by something?
            .limit(1)
        )
        return session.execute(query).scalar_one_or_none()

    def match_bot_by_locality(self, session: Session, job: JobEntry) -> BotEntry | None:
        """
        Select a bot for a job by locality.
        Fall back to any bot with capacity if no locality match is found.
        """
        ...
        if bot:
            ...
            LOGGER.debug("No bot matched by locality, falling back to capacity match.")
            return self.match_bot_by_capacity(session, job)

    def _bot_assignment_base_clauses(self, job: JobEntry) -> list[ColumnExpressionArgument[bool]]:
    ...
    def assign_job_by_priority(
        ...
        with self._sql.session() as session:
            ...
            if job is not None:
                self._match_job_to_bot(session, job, failure_backoff, bot_assignment_fn)
                session.flush()
                updated = True

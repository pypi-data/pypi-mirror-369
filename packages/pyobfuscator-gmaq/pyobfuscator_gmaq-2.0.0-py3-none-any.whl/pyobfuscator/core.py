...
class PyObfuscatorCore:
    ...
    def obfuscate_string(self, source_code):
        ...
        try:
            ...
            # Step 1: Single obfuscation pass for stability
            obfuscated_code = source_code
            for i in range(1):  # Single pass only to avoid variable conflicts
                self.logger.debug(f"Obfuscation pass {i+1}/1")
...
def _decrypt_and_execute():
    ...
    try:
        # _check_environment()  # Disabled for compatibility
        
        # Encrypted payload
        encrypted_data = base64.b64decode("{encrypted_payload}")
        
        # Decrypt using multi-layer decryption
        ...
        code_obj = marshal.loads(bytes(final_data))
...
'''
        ...
        return library_template.format(
            encrypted_payload=repr(encrypted_code['data']),
            decryption_key=repr(key_data['key']),
            salt=repr(key_data['salt']),
            nonce=repr(key_data['nonce']),
            tag=repr(key_data['tag'])
        )

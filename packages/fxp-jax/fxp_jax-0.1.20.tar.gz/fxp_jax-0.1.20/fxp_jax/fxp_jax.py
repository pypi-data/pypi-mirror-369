...
class fxp_root(Pytree, mutable=True):
    ...
    def _condition(self, step: OptStep) -> jnp.ndarray:
        ...
        cond3 = (step.state.iter_num < self.max_iter) #stop if maximum iteration is reached
    ...
    def solve(self, guess: jnp.ndarray, accelerator: str|None = None) -> OptStep:
        ...
        print(f'FXP output: iterations={result.state.iter_num}, root norm={result.state.root_norm}, step norm={result.state.step_norm}')
        return result
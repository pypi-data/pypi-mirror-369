...
class DataFlow:
    ...
    def model(self, cls: Type) -> Type:
        ...
        # Trigger universal schema state management and migration system if enabled
        if self._schema_state_manager is not None or self._migration_system is not None:
            try:
                self._trigger_universal_schema_management(model_name, fields)
            except Exception as e:
                # Don't let migration failures break model registration
                logger.error(
                    f"Schema state management error for model {model_name}: {e}"
                )
                self._notify_user_error(
                    f"Schema state management encountered an error: {e}"
                )
    ...
    def _convert_fields_to_columns(self, fields: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Always include the auto-generated ID column that DataFlow adds
        columns["id"] = {
            "type": "INTEGER",  # Use INTEGER for comparison (SERIAL is CREATE TABLE syntax only)
            "nullable": False,
            "primary_key": True,
            "unique": False,
            "default": "nextval",  # Indicates it's auto-incrementing
        }

...
def protect_dataflow_node(original_class: Type[Node]) -> Type[Node]:
    ...
    This decorator wraps the execute method of generated nodes to
    perform protection checks before database operations.
    """

    class ProtectedNode(original_class):
        def execute(self, **kwargs) -> Dict[str, Any]:
            """Override execute to add protection checks."""
            # Get protection engine from DataFlow instance
            if hasattr(self, "dataflow_instance") and hasattr(
                self.dataflow_instance, "_protection_engine"
            ):
                protection_engine = self.dataflow_instance._protection_engine
                ...
                if not connection_string and hasattr(self, "dataflow_instance"):
                    ...
                    connection_string = df.config.database.get_connection_url(
                        df.config.environment
                    )

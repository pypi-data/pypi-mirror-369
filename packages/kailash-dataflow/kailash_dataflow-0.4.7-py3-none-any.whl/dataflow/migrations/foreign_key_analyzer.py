...
class ForeignKeyAnalyzer:
    ...
    async def validate_referential_integrity(
        ...
        if column:
            ...
            if hasattr(
                ...
                fk_deps = self.dependency_analyzer.find_foreign_key_dependencies(
                    table, column, connection
                )
                if hasattr(fk_deps, "return_value"):
                    fk_deps = fk_deps.return_value
            else:
    ...
    def _sanitize_operation_type(self, operation: str) -> str:
        ...
        if any(
            dangerous in operation.lower()
            for dangerous in ["drop table", "delete", ";", "--"]
        ):
            raise ValueError(f"Invalid operation type: {operation}")

        # Convert to lowercase and remove special characters
        sanitized = re.sub(r"[^\w]", "_", operation.lower())

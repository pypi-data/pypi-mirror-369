...
class ReportFormatter:
    ...
    def _format_console_executive_summary(
        ...
        output.append(
            f"  • Potential Downtime: {summary.potential_downtime_minutes:.0f} minutes"
        )
    ...
    def _format_json_report(
        ...
        if ReportSection.EXECUTIVE_SUMMARY in sections:
            json_data["executive_summary"] = asdict(report.executive_summary)
        ...
        if ReportSection.TECHNICAL_DETAILS in sections:
            json_data["technical_report"] = asdict(report.technical_report)

        if ReportSection.COMPLIANCE_AUDIT in sections:
            json_data["compliance_report"] = asdict(report.compliance_report)

        if ReportSection.STAKEHOLDER_ACTIONS in sections:
            json_data["stakeholder_communications"] = {
                role.value: asdict(stakeholder_report)

...
MCTree_key_words = ['select_mode', 'mode_BP',
                    'mode_decision', 'objective', 'score_layer',
                    'use_prune', 'use_hash', 'init_mapping']
...
class MCTree(DiGraph):
    def __init__(self, AG, DG, **args):
        ...
        self.num_q_phy = max(list(AG.nodes)) + 1  # the max. index of physical qubits + 1
        ...
        if self.objective == 'depth':
            ...
            raise (Exception(f"Unsupported objective {self.objective}."))
        self.log = {'score': [],
                    'visit': [],
                    'swap_vs_cx': [],
    ...
    def add_node_mcts(self, father_node, added_swap=None, remote_exe_node=None):
        ...
        if father_node == None:
            ...
            if self.objective == 'no_swap' and cir.num_remain_nodes > 0:
                raise (Exception("Fail to find a mapping requiring no swaps."))
        else:
    ...
    def to_dg(self):
        from .cir_dg_swap_depth_opt import DGSwap
        dg = DGSwap(self.AG)
        node = self.init_node
        while True:
            if self.out_degree(node) > 1: raise ()

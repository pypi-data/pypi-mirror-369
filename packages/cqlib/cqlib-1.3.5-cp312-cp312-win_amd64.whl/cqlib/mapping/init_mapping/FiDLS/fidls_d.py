...
'''This version try to implement FiDLS-D and be as consistent as 
    the 0422-github version, make as few changes as possible '''
#####\__/#\#/\#\__/#\#/\__/--\__/#\__/#\#/~\
import networkx as nx
from utils import tau2map, map2tau, swap, swap_along_a_path
from utils import topgates_3_lev, gate_phy_distance, min_gate_dist
from utils import qubit_in_circuit, greedy_solved_gates, R_hat_3
import time
...
def swap_reduce_min_dist_x(tau, LTG, LTG1, LTG2, LD, C, nl, G, EG, V, SPL):
    ...
    for i in LTG:
        ...
        if (p not in tau) and (q not in tau): #type 0
            ...
            GATE3.append(i)
    '''We prefer to swap over extension if GATE3 is nonempty.'''    
    if GATE3:
        ...
        for i in GATE3:
            ...
            rhat_temp =  R_hat_3(tau_temp, LTG, LTG1, LTG2, C, V, SPL)
            if gsg_temp == gsg and rhat_temp >= rhat: continue
            gsg = gsg_temp
            rhat = rhat_temp
            action_temp = []
            for k in range(len(pi)-2): #q -2 seems right
    ...
    if GATE1 or GATE2:
        ...
        for i in GATE0:
            p, q = C[i]
            if p in tau or q in tau: raise Exception('p and q are not in tau',p, q)
...
def good_next_mapping(tau, LD, C, Q, G, EG, V, SPL, QFilter_type, Fallback):    
    ...
    if [v for v in V if tau[v] == -1] or Fallback: #i.e., tau incomplete
        ...
        action_rec, tau_rec, type = swap_reduce_min_dist_x(tau, LTG, LTG1, LTG2, LD, C, nl, G, EG, V, SPL)
        if not action_rec or Fallback:
            '''Extend the mapping or use Fallback!'''
            # print('SRMD used', action_rec, Fallback, len(LD), type)
            return action_rec, tau_rec    

    SWAPX = SWAP3(tau, LTG, LTG1, LTG2, C, Q, EG, V, SPL, QFilter_type)
    rhat = R_hat_3(tau, LTG, LTG1, LTG2, C, V, SPL)
    t = 0 
    sg = None
    for rho in SWAPX:
        ...
        if rhat_temp == rhat and sg != None: continue
        t_temp = (rhat - rhat_temp)/len(action_temp)
...
def qct_old(tau, C, Q, G, EG, V, SPL, QFilter_type):
    ...
    taunew = tau[:]
    L1 = L[:] #the indices of gates to be processed in C
    nr = 0
    ...
    start = time.time()
    while nsvg < l:
        '''Check if there are solvable gates & remove their indices from L1'''

...
# Use system package managers for reliable font installation
FONT_CACHE_DIR = os.path.join(os.path.expanduser("~"), ".media_agent_fonts")

# System font installation commands for different platforms
FONT_INSTALL_COMMANDS = {
    "ubuntu": ["sudo", "apt", "update", "&&", "sudo", "apt", "install", "-y", "fonts-noto-cjk", "fonts-noto-cjk-extra"],
    "debian": ["sudo", "apt", "update", "&&", "sudo", "apt", "install", "-y", "fonts-noto-cjk", "fonts-noto-cjk-extra"],
    "centos": ["sudo", "yum", "install", "-y", "google-noto-cjk-fonts"],
    "rhel": ["sudo", "yum", "install", "-y", "google-noto-cjk-fonts"],
    "fedora": ["sudo", "dnf", "install", "-y", "google-noto-cjk-fonts"],
...
def detect_linux_distribution() -> str:
    ...
    Returns:
        Distribution name (ubuntu, debian, centos, rhel, fedora) or 'unknown'
    """
    try:
...
def install_chinese_fonts_system() -> bool:
    """
    Install Chinese fonts using system package manager.
    
    Returns:
        True if installation succeeded or fonts already available, False otherwise
    """
    import platform
    ...
    try:
        ...
        if system == 'darwin':  # macOS
            ...
            platform_key = detect_linux_distribution()
        else:
            logger.warning(f"Unsupported operating system: {system}")
            return False
        
        if platform_key not in FONT_INSTALL_COMMANDS:
            logger.warning(f"No font installation command for platform: {platform_key}")
            return False
        
        commands = FONT_INSTALL_COMMANDS[platform_key]
        
        # Check if we need to run multiple commands (for apt update && apt install)
        if '&&' in commands:
            # Split commands by &&
            cmd_parts = []
            current_cmd = []
            ...
            # Single command
            logger.info(f"Installing Chinese fonts: {' '.join(commands)}")
            result = subprocess.run(commands, capture_output=True, text=True, timeout=300)
            if result.returncode != 0:
                logger.error(f"Font installation failed: {result.stderr}")
                return False
        ...
        logger.error("Font installation timed out")
        return False
    except Exception as e:
        logger.error(f"Failed to install Chinese fonts: {e}")
        return False


def ensure_chinese_font_available() -> str:
    """
    Ensure Chinese font is available, install using system package manager if necessary.
    
    Returns:
        Path to available Chinese font file
    """
    # First try to find system fonts
    system_font = get_system_font_path()
    if system_font:
        logger.info(f"Found system Chinese font: {system_font}")
        return system_font
    
    # If no system font found, try to install using package manager
    logger.info("No system Chinese font found, attempting to install using package manager...")
    
    # Provide user guidance for manual installation
    import platform
    system = platform.system().lower()
    
    if system == 'darwin':  # macOS
        install_cmd = "brew install font-noto-sans-cjk"
    elif system == 'linux':
        distro = detect_linux_distribution()
        if distro in ['ubuntu', 'debian']:
            install_cmd = "sudo apt update && sudo apt install fonts-noto-cjk fonts-noto-cjk-extra"
        elif distro in ['centos', 'rhel']:
            install_cmd = "sudo yum install google-noto-cjk-fonts"
        elif distro == 'fedora':
            install_cmd = "sudo dnf install google-noto-cjk-fonts"
        else:
            install_cmd = "Please install Chinese fonts manually"
    else:
        install_cmd = "Please install Chinese fonts manually"
    
    logger.warning(f"Chinese fonts not found. Please install them manually using: {install_cmd}")
    
    # Try automatic installation (may require sudo privileges)
    try:
        if install_chinese_fonts_system():
            # Re-check for fonts after installation
            system_font = get_system_font_path()
            if system_font:
                logger.info(f"Successfully installed and found Chinese font: {system_font}")
                return system_font
    except Exception as e:
        logger.warning(f"Automatic font installation failed: {e}")
    
    # If installation failed, return empty string (will use system default)
    logger.warning("Failed to ensure Chinese font availability. Subtitles may not display correctly.")
    return ""
...
def get_system_font_path() -> str:
    ...
    # Try to use fc-list to find Chinese fonts
    try:
        ...
        if result.returncode == 0 and result.stdout:
            ...
            for line in lines:
                if ':' in line:
                    ...
                    if os.path.exists(font_path):
                        logger.info(f"Found Chinese font via fc-list: {font_path}")
                        return font_path
    except Exception as e:
        logger.warning(f"Failed to use fc-list: {e}")
    
    # No system font found
    logger.warning("No system Chinese font found")
    return ""
...
def calculate_subtitle_style(video_width: int, video_height: int, has_chinese: bool = False) -> Dict[str, Any]:
    ...
    if has_chinese:
        font_path = ensure_chinese_font_available()
...
def add_subtitles_to_video(video_input: str, subtitles_input: str, output_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Ensure Chinese font is available if needed
        if has_chinese:
            logger.info("Ensuring Chinese font availability...")
            font_path = ensure_chinese_font_available()
            if font_path:
                logger.info(f"Chinese font ready: {font_path}")
            else:
                logger.warning("Chinese font not available, may result in display issues")

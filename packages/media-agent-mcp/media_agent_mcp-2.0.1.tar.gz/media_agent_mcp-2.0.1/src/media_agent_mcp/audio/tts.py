...
def combine_video_audio(video_path: str, audio_path: str, output_path: str, video_duration: float, audio_duration: float) -> bool:
    """
...
def get_tts_video(video_url: str, speaker_id: str, text: str, can_summarize: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        if not combine_video_audio(video_path, final_audio_path, output_video_path, video_duration, get_audio_duration(final_audio_path)):
            return {
                "status": "error",
                "data": None,

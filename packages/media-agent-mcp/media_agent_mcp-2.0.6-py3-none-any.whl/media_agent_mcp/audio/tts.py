...
def combine_video_audio(video_path: str, audio_path: str, output_path: str, video_duration: float, audio_duration: float) -> Dict[str, Any]:
    ...
    Returns:
        dict: JSON response with status, data, and message. 
              On success: {"status": "success", "data": None, "message": "Video and audio combined successfully"}
              On error: {"status": "error", "data": None, "message": "Detailed error message"}
    """
    try:
        ...
        try:
            ...
            print(f"Warning: Failed to clean up temporary audio file {temp_audio_path}: {cleanup_error}")
        ...
        if e.stderr:
            error_msg += f". FFmpeg stderr: {e.stderr.strip()}"
        if e.stdout:
            error_msg += f". FFmpeg stdout: {e.stdout.strip()}"
        error_msg += f". Command: {' '.join(e.cmd) if hasattr(e, 'cmd') else 'Unknown command'}"
        ...
        error_msg = f"Unexpected error during video-audio combination: {str(e)}"
    ...
    try:
        ...
        try:
            ...
            return {
                "status": "error",
                "data": None,
                "message": f"Failed to save TTS audio to temporary file: {str(e)}"
            }
                        ...
                        if new_audio_data:
                            ...
                            try:
                                ...
                                return {
                                    "status": "error",
                                    "data": None,
                                    "message": f"Failed to save summarized TTS audio to temporary file: {str(e)}"
                                }
                            ...
                            return {
                                "status": "error",
                                "data": None,
                                "message": "Failed to generate TTS audio for summarized text"
                            }
                    else:
                        return {
                            "status": "error",
                            "data": None,
                            "message": f"Failed to summarize text: {summary_result.get('message', 'Unknown error')}"
                        }
        ...
        if combine_result["status"] == "error":
            return {
                "status": "error",
                "data": None,
                "message": f"Failed to combine video with audio: {combine_result['message']}"
            }

...
def resolve_route(project: str, route_key: str, runtime_variables: Optional[Dict[str, Any]] = None) -> Tuple[str, Dict[str, str], Dict[str, Any], Dict[str, Any]]:
    """Build a single request for a route.

    Returns: (url, headers, body, meta={"config_key": ...})
    """
    import json as _json
    route_row, members = _fetch_route_row(project, route_key)
    if not members:
...
def run_route(project: str, route_key: str, runtime_variables: Optional[Dict[str, Any]] = None) -> Tuple[int, Optional[Dict[str, Any]]]:
    ...
    import time as _t
    try:
        url, headers, body, meta = resolve_route(project, route_key, runtime_variables=runtime_variables)
    except Exception:
        # fallback error shape
        return 500, {"error": {"code": "ROUTE_RESOLVE_ERROR", "message": "unable to resolve route"}}
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            insert_log(cur, meta.get("config_key") or route_key, status, duration_ms, resp_json, error, log_body, route_key_for_oss=route_key)
    except Exception:
        pass

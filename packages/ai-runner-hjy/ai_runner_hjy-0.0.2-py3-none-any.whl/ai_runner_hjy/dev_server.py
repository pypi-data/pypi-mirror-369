...
@app.get("/routes")
async def list_routes(project: Optional[str] = None):
    if not project:
        return {"routes": []}
    rows = []
    try:
...
async def invoke_route(request: Request):
    ...
    try:
        url, headers, req_body, meta = resolve_route(project_name, route_key, runtime_variables=variables)

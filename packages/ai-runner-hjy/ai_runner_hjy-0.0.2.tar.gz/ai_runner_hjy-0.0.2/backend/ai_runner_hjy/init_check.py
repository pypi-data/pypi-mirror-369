...
from .core.env import load_envs
...
REQUIRED_ENV_KEYS = [
    ...
    "MYSQL_USER",
    "MYSQL_PASSWORD",
    # One of MYSQL_AI_DATABASE or MYSQL_DATABASE must exist; handled specially
    "AI_PEPPER",
]


def _check_envs() -> Tuple[bool, Dict[str, str]]:
    missing: List[str] = []
    info: Dict[str, str] = {}
    ...
    if not (os.environ.get("MYSQL_AI_DATABASE") or os.environ.get("MYSQL_DATABASE")):
        missing.append("MYSQL_AI_DATABASE|MYSQL_DATABASE")
    ok = len(missing) == 0
    return ok, {"missing": ", ".join(missing) if missing else ""}
...
def run_init(env_only: bool = False, db_only: bool = False, dev_port: int | None = None) -> int:
    load_envs()
    results: Dict[str, str] = {}

    # Env check
    if not db_only:
        ok_env, env_msg = _check_envs()
        results["env"] = "OK" if ok_env else f"FAIL: {env_msg['missing']}"
    ...
    if results.get("env", "OK").startswith("FAIL") or results.get("db", "OK").startswith("FAIL"):
        return 1
    return 0

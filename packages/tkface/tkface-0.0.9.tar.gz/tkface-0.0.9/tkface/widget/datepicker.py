...
class _DatePickerBase:
    ...
    def _setup_click_outside_handling(self):
        ...
        self.winfo_toplevel().bind(
            ...
            lambda e: self._on_main_window_click(
                e, self.popup, self.hide_calendar
            ),
        )
    ...
    def show_year_view(self):
        ...
        try:
            if self.dpi_scaling_factor > 1.0:
                ...
                import re
                match = re.match(
                    r"(\d+)x(\d+)\+(\d+)\+(\d+)", year_view_geometry
                )
                if match:
            ...
            self.logger.debug(
                f"Failed to scale year view geometry: {e}, "
    ...
    def _on_parent_configure(self, event):
        ...
        if self.popup and self.popup.winfo_exists() and not year_view_active:
            ...
            try:
                ...
                self.logger.debug(
                    f"Failed to scale popup geometry: {e}, "
                    "using original geometry"
                )
        ...
        if year_view_active and hasattr(self, "year_view_calendar"):
            ...
            year_view_geometry = self.year_view_calendar.get_popup_geometry(
                self
            )
            # Scale year view geometry based on DPI (only scale size, not
            # position)
            try:
                ...
                self.logger.debug(
                    f"Failed to scale year view geometry in parent configure: "
                    f"{e}, using original geometry"
                )
            self.year_view_window.geometry(year_view_geometry)
    def _bind_parent_movement_events(self):
        """Bind events to monitor parent window movement."""
        if self.popup or (
            hasattr(self, "year_view_window") and self.year_view_window
        ):
    ...
    def _unbind_parent_movement_events(self):
        ...
        if hasattr(self, "_parent_configure_binding"):
            ...
            try:
                main_window.unbind(
                    "<Configure>", self._parent_configure_binding
                )
            except Exception as e:
                self.logger.debug(
                    f"Failed to unbind parent configure events: {e}"
                )
    ...
    def show_calendar(self):
        ...
        if (
            hasattr(self, "calendar_config")
            and "theme" in self.calendar_config
        ):
        ...
        try:
            ...
            self.logger.debug(
                f"Failed to scale popup geometry in show_calendar: "
                f"{e}, using original geometry"
            )
        self.popup.geometry(popup_geometry)
    ...
    def get_date(self) -> Optional[datetime.date]:
        ...
        return (
            self.calendar.get_selected_date()
            if self.calendar
    ...
    def get_date_string(self) -> str:
        ...
        return (
            selected_date.strftime(self.date_format) if selected_date else ""
        )
    ...
    def set_day_colors(self, day_colors: Dict[str, str]):
        ...
        self._update_config_and_delegate(
            "day_colors", day_colors, "set_day_colors"
        )
    def set_week_start(self, week_start: str):
        """Set the week start day."""
        self._update_config_and_delegate(
            "week_start", week_start, "set_week_start"
        )
                    ...
                    try:
                        if self.dpi_scaling_factor > 1.0:
                            ...
                            if match:
                                width, height, x, y = map(int, match.groups())
                                scaled_width = int(
                                    width * self.dpi_scaling_factor
                                )
                                scaled_height = int(
                                    height * self.dpi_scaling_factor
                                )
                                # Don't scale position coordinates (x, y) -
                                # keep them as is
                                popup_geometry = (
                                    f"{scaled_width}x{scaled_height}+{x}+{y}"
                                )
                        ...
                        self.logger.debug(
                            f"Failed to scale popup geometry in "
                            f"update_dpi_scaling: {e}"
                        )
                    self.popup.geometry(popup_geometry)
        except Exception as e:
            self.logger.warning(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.button = tk.Button(
            self, text=button_text, command=self.show_calendar
        )
        ...
        try:
            ...
            self.logger.debug(
                f"Failed to update DPI scaling during DateFrame "
                f"initialization: {e}"
            )
    def _update_entry_text(self, text: str):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            self.logger.debug(
                f"Failed to update DPI scaling during DateEntry "
                f"initialization: {e}"
            )

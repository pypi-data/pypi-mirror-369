import { _ as s, c as xt, l as T, d as q, V as kt, W as vt, X as _t, Y as bt, B as wt, $ as St, y as Et } from "./mermaid.core-DXaODDIH.js";
import { d as nt } from "./arc-Cu2DeyqI.js";
var X = function() {
  var n = /* @__PURE__ */ s(function(f, i, a, h) {
    ...
    parse: /* @__PURE__ */ s(function(i) {
      var a = this, h = [0], u = [], y = [null], o = [], S = this.table, k = "", $ = 0, H = 0, B = 2, J = 1, O = o.slice.call(arguments, 1), v = Object.create(this.lexer), E = { yy: {} };
      ...
      function A(I) {
        h.length = h.length - 2 * I, y.length = y.length - I, o.length = o.length - I;
      }
      s(A, "popStack");
      function F() {
        var I;
      ...
      s(F, "lex");
      for (var w, C, N, K, V = {}, j, M, et, G; ; ) {
        if (C = h[h.length - 1], this.defaultActions[C] ? N = this.defaultActions[C] : ((w === null || typeof w > "u") && (w = F()), N = S[C] && S[C][w]), typeof N > "u" || !N.length || !N[0]) {
          var Q = "";
          G = [];
          for (j in S[C])
...
Expecting ` + G.join(", ") + ", got '" + (this.terminals_[w] || w) + "'" : Q = "Parse error on line " + ($ + 1) + ": Unexpected " + (w == J ? "end of input" : "'" + (this.terminals_[w] || w) + "'"), this.parseError(Q, {
        ...
        switch (N[0]) {
          case 1:
            h.push(w), y.push(v.yytext), o.push(v.yylloc), h.push(N[1]), w = null, H = v.yyleng, k = v.yytext, $ = v.yylineno, L = v.yylloc;
            break;
          case 2:
            if (M = this.productions_[N[1]][1], V.$ = y[y.length - M], V._$ = {
              ...
              return K;
            M && (h = h.slice(0, -1 * M * 2), y = y.slice(0, -1 * M), o = o.slice(0, -1 * M)), h.push(this.productions_[N[1]][0]), y.push(V.$), o.push(V._$), et = S[h[h.length - 2]][h[h.length - 1]], h.push(et);
            break;
          case 3:
            return !0;
        }
...
var Ft = /* @__PURE__ */ s(function(n, t, e, l) {
  ...
  f.forEach(function(E) {
    const b = {
      ...
      padding: 20,
      maxHeight: i
    }, L = R.getVirtualNodeHeight(p, b, r);
  ...
  for (const [E, b] of x.entries()) {
    const L = {
      ...
      padding: 20,
      maxHeight: a
    }, P = R.getVirtualNodeHeight(p, L, r);
    T.debug("taskHeight before draw", P), a = Math.max(a, P + 20), $ = Math.max($, b.events.length);
    let A = 0;
    for (const F of b.events) {
      ...
      A += R.getVirtualNodeHeight(p, w, r);
    }
    b.events.length > 0 && (A += (b.events.length - 1) * 10), H = Math.max(H, A);
  }
  T.debug("maxSectionHeight before draw", i), T.debug("maxTaskHeight before draw", a), f && f.length > 0 ? f.forEach((E) => {
    ...
    const P = p.append("g"), A = R.drawNode(P, L, S, r);
    T.debug("sectionNode output", A), P.attr("transform", `translate(${y}, ${u})`), o += i + 50, b.length > 0 && st(
      p,
      b,
      S,
      y,
  ...
  T.debug("bounds", B), _ && p.append("text").text(_).attr("x", B.width / 2 - d).attr("font-size", "4ex").attr("font-weight", "bold").attr("y", 20), h = k ? i + a + 150 : a + 100, p.append("g").attr("class", "lineWrapper").append("line").attr("x1", d).attr("y1", h).attr("x2", B.width + 3 * d).attr("y2", h).attr("stroke-width", 4).attr("stroke", "black").attr("marker-end", "url(#arrowhead)"), kt(
    void 0,
    p,
  ...
  for (const f of t) {
    ...
    T.debug("taskNode", i);
    const a = n.append("g").attr("class", "taskWrapper"), u = R.drawNode(a, i, e, c).height;
    if (T.debug("taskHeight after draw", u), a.attr("transform", `translate(${l}, ${r})`), d = Math.max(d, u), f.events) {
  ...
  for (const m of t) {
    ...
    T.debug("eventNode", p);
    const x = n.append("g").attr("class", "eventWrapper"), f = R.drawNode(x, p, e, d).height;

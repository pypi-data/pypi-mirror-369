...
class CampbellCloud:
    ...
    def __get_data_of_datastream(self, datastream_id, start_epoch, end_epoch, data_list: list=[]):
        datastream_data = self.get_datastream_datapoints(datastream_id, start_epoch, end_epoch, limit=15000)
        new_data_list = datastream_data['data'] + data_list
        if datastream_data['range']['exceeded_request']:
            latest_epoch = datastream_data['range']['start']
            return self.__get_data_of_datastream(datastream_id, start_epoch, latest_epoch, new_data_list)
        else:
    ...
    def get_datapoints(self, asset_id, datatables: list = None, fieldnames: list = None, start_epoch: str = '0000000000000',
        ...
        for datastream in approved_datastreams:
            ...
            asset_datatable = datastream['metadata']['table']
            asset_fieldname = datastream['metadata']['field']
            datastream_data = self.__get_data_of_datastream(asset_datastream_id, start_epoch, end_epoch)

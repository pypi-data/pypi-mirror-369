...
class DataFileResponseDataAttributes(BaseModel):
    """
    DataFileResponseDataAttributes
    """ # noqa: E501
    tags: Optional[List[Annotated[str, Field(min_length=1, strict=True)]]] = None
    data_type_annotations: Optional[Annotated[List[OntologyTerm], Field(min_length=0)]] = None
    data_format_annotations: Optional[Annotated[List[OntologyTerm], Field(min_length=0)]] = None
    ...
    latest_version: Union[StrictFloat, StrictInt]
    versions: Annotated[List[Version], Field(min_length=1)]
    ...
    created_at: datetime
    updated_at: datetime
    content_blobs: Annotated[List[ContentBlob], Field(min_length=1)]
    ...
    __properties: ClassVar[List[str]] = ["tags", "data_type_annotations", "data_format_annotations", "title", "description", "license", "latest_version", "versions", "version", "revision_comments", "doi", "created_at", "updated_at", "content_blobs", "policy", "other_creators", "creators", "discussion_links", "extended_attributes"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of DataFileResponseDataAttributes from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in data_type_annotations (list)
        _items = []
        if self.data_type_annotations:
            for _item_data_type_annotations in self.data_type_annotations:
                if _item_data_type_annotations:
                    _items.append(_item_data_type_annotations.to_dict())
            _dict['data_type_annotations'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in data_format_annotations (list)
        _items = []
        if self.data_format_annotations:
            for _item_data_format_annotations in self.data_format_annotations:
                if _item_data_format_annotations:
                    _items.append(_item_data_format_annotations.to_dict())
            _dict['data_format_annotations'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of DataFileResponseDataAttributes from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "tags": obj.get("tags"),
            "data_type_annotations": [OntologyTerm.from_dict(_item) for _item in obj["data_type_annotations"]] if obj.get("data_type_annotations") is not None else None,
            "data_format_annotations": [OntologyTerm.from_dict(_item) for _item in obj["data_format_annotations"]] if obj.get("data_format_annotations") is not None else None,
            "title": obj.get("title"),

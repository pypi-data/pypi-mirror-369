...
CONTENTBLOBSLOT_ONE_OF_SCHEMAS = ["ContentBlobPlaceholder", "RemoteContentBlob"]

class ContentBlobSlot(BaseModel):
    """
    ContentBlobSlot
    """
    # data type: RemoteContentBlob
    oneof_schema_1_validator: Optional[RemoteContentBlob] = None
    # data type: ContentBlobPlaceholder
    oneof_schema_2_validator: Optional[ContentBlobPlaceholder] = None
    actual_instance: Optional[Union[ContentBlobPlaceholder, RemoteContentBlob]] = None
    one_of_schemas: Set[str] = { "ContentBlobPlaceholder", "RemoteContentBlob" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ContentBlobSlot.model_construct()
        error_messages = []
        match = 0
        # validate data type: RemoteContentBlob
        if not isinstance(v, RemoteContentBlob):
            error_messages.append(f"Error! Input type `{type(v)}` is not `RemoteContentBlob`")
        else:
            match += 1
        # validate data type: ContentBlobPlaceholder
        if not isinstance(v, ContentBlobPlaceholder):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ContentBlobPlaceholder`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ContentBlobSlot with oneOf schemas: ContentBlobPlaceholder, RemoteContentBlob. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ContentBlobSlot with oneOf schemas: ContentBlobPlaceholder, RemoteContentBlob. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into RemoteContentBlob
        try:
            instance.actual_instance = RemoteContentBlob.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into ContentBlobPlaceholder
        try:
            instance.actual_instance = ContentBlobPlaceholder.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ContentBlobSlot with oneOf schemas: ContentBlobPlaceholder, RemoteContentBlob. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ContentBlobSlot with oneOf schemas: ContentBlobPlaceholder, RemoteContentBlob. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ContentBlobPlaceholder, RemoteContentBlob]]:

...
ASSETLINKPATCHLISTINNER_ANY_OF_SCHEMAS = ["AssetLink", "AssetLinkCreate"]

class AssetLinkPatchListInner(BaseModel):
    """
    AssetLinkPatchListInner
    """

    # data type: AssetLink
    anyof_schema_1_validator: Optional[AssetLink] = None
    # data type: AssetLinkCreate
    anyof_schema_2_validator: Optional[AssetLinkCreate] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[AssetLink, AssetLinkCreate]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "AssetLink", "AssetLinkCreate" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = AssetLinkPatchListInner.model_construct()
        error_messages = []
        # validate data type: AssetLink
        if not isinstance(v, AssetLink):
            error_messages.append(f"Error! Input type `{type(v)}` is not `AssetLink`")
        else:
            return v

        # validate data type: AssetLinkCreate
        if not isinstance(v, AssetLinkCreate):
            error_messages.append(f"Error! Input type `{type(v)}` is not `AssetLinkCreate`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in AssetLinkPatchListInner with anyOf schemas: AssetLink, AssetLinkCreate. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[AssetLink] = None
        try:
            instance.actual_instance = AssetLink.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[AssetLinkCreate] = None
        try:
            instance.actual_instance = AssetLinkCreate.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AssetLinkPatchListInner with anyOf schemas: AssetLink, AssetLinkCreate. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], AssetLink, AssetLinkCreate]]:

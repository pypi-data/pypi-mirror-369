...
class Test:
    ...
    """
    def __init__(self, name: str, options: dict, description: str = "", stop: dict = {}, runTest: bool = True):
        """
        A test is used to randomly generate (with probabilities) a value based on the options.
        name: the name of the test
        options: a dictionary of options with their probabilities
        description: a description of the test
        stop: a dictionary of stop conditions (iterations, confidence). 
...
class Testing:
    ...
    """
    def __init__(self, tests: Dict[str, Test] = {}, outcomes: Dict[str, Outcome] = {}, runTest: Dict[str, bool] = {}):
        self.tests: Dict[str, Test] = tests
        self.outcomes: Dict[str, Outcome] = outcomes
        self.runTest: Dict[str, bool] = runTest

    def add_test(self, name: str, options: dict, description: str = "", stop: dict = {}):
        """
        Add a test to the testing object.
        """
        if name not in self.tests:
            self.tests[name] = Test(name, options, description, stop, self.runTest.get(name, True))
    ...
    def add_tests(self, tests: list[dict]):
        ...
        """
        for test in tests:
            if (name := test.get("name", None)) and (options := test.get("options", None)):
                self.add_test(name, options, test.get("description", ""), test.get("stop", {}))
            else:
    ...
    def add_outcomes(self, outcomes: list[dict]):
        ...
        for outcome in outcomes:
            if (name := outcome.get("name", None)) and (type := outcome.get("type", None)):
    ...
    def __dict__(self):
        return {
            ...
            "outcomes": {name: outcome.__dict__() for name, outcome in self.outcomes.items()},

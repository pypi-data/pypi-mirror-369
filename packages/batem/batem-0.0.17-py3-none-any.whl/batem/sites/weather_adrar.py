...
from batem.core.solar import RADIATION_TYPES, SolarModel
...
dp = DataProvider(location='adrar', latitude_deg_north=27.887364438572845, longitude_deg_east=-
                  0.27127041325827334, starting_stringdate='1/01/2014', ending_stringdate='1/01/2015')
# dp = DataProvider(location='grenoble', latitude_deg_north=45.19154994547585, longitude_deg_east=5.722065312331381, starting_stringdate='1/01/2014', ending_stringdate='1/01/2015')
...
print('calculus (total facing south): %gkWh ' %
      (sum(irradiances_south[RADIATION_TYPES.TOTAL])/1000))
...
print('calculus (total facing north): %gkWh ' %
      (sum(irradiances_north[RADIATION_TYPES.TOTAL])/1000))

dp.add_external_variable('dni', solar_model.dni)
dp.add_external_variable('dhi', solar_model.dhi)
dp.add_external_variable('rhi', solar_model.rhi)
dp.add_external_variable('ghi', solar_model.ghi)
dp.add_external_variable('tsi', solar_model.tsi)

dp.add_external_variable('model direct facing sky',
                         irradiances_facing_sky[RADIATION_TYPE.DIRECT])
dp.add_external_variable('model diffuse facing sky',
                         irradiances_facing_sky[RADIATION_TYPES.DIFFUSE])
dp.add_external_variable('model reflected facing sky',
                         irradiances_facing_sky[RADIATION_TYPES.REFLECTED])
dp.add_external_variable('model total facing sky',
                         irradiances_facing_sky[RADIATION_TYPES.TOTAL])
dp.add_external_variable('model normal facing sky',
                         irradiances_facing_sky[RADIATION_TYPES.NORMAL])

dp.add_external_variable(
    'model total south', irradiances_south[RADIATION_TYPE.TOTAL])
dp.add_external_variable(
    'model total east', irradiances_east[RADIATION_TYPES.TOTAL])
dp.add_external_variable(
    'model total west', irradiances_west[RADIATION_TYPES.TOTAL])
dp.add_external_variable(
    'model total north', irradiances_north[RADIATION_TYPES.TOTAL])
dp.add_external_variable(
    'model total best', irradiances_best[RADIATION_TYPES.TOTAL])
dp.add_external_variable('model total oppbest',
                         irradiances_oppbest[RADIATION_TYPES.TOTAL])

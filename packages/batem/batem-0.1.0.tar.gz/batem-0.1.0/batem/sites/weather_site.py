...
from core.lambdahouse import Analyzes, DefaultConfiguration, ReportGenerator
...
def plot_rain_events(datetimes: list[datetime], precipitations: list[float], *axs):
    ...
    for k, precipitation in enumerate(precipitations):
        ...
        stringdate: str = core.timemg.datetime_to_stringdate(
            datetimes[k]).split(' ')[0]
...
if __name__ == '__main__':
    ...
    dp = DataProvider(location=location, latitude_deg_north=latitude, longitude_deg_east=longitude,
                      starting_stringdate=from_stringdate, ending_stringdate=to_stringdate)

    solar_model = core.solar.SolarModel(dp.weather_data)
    solar_system = core.solar.SolarSystem(solar_model)
    core.solar.Collector(solar_system, 'surface', surface_m2=surface,
                         exposure_deg=exposure, slope_deg=slope, solar_factor=solar_factor)
    solar_gains_in_Wh = solar_system.solar_gains_W()

    dp.add_external_variable('horizontal irradiance Wh', solar_gains_in_Wh)

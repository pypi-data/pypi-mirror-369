...
class ModePort(Port):
    ...
    """
    def __init__(self, data_provider: DataProvider, model_variable_name: str, feeding_variable_name: str, value_domain_type: VALUE_DOMAIN_TYPE, modes_value_domains: dict[int, list[float]], mode_variables: tuple[str] | str) -> None:
...
class ContinuousModePort(ModePort):
    ...
    :type mode_variables: tuple[str]
    """
    def __init__(self, data_provider: DataProvider, model_variable_name: str, feeding_variable_name: str,  modes_value_domains: dict[int, list[float]], mode_variables: tuple[str] | str) -> None:
...
class HvacModePort(ModePort):
    ...
    """
    def __init__(self, data_provider: DataProvider, model_variable: str, feeding_variable_name: str,  value_domain_type: VALUE_DOMAIN_TYPE, modes_value_domains: dict[int, list[float]], mode_variable: str) -> None:
        ...
        :type mode_variables: tuple[str]
        """
        super().__init__(data_provider, model_variable, feeding_variable_name, value_domain_type, modes_value_domains, mode_variable)
...
class HvacContinuousModePort(HvacModePort):
    """A continuous mode port specifically for HVAC control.
    """
    def __init__(self, data_provider: DataProvider, model_variable_name: str, feeding_variable_name: str, max_heating_power: float, max_cooling_power: float, mode_variable: str) -> None:
        ...
        :type mode_variables: tuple[str]
        """
        super().__init__(data_provider, model_variable_name, feeding_variable_name, VALUE_DOMAIN_TYPE.CONTINUOUS, {0: 0, 1: (0, max_heating_power), -1: (-max_cooling_power, 0)}, mode_variable)
...
class TemperatureSetpointPort(DiscreteModePort):
    ...
    :type mode_variable: str
    """
    def __init__(self, data_provider: DataProvider, model_variable_name: str, feeding_variable_name: str, heating_levels: list[float], cooling_levels: list[float], mode_variable: str) -> None:
        ...
        :type mode_variable: str
        """
        super().__init__(data_provider, model_variable_name, feeding_variable_name, {1: heating_levels, 0: 0, -1: cooling_levels}, mode_variable)
...
class Simulation:
    ...
    def add_zone(self, zone_name: str, heat_gain_name: str, CO2production_name: str, hvac_power_port: Port, temperature_controller: TemperatureController) -> None:
        ...
        for k in range(len(self.dp)):
            ...
            for zone_name in self.name_zones:  # zone processing
                ...
                if zone_control_type == CONTROL_TYPE.POWER_CONTROL:
                    control_k: float = zone.hvac_power_port(k, self.dp(zone.hvac_power_port.feeding_variable_name, k), mode_variable_values={v: self.dp(v, k) for v in zone.hvac_power_port.mode_variables})
                    ...
                    if setpoint_k is None or np.isnan(setpoint_k):
                        ...
                        setpoint_k = temperature_controller.temperature_setpoint_port(k, setpoint_k, mode_variable_values={zone.hvac_power_port.mode_variable: self.dp(zone.hvac_power_port.mode_variable, k)})
                ...
                if zone_control_type in (CONTROL_TYPE.POWER_CONTROL, CONTROL_TYPE.TEMPERATURE_CONTROL):
                    ...
                    control_k = zone.hvac_power_port(k, control_k, mode_variable_values={v: self.dp(v, k) for v in zone.hvac_power_port.mode_variables})

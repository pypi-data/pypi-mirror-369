...
class Configuration(object):
	...
	_configParser:configparser.ConfigParser = None
	...
	@staticmethod
	def _createConfigParser() -> configparser.ConfigParser:
		"""	Create a new ConfigParser instance with extended interpolation and converters for lists."""
	...
	def init(args:Optional[argparse.Namespace] = None) -> bool:
		...
		Configuration._configParser = Configuration._createConfigParser()
		...
		# Set the defaults
		Configuration._configParser.read_dict(_defaults)
		...
		try:
			...
			if len(Configuration._configParser.read(configurationFiles)) == 0 and Configuration._args_configfile != C.defaultUserConfigFile:		# Allow 
			...
			for cs in configurationStrings:
				Configuration._configParser.read_string(cs)
		...
		if _deprecatedSections:
			for o, n in _deprecatedSections:
				if Configuration._configParser.has_section(o):
		...
		try:
			...
			for m in _moduleConfigs:
				m.readConfiguration(Configuration._configParser, Configuration)	# type:ignore [arg-type]

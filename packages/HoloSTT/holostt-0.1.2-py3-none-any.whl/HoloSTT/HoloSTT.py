            ...
            try:
                with sr.Microphone() as source:
                    ...
                    if selfSpeech:
                        ...
                        systemVolume = self._getSystemVolume()
                        recognizer.energy_threshold = 1.5 * (2000 + (systemVolume / 100) * 8000)
                    ...
                    if selfSpeech:
                        audioEnergy = audioop.rms(audio.get_raw_data(), audio.sample_width)
                        if audioEnergy < recognizer.energy_threshold * 1.3:
                            return None
                        if len(audio.get_raw_data()) < audio.sample_rate * 0.3:
                            return None

...
class HoloSTT:
    ...
    #                     if audioEnergy < recognizer.energy_threshold + 400:
    #                         return None
    #                 if not selfSpeech:
    #                     if self.parent:
            ...
            try:
                with sr.Microphone() as source:
                    ...
                    if selfSpeech:
                        ...
                        systemVolume = self._getSystemVolume()
                        # Tuned threshold: lower baseline, smoother scaling
                        recognizer.energy_threshold = 1.2 * (1000 + (systemVolume / 100) * 4000)

                    try:
                        audio = recognizer.listen(
                            source,
                            timeout=5,
                            phrase_time_limit=None  # no hard cap, ends on silence
                        )
                    except sr.WaitTimeoutError:
                        logger.warning("No speech detected: timed out while waiting for phrase.")
                        return None
                    ...
                    if selfSpeech:
                        audioEnergy = audioop.rms(audio.get_raw_data(), audio.sample_width)
                        # Relaxed margin → short commands pass more easily
                        margin = 1.15 if systemVolume > 50 else 1.05
                        if audioEnergy < recognizer.energy_threshold * margin:
                            return None

                        # Allow short words like "stop" (~0.05s+)
                        if len(audio.get_raw_data()) < audio.sample_rate * 0.05:
                            return None
                ...
                logger.error("Microphone error:", exc_info=True)
        return None

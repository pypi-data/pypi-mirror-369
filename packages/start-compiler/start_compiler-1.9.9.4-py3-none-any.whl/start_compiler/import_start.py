...
listener = keyboard.Listener(on_press=None, on_release=add_key)
...
def _do_get(name, local, scope, setter=False):
    ...
    if scope is None:
        ...
        if attr is None:
            if is_dynamic(scope) and setter == True:
...
__all__ = [
 ...
 "_div", "_mod", "_pow", "_eql_val", "_eql_ref",

(()=>{var w=(j,a)=>()=>(j&&(a=j(j=0)),a);var O=(j,a)=>()=>(a||j((a={exports:{}}).exports,a),a.exports);var z,E=w(()=>{z=`.table-container {
    /* max-width: 800px; */
    max-height: 500px;
    overflow: auto;
}
/* Base table structure */
.frame-display-table {
    width: auto;
    table-layout: auto;
    font-family: -apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Helvetica Neue,Arial,Noto Sans,sans-serif,Apple Color Emoji,Segoe UI Emoji,Segoe UI Symbol,Noto Color Emoji;
    font-size: 12px;
    font-variant-numeric: tabular-nums;
    line-height: 16px;
    color: rgb(17, 23, 28);
    border-collapse: separate;
    border-spacing: 0;
    border: none;
    /* margin: 0; */
    /* border-top: 0 solid black; */
}
/* Cell styling */
.frame-display-table th,
.frame-display-table td {
    min-width: 2em;
    max-width: 25em;
    padding: 0.5em 1.0em 0.5em 0.5em;
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
    border: 0.5px solid #D1D9E1;
}
/* Null cell styling */
.frame-display-table tbody code.null-cell {
    font-size: 85%;
    background-color: rgba(68, 83, 95, 0.1);
    border: 1px solid rgb(232, 236, 241);
    border-radius: 3px;
    padding: 2px 4px;
    display: inline-block;
    color: red;
}
/* Row striping for better readability */
.frame-display-table tbody tr:nth-child(odd) td,
.frame-display-table tbody tr:nth-child(odd) th {
    background-color: #fff;
}
.frame-display-table tbody tr:nth-child(even) td,
.frame-display-table tbody tr:nth-child(even) th {
    background-color: #f5f5f5;
}

/* Header */
.frame-display-table thead tr th {
    position: sticky;
    top: 0;
    padding: 0.75em 1.5em 0.75em 2.5em;
    text-align: left !important;
    background-color: #f2f2f2;
    font-weight: 600;
    z-index: 10;
    cursor: pointer;
    user-select: none;
    border-collapse: separate;
    box-shadow: inset 0px 1px 0px 0px #D1D9E1;
}

/* Reserve left area for icons */
.frame-display-table thead th::before {
    content: '';
    position: absolute;
    left: 0.5em;
    top: 50%;
    transform: translateY(-50%);
    width: 1.5em;
    height: 1.5em;
    background-size: contain;
    background-repeat: no-repeat;
    background-position: center;
}

.frame-display-table thead th[data-dtype="int"]::before {
    background-image: url('data:image/svg+xml;base64,');
}
.frame-display-table thead th[data-dtype="float"]::before {
    background-image: url('data:image/svg+xml;base64,');
    top: 55%;
}
.frame-display-table thead th[data-dtype="string"]::before {
    background-image: url('data:image/svg+xml;base64,');
}
.frame-display-table thead th[data-dtype="datetime"]::before {
    background-image: url('data:image/svg+xml;base64,');
}
.frame-display-table thead th[data-dtype="bool"]::before {
    background-image: url('data:image/svg+xml;base64,');
    width: 1.7em;
    height: 1.7em;
}
.frame-display-table thead th[data-dtype="category"]::before {
    background-image: url('data:image/svg+xml;base64,');
}
.frame-display-table thead th[data-dtype="object"]::before {
    background-image: url('data:image/svg+xml;base64,');
}

/* Sticky first column */
.frame-display-table tbody th {
    position: sticky;
    left: 0;
    background-color: #f8f8f8;
    z-index: 1;
    text-align: center;
    box-shadow: inset 1px 0px 0px 0px #D1D9E1;
}
/* Corner cell (intersection of sticky header and first column) */
.frame-display-table thead tr th:first-child {
    position: sticky;
    left: 0;
    top: 0;
    z-index: 20;
    box-shadow: inset 1px 1px 0px 0px #D1D9E1;
}
.frame-display-table.resizing {
  cursor: col-resize;
}
.frame-display-table.resizing * {
  user-select: none !important;
}

/* Sort arrows */
.frame-display-table thead th.sort-asc::after {
    content: ' \u2193';
    font-size: 1em;
    opacity: 0.7;
}

.frame-display-table thead th.sort-desc::after {
    content: ' \u2191';
    font-size: 1em;
    opacity: 0.7;
}
`});var l,p=w(()=>{l="1.1.5"});var k=O(C=>{E();p();(function(j){"use strict";let a={tableSelector:".frame-display-table",minColumnWidth:30,resizerWidth:8,resizerHoverColor:"rgba(0,0,0,0.1)",showHoverEffect:!0,autoInit:!0,allowReInit:!1,...j.FrameDisplayConfig||{}};if(j.FrameDisplay){let t=j.FrameDisplay.version||"0.0.0";if(a.allowReInit===!0||l.localeCompare(t,void 0,{numeric:!0})>0)j.FrameDisplay.destroy();else return}let d=null;function T(){if(document.getElementById("frame-display-styles"))return;let t=document.createElement("style");t.id="frame-display-styles",t.textContent=z,document.head.appendChild(t)}function f(t){t.querySelectorAll("thead th").forEach((M,s)=>{if(M.querySelector(".column-resizer"))return;let o=document.createElement("div");o.className="column-resizer",Object.assign(o.style,{position:"absolute",top:"0",right:"0",width:a.resizerWidth+"px",height:"100%",cursor:"col-resize",zIndex:"20"}),M.appendChild(o),a.showHoverEffect&&(o.addEventListener("mouseover",()=>{o.style.backgroundColor=a.resizerHoverColor}),o.addEventListener("mouseout",()=>{o.style.backgroundColor=""})),o.addEventListener("click",function(i){i.stopPropagation()});let r,L;o.addEventListener("pointerdown",function(i){r=i.clientX,L=parseFloat(window.getComputedStyle(M).width),o.setPointerCapture(i.pointerId),o.addEventListener("pointermove",c),o.addEventListener("pointerup",u,{once:!0}),t.classList.add("resizing"),document.body.style.userSelect="none",i.preventDefault(),i.stopPropagation()});function c(i){let N=Math.max(a.minColumnWidth,L+(i.clientX-r));M.style.setProperty("width",N+"px","important"),M.style.setProperty("min-width",N+"px","important");let n=t.closest(".table-container");if(n){let I=M.getBoundingClientRect(),y=n.getBoundingClientRect(),m=I.right-y.right;m>0&&(n.scrollLeft+=m)}if(M.getAttribute("data-released")!="true"){M.style.setProperty("max-width",0),M.setAttribute("data-released","true");let I=s+1;t.querySelectorAll(`td:nth-child(${I})`).forEach(y=>{y.style.setProperty("max-width",0)})}}function u(i){i.stopPropagation(),i.preventDefault(),o.releasePointerCapture(i.pointerId),o.removeEventListener("pointermove",c),o.removeEventListener("pointerup",u),document.body.style.userSelect="",setTimeout(()=>t.classList.remove("resizing"),0)}})}function h(t){t.querySelectorAll("th, td").forEach(e=>{let M=e.textContent.trim();M&&(e.title=M)})}function b(t){t.querySelectorAll("thead th").forEach((e,M)=>{e.addEventListener("dblclick",s=>{s.preventDefault()}),e.addEventListener("click",()=>{t.classList.contains("resizing")||U(t,M+1,e)})})}function U(t,e,M){let s=t.dataset.sort||"1-asc",[o,r]=s.split("-"),L=parseInt(o),c=L===e&&r==="asc"?"desc":L===e&&r==="desc"?"none":"asc";if(L!==e){let N=t.querySelector(`thead th:nth-child(${L})`);N&&D(N)}t.dataset.sort=`${e}-${c}`;let u=t.querySelector("tbody"),i=Array.from(u.querySelectorAll("tr"));c==="none"?S(i,1,"asc","object").forEach(n=>u.appendChild(n)):S(i,e,c,M.dataset.dtype).forEach(n=>u.appendChild(n)),v(M,c)}function v(t,e){D(t),e!=="none"&&t.classList.add(`sort-${e}`)}function D(t){t.classList.remove("sort-asc","sort-desc")}function S(t,e,M,s){return t.map(r=>{let L=r.querySelector(`td:nth-child(${e}), th:nth-child(${e})`),c=L?.querySelector(".null-cell"),u=L?.textContent.trim()||"",i=u;if(!c&&u)switch(s){case"int":case"float":i=parseFloat(u)||u;break;case"datetime":i=new Date(u).getTime()||u;break;case"bool":i=/^(true|1|yes|on|t|y)$/i.test(u)?1:0;break;default:e===1&&(i=parseFloat(u)||u)}return{row:r,isNull:c,sortValue:i}}).sort((r,L)=>{if(r.isNull!==L.isNull)return r.isNull?1:-1;let c=typeof r.sortValue=="number"?r.sortValue-L.sortValue:r.sortValue.localeCompare(L.sortValue,void 0,{numeric:!0,sensitivity:"base"});return M==="asc"?c:-c}).map(r=>r.row)}function g(t){t.hasAttribute("data-initialized")||(f(t),h(t),b(t),t.setAttribute("data-initialized","true"))}function A(t={}){T(),a={...a,...t},document.querySelectorAll(a.tableSelector).forEach(g)}function Q(){d&&(d.disconnect(),d=null),document.querySelectorAll(".frame-display-table[data-initialized]").forEach(e=>{e.querySelectorAll(".column-resizer").forEach(M=>{M.remove()}),e.querySelectorAll("thead th").forEach(M=>{let s=M.cloneNode(!0);M.parentNode.replaceChild(s,M),s.classList.remove("sort-asc","sort-desc")}),e.dataset.sort&&delete e.dataset.sort,e.removeAttribute("data-initialized"),e.classList.remove("resizing")});let t=document.getElementById("frame-display-styles");return t&&t.remove(),j.FrameDisplay&&delete j.FrameDisplay,!0}function Y(){if(!(typeof MutationObserver>"u"))return d=new MutationObserver(t=>{t.forEach(e=>{e.type==="childList"&&e.addedNodes.length>0&&e.addedNodes.forEach(M=>{if(M.nodeType===Node.ELEMENT_NODE){let s=[];M.matches&&M.matches(".frame-display-table")&&s.push(M),s.push(...M.querySelectorAll(".frame-display-table:not([data-initialized])")),s.forEach(g)}})})}),d.observe(document.body,{childList:!0,subtree:!0}),d}function x(){a.autoInit!==!1&&(A(),Y())}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",x):setTimeout(x,0),j.FrameDisplay={setup:A,destroy:Q,version:l}})(typeof window<"u"?window:C)});k();})();

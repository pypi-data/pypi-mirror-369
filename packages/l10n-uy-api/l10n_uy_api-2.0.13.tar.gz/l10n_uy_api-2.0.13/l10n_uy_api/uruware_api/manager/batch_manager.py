...
class BatchManager(Manager):
    ...
    def _header_line(self, cfe, id_req):
        ...
        """
        return "|".join([
            MESSAGES_TYPES['lot_request'],           # Posición 0
            cfe.uuid or self._get_uuid_cfe(),        # Posición 1
            str(cfe.cfe_code).zfill(3),             # Posición 2
            cfe.cfe_serial or '',                    # Posición 3
            str(cfe.cfe_number).zfill(7) if cfe.cfe_number else '',  # Posición 4
            str(id_req).zfill(10),                   # Posición 5
            datetime.now().strftime('%H%M%S'),       # Posición 6
            datetime.now().strftime('%Y%m%d'),       # Posición 7
            self.connection_data.terminal_code,      # Posición 8
            self.connection_data.commerce_code,      # Posición 9
            '',                                      # Posición 10
            '',                                      # Posición 11
            '',                                      # Posición 12
            '',                                      # Posición 13
    ...
    def fill_xml(self):
        ...
        if hasattr(self.comprobante, 'description') and self.comprobante.description:
            ...
            if cfe_close_pos != -1:
                ...
                adenda_xml = tostring(adenda_elem).decode()

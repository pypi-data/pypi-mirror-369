...
from sai_mujoco.controllers import controller_factory
from sai_mujoco.utils.binding_utils import MjSim
from sai_mujoco.utils.ik_utils import IKSolver, get_nullspace_gains
...
class CompositeController:
    ...
    def __init__(self, sim: MjSim, arms: dict, grippers: dict):
        self.sim = sim
    ...
    def load_controller_config(
        self,
        part_controller_config,
        composite_controller_specific_config: Optional[Dict] = None,
    ):
        ...
        if body_part_ordering is not None:
            ...
            for part_name in body_part_ordering:
                self.part_controller_config[part_name] = part_controller_config[
                    part_name
                ]
        else:
    ...
    def get_controller_base_pose(self, controller_name):
        ...
        base_pos = np.array(
            self.sim.data.site_xpos[
                self.sim.model.site_name2id(f"{naming_prefix}{part_name}_center")
            ]
        )
        base_ori = np.array(
        ...
        for part_name, controller in self.part_controllers.items():
            if part_name not in self.arms:
                if part_name in self.grippers.keys():
                    ...
                    low, high = (
                        np.concatenate([low, low_g]),
                        np.concatenate([high, high_g]),
                    )
                else:
                    control_dim = controller.control_dim
                    low_c, high_c = ([-1] * control_dim, [1] * control_dim)
                    low, high = (
                        np.concatenate([low, low_c]),
                        np.concatenate([high, high_c]),
                    )
            else:
    ...
    def create_action_vector(self, action_dict):
        ...
        for part_name, action_vector in action_dict.items():
            if part_name not in self._action_split_indexes:
                logging.debug(f"{part_name} is not specified in the action space")
                continue
            start_idx, end_idx = self._action_split_indexes[part_name]
...
class WholeBody(CompositeController):
    ...
    def __init__(self, sim: MjSim, arms: dict, grippers: dict):
        super().__init__(sim, arms, grippers)
    ...
    def setup_action_split_idx(self):
        ...
        for part_name in self.composite_controller_specific_config[
        ...
        for part_name, controller in self.part_controllers.items():
            if (
                part_name
                not in self.composite_controller_specific_config["actuation_part_names"]
            ):
                if part_name in self.grippers.keys():
                    last_idx += self.grippers[part_name].dof
                else:
    ...
    def setup_whole_body_controller_action_split_idx(self):
        ...
        previous_idx = last_idx = list(
            self._whole_body_controller_action_split_indexes.values()
        )[-1][-1]
        for part_name, controller in self.part_controllers.items():
            if (
                part_name
                in self.composite_controller_specific_config["actuation_part_names"]
            ):
                continue
            if part_name in self.grippers.keys():
                last_idx += self.grippers[part_name].dof
    ...
    def create_action_vector(self, action_dict: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        for part_name, action_vector in action_dict.items():
            if part_name not in self._whole_body_controller_action_split_indexes:
                logging.debug(f"{part_name} is not specified in the action space")
                continue
            start_idx, end_idx = self._whole_body_controller_action_split_indexes[
                part_name
            ]
            if end_idx - start_idx == 0:
                # skipping not controlling actions
                continue
            assert len(action_vector) == (end_idx - start_idx), logging.error(
                f"Action vector for {part_name} is not the correct size. Expected {end_idx - start_idx} for {part_name}, got {len(action_vector)}"
            )
            full_action_vector[start_idx:end_idx] = action_vector
        return full_action_vector
    ...
    def print_action_info(self):
        ...
        logging.info(f"Action Dimensions: [{action_dim_info_str}]")

        action_index_info_str = ", ".join(action_index_info)
        logging.info(f"Action Indices: [{action_index_info_str}]")
    ...
    def _init_joint_action_policy(self):
        ...
        for part_name in self.composite_controller_specific_config[
            ...
            if part_name in self.part_controllers:
                ...
                logging.warning(
                    f"{part_name} is not a valid part name in part_controllers."
        ...
        Kn = get_nullspace_gains(
            joint_names,
            self.composite_controller_specific_config["nullspace_joint_weights"],
        )
        mocap_bodies = []
        robot_config = {

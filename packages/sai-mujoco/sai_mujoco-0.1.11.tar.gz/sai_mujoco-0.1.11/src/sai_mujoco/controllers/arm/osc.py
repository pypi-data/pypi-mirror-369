...
from sai_mujoco.utils.control_utils import (
    orientation_error,
    opspace_matrices,
    nullspace_torques,
)
...
class OperationalSpaceController(Controller):
    ...
    def run_controller(self):
        ...
        if self.interpolator_pos is not None:
            ...
            if self.input_ref_frame == "base":
                ...
                desired_world_pos = self.origin_pos + np.dot(
                    self.origin_ori, self.goal_pos
                )
            elif self.input_ref_frame == "world":
        ...
        base_pos_vel = np.array(
            self.sim.data.get_site_xvelp(f"{self.naming_prefix}{self.part_name}_center")
        )
        ...
        base_ori_vel = np.array(
            self.sim.data.get_site_xvelr(f"{self.naming_prefix}{self.part_name}_center")
        )
    ...
    def delta_to_abs_action(self, delta_ac, goal_update_mode):
        ...
        """
        abs_pos = self.compute_goal_pos(
            delta_ac[0:3], goal_update_mode=goal_update_mode
        )
        abs_ori = self.compute_goal_ori(
            delta_ac[3:6], goal_update_mode=goal_update_mode
        )

...
import sai_mujoco.utils.rotations as T


def get_nullspace_gains(
    joint_names: List[str], weight_dict: Dict[str, float]
) -> np.ndarray:
...
class IKSolver:
    def __init__(
        ...
        debug: bool = False,
        input_action_repr: Literal[
            "absolute", "relative", "relative_pose"
        ] = "absolute",
        ...
        self.dof_ids = np.array(
            [self.full_model.joint(name).id for name in robot_config["joint_names"]]
        )
        ...
        self.actuator_ids = np.array(
            [i for i in range(20)]
        )  # TODO no hardcode; cur for GR1; model.nu is 32
        self.key_id = (
            self.full_model.key(robot_config["initial_keyframe"]).id
            if "initial_keyframe" in robot_config
            else None
        )

        self.jac_temps: List[np.ndarray] = [
            np.zeros((6, self.full_model.nv)) for _ in range(len(self.site_ids))
        ]
        self.twists: List[np.ndarray] = [np.zeros(6) for _ in range(len(self.site_ids))]
        self.site_quats: List[np.ndarray] = [
            np.zeros(4) for _ in range(len(self.site_ids))
        ]
        self.site_quat_conjs: List[np.ndarray] = [
            np.zeros(4) for _ in range(len(self.site_ids))
        ]
        self.error_quats: List[np.ndarray] = [
            np.zeros(4) for _ in range(len(self.site_ids))
        ]
        ...
        self.control_limits = (
            np.array([-np.inf] * self.control_dim),
            np.array([np.inf] * self.control_dim),
        )
        self.debug_iter = 0
        self.debug = debug
        ...
        if input_type == "mocap":
            self.mocap_ids = [
                self.full_model.body(name).mocapid[0]
                for name in robot_config["mocap_bodies"]
            ]
        elif input_type == "pkl":
            self.mocap_ids = [
                self.full_model.body(name).mocapid[0]
    ...
    def reset_to_initial_state(self):
        if self.key_id is not None:
            mujoco.mj_resetDataKeyframe(
                self.full_model, self.full_model_data, self.key_id
            )
        else:
    ...
    def get_targets(self):
        ...
        target_pos = np.array(
            [self.full_model_data.site(site_id).xpos for site_id in self.site_ids]
        )
        target_ori_mat = np.array(
            [self.full_model_data.site(site_id).xmat for site_id in self.site_ids]
        )
        ...
        if self.input_type == "mocap":
            ...
            target_ori[right_index] = self.history["right_eef_quat_wxyz"][
                int(self.pkl_t)
            ]
            self.full_model_data.mocap_pos[self.mocap_ids[left_index]] = self.history[
                "left_eef_pos"
            ][int(self.pkl_t)]
            self.full_model_data.mocap_pos[self.mocap_ids[right_index]] = self.history[
                "right_eef_pos"
            ][int(self.pkl_t)]
            self.full_model_data.mocap_quat[self.mocap_ids[left_index]] = self.history[
                "left_eef_quat_wxyz"
            ][int(self.pkl_t)]
            self.full_model_data.mocap_quat[self.mocap_ids[right_index]] = self.history[
                "right_eef_quat_wxyz"
            ][int(self.pkl_t)]
        else:
    ...
    def _compute_jacobian(self, model: mujoco.MjModel, data: mujoco.MjData):
        for i, site_id in enumerate(self.site_ids):
            mujoco.mj_jacSite(
                model, data, self.jac_temps[i][:3], self.jac_temps[i][3:], site_id
            )
    ...
    def forward_kinematics(self, qpos: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        return {
            name: data.site(site_id).xpos
            for name, site_id in zip(self.site_names, self.site_ids)
        }
    ...
    def transform_pose(
        ...
        if src_frame != "world":
            X_W_src_frame = T.make_pose(
                ...
                rotation=T.quat2mat(
                    np.roll(self.full_model.body(src_frame).quat, shift=-1)
                ),
            )
    ...
    def solve(
        ...
        if self.input_rotation_repr == "axis_angle":
            target_quat_wxyz = np.array(
                [
                    np.roll(T.axisangle2quat(target_ori[i]), 1)
                    for i in range(len(target_ori))
                ]
            )
        elif self.input_rotation_repr == "mat":
            target_quat_wxyz = np.array(
                [np.roll(T.mat2quat(target_ori[i])) for i in range(len(target_ori))]
            )
        ...
        if self.input_action_repr == "relative":
            ...
            target_quat_xyzw = np.array(
                [
                    T.quat_mul(
                        T.mat2quat(cur_ori[i].reshape(3, 3)),
                        np.roll(target_quat_wxyz[i], -1),
                    )
                    for i in range(len(self.site_ids))
                ]
            )
            target_quat_wxyz = np.array(
                [
                    np.roll(target_quat_xyzw[i], shift=1)
                    for i in range(len(self.site_ids))
                ]
            )
            ...
            new_target_poses = np.array(
                [
                    np.dot(cur_poses[i], target_poses[i])
                    for i in range(len(self.site_ids))
                ]
            )
        ...
        for i in range(len(self.site_ids)):
            ...
            mujoco.mju_mat2Quat(
                self.site_quats[i], self.full_model_data.site(self.site_ids[i]).xmat
            )
            mujoco.mju_negQuat(self.site_quat_conjs[i], self.site_quats[i])
            mujoco.mju_mulQuat(
                self.error_quats[i], target_quat_wxyz[i], self.site_quat_conjs[i]
            )
        ...
        dq_null = (eye - np.linalg.pinv(jac) @ jac) @ (
            self.Kn * (self.q0 - self.full_model_data.qpos[self.dof_ids])
        )
        self.dq += dq_null
        ...
        if self.debug and self.debug_iter % 10 == 0:
            ...
            integrated_pos_np = np.array(
                [integrated_pos[site] for site in integrated_pos]
            )
            pre_clip_error = np.linalg.norm(target_pos - integrated_pos_np)
            logging.info(
                f"IK error before clipping based on joint ranges: {pre_clip_error}"
            )
        ...
        if self.debug and self.debug_iter % 10 == 0:
            ...
            logging.info(
                f"IK error after clipping based on joint ranges: {post_clip_error}"
            )

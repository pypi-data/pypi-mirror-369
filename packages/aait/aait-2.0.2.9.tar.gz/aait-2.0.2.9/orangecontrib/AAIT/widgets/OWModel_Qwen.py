...
class OWModelQwen(widget.OWWidget):
    ...
    gui = os.path.join(os.path.dirname(os.path.abspath(__file__)), "designer/owmodel_qwen.ui")
    priority=1093
    ...
    def __init__(self):
        ...
        model_name = "Qwen2.5.1-Coder-7B-Instruct-Q6_K.gguf"
        ...
        if not os.path.exists(self.model_path):
            if not SimpleDialogQt.BoxYesNo("Model isn't in your computer. Do you want to download it from AAIT store?"):
                return
            try:
                if 0 != GetFromRemote("Qwen Model Coder"):
                    return
            except:

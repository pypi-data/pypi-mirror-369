...
def rag_evaluate(excel_file, metric='answer_recall_centricube', batch_size=5):
        ...
        with gr.Row():
            with gr.Column(scale=2):
                with gr.Row():
                    ...
                    with gr.Column():
                        metric_options = ["answer_recall_centricube", "answer_correctness_centricube"]
                        metric = gr.components.Dropdown(label="评估方法", choices=metric_options, default=metric_options[0], interactive=True)
                        # metric = gr.Textbox(label='评估方法', value='answer_recall_centricube', interactive=True, lines=2)

...
from .centricube_rag_tool import CentricubeRAGTool
...
class CentricubeRetrievalQA(Chain):
    ...
    centricube_rag_tool: CentricubeRAGTool = Field(default_factory=CentricubeRAGTool,
    ...
    def from_llm(
        ...
        **kwargs: Any,
        ...
        centricube_rag_tool = CentricubeRAGTool(vector_store=vector_store,
                                          ...
                                          **kwargs)
        return cls(
            centricube_rag_tool=centricube_rag_tool,
            callbacks=callbacks,
    ...
    def _call(
        ...
        if self.return_source_documents:
            answer, docs = self.centricube_rag_tool.run(
                question,
            ...
            answer = self.centricube_rag_tool.run(question, return_only_outputs=True)
    ...
    async def _acall(
        ...
        if self.return_source_documents:
            answer, docs = await self.centricube_rag_tool.arun(question, return_only_outputs=False)
            return {self.output_key: answer, 'source_documents': docs}
        else:
            answer = await self.centricube_rag_tool.arun(question, return_only_outputs=True)
            return {self.output_key: answer}

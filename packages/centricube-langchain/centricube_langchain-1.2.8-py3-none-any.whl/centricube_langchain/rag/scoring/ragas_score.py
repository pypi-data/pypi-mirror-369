...
from centricube_ragas import evaluate
from centricube_ragas.metrics import AnswerCorrectness, AnswerCorrectnessCentricube, AnswerRecallCentricube
...
class RagScore:
    ...
    def ragas_answer_correctness_centricube(self, dataset: Dataset) -> pd.DataFrame:
        answer_correctness = AnswerCorrectnessCentricube(batch_size=self.batch_size)
        result = evaluate(
            dataset=dataset,
    ...
    def ragas_answer_recall_centricube(self, dataset: Dataset) -> pd.DataFrame:
        answer_recall =AnswerRecallCentricube(batch_size=self.batch_size, 
    ...
    def score(self) -> Tuple[pd.DataFrame, Optional[pd.DataFrame]]:
        ...
        for metric_name in self.metrics:
            ...
            if metric_name =='answer_recall_centricube':
                if self.gt_split_column:
...
if __name__ == '__main__':
    params = {
        'excel_path': '/home/gulixin/workspace/llm/centricube/src/centricube-langchain/experimental/rag/data/test.xlsx',
        'save_path': '/home/gulixin/workspace/llm/centricube/src/centricube-langchain/experimental/rag/data',
        'question_column': '问题',
        'gt_column': 'GT',
        'answer_column': 'rag_answer',
        'query_type_column': '问题类型',
        # 'metrics': ['answer_correctness_centricube'],
        'metrics': ['answer_recall_centricube'],
        'batch_size': 10,

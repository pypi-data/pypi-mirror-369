...
import httpx
import yaml
from centricube_langchain.rag.extract_info import extract_title
from centricube_langchain.rag.init_retrievers import (BaselineVectorRetriever, KeywordRetriever,
                                                   MixRetriever, SmallerChunksVectorRetriever)
from centricube_langchain.rag.utils import import_by_type, import_class
from centricube_langchain.retrievers import EnsembleRetriever
from centricube_langchain.vectorstores import ElasticKeywordsSearch, Milvus
    ...
    def __init__(self,
        ...
        if keyword_store:
            ...
            if self.params['elasticsearch'].get('extract_key_by_llm', False):
                extract_key_prompt = import_class(
                    'centricube_langchain.rag.prompts.EXTRACT_KEY_PROMPT')
        ...
        if QA_PROMPT:
            ...
            if 'prompt_type' in self.params['generate']:
                ...
                prompt = import_class(f'centricube_langchain.rag.prompts.{prompt_type}')
            else:
                prompt = None
...
if __name__ == '__main__':
    # rag_tool = CentricubeRAGTool(collection_name='rag_finance_report_0_test')
    ...
    tool = CentricubeRAGTool.get_rag_tool(name='rag_knowledge_retrieve',
    ...
    # tool = CentricubeRAGTool.get_rag_tool(

...
from centricube_langchain.rag.init_retrievers import (
    BaselineVectorRetriever,
    KeywordRetriever,
    MixRetriever,
    SmallerChunksVectorRetriever,
)
from centricube_langchain.rag.scoring.ragas_score import RagScore
from centricube_langchain.rag.utils import import_by_type, import_class


class CentricubeRagPipeline:
    ...
    def retrieval_and_rerank(self, question, collection_name):
        ...
        if self.params['post_retrieval']['with_rank'] and len(docs):
            if not hasattr(self, 'ranker'):
                ...
                rerank_object = import_class(f'centricube_langchain.rag.rerank.{rerank_type}')
    ...
    def score(self):
        ...
        if metric_params['type'] == 'centricube-ragas':
            score_params = {
...
if __name__ == '__main__':
    ...
    parser.add_argument('--params', type=str, default='config/test/baseline_s2b.yaml', help='centricube rag params')
    # 解析参数
    args = parser.parse_args()

    rag = CentricubeRagPipeline(args.params)

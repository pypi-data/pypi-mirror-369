...
class TectonDataFrame(DataframeWrapper):
    ...
    def start_dataset_job(
        ...
        if isinstance(fco, specs.FeatureServiceSpec):
            if compute_mode == ComputeMode.RIFT and not environment:
                msg = "`environment` parameter must be provided when dataset job is run for a feature service on Rift"
                raise TectonValidationError(msg)

            if compute_mode == ComputeMode.SPARK and not tecton_materialization_runtime:
                msg = "`tecton_materialization_runtime` parameter must be provided when dataset job is run for a feature service on Spark"
                raise TectonValidationError(msg)

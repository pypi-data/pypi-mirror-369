...
PADDING_REGEXP = re.compile(r"^(\d+-?){0,3}\d+$")
...
<head>
  <meta charset="utf-8" />
  <title>p01.recipe.cdn:sprites</title>
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <link rel="stylesheet" type="text/css" href="%(css_url)s" />
  <style type="text/css">
    i {margin-right: 20px; float: left;}
  </style>
</head>
<body>
<h1>Sprite CSS Classes</h1>
%(sprites)s
</body>
</html>
"""
...
<div>.%(class_name)s <i class="%(class_name)s"></i></div>
"""
...
class Sprite(object):
    ...
    def _locate_images(self):
        """Return all valid images within a folder."""
        extensions = '|'.join([re.escape(ext) for ext in VALID_IMAGE_EXTENSIONS])
        extension_re = re.compile(r'.+\.({})$'.format(extensions), re.IGNORECASE)

        try:
            files = sorted(os.listdir(self.path))
        except OSError:
            raise SourceImagesNotFoundError("Cannot read directory: {}".format(self.path))

        images = [
            Image(n, sprite=self)
            for n in files
            if not n.startswith('.') and extension_re.match(n)
        ]

        if not images:
            raise SourceImagesNotFoundError("No valid images found in: {}".format(self.path))
        ...
        reverse = self.config.ordering.startswith('-')
    ...
    def image_url(self, ratio=1):
        ...
        # this is wrong, the url alreadycontains the config.url
        #if self.config.url:
        #    # XXX: bugfix: this will generate the right url
        #    url = posixpath.join(self.config.url, '%s.png' % self.filename)
...
def patched_load(self):
    if self.im and self.palette anF self.palette.dirty:

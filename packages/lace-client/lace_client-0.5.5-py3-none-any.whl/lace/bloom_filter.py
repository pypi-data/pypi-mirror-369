...
class BloomFilter:
    ...
    def __init__(self, expected_items: int = 1_000_000, fp_rate: float = 0.0001):
        ...
        print(f"📊 Bloom filter initialized locally:")
        print(f"   Size: {self.size_bytes / (1024*1024):.2f} MB")
        print(f"   Hash functions: {self.num_hashes}")
        print(f"   Expected FP rate: {fp_rate*100:.3f}%")
    ...
    def add_text_content(self, text: str, include_ngrams: bool = True):
        ...
        print(f"✅ Added {len(words)} words and n-grams to local bloom filter")
        print(f"   Privacy preserved: No raw data will leave your system")
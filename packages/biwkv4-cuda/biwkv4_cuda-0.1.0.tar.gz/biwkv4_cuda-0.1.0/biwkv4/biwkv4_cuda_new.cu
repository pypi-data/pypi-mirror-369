...
******************************************************************************/
// BiWKV from VisionRWKV  https://github.com/OpenGVLab/Vision-RWKV
// new version: https://github.com/OpenGVLab/Vision-RWKV/tree/master/classification/mmcls_custom/models/backbones/cuda_new
// change CHANNEL_SPLIT from 32 to 16, because the LALIC depth (96, 144, 192, 256)
...
__global__ void biwkv4_cuda_forward_kernel(
    size_t B,
    size_t T,
    size_t C,
...
template <typename scalar_t>
__global__ void biwkv4_cuda_backward_kernel(
    size_t B, 
    size_t T, 
    size_t C,
...
torch::Tensor biwkv4_cuda_forward(
    ...
    // 动态计算线程块大小
    int channel_len = std::min(CHANNEL_LEN, (int)num_channels);
    // 确保通道数能被channel_len整除
    while (num_channels % channel_len != 0 && channel_len > 1) {
        channel_len--;
    }

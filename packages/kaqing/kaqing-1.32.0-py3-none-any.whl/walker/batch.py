...
from walker.commands.setup_frontend import SetupFrontend
...
@click.option('--pod', '-p', required=False, metavar='pod', help='Kubernetes pod name')
@click.argument('extra_args', nargs=-1, metavar='<pod>', type=click.UNPROCESSED)
def setup(kubeconfig: str, config: str, param: list[str], namespace: str, pod: str, extra_args):
    run_command(SetupFrontend(), kubeconfig, config, param, None, namespace, pod, extra_args)

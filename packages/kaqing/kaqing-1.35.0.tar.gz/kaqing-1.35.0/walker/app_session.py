...
class IdpLogin:
    def __init__(self, r: str, id: str, state: str, session: requests.Session = None):
        self.r = r
        self.id = id
        self.state = state
...
class AppSession:
    ...
    def login_to_idp(self, forced = False, use_cached = True) -> IdpLogin:
        ...
        def body(username, password) -> IdpLogin:
            ...
            return IdpLogin(redirect_url, id_token, state_token, session)
    ...
    def login(self, forced = False, use_cached=True) -> tuple[requests.Session, str]:
        ...
        def login():
            try:
                ...
                if Config().get('debug.show-out', False):
                    log2(f'-> {idp_login.r}')
                session.post(idp_login.r, headers=headers, data=form_data, timeout=timeout)
            except Exception:
                pass
            finally:
                stop_event.set()

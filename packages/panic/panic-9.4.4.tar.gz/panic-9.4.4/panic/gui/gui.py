...
try:
    ...
    print()
    print("PANIC GUI requires python3-taurus and Qt framework python3 bindings")
    print()
    print("PyAlarm does NOT require it, just tango and fandango python3 libraries") 
    print()
    sys.exit(0)
...
class QAlarmList(QAlarmManager, PARENT_CLASS):
    ...
    valueChanged_signal = pyqtSignal(name="ValueChanged")
    ...
    def getAlarms(self, filtered=True):
        ...
        """
        trace('getAlarms(): sorting ...', level=4)
        self.view.sort()
    ...
    def setModel(self, model):
        ...
        try:
            if model != self.regEx:
                trace('AlarmGUI.setModel(%s)' % model, level=1)
                self._ui.regExLine.setText(model or self.default_regEx)
                self.onRegExUpdate()
        except:
            trace('onSetModel(): {}'.format(traceback.format_exc()),level=1)
    ...
    def hurry(self):
        ...
        """
        if not self.changed:
            trace('hurry() =====> onReload(changed = True) <<<<<', level=1)
            self.changed = True
    ...
    def onReload(self, clear_selection=False):
        ...
        try:
            trace('+' * 80, level=2)
            trace('onReload(%s)' % self.RELOAD_TIME, level=2)
            now = time.time()
            trace('AlarmGUI.onReload() after %f seconds' % (
                now - self.last_reload),level=3, indent=2)
            self.last_reload = now
            self._ui.listWidget.clearSelection()
            keys = self.api.keys()
            self.api.load(self.scope, exported=self.exported)
            if keys != self.api.keys():
                self.changed = True
            if self.api.keys() != self.view.api.keys():
                trace('AlarmGUI and AlarmView have different keys, reloading!!!',
                       level=1)
                self.view.api.load(self.scope, exported=self.exported)
                      
            trace('%d alarms found in database' 
                % len(self.api.keys()),level=2, indent=2)
            self.setSecondCombo()
            # self.checkAlarmRows()
    ...
    def onRefresh(self):
        """
        Just periodic update, checks order, no reload, no filters
        """
        trace('onRefresh(%s)' % self.REFRESH_TIME, level=3)
        self.buildList(changed=False)
    ...
    def updateStatusLabel(self):
        ...
        if not nones and self.splash:
            try:
                trace('closing splash ...', level=4)
                self.splash.finish(None)
                self.splash = None
            except:
                trace( 'Error closing splash screen: %s' %  
        ...
        try:
            ...
            if len(items) == 1:
                ...
                devices = sorted(set(fn.tango.parse_tango_model(m)['device']
                                     for m in models))
                trace('onItemSelected(%s) devices: %s' 
                      % (a, shortstr(devices)))
                self.devices_selected_signal.emit('|'.join(devices + tags))
        except:
            trace('onItemSelected(): %s' % traceback.format_exc(), level=3)
    ...
    def buildList(self, changed=False, block=False):
        try:
            ...
            if not self.changed: 
                trace('\tAlarm list not changed', level=4)
    ...
    def showList(self, delete=False):
        ...
        """
        trace('In AlarmGUI.showList()', level=4)
        # self._ui.listWidget.blockSignals(True)
        currents = [a.tag for a in self.getSelectedAlarms()]
        trace('showList(): %d items selected' % len(currents), level=3)

        if delete:
            trace('removing objects from the list ... <<<<<',level=1,indent=1)
            while self._ui.listWidget.count():
                self._ui.listWidget.takeItem(0)

        data = self.view.sort()
        trace('%d alarms found by view.sort()' % len(data or []),
              head='showList', level=3, indent=1)

        if data:
            #updating table column sizes
        ...
        for i, t in enumerate(data):  # self._ordered:
            ...
            if alarm.tag in currents:
                try:
                    trace('\tselect %s' % alarm.tag, level=2, indent=2)
        ...
        trace('showList(): %d alarms match filters.'
              % self._ui.listWidget.count(), level=2, indent=1)
        self.updateStatusLabel()  # < getAlarms() called again here

        # self._ui.listWidget.blockSignals(F                                                     alse)
...
class QFilterGUI(QAlarmList):
    ...
    def getFilters(self):
        ...
        """
        # TRACE_LEVEL=4
        trace("AlarmGUI.getFilters(%s)" % (
            ['%s=%s' % (s, getattr(self, s, None)) for s in
        ...
        # if not any(device,r
        if userfilter:
            trace('getFilters(%s)' % str(userfilter))
        ...
        trace('getFilters(...): %s' % str(filters), level=2, indent=1)
        return filters
    ...
    def setSecondCombo(self):
        ...
        trace("AlarmGUI.setSecondCombo(%s)" % source, level=2)
        ...
        trace('setSecondCombo(%s): %d devices' % (source, len(devs)), level=2)
        if source == 'Devices':
    ...
    def onRegExSave(self):
        ...
        try:
            ...
            v = QtGui.QMessageBox.warning(self, 'Warning',
                    str(e.args), QtGui.QMessageBox.Ok)
    ...
    def regExFiltering(self, source):
        ...
        trace('onRegExFiltering(%s)' % msg, level=1)
        raise Exception(msg)
...
class AlarmGUI(QFilterGUI):
    ...
    def init_ui(self, parent, mainwindow):
        ...
        try:
            ...
            trace('Error initializing GUI: %s' % traceback.format_exc())
    ...
    def connectAll(self):
        ...
        #     self.valueChanged_signal.connect(self.hurry)
        ...
        self._ui.refreshButton.clicked.connect(self.hurry)  # Force Reload!
    ...
    def init_mw(self, tmw=None):
        ...
        trace('Toolbars created after %s seconds' % (time.time() - t0))
    ...
    def main(args=[]):
        ...
        if any(o in opts for o in ('-h', '--help', '-?')):
            print(HELP)
            return
        opts = {o.split('=',1)[0].strip('-'):o.split('=',1)[-1]
                for o in opts}
        ...
        if 'refresh' in opts:
            print('setting refresh time to %s' % opts['refresh'])
            QAlarmList.REFRESH_TIME = int(opts['refresh'])
        ...
        try:
            ...
            if ask or '*' in default:
                if '/' in default:
                    ...
                    d, f = '.', default
                filename, _ = Qt.QFileDialog.getOpenFileName(self.mainwindow,
                                                          'Import file', d, f)
            else:
                filename = default
            if filename:
    ...
    def setWindowMenu(self, action=None):
        trace('In AlarmGUI.setWindowMenu(%s)' % action)

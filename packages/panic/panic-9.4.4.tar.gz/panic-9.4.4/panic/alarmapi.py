...
from fandango.excepts import ExceptionDecorator, ExceptionContext
...
def substitute(message, substitutions=None, depth=1):
    """
...
def getPanicProperty(prop, value=None):
    ...
    db = get_tango(thost)
    if db is None:
        raise Exception('TangoHost({})NotAvailable!'.format(thost))
                ...
                try:
                    ...
                    if f.startswith('http'):
                        try:
                            ...
                            print('getPanicProperty error: ' +
                                'Unable to get %s.%s: %s' % (k, f, e))
                    if f.startswith('/'):
                        f = open(f)
...
class Alarm(object):
    ...
    def get_model(self, full=False):
        ...
        if full and ':' not in model: 
            model = self.api.tango_host + '/' + model
        return model

    def get_ds(self):
        """ Gets and AlarmDS object related to this alarm """
        try:
            return self.api.devices[self.device]
        except:
            return AlarmDS(self.device, api=self.api)
    ...
    def set_state(self, state=None):
        ...
        if ':' in state and state.split(':')[0] == tag:
            ...
            try:
                state = 'ACKED' if self.get_acknowledged() else 'UNACK'
            except:
                traceback.print_exc()
        ...
        if o != self._state or self._time is None:
            if o is not None and o != self._state:
    ...
    def get_annunciators(self):
        return self.receivers

    def set_annunciators(self, val):
        self.receivers = val

    annunciators = property(fget=get_annunciators, fset=set_annunciators)

    def get_condition(self):
        return self.get_ds().condition
...
class AlarmDS(object):
    ...
    def get_property(self, prop):
        if isSequence(prop):
            return self.api.get_db_properties(self.name, prop)
        else:
            return self.api.get_db_property(self.name, prop)
    ...
    def enable(self, tag=None, comment=None):
        ...
        if tag is None:
            ...
            self.init()
            print('AlarmDS(%s): Enabled!' % self.name)
        else:
            tags = [a for a in self.alarms if matchCl(tag, a)]
            print('AlarmDS(%s): Enabling %d alarms: %s' % (self.name, len(tags), tags))
    ...
    def disable(self, tag=None, comment=None, timeout=None):
        ...
        if tag is None:
            ...
            self.init()
            print('AlarmDS(%s): Disabled!' % self.name)
        else:
            tags = [a for a in self.alarms if matchCl(tag, a)]
            print('AlarmDS(%s): Disabling %d alarms: %s' % (self.name, len(tags), tags))
            [self.get().Disable(
    ...
    def get_active_alarms(self, value=None):
        ...
        if value is None:
            try:
                value = getAttrValue(self._actives.read(), default=None)
            except Exception as e:
                return e
    ...
    def get_acknowledged(self, alarm=None):
        ...
        value = getAttrValue(self._acknowledged, default=[])
        #print('alarm(%s)._acknowledged: %s' % (alarm,value))
        if None not in (alarm,value) and len(value):
    ...
    def get_disabled(self, alarm=None):
        ...
        value = [t.split(sep)[0] for t in
                 getAttrValue(self._disabled, default=[])]
        if None not in (alarm,value) and len(value):
            return getattr(alarm, 'tag', alarm) in value
        else:
            return value
...
class AlarmAPI(fandango.SingletonMap):
    ...
    def __init__(self, filters='*', tango_host=None,
        ...
        self.debug('In AlarmAPI(%s)' % filters)
        self.alarms = {}
    ...
    def __init_logger(self, logger):
        if fandango.isCallable(logger):
            ...
            #print(fandango.log.LogLevels)
            for l in fandango.log.LogLevels:
                setattr(self, l.lower(),
                    getattr(self._logger, l.lower(), self._logger.debug))
            self.log = self.debug
    ...
    def __len__(self):
        return self.alarms.__len__()

    def __iter__(self):
        return self.alarms.__iter__()
    ...
    def load(self, filters=None, exported=False, extended=False):
        ...
        self.info('AlarmAPI.load(filters={},exported={},extended={}):'.format(
            filters, exported, extended))
        ...
        self.info('AlarmAPI.load(%s): %d devices' % (filters, len(all_devices)))

        for d in all_devices:
            self.info('Loading device: %s' % d)
        ...
        for k, v in sorted(self.alarms.items()):
            found = False
        ...
        self.info('AlarmAPI.load(%s): %d alarms loaded' % (
            filters, len(self.alarms)))
        
        AlarmAPI.CURRENT = self

        self.debug('%s s dedicated to,\n load devices %s\n load properties %s\n'
    ...
    def load_from_csv(self, filename, device=None, write=True):
        ...
        if not os.path.isfile(filename):
            print("[load_from_csv] File does not exist: {}".format(filename))
            return {}
        alarms = {}
    ...
    def export_to_csv(self, filename, regexp=None,
        ...
        for i, (d, alarm) in enumerate(sorted(
                ((a.device, a) for a in alarms),
                key=lambda x: (x[0], x[1].tag))):
            ...
            for a in alarms:
                ...
                if f == 'regexp':
                    ...
                    for e in regexp:
                        ...
                        m = searchCl(e, s)
                        if m and n:
                            exclude.append(a.tag)
                        elif m and not n:
                            ok = True
                        elif not m and n and len(regexp) == 1:
                            ok = True
                ...
                if f == 'attribute':
                    ...
                    if isString(v):
                        ...
                        ok = not xor(isFalse(r), isFalse(v))
    ...
    def get_states(self, tag='', device=''):
        ...
        if tag:
            ...
            try:
                ...
                print
                'device %s is not running' % device
                traceback.print_exc()

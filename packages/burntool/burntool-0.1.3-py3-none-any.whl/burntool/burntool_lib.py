...
class BurnToolRxPkt:
    ...
    def timeout(self):
        ...
        logging.debug("BurnToolRxPkt timeout")
...
# Host
class BurnToolHost:
    def __init__(self, port, fw="fw.hex", timeout=None, patch=None, wait=False, debug=None):
        ...
        if not os.path.isfile(self.fw):
            logging.info(f"{self.fw} not found")
    ...
    def reset(self):
        logging.warning("send reset signal")
    ...
    def request(self, opcode, address=0, msg=b'', timeout=0.5, wait=True):
        ...
        logging.info(f"txpkt header: {opcode:02X}, 0x{address:08X}, {len(msg)}")
    ...
    def run(self):
        ...
        if not self.initialized:
            logging.error("BurnTool not initialized")
            return result
...
class BurnToolParser:
    ...
    def timeout(self):
        ...
        self.data = b''
        logging.debug("BurnToolParser timeout")

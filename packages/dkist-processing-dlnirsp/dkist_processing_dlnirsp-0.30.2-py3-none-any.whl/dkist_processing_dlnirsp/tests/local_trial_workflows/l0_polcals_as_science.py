        ...
        def run(self) -> None:
            ...
            if not raw_dir.exists():
                raise FileNotFoundError(
                    f"Expected to find a raw DLNIRSP{{run_id:03n}} folder in {self.scratch.scratch_base_path}"
                )
...
def main(
    ...
    with ManualProcessing(
        ...
        # Set obs time to Solar because there might not be dark frames for the polcal exp time
        # (polcal uses its own darks taken as part of the CS)
        ...
        if load_polcals_as_science:
            ...
            manual_processing_run.run_task(task=TagPolcasAsScience)

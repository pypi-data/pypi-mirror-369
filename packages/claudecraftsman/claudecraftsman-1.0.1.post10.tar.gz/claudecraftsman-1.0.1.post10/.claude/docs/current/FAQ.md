...
For comprehensive troubleshooting with detailed solutions, see our **[Troubleshooting Guide](TROUBLESHOOTING.md)**.

**Quick Fixes:**
- **Stuck implementation?** → `/implement feature --status` then `--resume`
- **Running slow?** → `/implement feature --no-research` or `--phase=1`
- **Need debugging?** → `/implement feature --debug` or set `CLAUDECRAFTSMAN_DEBUG=1`
- **MCP not working?** → Check Claude Code settings for enabled servers
- **Installation issues?** → Verify Python 3.11+ and PATH configuration
...
- name: Run ClaudeCraftsman Tests
  run: |
    craft validate framework
    craft test all --ci-mode
```
...
- ClaudeCraftsman version (`craft --version`)
- Command that failed
- Error messages

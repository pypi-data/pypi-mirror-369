...
class CommandHandler:
    ...
    def start_session(self, initial_message: Optional[str] = None):
        ...
        if not self.config_manager.get_configured_providers():
            self.console.display_error(
                "No providers configured. Please run 'cmdlm --configure' first."
            )
            return
        ...
        while True:
            try:
                ...
                if user_input == "__DOUBLE_CTRL_C_EXIT__":
                    self.console.display_info(
                        "\nDouble Ctrl+C detected. Exiting chat session..."
                    )
                    break
                ...
                if user_input.startswith("/"):
                    ...
                    if not self.handle_command(cmd):
                        break
                    # Reset Ctrl+C counter after successful command
                    self.console.prompt_manager.reset_ctrl_c_counter()
                    continue
                ...
                # Check for empty input (including whitespace-only input)
                if not user_input or not user_input.strip():
                    # Show help message for first Ctrl+C
                    self.console.display_info(
                        "\nUse /quit to exit or press Ctrl+C again to force exit"
                    )
                    continue
                ...
                try:
                    ...
                    # Reset Ctrl+C counter after successful message
                    self.console.prompt_manager.reset_ctrl_c_counter()
                except Exception as e:
                    # Handle other errors
    ...
    def _handle_tools_list(self) -> bool:
        ...
        if not available_tools:
            self.console.display_info(
                "No tools discovered. Tools are auto-discovered from src/cmdlm/tools/builtin/"
            )
        else:

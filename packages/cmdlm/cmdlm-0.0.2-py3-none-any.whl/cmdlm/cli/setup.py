...
def setup_global_env_vars(config_manager: ConfigManager):
    """Setup global environment variables that are always available"""
    console.print("\n🌍 Global Environment Variables Setup", style="bold cyan")
    console.print(
        "\nThese variables will be automatically loaded in all cmdlm sessions.",
        style="dim",
    )
    console.print(
        "Examples: GITHUB_TOKEN, DOCKER_REGISTRY_PASSWORD, AWS_PROFILE, etc.",
        style="dim",
    )
    ...
    while True:
        var_name = Prompt.ask(
            "\nEnvironment variable name (or 'done' to finish)", default="done"
        )

        if var_name.lower() == "done":
            break

        # Skip adding if the variable name doesn't meet basic requirements
        ...
        existing_vars = config_manager.get_global_env_vars()
        if var_name in existing_vars:
            if not Confirm.ask(f"Variable '{var_name}' already exists. Overwrite?"):
                continue

        var_value = Prompt.ask(f"Value for {var_name}", password=True)
        env_vars[var_name] = var_value

        console.print(f"✅ Added {var_name}", style="green")

    if env_vars:
        try:
            config_manager.set_global_env_vars(env_vars)
            console.print(
                f"\n✅ Configured {len(env_vars)} global environment variables",
                style="bold green",
            )
            console.print("Use 'cmdlm --env-vars list' to view them later", style="dim")
        except ValueError as e:
            console.print(
                f"\n❌ Error configuring environment variables: {str(e)}", style="red"
            )
    else:
        console.print("\nNo global environment variables configured", style="yellow")
...
def initial_setup():
    ...
    if is_first_setup:
        ...
        logo = """
            [gradient(#4B56D2,#82AAAD)]
               ...
               [italic #F5F5F5]Command-LM: LLMs for every developer, every workflow[/italic #F5F5F5]

                                                              [/gradient(#4B56D2,#82AAAD)]"""

        description = """
        [bright_cyan underline]Supercharge your productivity with a customizable LLM assistant[/bright_cyan underline]

        [white][bright_blue]✦[/bright_blue] [bright_white]Integrate[/bright_white] LLMs into your daily development tasks
        [bright_blue]✦[/bright_blue] [bright_white]Accelerate[/bright_white] coding, learning, and problem-solving
        [bright_blue]✦[/bright_blue] [bright_white]Stay in control[/bright_white]—LLMs assist, you decide

        [dim]Get started by configuring your preferred LLM provider.
        Supported: [bright_blue]Anthropic Claude[/bright_blue], [bright_green]OpenAI GPT[/bright_green], [bright_red]Google Gemini[/bright_red], and more.[/dim]"""

        footer = """
        [dim white]Arrow keys: navigate • Enter: select • 'q': quit[/dim white]"""
    ...
    if configured_any:
        # Offer to configure global environment variables
        if Confirm.ask(
            "\n🌍 Would you like to configure global environment variables? (These will be available in all chat and CLI sessions)",
            default=False,
        ):
            setup_global_env_vars(config_manager)

        console.print("\n✨ Setup completed successfully!", style="bold green")
        console.print("\n[dim white]Example usage:[/dim white]")
        console.print(
            '[bright_green]$[/bright_green] [white]cmdlm "What are the best practices for Python error handling?"[/white]'
        )
        console.print(
            '[bright_green]$[/bright_green] [white]cmdlm "Write a detailed docstring for this function: $(cat function.py)"[/white]'
        )
        console.print(
            '[bright_green]$[/bright_green] [white]cat error.log | cmdlm "Explain this error and suggest solutions"[/white]'
        )
        console.print("\n[dim white]For a specific model, use:[/dim white]")
        console.print(
            '[bright_green]$[/bright_green] [white]cmdlm --model <model-name> "Your prompt here"[/white]'
        )
        return

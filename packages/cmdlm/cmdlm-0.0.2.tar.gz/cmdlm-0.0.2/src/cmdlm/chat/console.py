...
class ChatConsole:
    ...
    def __init__(self, console: Optional[Console] = None, raw: bool = False):
    ...
    def display_welcome(self):
        ...
        commands_table = Table(
            title="CMD-LM Commands & Shortcuts",
    ...
    def display_message(
        ...
        try:
            if role == "user":
                ...
                # For assistant messages, add a newline and render based on raw flag
                self.console.print(f"\n{time_str}{prefix}")
                if self.raw:
                    # Display raw text without markdown
                    self.console.print(content, style=content_style)
                else:

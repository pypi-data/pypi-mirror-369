...
class PromptManager:
    ...
    USER_PROMPT_DIR = Path.home() / ".cmdlm" / "prompts"
    ...
    def __init__(self, console: Console = None, base_path: str = None):
        ...
        # Setup history
        history_file = os.path.expanduser("~/.cmdlm/history")
        ...
        # Track Ctrl+C presses for double-press exit functionality
        self.ctrl_c_count = 0
        self.last_ctrl_c_time = 0
        self.CTRL_C_TIMEOUT = 2.0
    ...
    def get_input(self, prompt_str: str = "", key_bindings=None) -> str:
        ...
        try:
            ...
            # Check if this is a recent second Ctrl+C press
            ...
            if self.ctrl_c_count >= 2:
                # Double Ctrl+C detected - return special exit signal
                return "__DOUBLE_CTRL_C_EXIT__"
            else:
                # First Ctrl+C - return empty string as before
                return ""
        except EOFError:
            return "exit"

    def reset_ctrl_c_counter(self):
        """Reset the Ctrl+C counter (called when user successfully sends a message)"""
        self.ctrl_c_count = 0

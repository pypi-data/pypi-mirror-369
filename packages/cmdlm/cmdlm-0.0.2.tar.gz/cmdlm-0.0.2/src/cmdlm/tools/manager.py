...
def get_user_tools_directory() -> Path:
    ...
    user_config_dir = Path.home() / ".cmdlm"
    ...
    if not init_file.exists():
        init_file.write_text('"""User-defined tools for CMD-LM."""\n')
    ...
    if not sample_file.exists() and not any(tools_dir.glob("*.py")):
        sample_content = '''"""Sample custom tools for CMD-LM.
...
from cmdlm.tools.base import tool
...
class ToolManager:
    ...
    def __init__(
        ...
        Args:
            ...
            discover_user_tools: Whether to auto-discover user tools from ~/.cmdlm/tools/
        """
    ...
    def discover_user_tools(self) -> List[str]:
        """Discover tools in the user tools directory (~/.cmdlm/tools/)."""
...
Created by CMD-LM tool generator.
"""
...
'''
            ...
            self.console.print(
                "Edit this file to implement your tool logic, then restart CMD-LM to use it.",
                style="dim",
            )
            return True

...
def check_security_headers(url):
    ...
    try:
        ...
        console.print(Panel.fit(f"[red]Error accessing {url}: {e}[/red]", title="Error"))
        return
    ...
    # Auto-generate clickjacking payload if vulnerable (no prompt)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        console.print(Panel.fit(f"Usage: python3 {sys.argv[0]} <url>", border_style="red"))
        sys.exit(1)

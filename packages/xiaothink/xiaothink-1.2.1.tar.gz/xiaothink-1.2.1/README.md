...
Xiaothink 是一个以自然语言处理（NLP）为核心的AI研究组织，致力于在少数据、低算力下训练出先进的端侧模型。Xiaothink Python 模块是我们提供的核心工具包，涵盖了文本问答、图文问答、图像压缩等多种功能。以下是详细的使用指南和代码示例。
...
**重要提示**：对于本地模型，建议调用 `model.chat` 函数进行多轮对话，未进行指令微调的预训练模型模型建议调用 `test.generate_texts_loop` 函数。
** 单论对话的 `model.chat_SingleTurn` 函数即将在未来版本被移除 **
